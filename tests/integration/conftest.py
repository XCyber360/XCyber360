...
from xcyber360_testing import session_parameters
from xcyber360_testing.constants import platforms
from xcyber360_testing.constants.platforms import WINDOWS
from xcyber360_testing.constants.daemons import XCYBER360_MANAGER, API_DAEMONS_REQUIREMENTS
from xcyber360_testing.constants.paths import ROOT_PREFIX
from xcyber360_testing.constants.paths.api import RBAC_DATABASE_PATH
from xcyber360_testing.constants.paths.logs import ACTIVE_RESPONSE_LOG_PATH, XCYBER360_LOG_PATH, ALERTS_JSON_PATH, \
                                               XCYBER360_API_LOG_FILE_PATH, XCYBER360_API_JSON_LOG_FILE_PATH
from xcyber360_testing.constants.paths.configurations import XCYBER360_CLIENT_KEYS_PATH
from xcyber360_testing.logger import logger
from xcyber360_testing.tools import socket_controller
from xcyber360_testing.tools.monitors import queue_monitor
from xcyber360_testing.tools.monitors.file_monitor import FileMonitor
from xcyber360_testing.tools.simulators.agent_simulator import create_agents, connect
from xcyber360_testing.tools.simulators.authd_simulator import AuthdSimulator
from xcyber360_testing.tools.simulators.remoted_simulator import RemotedSimulator
from xcyber360_testing.utils import configuration, database, file, mocking, services
from xcyber360_testing.utils.file import remove_file, truncate_file
from xcyber360_testing.utils.manage_agents import remove_agents
import xcyber360_testing.utils.configuration as xcyber360_configuration
from xcyber360_testing.utils.services import control_service
...
def load_xcyber360_basic_configuration():
    ...
    backup_ossec_configuration = configuration.get_xcyber360_conf()

    # Write new configuration
    configuration.write_xcyber360_conf(minimal_configuration)

    yield

    # Restore the ossec.conf backup
    configuration.write_xcyber360_conf(backup_ossec_configuration)


@pytest.fixture()
def backup_xcyber360_configuration() -> None:
    """Backup xcyber360 configuration. Saves the initial configuration and restores it after the test."""
    # Save configuration
    backup_config = configuration.get_xcyber360_conf()

    yield

    # Restore configuration
    configuration.write_xcyber360_conf(backup_config)


@pytest.fixture()
def set_xcyber360_configuration(test_configuration: dict) -> None:
    """Set xcyber360 configuration
    ...
    test_config = configuration.set_section_xcyber360_conf(test_configuration.get('sections'))

    # Set new configuration
    configuration.write_xcyber360_conf(test_config)
...
def configure_local_internal_options_function(request):
    ...
    backup_local_internal_options = xcyber360_configuration.get_local_internal_options_dict()

    logger.debug(f"Set local_internal_option to {str(local_internal_options)}")
    xcyber360_configuration.set_local_internal_options_dict(local_internal_options)

    yield

    logger.debug(f"Restore local_internal_option to {str(backup_local_internal_options)}")
    xcyber360_configuration.set_local_internal_options_dict(backup_local_internal_options)


@pytest.fixture()
def restart_xcyber360_function(request):
    ...
    if len(daemons) == 0:
        ...
        # Stop a list daemons in order (as Xcyber360 does)
        daemons.reverse()
        for daemon in daemons:
...
def file_monitoring(request):
    ...
    if hasattr(request.module, 'file_to_monitor'):
        ...
        file_to_monitor = XCYBER360_LOG_PATH
...
def truncate_monitored_files_implementation() -> None:
    ...
    if services.get_service() == XCYBER360_MANAGER:
        log_files = [XCYBER360_LOG_PATH, ALERTS_JSON_PATH, XCYBER360_API_LOG_FILE_PATH,
                     XCYBER360_API_JSON_LOG_FILE_PATH, XCYBER360_CLIENT_KEYS_PATH]
    else:
        log_files = [XCYBER360_LOG_PATH]
...
def daemons_handler_implementation(request: pytest.FixtureRequest) -> None:
    """Helper function to handle Xcyber360 daemons.
    ...
    The  `daemons_handler_configuration` should be a dictionary with the following keys:
        daemons (list, optional): List with every daemon to be used by the module. In case of empty a ValueError
            will be raised
        all_daemons (boolean): Configure to restart all xcyber360 services. Default `False`.
    ...
    if config := getattr(request.module, 'daemons_handler_configuration', None):
        ...
        if 'all_daemons' in config:
            logger.debug(f"Xcyber360 control set to {config['all_daemons']}")
        ...
        logger.debug("Xcyber360 control set to 'all_daemons'")
        all_daemons = True

    try:
        if all_daemons:
            logger.debug('Restarting xcyber360 using xcyber360-control')
    ...
    if all_daemons:
        logger.debug('Stopping xcyber360 using xcyber360-control')
...
@pytest.fixture(scope='module')
def restart_xcyber360_daemon_after_finishing_module(daemon: str = None) -> None:
    """Restart a Xcyber360 daemons and clears the xcyber360 log after the test module finishes execution.
    Args:
        daemon (str): provide which daemon to restart. If None, all daemons will be restarted.
    """
    yield
    file.truncate_file(XCYBER360_LOG_PATH)
    services.control_service("restart", daemon=daemon)
...
def configure_local_internal_options_handler(request: pytest.FixtureRequest, test_metadata) -> None:
    ...
    The `local_internal_options` is a dict with keys and values as the Xcyber360 `local_internal_options` format.
...
def configure_sockets_environment_implementation(request: pytest.FixtureRequest) -> None:
    ...
    # Stop xcyber360-service and ensure all daemons are stopped
    services.control_service('stop')
...
def mock_agent_module():
    """Fixture to create a mocked agent in xcyber360 databases"""
...
def autostart_simulators(request: pytest.FixtureRequest) -> None:
    """
    Fixture for starting simulators in xcyber360-agent executions.

    This fixture starts both Authd and Remoted simulators only in the cases where the service is not
    XCYBER360_MANAGER, and when the test function is not already using the simulator fixture, if it does
    use one of them, only start the remaining simulator.

    This is required so all xcyber360-agent instances are being tested with the xcyber360-manager connection
    being mocked.
    """
    create_authd = 'authd_simulator' not in request.fixturenames
    create_remoted = 'remoted_simulator' not in request.fixturenames

    if services.get_service() is not XCYBER360_MANAGER:
