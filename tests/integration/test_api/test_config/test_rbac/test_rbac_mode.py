...
brief: These tests will check if the 'rbac_mode' (Role-Based Access Control) setting of the API
       ...
       'whitelist mode' and 'blacklist mode'. The Xcyber360 API is an open source 'RESTful' API
       that allows for interaction with the Xcyber360 manager from a web browser, command line tool
...
references:
    ...
    - https://documentation.xcyber360.com/current/user-manual/api/configuration.html#rbac-mode
...
from xcyber360_testing.constants.api import CONFIGURATION_TYPES, MANAGER_INFORMATION_ROUTE
