"""
copyright: Copyright (C) 2015, Xcyber360 Inc.
...
references:
    ...
    - https://documentation.xcyber360.com/current/user-manual/api/configuration.html
...
from xcyber360_testing.constants.api import CONFIGURATION_TYPES, MANAGER_CONFIGURATION_ROUTE
from xcyber360_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from xcyber360_testing.modules.api.utils import login, get_base_url
from xcyber360_testing.utils.configuration import get_test_cases_data, load_configuration_template, get_xcyber360_conf
...
def test_limit_eps(test_configuration, test_metadata, set_xcyber360_configuration, add_configuration,
    ...
    xcyber360_config = " ".join(get_xcyber360_conf())
    assert f"<maximum>{test_metadata['expected_max_value']}</maximum>" in xcyber360_config
    assert f"<timeframe>{test_metadata['expected_timeframe_value']}</timeframe>" in xcyber360_config
