...
references:
    - https://documentation.xcyber360.com/current/user-manual/api/configuration.html#logs
...
from xcyber360_testing.constants.api import CONFIGURATION_TYPES, XCYBER360_API_USER, LOGIN_ROUTE
from xcyber360_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from xcyber360_testing.constants.paths.logs import XCYBER360_API_JSON_LOG_FILE_PATH, XCYBER360_API_LOG_FILE_PATH
from xcyber360_testing.modules.api.patterns import API_TIMEOUT_ERROR_MSG, API_LOGIN_REQUEST_MSG
from xcyber360_testing.modules.api.utils import login
...
def test_logs_formats(test_configuration, test_metadata, add_configuration, truncate_monitored_files, daemons_handler):
    ...
    if 'json' in current_formats:
        json_file_monitor = file_monitor.FileMonitor(XCYBER360_API_JSON_LOG_FILE_PATH)
        if current_level == 'error':
    ...
    if 'plain' in current_formats:
        plain_file_monitor = file_monitor.FileMonitor(XCYBER360_API_LOG_FILE_PATH)
        if current_level == 'error':
