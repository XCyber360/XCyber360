...
brief: These tests will check if the response_postprocessing middleware of the API handled by the 'xcyber360-apid' daemon is
       working properly. The Xcyber360 API is an open source 'RESTful' API that allows the interaction with the Xcyber360
...
from xcyber360_testing.constants.api import XCYBER360_API_PROTOCOL
from xcyber360_testing.modules.api.utils import login, get_base_url, set_authorization_header
from xcyber360_testing.utils.configuration import get_test_cases_data
...
def test_response_postprocessing(test_configuration, test_metadata, truncate_monitored_files, daemons_handler,
    ...
    if use_login_token:
        ...
        session.mount(f"{XCYBER360_API_PROTOCOL}://", adapter)
