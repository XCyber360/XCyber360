...
import { XCYBER360_ROLE_ADMINISTRATOR_ID } from '../../../common/constants';
  ...
  dashboardSecurity.isAdministratorUser = async function (context, request) {
    try {
      ...
      if (
        ...
        !decodedToken.rbac_roles.includes(XCYBER360_ROLE_ADMINISTRATOR_ID)
      ) {
        return {
          administrator: false,
      ...
      const responseTokenIsWorking =
        await context.xcyber360_core.api.client.asCurrentUser.request(
          'GET',
          '/',
          {},
          { apiHostID },
