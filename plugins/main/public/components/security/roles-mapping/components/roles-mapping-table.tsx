...
import {
  UI_LOGGER_LEVELS,
  XCYBER360_API_RESERVED_WUI_SECURITY_RULES,
    ...
    },
    {
      ...
      render(item, obj) {
        if (WzAPIUtils.isReservedID(item)) {
          if (XCYBER360_API_RESERVED_WUI_SECURITY_RULES.includes(obj.id)) {
            return (
              <EuiFlexGroup>
                <EuiBadge color='primary'>Reserved</EuiBadge>
                <EuiToolTip
                  position='top'
                  content='wui_ rules belong to xcyber360-wui API user'
                >
