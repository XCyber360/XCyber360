/*
 * Xcyber360 app - React component building the configuration component.
 *
...
const WzConfigurationSettingsProvider = props => {
  ...
  const {
    ...
    transformPluginSettingsToFormFields(
      ...
      Object.fromEntries(
        getXcyber360CorePlugin().configuration._settings.entries(),
      ),
    ),
  );
  ...
  const visibleSettings = Object.entries(fields).map(
    ([fieldKey, fieldForm]) => {
      const pluginSetting =
        getXcyber360CorePlugin().configuration._settings.get(fieldKey);
      const _categoryMeta = getXcyber360CorePlugin().configuration._categories.get(
  ...
  const visibleCategories = groupSettingsByCategory(
    ...
    Array.from(getXcyber360CorePlugin().configuration._categories.values()),
  );
    ...
    try {
      const settingsToUpdate = Object.entries(changed).reduce(
        (accum, [pluginSettingKey, currentValue]) => {
          const pluginSetting =
            getXcyber360CorePlugin().configuration._settings.get(pluginSettingKey);
            ...
            searchBarFilters={[
              {
                ...
                name: 'Categories',
                multiSelect: 'or',
                options: getXcyber360CorePlugin()
          ...
          {visibleCategories &&
            visibleCategories.map(({ category, settings }) => {
              const { description, documentationLink, title } = Array.from(
                getXcyber360CorePlugin().configuration._categories.values(),
