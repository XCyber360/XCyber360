/*
 * Xcyber360 app - Saved Objects management service
...
import {
  ...
  XCYBER360_INDEX_TYPE_ALERTS,
  XCYBER360_INDEX_TYPE_MONITORING,
  XCYBER360_INDEX_TYPE_STATISTICS,
...
export class SavedObject {
   ...
   */
  static async getListOfXcyber360ValidIndexPatterns(defaultIndexPatterns, where) {
    let result = [];
  ...
  /**
   * Creates the 'xcyber360-alerts-*'  index pattern
   */
  static async createXcyber360IndexPattern(pattern) {
    try {
      const fields = await SavedObject.getIndicesFields(
        pattern,
        XCYBER360_INDEX_TYPE_ALERTS,
  ...
  static getIndicesFields = async (pattern, indexType) => {
    try {
      ...
      switch (indexType) {
        case XCYBER360_INDEX_TYPE_MONITORING:
          return FieldsMonitoring;
        case XCYBER360_INDEX_TYPE_STATISTICS:
          return FieldsStatistics;
        case XCYBER360_INDEX_TYPE_ALERTS:
          return KnownFields;
        default:
   ...
   * It is usefull to validate if the endpoint works as expected. Related issue: https://github.com/XCyber360/XCyber360/tree/master/dashboard/plugins/issues/4293
...
Restart the ${PLUGIN_PLATFORM_NAME} service to initialize the index. More information in troubleshooting guide: ${webDocumentationLink(
        'user-manual/xcyber360-dashboard/troubleshooting.html#saved-object-for-index-pattern-not-found',
      )}.`);
    }
  }
