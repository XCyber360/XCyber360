/*
 * Xcyber360 app - Authentication service for Xcyber360
...
/**
 * Xcyber360 user authentication class
 */
export class WzAuthentication {
  ...
  static async refresh(force = false) {
    try {
      ...
      store.dispatch(
        updateUserAccount(
          getXcyber360CorePlugin().dashboardSecurity.getAccountFromJWTAPIDecodedToken(
            jwtPayload,
          ),
        ),
      );
  ...
  /**
   * Sends a request to the Xcyber360's API to delete the user's token.
   *
   * @returns {Object}
   */
