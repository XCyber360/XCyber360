/*
 * Xcyber360 app - Reporting service
...
export class ReportingService {
  ...
  removeAgentStatusVis(idArray) {
    const monitoringEnabled =
      this.xcyber360Config.getConfig()['xcyber360.monitoring.enabled'];
    if (!monitoringEnabled) {
      const visArray = idArray.filter(vis => {
        return vis !== 'Xcyber360-App-Overview-General-Agents-status';
      });
      return visArray;
    }
    return idArray;
    ...
    return await Promise.all(
      Array.from(domVisualizations).map(async node => {
        return {
          ...
          See https://github.com/XCyber360/XCyber360/tree/master/dashboard/plugins/issues/6900#issuecomment-2275495245
          */
