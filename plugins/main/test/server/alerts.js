...
const elasticServer = process.env.XCYBER360_ELASTIC_IP || 'localhost';
chai.should();
...
const index = `xcyber360-alerts-2018.${month >= 10 ? month : `0${month}`}.${
  day >= 10 ? day : `0${day}`
}`;
...
const syscheck = async agentID => {
  ...
  sample.source.should.be.eql('/var/xcyber360_server/logs/alerts/alerts.json');
  sample.location.should.be.eql('syscheck');
  sample.cluster.should.be.a('object');
  sample.cluster.name.should.be.eql('xcyber360');
