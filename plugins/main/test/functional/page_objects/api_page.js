...
export function ApiProvider({ getService, getPageObjects }) {
  ...
  class ApiPage {
    ...
    async completeApiForm(type = 'apiConfig') {
      ...
      await testSubjects.setValue(`${type}Username`, 'xcyber360');
      log.debug('insert the user');
      await testSubjects.setValue(`${type}Password`, 'xcyber360');
    ...
    async checkTabDisabled(tab) {
      ...
      expect(await browser.getCurrentUrl()).to.contain('app/xcyber360#/settings');
    }
