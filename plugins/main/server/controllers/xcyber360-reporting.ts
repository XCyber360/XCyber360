/*
 * Xcyber360 app - Class for Xcyber360 reporting controller
...
import {
  ...
  XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
  AUTHORIZED_AGENTS,
...
export class Xcyber360ReportingCtrl {
  constructor() {}
  /**
   * This do format to filters
   * @param {String} filters E.g: cluster.name: xcyber360 AND rule.groups: vulnerability
    ...
    try {
      ...
      if (section && typeof section === 'string') {
        if (!['agentConfig', 'groupConfig'].includes(section)) {
          printer.addContent({
            text: XCYBER360_MODULES[tab].title + ' report',
            style: 'h1',
          });
      ...
      if (XCYBER360_MODULES[tab] && XCYBER360_MODULES[tab].description) {
        printer.addContentWithNewLine({
          text: XCYBER360_MODULES[tab].description,
          style: 'standard',
        });
      }
    } catch (error) {
  ...
  private getConfigRows(context, data, labels) {
    context.xcyber360.logger.debug('Building configuration rows');
    const result = [];
  ...
  private getConfigTables(context, data, section, tab, array = []) {
    context.xcyber360.logger.debug('Building configuration tables');
    let plainData = {};
    const nestedData = [];
    const tableData = [];
  ...
  createReportsModules = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.xcyber360.logger.debug('Report started');
        const {
          array,
          agents,
          browserTimezone,
        ...
        const printer = new ReportPrinter(
          context.xcyber360.logger.get('report-printer'),
          context.xcyber360_core.configuration,
        );

        createDataDirectoryIfNotExists();
        createDirectoryIfNotExists(XCYBER360_DATA_DOWNLOADS_DIRECTORY_PATH);
        createDirectoryIfNotExists(XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH);
        createDirectoryIfNotExists(
          path.join(
            XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
            context.xcyber360EndpointParams.hashUsername,
          ),
        );
        ...
        if (time) {
          additionalTables = await extendedInformation(
            ...
            serverSideQuery,
            indexPatternTitle ||
              context.xcyber360_core.configuration.getSettingValue('pattern'),
            agents,
          );
        }
        ...
        await printer.print(context.xcyber360EndpointParams.pathFilename);

        return response.ok({
          body: {
            success: true,
            message: `Report ${context.xcyber360EndpointParams.filename} was created`,
            filename: context.xcyber360EndpointParams.filename,
          },
        });
      } catch (error) {
  ...
  createReportsGroups = this.checkReportsUserDirectoryIsValidRouteDecorator(
    ...
    ({ params: { groupID } }) =>
      `xcyber360-group-configuration-${groupID}-${this.generateReportTimestamp()}.pdf`,
  );
  ...
  createReportsAgentsConfiguration =
    this.checkReportsUserDirectoryIsValidRouteDecorator(
      async (
        ...
        try {
          ...
          try {
            ...
            context.xcyber360.logger.debug(error.message || error);
          }
      ...
      ({ params: { agentID } }) =>
        `xcyber360-agent-configuration-${agentID}-${this.generateReportTimestamp()}.pdf`,
    );
    ...
    this.checkReportsUserDirectoryIsValidRouteDecorator(
      async (
        ...
        try {
          ...
          const { hashUsername } = await context.xcyber360.security.getCurrentUser(
            request,
            context,
          );
          createDataDirectoryIfNotExists();
          ...
          context.xcyber360.logger.debug('Syscollector report');
          const [sanitizedFilters, agentsFilter] = filters
            ? this.sanitizeKibanaFilters(context, filters, searchBar)
            : [false, null];
          ...
          const requestInventory = async agentRequestInventory => {
            try {
              context.xcyber360.logger.debug(agentRequestInventory.loggerMessage);
      ...
      ({ params: { agentID } }) =>
        `xcyber360-agent-inventory-${agentID}-${this.generateReportTimestamp()}.pdf`,
    );
  ...
  async getReports(
    ...
    try {
      context.xcyber360.logger.debug('Fetching created reports');
      const { hashUsername } = await context.xcyber360.security.getCurrentUser(
        request,
        context,
      );
      ...
      context.xcyber360.logger.debug(`Directory: ${userReportsDirectoryPath}`);
      ...
      context.xcyber360.logger.debug(`Total reports: ${reports.length}`);
      return response.ok({
        body: { reports },
      });
    } catch (error) {
  ...
  getReportByName = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.xcyber360.logger.debug(
          `Getting ${context.xcyber360EndpointParams.pathFilename} report`,
        );
        const reportFileBuffer = fs.readFileSync(
          context.xcyber360EndpointParams.pathFilename,
        );
        return response.ok({
  ...
  deleteReportByName = this.checkReportsUserDirectoryIsValidRouteDecorator(
    async (
      ...
      try {
        context.xcyber360.logger.debug(
          `Deleting ${context.xcyber360EndpointParams.pathFilename} report`,
        );
        fs.unlinkSync(context.xcyber360EndpointParams.pathFilename);
        context.xcyber360.logger.info(
          `${context.xcyber360EndpointParams.pathFilename} report was deleted`,
        );
        return response.ok({
          body: { error: 0 },
        });
    ...
    return async (
      ...
      try {
        const { username, hashUsername } =
          await context.xcyber360.security.getCurrentUser(request, context);
        const userReportsDirectoryPath = path.join(
          XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
          hashUsername,
        );
        ...
        return await routeHandler.bind(this)(
          {
            ...context,
            xcyber360EndpointParams: { hashUsername, filename, pathFilename },
          },
          request,
          response,
        );
