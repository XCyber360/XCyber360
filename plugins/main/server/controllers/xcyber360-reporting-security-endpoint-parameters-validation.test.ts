...
import { Xcyber360ReportingRoutes } from '../routes/xcyber360-reporting';
import md5 from 'md5';
import {
...
beforeAll(async () => {
  // Create <PLUGIN_PLATFORM_PATH>/data/xcyber360 directory.
  createDataDirectoryIfNotExists();
  // Create <PLUGIN_PLATFORM_PATH>/data/xcyber360/downloads directory.
  createDirectoryIfNotExists(XCYBER360_DATA_DOWNLOADS_DIRECTORY_PATH);
  // Create <PLUGIN_PLATFORM_PATH>/data/xcyber360/downloads/reports directory.
  createDirectoryIfNotExists(XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH);
  // Create report files
  [
    { name: md5('admin'), files: ['xcyber360-module-overview-general-1234.pdf'] },
    {
      name: md5('../../etc'),
      files: ['xcyber360-module-overview-general-1234.pdf'],
    },
  ].forEach(({ name, files }) => {
    createDirectoryIfNotExists(
      path.join(XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, name),
    );
  ...
  // Register routes
  Xcyber360ReportingRoutes(router);
...
afterAll(async () => {
  // Remove <PLUGIN_PLATFORM_PATH>/data/xcyber360 directory.
  execSync(`rm -rf ${XCYBER360_DATA_ABSOLUTE_PATH}`);
...
describe('[endpoint][security] GET /reports/{name} - Parameters validation', () => {
  it.each`
    ...
    ${'Get report by filename'} | ${'admin'}     | ${'xcyber360-module-overview-general-1234.pdf'}               | ${200}             | ${null}
    ${'Invalid parameters'}     | ${'admin'}     | ${'..%2fxcyber360-module-overview-general-1234.pdf'}          | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'}     | ${'admin'}     | ${'custom..%2fxcyber360-module-overview-general-1234.pdf'}    | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}        | ${'admin'}     | ${'../custom..%2fxcyber360-module-overview-general-1234.pdf'} | ${404}             | ${/Not Found/}
    ${'Get report by filename'} | ${'../../etc'} | ${'xcyber360-module-overview-general-1234.pdf'}               | ${200}             | ${null}
    ${'Invalid parameters'}     | ${'../../etc'} | ${'..%2fxcyber360-module-overview-general-1234.pdf'}          | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'}     | ${'../../etc'} | ${'custom..%2fxcyber360-module-overview-general-1234.pdf'}    | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}        | ${'../../etc'} | ${'../custom..%2fxcyber360-module-overview-general-1234.pdf'} | ${404}             | ${/Not Found/}
  `(
    ...
    async ({
      ...
      const response = await supertest(innerServer.listener)
        ...
        .send({
          ...
          tables: [],
          section: 'overview',
          indexPatternTitle: 'xcyber360-alerts-*',
          apiId: 'default',
        })
        .expect(responseStatusCode);
      if (responseBodyMessage) {
...
describe('[endpoint][security] DELETE /reports/{name} - Parameters validation', () => {
  it.each`
    testTitle               | username       | filename                                               | responseStatusCode | responseBodyMessage
    ${'Delete report file'} | ${'admin'}     | ${'xcyber360-module-overview-general-1234.pdf'}            | ${200}             | ${null}
    ${'Invalid parameters'} | ${'admin'}     | ${'..%2fxcyber360-module-overview-general-1234.pdf'}       | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'} | ${'admin'}     | ${'custom..%2fxcyber360-module-overview-general-1234.pdf'} | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}    | ${'admin'}     | ${'../xcyber360-module-overview-general-1234.pdf'}         | ${404}             | ${/Not Found/}
    ${'Delete report file'} | ${'../../etc'} | ${'xcyber360-module-overview-general-1234.pdf'}            | ${200}             | ${null}
    ${'Invalid parameters'} | ${'../../etc'} | ${'..%2fxcyber360-module-overview-general-1234.pdf'}       | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Invalid parameters'} | ${'../../etc'} | ${'custom..%2fxcyber360-module-overview-general-1234.pdf'} | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
    ${'Route not found'}    | ${'../../etc'} | ${'../xcyber360-module-overview-general-1234.pdf'}         | ${404}             | ${/Not Found/}
  `(
