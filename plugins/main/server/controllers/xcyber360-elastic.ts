/*
 * Xcyber360 app - Class for Xcyber360-Elastic functions
...
import {
  XCYBER360_SAMPLE_ALERTS_INDEX_SHARDS,
  XCYBER360_SAMPLE_ALERTS_INDEX_REPLICAS,
...
import {
  XCYBER360_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS,
  XCYBER360_SAMPLE_ALERTS_DEFAULT_NUMBER_ALERTS,
} from '../../common/constants';
import { XCYBER360_INDEXER_NAME } from '../../common/constants';
import { routeDecoratorProtectedAdministrator } from './decorators';

export class Xcyber360ElasticCtrl {
  constructor() {}
  ...
  async buildSampleIndexByCategory(
    ...
    return `${await context.xcyber360_core.configuration.get(
  ...
  async getTemplate(
    ...
    try {
      ...
      if (!templates || typeof templates !== 'string') {
        throw new Error(
          `An unknown error occurred when fetching templates from ${XCYBER360_INDEXER_NAME}`,
        );
      }
      ...
      return ErrorResponse(
        `Could not retrieve templates from ${XCYBER360_INDEXER_NAME} due to ${
          error.message || error
        }`,
        4002,
        500,
  ...
  async getCurrentPlatform(
    ...
    try {
      return response.ok({
        body: {
          platform: context.xcyber360.security.platform,
        },
      });
    } catch (error) {
      context.xcyber360.logger.error(error.message || error);
   ...
   * @param {String} id Index-pattern id to use in the visualizations. Eg: 'xcyber360-alerts'
   */
  async buildVisualizationsRaw(context, app_objects, id, namespace = false) {
    const config = await context.xcyber360_core.configuration.get();
    let monitoringPattern = `${config['xcyber360.monitoring.pattern']}`;
    context.xcyber360.logger.debug(`Building ${app_objects.length} visualizations`);
    context.xcyber360.logger.debug(`Index pattern ID: ${id}`);
    const visArray = [];
    ...
    for (let element of app_objects) {
      ...
      if (
        ...
        const isMonitoring = defaultStr.includes('xcyber360-monitoring');
        if (isMonitoring) {
          ...
          aux_source.kibanaSavedObjectMeta.searchSourceJSON =
            defaultStr.replace(/xcyber360-alerts/g, id);
        }
      }

      // Replace index-pattern for selector visualizations
      if (typeof (aux_source || {}).visState === 'string') {
        aux_source.visState = aux_source.visState.replace(/xcyber360-alerts/g, id);
      }
   ...
   * @param {String} name Cluster name. Eg: 'xcyber360'
  ...
  buildClusterVisualizationsRaw(
    ...
    try {
      ...
      for (const element of app_objects) {
        ...
        aux_source = aux_source.replace(/xcyber360-alerts/g, id);
  ...
  async haveSampleAlerts(
    ...
    try {
      // Check if xcyber360 sample alerts index exists
      const results = await Promise.all(
        Object.keys(XCYBER360_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS).map(
          async category =>
  ...
  /**
   * This creates sample alerts in xcyber360-sample-alerts
  ...
  createSampleAlerts = routeDecoratorProtectedAdministrator(1000)(
    async (
      ...
      try {
        ...
        const sampleAlerts = XCYBER360_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS[
          request.params.category
        ]
          .map(typeAlert =>
            generateAlerts(
        ...
        if (!existsSampleIndex.body) {
          // Create xcyber360 sample alerts index

          const configuration = {
            settings: {
              index: {
                number_of_shards: XCYBER360_SAMPLE_ALERTS_INDEX_SHARDS,
                number_of_replicas: XCYBER360_SAMPLE_ALERTS_INDEX_REPLICAS,
              },
            },
          };
          ...
          context.xcyber360.logger.info(`Index ${sampleAlertsIndex} created`);
        }
  ...
  deleteSampleAlerts = routeDecoratorProtectedAdministrator(1000)(
    async (
      ...
      // Delete Xcyber360 sample alert index
      const sampleAlertsIndex = await this.buildSampleIndexByCategory(
        context,
        request.params.category,
      );
      ...
      try {
        ...
        if (existsSampleIndex.body) {
          // Delete Xcyber360 sample alerts index
          await context.core.opensearch.client.asCurrentUser.indices.delete({
            index: sampleAlertsIndex,
          });
          context.xcyber360.logger.info(`Deleted ${sampleAlertsIndex} index`);
          return response.ok({
