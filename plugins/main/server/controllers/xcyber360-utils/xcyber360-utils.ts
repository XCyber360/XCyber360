...
export class Xcyber360UtilsCtrl {
  /**
   * Constructor
   * @param {*} server
   */
  ...
  async getConfiguration(
    ...
    try {
      context.xcyber360.logger.debug('Getting configuration');
      const configuration = await context.xcyber360_core.configuration.get();
  ...
  updateConfiguration = compose(
    ...
    async (
      ...
      const { requirements, update: updatedConfiguration } =
        await context.xcyber360_core.configuration.set(updatedSettings);
      context.xcyber360.logger.debug('Configuration updated');
  ...
  uploadFile = compose(
    ...
    async (
      ...
      const pluginSetting = context.xcyber360_core.configuration._settings.get(key);
      ...
      context.xcyber360.logger.debug(`Directory: ${targetDirectory}`);
      ...
      context.xcyber360.logger.debug(`Storing file on : ${files.join(', ')}`);
      ...
      const { requirements, update } =
        await context.xcyber360_core.configuration.set(updatedConfiguration);
  ...
  deleteFile = compose(
    ...
    async (
      ...
      // Update the setting in the configuration cache
      const { requirements, update } =
        await context.xcyber360_core.configuration.clear(key);
