/*
 * Xcyber360 app - Add delayed jobs to a queue.
...
import { XCYBER360_QUEUE_CRON_FREQ } from '../../../common/constants';
...
async function executePendingJobs(context: any) {
  try {
    ...
    context.xcyber360.logger.debug(`Pending jobs: ${pendingJobs.length}`);
...
export function jobQueueRun(context) {
  cron.schedule(XCYBER360_QUEUE_CRON_FREQ, async () => {
    try {
