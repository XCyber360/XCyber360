...
def getXcyber360Info(xcyber360_home):
    xcyber360_control = os.path.join(xcyber360_home, "bin", "xcyber360-control")
    xcyber360_env_vars = {}
    try:
        proc = subprocess.Popen([xcyber360_control, "info"], stdout=subprocess.PIPE)
        (stdout, stderr) = proc.communicate()
    except Exception as e:
        print("Seems like there is no Xcyber360 installation.")
        return None
...
def provisionDR():
    ...
    for file in os.listdir(rules_dir):
        ...
        if os.path.isfile(file_fullpath) and re.match(r'^test_(.*_)?rules.xml$', file):
            shutil.copy2(file_fullpath, args.xcyber360_home + "/etc/rules")
    ...
    for file in os.listdir(decoders_dir):
        ...
        if os.path.isfile(file_fullpath) and re.match(r'^test_(.*_)?decoders.xml$', file):
            shutil.copy2(file_fullpath, args.xcyber360_home + "/etc/decoders")



def cleanDR():
    rules_dir = args.xcyber360_home + "/etc/rules"
    decoders_dir = args.xcyber360_home + "/etc/decoders"
...
class OssecTester(object):
    ...
    def buildCmd(self, rule, alert, decoder):
        cmd = ['%s/xcyber360-logtest' % (self._ossec_path), ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='This script tests Xcyber360 rules.')
    parser.add_argument('--path', '-p', default='/var/ossec', dest='xcyber360_home',
                        help='Use -p or --path to specify Xcyber360 installation path')
    ...
    xcyber360_info = getXcyber360Info(args.xcyber360_home)
    if xcyber360_info is None:
        sys.exit(1)
    ...
    provisionDR()
    OT = OssecTester(args.xcyber360_home)
