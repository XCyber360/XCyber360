...
def generateNodeNames():
    nodes = ['xcyber360', 'node1', 'node2', 'node3', 'node4', 'node5']
    return nodes
...
def generateRandomXcyber360():
    xcyber360 = {}
    xcyber360['cluster'] = {'name':random.choice(['xcyber360.manager', 'xcyber360']), 'node':random.choice(['master','worker-01','worker-02','worker-03'])}
    return(xcyber360)
...
def generateRandomData(number):
    for i in range(0, int(number)):    
        yield {
            ...
            'xcyber360': generateRandomXcyber360()
        }
...
def verifySettings():
    ...
    if not verified:
        ...
        index = input("\nEnter the index name [default=xcyber360-fleet-groups-default]: \n")
        if index == '':
            index = 'xcyber360-fleet-groups-default'
