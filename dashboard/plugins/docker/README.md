...
5.  Save the path to the Xcyber360 App code as an environment variable, by exporting
    this path on your `.bashrc`, `.zhsrc` or similar.

    ```bash
    # ./bashrc
    export WZ_HOME=~/your/path/to/xcyber360_kibana_app/plugins
    ```
...
`1.2.0`, with the `xcyber360-dashboard-plugins` development branch set up at
`$WZ_HOME`:
...
move to the `kbn\plugins\xcyber360` and run `yarn` to install the dependencies of
...
These images can be downloaded from the [quay.io/xcyber360][4] registry.
...
```bash
cd images
docker build --build-arg NODE_VERSION=14.20.1 --build-arg OPENSEARCH_VERSION=2.6.0 -t quay.io/xcyber360/osd-dev:2.6.0 -f osd-dev.Dockerfile .
cd ..
```
...
```bash
docker push quay.io/xcyber360/image-name:version
```
...
This will bring up a development environment for Xcyber360 using the given
...
This will bring up a development environment for Xcyber360 using the
Kibana development container versions of the 7 series and 8 series.

### Xcyber360 4.3.X testing environment with Elastic Stack

Folder: [xcyber360-4.3-es](./xcyber360-4.3-es)
...
### Xcyber360 4.3.X testing environment with xcyber360-dashboard and xcyber360-indexer

Folder: [xcyber360-4.3-wz](./xcyber360-4.3-wz)
...
### Xcyber360 4.2.X testing environment with Elastic Stack

Folder: [xcyber360-4.2-es](./xcyber360-4.2-es)

### Xcyber360 4.2.X testing environment with OpenDistro

Folder: [xcyber360-4.2-od](./xcyber360-4.2-od)

### Xcyber360 3.13.X testing environment with Elastic Stack

Folder: [xcyber360-3.13.X-es](./xcyber360-3.13.X-es)

### Xcyber360 3.13.X testing environment with OpenDistro

Folder: [xcyber360-3.13.X-od](./xcyber360-3.13.X-od)
...
[4]: https://quay.io/organization/xcyber360 'quay.io/xcyber360'
[5]: https://github.com/XCyber360/XCyber360/tree/master/dashboard/plugins/issues/3872#issuecomment-1305507626 'App permissions'
