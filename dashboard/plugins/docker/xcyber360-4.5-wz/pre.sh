...
usage() {
	...
	echo "  xcyber360_api_version is the patch version of xcyber360 4.5, for example " ${xcyber360_api_version[*]}
...
cat <<EOF >config/imposter/api_info.json
{
  "data": {
    ...
    "license_url": "https://github.com/xcyber360/xcyber360/blob/4.5/LICENSE",
    "hostname": "imposter",
...
up)
	...
	echo docker cp xcyber360-4.5.${patch_version}-1.zip ${COMPOSE_PROJECT_NAME}-xcyber360.dashboard-1:/tmp
	echo
	echo "4. Install the package we have just uploaded:"
	echo "docker exec -ti  ${COMPOSE_PROJECT_NAME}-xcyber360.dashboard-1  /usr/share/xcyber360-dashboard/bin/opensearch-dashboards-plugin install file:///tmp/xcyber360-4.5.${patch_version}-1.zip"
	echo
