...
usage() {
  echo
  echo "./dev.sh os_version osd_version /xcyber360_app_src action [saml/server] [server_version]"
  echo
  echo "where"
  echo "  os_version is one of " ${os_versions[*]}
  echo "  osd_version is one of " ${osd_versions[*]}
  echo "  xcyber360_app_src is the path to the xcyber360 application source code"
...
profile="standard"
export XCYBER360_DASHBOARD_CONF=./config/${OSD_MAJOR}/osd/opensearch_dashboards.yml
export SEC_CONFIG_FILE=./config/${OSD_MAJOR}/os/config.yml
if [[ "$5" =~ "saml" ]]; then
  cat /etc/hosts | grep -q "idp" || exit_with_message "Add idp to /etc/hosts"

  profile="saml"
  export XCYBER360_DASHBOARD_CONF=./config/${OSD_MAJOR}/osd/opensearch_dashboards_saml.yml
...
if [[ "$5" =~ "server" ]]; then
  ...
  if [[ ! " ${wzs_version[*]} " =~ " ${6} " ]]; then
    echo "Xcyber360 server version ${6} not found in ${wzs_version[*]}"
    echo
    exit -1
  fi
  export XCYBER360_STACK="${6}"
fi
...
up)
  ...
  if [[ "$5" =~ "server" ]]; then
    ...
    echo "docker run --name ${COMPOSE_PROJECT_NAME}-agent-\$(date +%s) --network os-dev-${OS_VERSION} --label com.docker.compose.project=${COMPOSE_PROJECT_NAME} --env XCYBER360_AGENT_VERSION=${XCYBER360_STACK} -d ubuntu:20.04 bash -c '"
    echo "  apt update -y"
    echo "  apt install -y curl lsb-release"
    echo "  curl -so \xcyber360-agent-\${XCYBER360_AGENT_VERSION}.deb \\"
    echo "    https://packages.xcyber360.com/4.x/apt/pool/main/w/xcyber360-agent/xcyber360-agent_\${XCYBER360_AGENT_VERSION}-1_amd64.deb \\"
    echo "    && XCYBER360_MANAGER='xcyber360.manager' XCYBER360_AGENT_GROUP='default' dpkg -i ./xcyber360-agent-\${XCYBER360_AGENT_VERSION}.deb"
    echo
