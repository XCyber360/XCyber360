Feature: Validate that the config to added new agent
    ...
    Scenario Outline: Validate the information to add a new Agent
        ...
        Examples:
            ...
            | 'Choose the Operating system' | 'Red Hat / CentOS,Debian / Ubuntu,Windows,MacOS' | 'Xcyber360 server address' | 'This is the address the agent uses to communicate with the Xcyber360 server. It can be an IP address or a fully qualified domain name (FQDN).' | '172.19.0.4       ' | 'Assign the agent to a group' | 'Select one or more existing groups' | 'Select group    ' | 'Install and enroll the agent' | 'Please select the Operating system.' |
