  ...
  try {
    ...
    if (agentsData.length) {
      ...
      printer.addSimpleTable({
        ...
        items: agentsData
          .filter(agent => agent) // Remove undefined agents when Xcyber360 API no longer finds and agentID
          .map(agent => {
            return {
              ...agent,
              os:
...
export async function extendedInformation(
  ...
  try {
    ...
    const agents = await context.xcyber360.api.client.asCurrentUser.request(
      'GET',
      '/agents',
      { params: { limit: 1 } },
      { apiHostID: apiId },
