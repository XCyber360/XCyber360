// To run this file:
// yarn test:jest --testEnvironment node --verbose server/routes/xcyber360-utils
...
import { Xcyber360UtilsRoutes } from './xcyber360-utils';
import fs from 'fs';
...
// Register settings
context.xcyber360_core.configuration._settings.set('pattern', {
  validate: () => undefined,
...
beforeAll(async () => {
  ...
  // Register routes
  Xcyber360UtilsRoutes(router, { configuration: context.xcyber360_core.configuration });
...
describe('[endpoint] PUT /utils/configuration - protected route', () => {
  ...
  `(
    ...
    async ({
      ...
      context.xcyber360_core.configuration.get.mockReturnValueOnce(
        isConfigurationAPIEditable,
      );
      context.xcyber360_core.dashboardSecurity.isAdministratorUser.mockReturnValueOnce(
        {
...
describe.skip('[endpoint] GET /utils/configuration', () => {
  it(`Get plugin configuration and ensure the hosts is not returned GET /utils/configuration - 200`, async () => {
    ...
    context.xcyber360_core.configuration.get.mockReturnValueOnce(initialConfig);
    const response = await supertest(innerServer.listener)
...
describe.skip('[endpoint] PUT /utils/configuration', () => {
  beforeAll(() => {
    context.xcyber360_core.configuration._settings = new Map();
    context.xcyber360_core.configuration._settings.set('pattern', {
      isConfigurableFromSettings: true,
    });
    ...
    context.xcyber360_core.configuration._settings.set('timeout', {
      isConfigurableFromSettings: true,
    });
    context.xcyber360_core.configuration._settings.set('cron.statistics.apis', {
      isConfigurableFromSettings: true,
    });
  });
  ...
  afterAll(() => {
    ...
    context.xcyber360_core.configuration._settings = null;
  });
  ...
  `(
    ...
    async ({ responseStatusCode, settings }) => {
      ...
      context.xcyber360_core.configuration.get.mockReturnValueOnce(initialConfig);
      context.xcyber360_core.configuration.set.mockReturnValueOnce(settings);
      const response = await supertest(innerServer.listener)
        .put('/utils/configuration')
        .send(settings)
        .expect(responseStatusCode);
  ...
  it.skip.each`
    ...
    ${'xcyber360.monitoring.creation'}      | ${'h'}                                                           | ${200}             | ${null}
    ${'xcyber360.monitoring.creation'}      | ${'d'}                                                           | ${200}             | ${null}
    ${'xcyber360.monitoring.creation'}      | ${'w'}                                                           | ${200}             | ${null}
    ${'xcyber360.monitoring.creation'}      | ${'m'}                                                           | ${200}             | ${null}
    ${'xcyber360.monitoring.creation'}      | ${'test'}                                                        | ${400}             | ${'[request body.xcyber360.monitoring.creation]: types that failed validation:\n- [request body.xcyber360.monitoring.creation.0]: expected value to equal [h]\n- [request body.xcyber360.monitoring.creation.1]: expected value to equal [d]\n- [request body.xcyber360.monitoring.creation.2]: expected value to equal [w]\n- [request body.xcyber360.monitoring.creation.3]: expected value to equal [m]'}
    ${'xcyber360.monitoring.enabled'}       | ${true}                                                          | ${200}             | ${null}
    ${'xcyber360.monitoring.enabled'}       | ${0}                                                             | ${400}             | ${'[request body.xcyber360.monitoring.enabled]: expected value of type [boolean] but got [number]'}
    ${'xcyber360.monitoring.frequency'}     | ${100}                                                           | ${200}             | ${null}
    ${'xcyber360.monitoring.frequency'}     | ${40}                                                            | ${400}             | ${'[request body.xcyber360.monitoring.frequency]: Value should be greater or equal than 60.'}
    ${'xcyber360.monitoring.frequency'}     | ${1.2}                                                           | ${400}             | ${'[request body.xcyber360.monitoring.frequency]: Number should be an integer.'}
    ${'xcyber360.monitoring.frequency'}     | ${''}                                                            | ${400}             | ${'[request body.xcyber360.monitoring.frequency]: expected value of type [number] but got [string]'}
    ${'xcyber360.monitoring.pattern'}       | ${'test'}                                                        | ${200}             | ${null}
    ${'xcyber360.monitoring.pattern'}       | ${'test*'}                                                       | ${200}             | ${null}
    ${'xcyber360.monitoring.pattern'}       | ${''}                                                            | ${400}             | ${'[request body.xcyber360.monitoring.pattern]: value has length [0] but it must have a minimum length of [1].'}
    ${'xcyber360.monitoring.pattern'}       | ${true}                                                          | ${400}             | ${'[request body.xcyber360.monitoring.pattern]: expected value of type [string] but got [boolean]'}
    ${'xcyber360.monitoring.pattern'}       | ${'-test'}                                                       | ${400}             | ${"[request body.xcyber360.monitoring.pattern]: It can't start with: -, _, +, .."}
    ${'xcyber360.monitoring.pattern'}       | ${'_test'}                                                       | ${400}             | ${"[request body.xcyber360.monitoring.pattern]: It can't start with: -, _, +, .."}
    ${'xcyber360.monitoring.pattern'}       | ${'+test'}                                                       | ${400}             | ${"[request body.xcyber360.monitoring.pattern]: It can't start with: -, _, +, .."}
    ${'xcyber360.monitoring.pattern'}       | ${'.test'}                                                       | ${400}             | ${"[request body.xcyber360.monitoring.pattern]: It can't start with: -, _, +, .."}
    ${'xcyber360.monitoring.pattern'}       | ${'test\\'}                                                      | ${400}             | ${'[request body.xcyber360.monitoring.pattern]: It can\'t contain invalid characters: \\, /, ?, ", <, >, |, ,, #.'}
    ${'xcyber360.monitoring.pattern'}       | ${'test/'}                                                       | ${400}             | ${'[request body.xcyber360.monitoring.pattern]: It can\'t contain invalid characters: \\, /, ?, ", <, >, |, ,, #.'}
    ${'xcyber360.monitoring.pattern'}       | ${'test?'}                                                       | ${400}             | ${'[request body.xcyber360.monitoring.pattern]: It can\'t contain invalid characters: \\, /, ?, ", <, >, |, ,, #.'}
    ${'xcyber360.monitoring.pattern'}       | ${'test"'}                                                       | ${400}             | ${'[request body.xcyber360.monitoring.pattern]: It can\'t contain invalid characters: \\, /, ?, ", <, >, |, ,, #.'}
    ${'xcyber360.monitoring.pattern'}       | ${'test<'}                                                       | ${400}             | ${'[request body.xcyber360.monitoring.pattern]: It can\'t contain invalid characters: \\, /, ?, ", <, >, |, ,, #.'}
    ...
    ${'xcyber360.monitoring.pattern'}       | ${'test,'}                                                       | ${400}             | ${'[request body.xcyber360.monitoring.pattern]: It can\'t contain invalid characters: \\, /, ?, ", <, >, |, ,, #.'}
    ${'xcyber360.monitoring.pattern'}       | ${'test#'}                                                       | ${400}             | ${'[request body.xcyber360.monitoring.pattern]: It can\'t contain invalid characters: \\, /, ?, ", <, >, |, ,, #.'}
    ${'xcyber360.monitoring.replicas'}      | ${0}                                                             | ${200}             | ${null}
    ${'xcyber360.monitoring.replicas'}      | ${-1}                                                            | ${400}             | ${'[request body.xcyber360.monitoring.replicas]: Value should be greater or equal than 0.'}
    ${'xcyber360.monitoring.replicas'}      | ${1.2}                                                           | ${400}             | ${'[request body.xcyber360.monitoring.replicas]: Number should be an integer.'}
    ${'xcyber360.monitoring.replicas'}      | ${'custom'}                                                      | ${400}             | ${'[request body.xcyber360.monitoring.replicas]: expected value of type [number] but got [string]'}
    ${'xcyber360.monitoring.shards'}        | ${1}                                                             | ${200}             | ${null}
    ${'xcyber360.monitoring.shards'}        | ${-1}                                                            | ${400}             | ${'[request body.xcyber360.monitoring.shards]: Value should be greater or equal than 1.'}
    ${'xcyber360.monitoring.shards'}        | ${1.2}                                                           | ${400}             | ${'[request body.xcyber360.monitoring.shards]: Number should be an integer.'}
    ${'xcyber360.monitoring.shards'}        | ${'custom'}                                                      | ${400}             | ${'[request body.xcyber360.monitoring.shards]: expected value of type [number] but got [string]'}
  `(
