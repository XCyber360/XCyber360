// To launch this file
// yarn test:jest --testEnvironment node --verbose server/routes/xcyber360-reporting
...
import { Xcyber360UtilsCtrl } from '../controllers/xcyber360-utils/xcyber360-utils';
import md5 from 'md5';
...
beforeAll(async () => {
  ...
  // Create <PLUGIN_PLATFORM_PATH>/data/xcyber360/config directory.
  createDirectoryIfNotExists(XCYBER360_DATA_CONFIG_DIRECTORY_PATH);
...
describe('[endpoint] GET /reports', () => {
  ...
  beforeAll(() => {
    ...
    directories.forEach(({ username, files }) => {
      ...
      createDirectoryIfNotExists(
        path.join(XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, hashUsername),
      );
      if (files) {
  ...
  afterAll(async () => {
    // Remove <PLUGIN_PLATFORM_PATH>/data/xcyber360/downloads directory.
    execSync(`rm -rf ${XCYBER360_DATA_DOWNLOADS_DIRECTORY_PATH}`);
  });
...
describe('[endpoint] PUT /utils/configuration', () => {
  ...
  beforeAll(() => {
    ...
    Object.entries(SettingsDefinitions).forEach(([key, value]) =>
      context.xcyber360_core.configuration._settings.set(key, value),
    );
  });
  ...
  `(
    ...
    async ({ footer, header, responseStatusCode, expectedMD5, tab }) => {
      ...
      context.xcyber360_core.configuration.getCustomizationSetting.mockImplementation(
        (...settings) => ({
          then: fn =>
            fn(
              Object.fromEntries(
      ...
      context.xcyber360_core.dashboardSecurity.isAdministratorUser.mockImplementation(
        () => ({ administrator: true }),
      );

      // Set custom report header and footer
      if (typeof footer === 'string' || typeof header === 'string') {
        context.xcyber360_core.configuration.set.mockReturnValueOnce({
      ...
      const reportPath = `${XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH}/${userPath}/${fileName}`;
