...
import {
  XCYBER360_STATISTICS_DEFAULT_INDICES_SHARDS,
  XCYBER360_STATISTICS_DEFAULT_INDICES_REPLICAS,
...
export class SaveDocument {
  ...
  async save(doc: object[], indexConfig: IIndexConfiguration) {
    ...
    try {
      ...
      this.context.xcyber360.logger.debug('Bulk data');
      const response = await this.esClientInternalUser.bulk(
        createDocumentObject,
      );
      this.context.xcyber360.logger.debug(
  ...
  private async checkIndexAndCreateIfNotExists(index, shards, replicas) {
    try {
      await tryCatchForIndexPermissionError(index)(async () => {
        ...
        if (!exists.body) {
          this.context.xcyber360.logger.debug(`Creating ${index} index`);
          await this.esClientInternalUser.indices.create({
            ...
            body: {
              settings: {
                index: {
                  number_of_shards:
                    shards ?? XCYBER360_STATISTICS_DEFAULT_INDICES_SHARDS,
                  number_of_replicas:
                    replicas ?? XCYBER360_STATISTICS_DEFAULT_INDICES_REPLICAS,
                },
              },
            },
          });
          this.context.xcyber360.logger.info(`${index} index created`);
        }
      })();
    } catch (error) {
  ...
  private async addIndexPrefix(index): string {
    const prefix = await this.context.xcyber360_core.configuration.get(
      'cron.prefix',
    );
    return `${prefix}-${index}`;
  }
