...
import { XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH } from '../../../common/constants';
...
export default function migrateReportsDirectoryName(context) {
  ...
  const createLog = (level: string) => message => {
    context.xcyber360.logger[level](`migration:reportsDirectoryName: ${message}`);
  };
  ...
  try {
    ...
    if (!fs.existsSync(XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH)) {
      logger.debug(
    ...
    logger.debug(
      `Reading reports directory: ${XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH}`,
    );
    fs.readdirSync(XCYBER360_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, {
      withFileTypes: true,
    }).forEach(fileDirent => {
