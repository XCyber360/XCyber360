...
export default async function sanitizeUploadedSVG(context) {
  ...
  const createLog = (level: string) => message => {
    context.xcyber360.logger[level](`sanitize:sanitizeUploadedSVG: ${message}`);
  };
  ...
  try {
    ...
    logosSettingKeys.forEach(async logoKey => {
      const logoSetting =
        context.xcyber360_core.configuration._settings.get(logoKey);
      ...
      if (originalSVGString.replace(/\s/g, '') != cleanSVG.replace(/\s/g, '')) {
        ...
        await context.xcyber360_core.configuration.set({
