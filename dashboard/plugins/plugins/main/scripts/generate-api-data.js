...
node ${cliFilePath} --spec https://raw.githubusercontent.com/xcyber360/xcyber360/master/api/api/spec/spec.yaml --output file --output-directory common/api-info

- Get API data from URL spec file using the plugin version and save to files. Run from project root path.
node ${cliFilePath} --spec https://raw.githubusercontent.com/xcyber360/xcyber360/$(node -e \"console.log(require('./package.json').version.split('.').splice(0,2).join('.'))\")/api/api/spec/spec.yaml --output file --output-directory common/api-info

- Unused: Get API data from spec file and print to stdout (API branch master). Run from project root path.
node ${cliFilePath} --spec https://raw.githubusercontent.com/xcyber360/xcyber360/master/api/api/spec/spec.yaml --output stdout
  ...
  try {
    ...
    function extractDataEndpoints(spec, configuration) {
      ...
      function generateEndpointDocumentationLink(operationId) {
        return `https://documentation.xcyber360.com/${apiVersionShort}/user-manual/api/reference.html#operation/${operationId}`;
      }
      ...
      const endpoints = Object.entries(spec.paths).reduce(
        (accum, [endpointPath, endpointData]) => {
          Object.keys(endpointData).forEach(httpMethod => {
            ...
            //Xcyber360 prefix is removed due issue [#6155](https://github.com/XCyber360/XCyber360/tree/master/dashboard/plugins/pull/6155)
            ...
            accum[httpMethodUppercase] = [
              ...accum[httpMethodUppercase],
              {
                name,
                documentation,
                description: description.replace(/Xcyber360/g, 'Server'),
                summary: summary.replace(/Xcyber360/g, 'Server'),
                tags,
                ...(args.length ? { args } : {}),
                ...(query.length
                  ? {
    ...
    function extractDataSecurityActions(spec, configuration) {
      /* Replicate the logic of GET /security/actions endpoint response
        Based on https://github.com/xcyber360/xcyber360/blob/v4.4.3/framework/xcyber360/security.py#L1090-L1105
      */
      let data = {};
