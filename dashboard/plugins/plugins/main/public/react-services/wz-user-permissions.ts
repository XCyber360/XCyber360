/*
 * Xcyber360 app - React hook for get query of plugin platform searchBar
...
import xcyber360Permissions from '../../common/api-info/security-actions';
...
export class WzUserPermissions{
  ...
  static checkMissingUserPermissions = (requiredPermissions, userPermissions) => {
    const filtered = requiredPermissions.filter(permission => {
      ...
      let actionResource = (typeof permission === 'string' && xcyber360Permissions[actionName].resources.length === 1) ? (xcyber360Permissions[actionName].resources[0] + ':*') : permission.resource;
      ...
      const existInXcyber360Permissions = (userResource) => {
        return !!xcyber360Permissions[actionName].resources.find(function (resource) {
          return (
            resource ===
            userResource
              .split('&')
      ...
      const notAllowInXcyber360Permissions = (userResource) => {
        if (userResource !== RESOURCE_ANY) {
          return existInXcyber360Permissions(userResource)
      ...
      return userPermissions[actionName][actionResource]
        ? notAllowInXcyber360Permissions(actionResource)
