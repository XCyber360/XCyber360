...
function calculateWhichGroup(editor, firstTime = false, groups = []) {
  try {
    ...
    try {
      ...
      $('#play_button').hide();
      $('#xcyber360_dev_tools_documentation').hide();
      return null;
    }

    if (!$('#play_button').is(':visible')) $('#play_button').show();
    if (!$('#xcyber360_dev_tools_documentation').is(':visible'))
      $('#xcyber360_dev_tools_documentation').show();
    const currentPlayButton = $('#play_button').offset();
    $('#play_button').offset({
      top: cords.top /*+ 2*/,
      left: currentPlayButton.left,
    });
    $('#xcyber360_dev_tools_documentation').offset({
      top: cords.top /*+ 2*/,
    });
    ...
    if (desiredGroup[0]) {
      ...
      if (apiEndpoint && apiEndpoint.documentation) {
        $('#xcyber360_dev_tools_documentation')
          .attr('href', apiEndpoint.documentation)
          .show();
      } else {
        $('#xcyber360_dev_tools_documentation').attr('href', '').hide();
      }
    }
        ...
        >
          <span className='wz-headline-title wz-dev-title'>
            ...
            <a
              ...
              id='xcyber360_dev_tools_documentation'
            ></a>
          </span>
