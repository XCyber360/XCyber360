...
export const WzSecurity = compose(
  ...
  const isNotRunAs = allowRunAs => {
    let runAsWarningTxt = '';
    switch (allowRunAs) {
      case getXcyber360CorePlugin().API_USER_STATUS_RUN_AS.HOST_DISABLED:
        ...
        break;
      case getXcyber360CorePlugin().API_USER_STATUS_RUN_AS.USER_NOT_ALLOWED:
        runAsWarningTxt =
          'The role mapping has no effect because the current API user has allow_run_as disabled.';
        break;
      case getXcyber360CorePlugin().API_USER_STATUS_RUN_AS.ALL_DISABLED:
