...
import { Xcyber360Security } from '../../../factories/xcyber360-security';
...
export const RolesMapping = withUserAuthorizationPrompt([
  ...
  const getInternalUsers = async () => {
    try {
      const xcyber360Security = new Xcyber360Security();
      const users = await xcyber360Security.security.getUsers();
      const _users = users
        .map((item, idx) => {
          return {
            id: idx,
