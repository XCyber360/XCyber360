/*
 * Xcyber360 app - React component building the API entries table.
 *
...
import {
  getXcyber360CheckUpdatesPlugin,
  getXcyber360CorePlugin,
...
export const ApiTable = compose(withErrorBoundary)(
  class ApiTable extends Component {
    ...
    async getApisAvailableUpdates(queryApi = false, forceQuery = false) {
      try {
        ...
        const availableUpdates =
          await getXcyber360CheckUpdatesPlugin().getAvailableUpdates(
            queryApi,
            forceQuery,
          );
    ...
    componentDidMount() {
      this.refresh();
      this.xcyber360Config = new Xcyber360Config().getConfig();
      this.isUpdatesEnabled = !this.xcyber360Config?.['xcyber360.updates.disabled'];
    ...
    render() {
      const { DismissNotificationCheck } = getXcyber360CheckUpdatesPlugin();
        ...
        },
        {
          ...
          render: value => {
            return value ===
              ...
              getXcyber360CorePlugin().API_USER_STATUS_RUN_AS.USER_NOT_ALLOWED ? (
              <EuiToolTip
                position='top'
                        ...
                        <EuiToolTip
                          ...
                          content={
                            this.state.availableUpdates?.last_check_date
                              ? getXcyber360CorePlugin().utils.formatUIDate(
                                  this.state.availableUpdates.last_check_date,
                                )
                              : '-'
                          }
                                    ...
                                    {[
                                      {
                                        label: 'For Systemd',
                                        command:
                                          'sudo systemctl status xcyber360-manager',
                                      },
                                      {
                                        label: 'For SysV Init',
                                        command:
                                          'sudo service xcyber360-manager status',
                                      },
                                    ].map(({ label, command }) => (
                                      <>
