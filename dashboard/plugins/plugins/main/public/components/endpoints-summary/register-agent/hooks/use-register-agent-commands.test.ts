  ...
  options: [
    {
      architecture: '32/64',
      urlPackage: props =>
        `https://packages.xcyber360.com/4.x/yum/xcyber360-agent-${props.xcyber360Version}-1.x86_64`,
      installCommand: props => `sudo yum install -y ${props.urlPackage}`,
      startCommand: props => `sudo systemctl start xcyber360-agent`,
    },
    {
      architecture: 'x64',
      urlPackage: props =>
        `https://packages.xcyber360.com/4.x/apt/pool/main/w/xcyber360-agent/ xcyber360-agent_${props.xcyber360Version}-1_${props.architecture}`,
      installCommand: props =>
        `curl -so xcyber360-agent.deb ${props.urlPackage} && sudo dpkg -i ./xcyber360-agent.deb`,
      startCommand: props => `sudo systemctl start xcyber360-agent`,
    },
  ],
};
