...
export const osdfucatePasswordInCommand = (password: string, commandText: string, os: tOperatingSystem['name']): string => {
  ...
  switch (osName){
    case 'macos':
    {
      const regex = /XCYBER360_REGISTRATION_PASSWORD=\'((?:\\'|[^']|[\"'])*)'/g;
    ...
    case 'windows':
      {
        const replacedString = command.replace(
          `XCYBER360_REGISTRATION_PASSWORD=\'${scapeSpecialCharsForWindows(password)}'`,
          () => {
            return `XCYBER360_REGISTRATION_PASSWORD=\'${'*'.repeat(scapeSpecialCharsForWindows(password).length)}\'`;
          }
        );
        return replacedString;
      }
    ...
    {
      const replacedString = command.replace(
        `XCYBER360_REGISTRATION_PASSWORD=\$'${scapeSpecialCharsForLinux(password)}'`,
        () => {
          return `XCYBER360_REGISTRATION_PASSWORD=\$\'${'*'.repeat(scapeSpecialCharsForLinux(password).length)}\'`;
        }
      );
      return replacedString;
    }
