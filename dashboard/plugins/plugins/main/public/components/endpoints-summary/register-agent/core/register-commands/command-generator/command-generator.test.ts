...
import { DuplicatedOSException, DuplicatedOSOptionException, NoOSSelectedException, Xcyber360VersionUndefinedException } from '../exceptions';
...
export type tOptionalParameters = 'server_address' | 'agent_name' | 'agent_group' | 'protocol' | 'xcyber360_password';
...
const optionalParams: tOptionalParams<tOptionalParameters> = {
  ...
  protocol: {
    property: 'XCYBER360_MANAGER_PROTOCOL',
    getParamCommand: props => `${props.property}=${props.value}`,
  },
  agent_group: {
...
describe('Command Generator', () => {
  ...
  it('should return commands with the filled optional params', () => {
    ...
    expect(commands).toEqual({
      ...
      optionals: {
        ...
        xcyber360_password: optionalParams.xcyber360_password.getParamCommand({
          property: optionalParams.xcyber360_password.property,
          value: optionalValues.xcyber360_password,
          name: 'xcyber360_password',
        }),
      },
    });
  });
  ...
  it('should return an ERROR when receive an empty version', () => {
    try {
      ...
      if (error instanceof Error)
        expect(error).toBeInstanceOf(Xcyber360VersionUndefinedException);
    }
  });
