...
export const getAllOptionals = (
  ...
  const paramNameOrderList: (keyof IOptionalParameters<tOptionalParameters>)[] =
    ['serverAddress', 'xcyber360Password', 'agentGroups', 'agentName', 'protocol'];
...
export const getDEBAMD64InstallCommand = (
  ...
  const { optionals, urlPackage, xcyber360Version } = props;
  const packageName = `xcyber360-agent_${xcyber360Version}-1_amd64.deb`;
...
export const getDEBARM64InstallCommand = (
  ...
  const { optionals, urlPackage, xcyber360Version } = props;
  const packageName = `xcyber360-agent_${xcyber360Version}-1_arm64.deb`;
  return `wget ${urlPackage} && sudo ${
    optionals && getAllOptionals(optionals)
  }dpkg -i ./${packageName}`;
};
...
export const getRPMAMD64InstallCommand = (
  ...
  const { optionals, urlPackage, xcyber360Version, architecture } = props;
  const packageName = `xcyber360-agent-${xcyber360Version}-1.x86_64.rpm`;
...
export const getRPMARM64InstallCommand = (
  ...
  const { optionals, urlPackage, xcyber360Version, architecture } = props;
  const packageName = `xcyber360-agent-${xcyber360Version}-1.aarch64.rpm`;
  return `curl -o ${packageName} ${urlPackage} && sudo ${
    optionals && getAllOptionals(optionals)
  }rpm -ihv ${packageName}`;
};
...
export const getWindowsInstallCommand = (
  ...
  return `Invoke-WebRequest -Uri ${urlPackage} -OutFile \$env:tmp\\xcyber360-agent; msiexec.exe /i \$env:tmp\\xcyber360-agent /q ${
...
export const getMacOsInstallCommand = (
  ...
  if (optionalsForCommand?.xcyber360Password) {
    /**
    ...
    */
    const scapedPasswordLength = JSON.stringify(
      optionalsForCommand?.xcyber360Password,
    ).length;
    // We need to remove the " added by JSON.stringify
    optionalsForCommand.xcyber360Password = `${JSON.stringify(
      optionalsForCommand?.xcyber360Password,
  ...
  const macOSInstallationSetEnvVariablesScript = macOSInstallationOptions
    ? `echo "${macOSInstallationOptions}" > /tmp/xcyber360_envs && `
    : ``;

  // Merge environment variables with installation script
  const macOSInstallationScript = `curl -so xcyber360-agent.pkg ${urlPackage} && ${macOSInstallationSetEnvVariablesScript}sudo installer -pkg ./xcyber360-agent.pkg -target /`;
...
export const getMacosStartCommand = (
  ...
  return `sudo /Library/xcyber360_agent/bin/xcyber360-control start`;
};
