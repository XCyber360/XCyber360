/*
 * Xcyber360 app - Fetch API function and utils.
    ...
    for (const section of sections) {
      ...
      try {
        ...
        if (agentId === '000') {
           ...
           * because the /agents/{agent_id}/config/logcollector/socket response has property named "target" instead of "socket" in versions before Xcyber360 4.9.0
 ...
 * @param {*} location
 * @param updateXcyber360NotReadyYet
...
export const handleError = async (
  ...
  try {
    if (messageIsString && message.includes('ERROR3099')) {
      updateXcyber360NotReadyYet('Server not ready yet.');
      await makePing(updateXcyber360NotReadyYet, isCluster);
      return;
    }
...
export const checkDaemons = async isCluster => {
  try {
    ...
    const isValid =
      execd && modulesd && xcyber360db && (isCluster ? clusterd : true);
...
export const makePing = async (
  ...
  try {
    ...
    return Promise.resolve('Xcyber360 is ready');
  } catch (error) {
...
/**
 * Get Cluster status from Xcyber360 API
 * @returns {Promise}
 */
...
export const restartNodeSelected = async (
  ...
  try {
    ...
    return await makePing(updateXcyber360NotReadyYet, isCluster);
  } catch (error) {
    throw error;
  }
};
 ...
 */
export const restartClusterOrManager = async updateXcyber360NotReadyYet => {
  try {
    const clusterStatus = (((await clusterReq()) || {}).data || {}).data || {};
    const isCluster =
      clusterStatus.enabled === 'yes' && clusterStatus.running === 'yes';
