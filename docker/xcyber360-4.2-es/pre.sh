...
usage() {
	...
	echo "  xcyber360_api_version is the patch version of xcyber360 4.2, for example " ${xcyber360_api_version[*]}
...
case "$3" in
    ...
    echo "1. Copy the pre-release package to the running Kibana container:"
		echo "docker cp xcyber360_kibana-4.2.${patch_version}_${ES_VERSION}-1.zip ${COMPOSE_PROJECT_NAME}-kibana-1:/tmp"
    echo
    echo "2. Install the pre-release package:"
		echo "docker exec -ti ${COMPOSE_PROJECT_NAME}-kibana-1 /usr/share/kibana/bin/kibana-plugin install file:///tmp/xcyber360_kibana-4.2.${patch_version}_${ES_VERSION}-1.zip"
    echo
