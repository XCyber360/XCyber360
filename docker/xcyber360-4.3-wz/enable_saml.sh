...
indexer="$1-xcyber360.indexer-1"
dashboard="$1-xcyber360.dashboard-1"
...
certs="/usr/share/xcyber360-indexer/config/certs"
ca="$certs/ca.pem"
cert="$certs/admin.pem"
key="$certs/admin-key.pem"

securityadmin="bash /usr/share/xcyber360-indexer/plugins/opensearch-security/tools/securityadmin.sh"
config_path="/usr/share/xcyber360-indexer/plugins/opensearch-security/securityconfig/"

echo "To update configuration in indexer, you can run:"
echo docker exec -e JAVA_HOME=/usr/share/xcyber360-indexer/jdk $indexer $securityadmin -cacert $ca -cert $cert -key $key -cd $config_path
