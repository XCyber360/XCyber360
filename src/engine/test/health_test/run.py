...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Run Behave tests for Xcyber360.')
...
def check_config_file():
    global environment_directory
    global XCYBER360_DIR

    if not environment_directory:
        environment_directory = os.path.join(XCYBER360_DIR, 'environment')
...
def run_test_health():
    ...
    engine_src_dir = os.path.join(XCYBER360_DIR, "src", "engine")
    ...
    command = ["python3", os.path.join(health_test_dir, "health_test.py"), XCYBER360_DIR, environment_directory]
