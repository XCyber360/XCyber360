...
class OpensearchManagement:
    ...
    def init_opensearch(self, template_path: Path):
        ...
        env_vars = {
            ...
            'OPENSEARCH_INITIAL_ADMIN_PASSWORD': 'Xcyber360Test99$',
...
def load_indexer_output(engine_handler: EngineHandler) -> None:
    indexer_output = {
        ...
        "metadata": {
            ...
            "description": "Output integrations events to xcyber360-indexer",
            "compatibility": "",
            "versions": [""],
            "references": [""],
            "author": {
...
def update_xcyber360_core_message(xcyber360_core_message_file, engine_handler):
    request = api_catalog.ResourcePut_Request()
    request.name = "decoder/core-xcyber360-message/0"
    request.namespaceid = "system"
    request.content = xcyber360_core_message_file.read_text()
    request.format = api_catalog.ResourceFormat.yml
    print(f"Updating xcyber360-core-message...\n{request}")
    error, response = engine_handler.api_client.send_recv(request)
    if error:
        raise Exception(error)
...
def modify_core_xcyber360_decoder(file_path: Path, add_hash: bool = True):
    """
    Modifies the given Xcyber360 YAML file to add or remove 'event_hash' in 'normalize'.
...
def decoder_health_test(env_path: Path, integration_name: Optional[str] = None, skip: Optional[List[str]] = None):
    ...
    CORE_XCYBER360_DECODER_PATH = env_path / 'ruleset' / 'decoders' / 'xcyber360-core' / 'core-xcyber360-message.yml'
    ...
    try:
        ...
        opensearch_management.init_opensearch(env_path / 'ruleset' / 'schemas' / 'xcyber360-template.json')
        log = (env_path / "logs/engine.log").as_posix()
        engine_handler.start(log)
        print("Engine started.")
        print("Update xcyber360-core-message decoder")
        if not exist_index_output(engine_handler):
            load_indexer_output(engine_handler)
            load_indexer_output_in_policy(engine_handler)
        modify_core_xcyber360_decoder(CORE_XCYBER360_DECODER_PATH)
        update_xcyber360_core_message(CORE_XCYBER360_DECODER_PATH, engine_handler)
        ...
        print("Restart xcyber360-core-message decoder changes")
        modify_core_xcyber360_decoder(CORE_XCYBER360_DECODER_PATH, add_hash=False)
        update_xcyber360_core_message(CORE_XCYBER360_DECODER_PATH, engine_handler)
        engine_handler.stop()
