...
INTEGRATION_WITHOUT_TESTS = 'xcyber360-core'
...
def verify_event_processing(all_assets: dict, expected_json_files):
    """
    Compare the assets in 'xcyber360.decoders' in the '_expected.json' files with the list of all_assets.
        ...
        try:
            with open(json_file, 'r') as f:
                ...
                for expected in expected_data:
                    decoders = expected.get('xcyber360', {}).get('decoders', [])
                    for decoder in decoders:
                        if decoder in all_assets['decoders']:
                            all_assets['decoders'].remove(decoder)

                    rules = expected.get('xcyber360', {}).get('rules', [])
                    for rule in rules:
...
def verify(all_assets: dict, ruleset_path: Path):
    ...
    if all_assets['decoders']:
        missing_assets.append(f"These assets were not found in 'xcyber360.decoders': {', '.join(all_assets['decoders'])}")
    if all_assets['rules']:
        missing_assets.append(f"These assets were not found in 'xcyber360.rules': {', '.join(all_assets['rules'])}")
