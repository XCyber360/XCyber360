...
then
    ...
    systemctl stop xcyber360-manager.service

    # Backup Xcyber360 files
    ...
    chgrp xcyber360 $CONFIG_DST_DIR/ossec.conf
    chgrp xcyber360 $CONFIG_DST_DIR/local_internal_options.conf

    mkdir -p $RULES_DST_DIR
    mkdir -p $DECODERS_DST_DIR
    cp $RULES_SRC_DIR/* $RULES_DST_DIR
    cp $DECODERS_SRC_DIR/* $DECODERS_DST_DIR
    chown -R root:xcyber360 $RULES_DST_DIR
    chown -R root:xcyber360 $DECODERS_DST_DIR

    # Start Xcyber360

    systemctl start xcyber360-manager.service

    # Sleep to wait for xcyber360-analysisd to start up
    sleep 5

    # Run stats collector script

    python3 ./utils/monitor.py -s $STATS_MONITOR_POLL_TIME_SECS -b xcyber360-analysisd -n $TEST_NAME &
...
then
    ...
    ./main server --event_queue_tasks 0                                    \
                  ...
                  --api_queue_tasks 0                                      \
                  --api_socket "/run/xcyber360-server/engine-api.socket"       \
                  --server_threads 1                                       \
