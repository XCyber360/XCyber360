Feature: Router Routes API Management
  ...
  Scenario: Add new existing priority route to router via API
    ...
    And I create a "default" route with priority "255" that uses the filter "filter/allow-all/0" and points to policy "policy/xcyber360/0"
    When I send a request to the router to add a new route called "dummy" with the data from policy:"policy/xcyber360/0" filter:"filter/allow-all/0" priority:"255"
  ...
  Scenario: Add new existing named route to router via API
    Given I have a policy "policy/xcyber360/0" that has an integration called "xcyber360-core-test" loaded
    And I create a "default" route with priority "255" that uses the filter "filter/allow-all/0" and points to policy "policy/xcyber360/0"
    When I send a request to the router to add a new route called "default" with the data from policy:"policy/xcyber360/0" filter:"filter/allow-all/0" priority:"200"
  ...
  Scenario: Add new route to router via API
    Given I have a policy "policy/xcyber360/0" that has an integration called "xcyber360-core-test" loaded
    And I create a "default" route with priority "255" that uses the filter "filter/allow-all/0" and points to policy "policy/xcyber360/0"
    When I send a request to the router to add a new route called "dummy" with the data from policy:"policy/xcyber360/0" filter:"filter/allow-all/0" priority:"200"
  ...
  Scenario: View of all the information of a certain route via API
    ...
    Then I should receive all the "default" route information. Filter "filter/allow-all/0", policy "policy/xcyber360/0", priority "255"

  Scenario: View a list of routes in the router via API
    Given I have a policy "policy/xcyber360/0" that has an integration called "xcyber360-core-test" loaded
    And I create a "default" route with priority "255" that uses the filter "filter/allow-all/0" and points to policy "policy/xcyber360/0"
    When I send a request to the router to add a new route called "dummy" with the data from policy:"policy/xcyber360/0" filter:"filter/allow-all/0" priority:"200"
    And I send a request to the router to add a new route called "other-dummy" with the data from policy:"policy/xcyber360/0" filter:"filter/allow-all/0" priority:"500"
