Feature: Catalog API Management
  ...
  Scenario: Try to create a resource that is not a collection type
    ...
    When I send a request to publish in the "yml" format in the namespace "xcyber360" a new resource of type "non-exist" that contains
      """
      nothing
      """
  ...
  Scenario: Try to create a resource with a invalid name
    Given I have a clear catalog
    When I send a request to publish in the "yml" format in the namespace "xcyber360" a new resource of type "decoder" that contains
      """
  ...
  Scenario: Try to create a resource in invalid namespace
    Given I have a clear catalog
    When I send a request to publish in the "yml" format in the namespace "xcyber360/other" a new resource of type "decoder" that contains
      """
      name: decoder/testing/0
      """
    Then I should receive a failed response indicating "Invalid namespace 'xcyber360/other': NamespaceId must have only one part and cannot be empty"
  ...
  Scenario: Try to create a resource whose type does not match its name
    Given I have a clear catalog
    When I send a request to publish in the "yml" format in the namespace "xcyber360" a new resource of type "filter" that contains
      """
      name: decoder/testing/0
      """
  ...
  Scenario: Try to create the same resource in the same namespace
    ...
    When I send a request to publish in the "yml" format in the namespace "xcyber360" a new resource of type "decoder" that contains
      ...
      name: decoder/testing/0
      """
    And I send a request to publish in the "yml" format in the namespace "xcyber360" a new resource of type "decoder" that contains
      """
      name: decoder/testing/0
      """
  ...
  Scenario: Try to create a resource whose content does not correspond to the type
    Given I have a clear catalog
    When I send a request to publish in the "json" format in the namespace "xcyber360" a new resource of type "decoder" that contains
      """
      name: decoder/testing/0
      """
  ...
  Scenario: Try to get a resource with different format
    ...
    When I send a request to publish in the "yml" format in the namespace "xcyber360" a new resource of type "decoder" that contains
      ...
      """
    And I send a request to get the resource "decoder/testing/0" with format "yaml" in the namespace "xcyber360"
    Then I should receive a success response
  ...
  Scenario: Try to get non-exist resource
    Given I have a clear catalog
    When I send a request to get the resource "decoder" with format "yaml" in the namespace "xcyber360"
    Then I should receive a failed response indicating "Collection 'decoder' does not exist on namespace 'xcyber360'"
  ...
  Scenario: Try to delete resource in namespace selected
    ...
    When I send a request to publish in the "yml" format in the namespace "xcyber360" a new resource of type "decoder" that contains
      ...
      """
    And I send a request to delete the resource "decoder/testing/0" in the namespace "xcyber360"
    Then I should receive a success response
    When I send a request to get the resource "decoder/testing/0" with format "yaml" in the namespace "xcyber360"
  ...
  Scenario: Try to delete collection in namespace selected
    ...
    When I send a request to publish in the "yml" format in the namespace "xcyber360" a new resource of type "decoder" that contains
      ...
      """
    And I send a request to delete the resource "decoder" in the namespace "xcyber360"
    Then I should receive a success response
    When I send a request to get the resource "decoder" with format "yaml" in the namespace "xcyber360"
    Then I should receive a failed response indicating "Collection 'decoder' does not exist on namespace 'xcyber360'"
  ...
  Scenario: Try to update the name of a resource
    ...
    When I send a request to publish in the "yml" format in the namespace "xcyber360" a new resource of type "decoder" that contains
      ...
      """
    And I send a request to update in the "yml" format in the namespace "xcyber360" the resource "decoder/testing/0" that contains
      """
      {
  ...
  Scenario: Try to validate the resource valid
    ...
    When I send a request to validate in the "yml" format in the resource "decoder/testing/0" that contains
      """
        name: decoder/testing/0
        check: $xcyber360.origin == /var/log/apache2/access.log
        parse|message:
          - <event.code> <event.kind> <event.action>
        normalize:
          - check: $event.code == '1'
