Feature: Tester API Management
  ...
  Scenario: Add a repeated session for testing via API
    Given I have a policy "policy/xcyber360/0" that has an integration called "xcyber360-core-test" loaded
    And I create a "default" session that points to policy "policy/xcyber360/0"
    When I send a request to the tester to add a new session called "default" with the data from policy:"policy/xcyber360/0"
  ...
  Scenario: Add a new session for testing via API
    Given I have a policy "policy/xcyber360/0" that has an integration called "xcyber360-core-test" loaded
    And I create a "default" session that points to policy "policy/xcyber360/0"
    When I send a request to the tester to add a new session called "test1" with the data from policy:"policy/xcyber360/0"
    Then I should receive a success response

  Scenario: Remove specific session via API
    Given I have a policy "policy/xcyber360/0" that has an integration called "xcyber360-core-test" loaded
    And I create a "default" session that points to policy "policy/xcyber360/0"
    When I send a request to the tester to add 5 sessions called "dummy" with policy "policy/xcyber360/0"
  ...
  Scenario: Change sync to OUTDATED of specific session via API
    Given I have a policy "policy/xcyber360/0" that has an integration called "xcyber360-core-test" loaded
    And I create a "default" session that points to policy "policy/xcyber360/0"
    When I send a request to the policy "policy/xcyber360/0" to add an integration called "other-xcyber360-core-test"
  ...
  Scenario: Change sync to ERROR of specific session via API
    Given I have a policy "policy/xcyber360/0" that has an integration called "xcyber360-core-test" loaded
    And I create a "default" session that points to policy "policy/xcyber360/0"
    When I send a request to delete the policy "policy/xcyber360/0"
    And I send a request to the tester to get the session "default"
  ...
  Scenario: Send events to specific session without debug session via API
    Given I have a policy "policy/xcyber360/0" that has an integration called "other-xcyber360-core-test" loaded
    And I create a "test" session that points to policy "policy/xcyber360/0"
    ...
    Then I should receive the next output: "{"output":"{\"xcyber360\":{\"queue\":50,\"location\":\"any\"},\"event\":{\"original\":\"hi! i am an event test!\"}}"}"
  ...
  Scenario: Send events to specific session with low debug via API
    ...
    Then I should receive the next output: "{"assetTraces":[{"asset":"decoder/other-test-message/0","success":true}],"output":"{\"xcyber360\":{\"queue\":50,\"location\":\"any\"},\"event\":{\"original\":\"hi! i am an event test!\"}}"}"
  ...
  Scenario: Send events to specific session with high debug via API
    ...
    Then I should receive the next output: "{"assetTraces":[{"asset":"decoder/other-test-message/0","success":true,"traces":["[check: $xcyber360.queue == 50] -> Success"]}],"output":"{\"xcyber360\":{\"queue\":50,\"location\":\"any\"},\"event\":{\"original\":\"hi! i am an event test!\"}}"}"
