...
OUTPUT_DIR=$(realpath $(mktemp -d -t xcyber360-rtr-XXXXXX))
CONTAINER_NAME=xcyber360-rtr-engine
RTR_DIR=$SRC_DIR/.rtr
RTR_BIN=rtr.py
...
Help() {
    ...
    echo "o     Output directory. It creates a temporary directory in '/tmp/xcyber360-rtr-XXXXXX' if not specified."
    ...
    echo "          ./rtr.sh -o /tmp/rtr -s /home/user/xcyber360/src/engine -i ./rtr_inputs/rtr_ut.json -v -b no -t 8"
    echo "      Run all steps in parallel mode forcing 8 threads in a fixed folder: "
    echo "          ./rtr.sh -o /tmp/rtr -s /home/user/xcyber360/src/engine -p -t 8"
}
