...
// Parse Events
static void parseXcyber360Event_batch(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        try
        {
            ...
            base::Event e;
            benchmark::DoNotOptimize(e = base::parseEvent::parseXcyber360Event(sampleEventsStr[current]));
...
BENCHMARK(parseXcyber360Event_batch)->Threads(1)->Threads(2)->Threads(4)->UseRealTime();
...
static void copyEvents_batch(benchmark::State& state)
{
    ...
    std::transform(sampleEventsStr.begin(),
                   ...
                   [](const auto& e) { return base::parseEvent::parseXcyber360Event(e); });
