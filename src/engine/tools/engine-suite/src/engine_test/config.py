import os
DEFAULT_CONFIG_FILE = "/var/lib/xcyber360-server/engine/engine-test.conf"
    ...
    def get_config_file():
        ...
        if Config.config_file == DEFAULT_CONFIG_FILE and not os.path.exists(Config.config_file):
            try:
                ...
                try:
                    import grp
                    gid = grp.getgrnam("xcyber360").gr_gid
                    os.chown(Config.config_file, -1, gid)
                except ImportError as e:
                    print(f"Warning: xcyber360 group cannot be set for {Config.config_file}. Error: {e}")
