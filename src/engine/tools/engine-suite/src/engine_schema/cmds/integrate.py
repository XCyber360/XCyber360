...
DEFAULT_API_SOCK = '/run/xcyber360-server/engine-api.socket'
DEFAULT_INDEXER_DIR = '/etc/filebeat/'
DEFAULT_FIELDS_DIR = '/home/vagrant/engine/xcyber360/src/engine/ruleset/schemas/'
...
def run(args, resource_handler: rs.ResourceHandler):
    ...
    print(f'Overriding xcyber360-template.json in {indexer_path}...')
    resource_handler.save_file(
        indexer_path, 'xcyber360-template', jmappings, rs.Format.JSON)
    # Update logpar_types in the catalog
    print(f'Updating logpar configuration...')
    resource_handler.update_catalog_file(
        api_socket, 'schema', 'schema/xcyber360-logpar-types/0', jlogpar, rs.Format.JSON)
    # Update schema
    ...
    print('     systemctl restart xcyber360-manager')
...
def configure(subparsers):
    ...
    parser_integrate.add_argument('--indexer-dir', type=str, default=DEFAULT_INDEXER_DIR,
                                  help=f'[default="{DEFAULT_INDEXER_DIR}"] Path to directory where the xcyber360-template.json indexer file is located')
