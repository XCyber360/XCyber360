...
def run(args, resource_handler: rs.ResourceHandler):
    ...
    Generates the xcyber360 schema and all needed configuration, optionally add \
ECS field modules and apply changes on an engine instance through the API socket
    ...
    resource_handler.save_file(
        output_dir, 'xcyber360-template', jmappings, rs.Format.JSON)
    resource_handler.save_file(
        output_dir, 'xcyber360-logpar-types', jlogpar, rs.Format.JSON)
    resource_handler.save_file(
