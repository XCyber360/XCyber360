/*
 * Xcyber360 Indexer Connector - Component tests
...
extern "C" struct passwd* __wrap_getpwnam(const char* name)
{
    ...
    if (strcmp(name, "xcyber360") == 0) // Simulate only "xcyber360" user existing
    {
        dummy_passwd.pw_name = strcpy(dummy_name, "xcyber360");
        dummy_passwd.pw_uid = 1000;
        dummy_passwd.pw_gid = 1000;
        dummy_passwd.pw_dir = strcpy(dummy_dir, "/home/xcyber360");
...
extern "C" struct group* __wrap_getgrnam(const char* name)
{
    ...
    if (strcmp(name, "xcyber360") == 0) // Simulate only "xcyber360" group existing
    {
        dummy_group.gr_name = strcpy(dummy_name, "xcyber360");
...
extern "C" int __wrap_chown(const char* path, uid_t owner, gid_t group)
{
    ...
    if (strcmp(path, "/tmp/xcyber360-server/root-ca-merged.pem") == 0)
    {
...
TEST_F(IndexerConnectorTest, ConnectionWithCertsArray)
{
    ...
    const std::string mergedCertFile = "/tmp/xcyber360-server/root-ca-merged.pem";
