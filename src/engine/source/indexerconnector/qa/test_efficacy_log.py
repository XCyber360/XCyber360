...
def test_initialize_indexer_connector(opensearch):
    ...
    ## Remove folder queue/indexer/db/xcyber360-states-vulnerabilities
    if Path("queue/indexer/db/xcyber360-states-vulnerabilities").exists():
        for file in Path("queue/indexer/db/xcyber360-states-vulnerabilities").glob("*"):
            file.unlink()
        Path("queue/indexer/db/xcyber360-states-vulnerabilities").rmdir()
    ...
    while counter < 10:
        ...
        response = requests.get(url)
        if response.status_code == 200 and 'xcyber360-states-vulnerabilities' in response.text:
...
def test_add_bulk_indexer_connector(opensearch):
    ...
    counter = 0
    while counter < 10:
        url = 'http://localhost:9200/xcyber360-states-vulnerabilities/_search'
        query = {
            "query": {
                "match_all": {}
            }
