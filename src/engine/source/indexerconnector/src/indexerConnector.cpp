...
constexpr auto XCYBER360_OWNER {"xcyber360"};
constexpr auto XCYBER360_GROUP {"xcyber360"};
constexpr auto MERGED_CA_PATH {"/tmp/xcyber360-server/root-ca-merged.pem"};
...
static void mergeCaRootCertificates(const std::vector<std::string>& filePaths, std::string& caRootCertificate)
{
    ...
    struct passwd* pwd = getpwnam(XCYBER360_OWNER);
    struct group* grp = getgrnam(XCYBER360_GROUP);
