...
static inline MapOp opBuilderWdbGenericQuery(const std::vector<OpArg>& opArgs,
                                             ...
                                             const std::shared_ptr<xcyber360db::IWDBManager>& wdbManager)
{
               ...
               base::ConstEvent event) -> MapResult
    {
        ...
        if (doReturnPayload)
        {
            ...
            {
                auto retCode = xcyber360db::qrcToStr(resultCode);
        ...
        {
            json::Json result;
            auto ok = xcyber360db::QueryResultCodes::OK == resultCode;
            result.setBool(ok);
            if (!ok)
            {
                RETURN_FAILURE(
                    runState, result, failureTrace + fmt::format("Result code is '{}'", xcyber360db::qrcToStr(resultCode)));
            }
            RETURN_SUCCESS(runState, result, successTrace);
        }
    };
...
MapBuilder getWdbUpdateBuilder(const std::shared_ptr<xcyber360db::IWDBManager>& wdbManager)
{
...
MapBuilder getWdbQueryBuilder(const std::shared_ptr<xcyber360db::IWDBManager>& wdbManager)
{
    return [wdbManager](const std::vector<OpArg>& opArgs, const std::shared_ptr<const IBuildCtx> buildCtx) -> MapOp
    {
