...
searchAndParse(const std::string& query, std::shared_ptr<xcyber360db::IWDBHandler> wdb, bool parse = true)
{
    std::string retStr {};
    ...
    if (xcyber360db::QueryResultCodes::OK == rescode && payload.has_value())
    {
...
void insertCompliance(const DecodeCxt& ctx, const int checkID)
{
    ...
    for (const auto& [key, jsonValue] : compliance.value())
    {
        ...
        if (xcyber360db::QueryResultCodes::OK != res)
        {
            LOG_WARNING(
...
std::optional<std::string> handleCheckEvent(const DecodeCxt& ctx)
{
    ...
    if (xcyber360db::QueryResultCodes::OK != resSavePolicy)
    {
...
bool SaveScanInfo(const DecodeCxt& ctx, bool update)
{
    ...
    if (xcyber360db::QueryResultCodes::OK != queryResult)
    {
...
void insertPolicyInfo(const DecodeCxt& ctx)
{
    ...
    if (xcyber360db::QueryResultCodes::OK != result)
    {
...
bool deletePolicyAndCheck(const DecodeCxt& ctx, const std::string& policyId)
{
    ...
    if (xcyber360db::QueryResultCodes::OK != resDelPolicy)
    {
    ...
    if (xcyber360db::QueryResultCodes::OK != delCheckResultCode)
    {
        LOG_WARNING(
...
void deletePolicyCheckDistinct(const DecodeCxt& ctx, const std::string& policyId, const int scanId)
{
    ...
    if (xcyber360db::QueryResultCodes::OK != resultCode)
    {
...
TransformBuilder getBuilderSCAdecoder(const std::shared_ptr<xcyber360db::IWDBManager>& wdbManager,
