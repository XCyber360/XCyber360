    ...
    void SetUp() override
    {
        ...
        EXPECT_CALL(*m_mockStore, readDoc(testing::_))
            .WillRepeatedly(testing::Invoke(
                [&](const base::Name& name)
                {
                    if (name == "integration/xcyber360-core-fake/0")
                    {
...
{
    router::prod::EntryPost entry("route", "policy/xcyber360/0", "filter/allow-all/0", 10);
...
{
    router::prod::EntryPost entry("route", "policy/xcyber360/0", "filter/allow-all/0", 10);
    router::prod::EntryPost entryTwo("routeTwo", "policy/xcyber360/0", "filter/allow-all/0", 11);
