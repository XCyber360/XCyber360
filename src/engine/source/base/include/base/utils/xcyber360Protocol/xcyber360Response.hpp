#ifndef _BASE_UTILS_XCYBER360_RESPONSE_HPP
#define _BASE_UTILS_XCYBER360_RESPONSE_HPP
...
public:
    ...
    /**
     * @brief  Construct a new Xcyber360 Response object
     *
     * @param data Data to be sent, it can be a json object or a string
     * @param error Error code (0 if no error)
     * @param message Optional message
     */
    explicit Xcyber360Response(const json::Json& data, int error = 0, std::string_view message = "") noexcept
        : m_data(data)
        , m_error(error)
    {
     ...
     */
    explicit Xcyber360Response(json::Json&& data, int error = 0, std::string_view message = "") noexcept
        : m_data(data)
        , m_error(error)
    {
        m_message = message.empty() ? std::nullopt : std::optional<std::string> {message};
     ...
     */
    explicit Xcyber360Response(std::string_view message) noexcept
        : m_data(json::Json {R"({})"})
     ...
     * @param message Messsage to be sent as a response
     */
    explicit Xcyber360Response(json::Json& data, std::string_view message) noexcept
        : m_data(data)
        , m_error(static_cast<int>(RESPONSE_ERROR_CODES::OK))
        , m_message(message) {};
     ...
     */
    explicit Xcyber360Response() noexcept
        : m_data(json::Json {R"({})"})
        , m_error(static_cast<int>(RESPONSE_ERROR_CODES::OK))
        , m_message() {};
    ...
    /**
     * @brief Create a Xcyber360Response object from a string
     *
     * @param response Response as a string.
     * @return Xcyber360Response object.
     ...
     * }
     */
    static Xcyber360Response fromStr(std::string_view response)
    {
        ...
        auto ret = Xcyber360Response {std::move(data.value()), error.value()};
     ...
     *
     */
    static Xcyber360Response invalidJsonRequest()
    {
        return Xcyber360Response(json::Json(R"({})"),
     ...
     *
     */
    static Xcyber360Response invalidSize()
    {
        return Xcyber360Response(
     ...
     *
     */
    static Xcyber360Response unknownError()
    {
        return Xcyber360Response(
     ...
     *
     */
    static Xcyber360Response invalidRequest(const std::string& message)
    {
        return Xcyber360Response(json::Json(R"({})"),
     ...
     * process  the respons
     */
    static Xcyber360Response internalError(const std::string& message)
    {
        return Xcyber360Response(json::Json(R"({})"),
     ...
     *
     */
    static Xcyber360Response busyServer()
    {
        return Xcyber360Response(
...
#endif // _API_XCYBER360_RESPONSE_HPP
