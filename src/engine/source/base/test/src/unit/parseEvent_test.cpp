...
void execute(const UseCase& useCase)
{
    ...
    auto e = base::parseEvent::parseXcyber360Event(event);
...
TEST(parseXcyber360Event, InvalidEventFormat)
{
    const std::string event {"Invalid event format string"};
    ASSERT_THROW(base::parseEvent::parseXcyber360Event(event), std::runtime_error);
}

TEST(parseXcyber360Event, InvalidShortEvent)
{
    const std::string event {"x::"};
    ASSERT_THROW(base::parseEvent::parseXcyber360Event(event), std::runtime_error);
}

TEST(parseXcyber360Event, Forms)
{
