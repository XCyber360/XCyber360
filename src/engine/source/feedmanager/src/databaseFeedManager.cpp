/*
 * Xcyber360 Vulnerability scanner - Database Feed Manager
...
const std::filesystem::path XCYBER360_LIB_PATH {"/var/lib/xcyber360-server/"};            //< Path to the lib server files.
const std::filesystem::path TMP_PATH {"/tmp/xcyber360-server"};                       //< Path to the tmp files.
const std::filesystem::path XCYBER360_LIB_TMP_PATH {XCYBER360_LIB_PATH / "tmp/"};         //< Path to the lib server tmp files.
const std::filesystem::path XZ_FILE_PATH {TMP_PATH / (CONTENT_NAME + ".tar.xz")}; //< Path of the compressed db
const std::filesystem::path TAR_FILE_PATH {XCYBER360_LIB_TMP_PATH / (CONTENT_NAME + ".tar")}; //< Path to the tar db.
const std::filesystem::path LEGACY_DB_PATH {XCYBER360_LIB_TMP_PATH / "queue/"};           //< Path to the legacy database.
const std::filesystem::path VD_FEED_DB_BASE_PATH {XCYBER360_LIB_PATH / "vd/"};            //< Path to the current database.
const std::filesystem::path VD_UPDATER_DB_BASE_PATH {XCYBER360_LIB_PATH / "vd_updater/"}; //< Path to the updater database.
    ...
    : m_mutex(mutex)
{
    try
    {
        if (std::filesystem::exists(XZ_FILE_PATH))
        {
            ...
            if (!std::filesystem::exists(XCYBER360_LIB_TMP_PATH))
            {
                std::filesystem::create_directories(XCYBER360_LIB_TMP_PATH);
            }
            ...
            fs::ArchiveHelper::decompress(TAR_FILE_PATH, XCYBER360_LIB_TMP_PATH, extractOnly);
            ...
            std::filesystem::rename(LEGACY_DB_PATH / "vd", XCYBER360_LIB_PATH / "vd");
            std::filesystem::rename(LEGACY_DB_PATH / "vd_updater", XCYBER360_LIB_PATH / "vd_updater");
