...
/**
 * @brief This function takes a weak pointer to a policy API and a xcyber360 request, and returns a tuple
 * containing the request, a shared pointer to the policy API, and the policy name. The request and policy name are
 * extracted from the xcyber360 request. If the policy name is empty or invalid, an error response is
...
{
    using namespace ::com::xcyber360::api::engine;
    ...
    auto res = ::api::adapter::fromXcyber360Request<RequestT, ResponseT>(wRequest);
...
api::HandlerSync storePost(const std::shared_ptr<policy::IPolicy>& policyManager)
{
    ...
    return [wpPolicyAPI = std::weak_ptr(policyManager)](const api::wpRequest& wRequest) -> api::wpResponse
    {
        ...
        return ::api::adapter::toXcyber360Response<ResponseType>(eResponse);
    };
}
...
api::HandlerSync policiesGet(const std::shared_ptr<policy::IPolicy>& policyManager)
{
    ...
    return [wpPolicyAPI = std::weak_ptr(policyManager)](const api::wpRequest& wRequest) -> api::wpResponse
    {
        ...
        // Validate the eRequest
        auto res = ::api::adapter::fromXcyber360Request<RequestType, ResponseType>(wRequest);
        if (std::holds_alternative<api::wpResponse>(res))
        {
            return std::move(std::get<api::wpResponse>(res));
        }
