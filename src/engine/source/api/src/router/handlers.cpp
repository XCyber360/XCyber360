...
/**
 * @brief Get the request from the xcyber360 request and validate the router
 *
 * @tparam RequestType
 * @tparam ResponseType
 * @param wRequest The xcyber360 request to convert
                          ...
                          const std::weak_ptr<api::policy::IPolicy>& policy)
{
    return [wRouter = router, wPolicyManager = policy](const api::wpRequest& wRequest) -> api::wpResponse
    {
        ...
        // Adapt the response to xcyber360 api
        return ::api::adapter::toXcyber360Response<ResponseType>(eResponse);
    };
}
...
api::HandlerSync queuePost(const std::weak_ptr<::router::IRouterAPI>& router)
{
    return [wRouter = router](const api::wpRequest& wRequest) -> api::wpResponse
    {
        ...
        auto& [router, eRequest] = std::get<RouterAndRequest<RequestType>>(res);
        const auto postRes = router->postStrEvent(eRequest.xcyber360_event());
