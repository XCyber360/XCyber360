...
/**
 * @brief Get the request from the xcyber360 request and validate the manager
 *
 * @tparam RequestType
 * @tparam ResponseType
...
namespace eGeo = ::com::xcyber360::api::engine::geo;
...
api::HandlerSync listDbCmd(const std::weak_ptr<::geo::IManager>& geoManager)
{
    return [geoManager](const api::wpRequest& wpRequest) -> api::wpResponse
    {
        ...
        return api::adapter::toXcyber360Response(response);
    };
}
