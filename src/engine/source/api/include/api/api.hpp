...
public:
    ...
    void processRequest(const std::string& message, std::function<void(const std::string&)> callbackFn)
    {
        ...
        {
            auto wresponse = base::utils::xcyber360Protocol::Xcyber360Response::invalidJsonRequest();
        ...
        if (!wrequest.isValid())
        {
            auto wresponse = base::utils::xcyber360Protocol::Xcyber360Response::invalidRequest(wrequest.error().value());
            callbackFn(wresponse.toString());
            return;
        }

        processXcyber360Request(wrequest, [=](const wpResponse& wresponse) { callbackFn(wresponse.toString()); });
    }
     ...
     *
     * @param wrequest A Xcyber360 request
     * @param callbackFn A callback function that will be invoked with the generated response to respond to the request
     *
     */
    void processXcyber360Request(const wpRequest& wrequest, std::function<void(const wpResponse&)> callbackFn)
    {
        ...
        catch (const std::exception& e)
        {
            ...
            wresponse = base::utils::xcyber360Protocol::Xcyber360Response::unknownError();
