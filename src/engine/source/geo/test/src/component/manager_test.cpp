...
TEST_F(GeoManagerTest, MultithreadDeleteAddLookup)
{
    ...
    auto locatorFn = [&errorMsg, &error, times](std::shared_ptr<ILocator> locator)
    {
        for (auto i = 0; i < times; i++)
        {
            ...
            {
                if (base::getResponse(res) != "Xcyber360")
                {
                    setError(
                        error,
                        errorMsg,
                        fmt::format("Locator thread got response '{}' which is not 'Xcyber360'", base::getResponse(res)));
                }
            }
        }
    };
            ...
            else
            {
                ...
                else
                {
                    if (base::getResponse(res) != "Xcyber360")
                    {
                        setError(error,
                                 errorMsg,
                                 fmt::format("Locator thread got response '{}' which is not 'Xcyber360'",
