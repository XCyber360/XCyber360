...
namespace eRouter = ::com::xcyber360::api::engine::router;
            ...
            const std::string& policy)
{
    ...
    const auto request = utils::apiAdapter::toXcyber360Request<RequestType>(command, details::ORIGIN_NAME, eRequest);
    const auto response = client->send(request);
    utils::apiAdapter::fromXcyber360Response<ResponseType>(response); // Validate response
}
...
void runDelete(std::shared_ptr<apiclnt::Client> client, const std::string& nameStr)
{
    ...
    const auto request = utils::apiAdapter::toXcyber360Request<RequestType>(command, details::ORIGIN_NAME, eRequest);
    const auto response = client->send(request);
    utils::apiAdapter::fromXcyber360Response<ResponseType>(response);
}
...
void runIngest(std::shared_ptr<apiclnt::Client> client, const std::string& event)
{
    ...
    // Prepare the request
    RequestType eRequest;
    eRequest.set_xcyber360_event(event);
