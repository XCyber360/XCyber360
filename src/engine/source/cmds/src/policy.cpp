...
{
namespace ePolicy = ::com::xcyber360::api::engine::policy;
...
// Default assets
const std::vector<std::pair<std::string, std::string>> DEFAULT_ASSETS = {{"integration/xcyber360-core/0", "system"},
                                                                         {"integration/syslog/0", "xcyber360"},
                                                                         {"integration/system/0", "xcyber360"},
                                                                         {"integration/windows/0", "xcyber360"},
                                                                         {"integration/apache-http/0", "xcyber360"},
                                                                         {"integration/suricata/0", "xcyber360"}};
// Default namespaces parent
const std::vector<std::pair<std::string, std::string>> DEFAULT_PARENTS = {{"user", "decoder/integrations/0"},
                                                                          {"xcyber360", "decoder/integrations/0"}};
...
void runRemovePolicy(std::shared_ptr<apiclnt::Client> client, const std::string& policyName)
{
    ...
    const auto request = utils::apiAdapter::toXcyber360Request<RequestType>(command, details::ORIGIN_NAME, eRequest);
    const auto response = client->send(request);
    const auto eResponse = utils::apiAdapter::fromXcyber360Response<ResponseType>(response);
};
...
void runAddPolicy(std::shared_ptr<apiclnt::Client> client, const std::string& policyName, bool forceEmpty)
{
    ...
    if (!forceEmpty)
    {
        try
        {
            ...
            {
                const auto req = utils::apiAdapter::toXcyber360Request(configCommand, details::ORIGIN_NAME, *configRequest);
                const auto responseStr = client->send(req);
                const auto eResponse = utils::apiAdapter::fromXcyber360Response<ResponseType>(responseStr);
            }
        }
