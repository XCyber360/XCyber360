...
void runStart(ConfHandler confManager)
{
    ...
    std::shared_ptr<xcyber360db::WDBManager> wdbManager;
    ...
    try
    {
        ...
        // HLP
        {
            ...
            base::Name hlpConfigFileName({"schema", "xcyber360-logpar-types", "0"});
        ...
        // Indexer Connector
        { // TODO Change index to `xcyber360-alerts-5.x-%{+yyyyy.MM.dd}` when supported placeholder is available.
            ...
            IndexerConnectorOptions indexerConnectorOptions {
                ...
                .password = getEnvOrDefault("WENGINE_ICONNECTOR_PASSWORD", "Xcyber360Engine5+"),
        ...
        // Builder and registry
        {
            ...
            builderDeps.wdbManager =
                std::make_shared<xcyber360db::WDBManager>(std::string(xcyber360db::WDB_SOCK_PATH), builderDeps.sockFactory);
        ...
        // API Server
        {
             ...
             *
             * @apiDescription This endpoint receives events to be processed by the Xcyber360-Engine security policy. It
        ...
        // Server
        {
            ...
            apiClientFactory->setErrorResponse(base::utils::xcyber360Protocol::Xcyber360Response::unknownError().toString());
            apiClientFactory->setBusyResponse(base::utils::xcyber360Protocol::Xcyber360Response::busyServer().toString());
...
{
    auto serverApp = app->add_subcommand("server", "Start/Stop a Xcyber360 engine instance.");
    ...
    // Start subcommand
    auto startApp = serverApp->add_subcommand("start", "Start a Xcyber360 engine instance");
