...
using wpResponse = base::utils::xcyber360Protocol::Xcyber360Response;
using wpRequest = base::utils::xcyber360Protocol::Xcyber360Request;

namespace apiAdapter
{
/**
 * @brief Converts an eMessage and command into a Xcyber360Request.
 *
 ...
 * @return base::utils::xcyber360Protocol::Xcyber360Request Xcyber360Request containing the serialized eMessage.
 * @throw ClientException if the serialization fails.
 */
template<typename T>
wpRequest toXcyber360Request(const std::string& command, const std::string& origin, const T& eMessage)
{
    ...
    // Create and return the Xcyber360Request object
    return wpRequest::create(command, origin, params);
}

/**
 * @brief Parses the response data from a Xcyber360 API call into a protocol buffer (eMessage of type T).
 *
 ...
 * @param wResponse The response from the Xcyber360 API call
 ...
 */
template<typename T>
T fromXcyber360Response(const wpResponse& wResponse)
{
    // The status code used in the protocol buffer message to indicate success
    using StatusCode = ::com::xcyber360::api::engine::ReturnStatus;
    ...
    // Check if the Xcyber360 API call was successful
    if (wResponse.error())
    {
        // Throw an exception with the error message from the Xcyber360 response
