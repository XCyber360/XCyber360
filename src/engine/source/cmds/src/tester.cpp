...
namespace eTest = ::com::xcyber360::api::engine::tester;
...
void run(std::shared_ptr<apiclnt::Client> client, const Parameters& parameters)
{
    ...
    const auto getRequest =
        utils::apiAdapter::toXcyber360Request<eTest::SessionGet_Request>(commandGet, details::ORIGIN_NAME, eGetRequest);
    const auto responseGet = client->send(getRequest);
    const auto eResponseGet = utils::apiAdapter::fromXcyber360Response<eTest::SessionGet_Response>(responseGet);
...
void configure(CLI::App_p app)
{
    ...
    testApp
        ->add_option(
            "-t, --client_timeout", parameters->clientTimeout, "Sets the timeout for the client in miliseconds.")
