...
TEST_F(UrlRequestUnitTest, GetFileHttp)
{
    ...
    EXPECT_CALL(*request, setOption(optUrl, "http://www.xcyber360.com/")).Times(1);
    ...
    GetRequest::builder(request).url("http://www.xcyber360.com/").outputFile("/tmp/hello_world.html").execute();
}
...
TEST_F(UrlRequestUnitTest, GetFileWithUnixSocket)
{
    ...
    EXPECT_CALL(*request, setOption(optUnixSocketPath, "/tmp/xcyber360-agent.sock")).Times(1);
    EXPECT_CALL(*request, setOption(optUrl, "http://www.xcyber360.com/")).Times(1);
    EXPECT_CALL(*request, setOption(optCustomRequest, "GET")).Times(1);
    EXPECT_CALL(*request, setOption(optWriteData, SafeMatcherCast<void*>(_))).Times(1);
    EXPECT_CALL(*request, setOption(optWriteFunction, zero)).Times(1);
    ...
    GetRequest::builder(request)
        .url("http://www.xcyber360.com/")
        .unixSocketPath("/tmp/xcyber360-agent.sock")
...
TEST_F(UrlRequestUnitTest, GetApiRequest)
{
    ...
    EXPECT_CALL(*request, setOption(optUserAgent, "Xcyber360-Agent/1.0")).Times(1);
...
TEST_F(UrlRequestUnitTest, PostApiRequestWithPostFields)
{
    ...
    EXPECT_CALL(*request, setOption(optCainfo, "cert.ca")).Times(1);
    EXPECT_CALL(*request, setOption(optTimeout, 10)).Times(1);
    EXPECT_CALL(*request, setOption(optPostFields, R"({"name":"xcyber360"})")).Times(1);
    EXPECT_CALL(*request, setOption(optPostFieldSize, std::string(R"({"name":"xcyber360"})").length())).Times(1);
    EXPECT_CALL(*request, setOption(optVerifyPeer, true)).Times(1);
    EXPECT_CALL(*request, execute()).Times(1);
...
TEST_F(UrlRequestUnitTest, HttpsCertExists)
{
    ...
    EXPECT_CALL(*request, setOption(optUrl, "https://www.xcyber360.com/")).Times(1);
    ...
    EXPECT_NO_THROW(getRequest.url("https://www.xcyber360.com/").execute());
}
...
TEST_F(UrlRequestUnitTest, HttpsNoCertNotExists)
{
    ...
    EXPECT_NO_THROW(getRequestNoCert.url("https://www.xcyber360.com/").execute());
}
