...
# xcyber360-control        This shell script takes care of starting
...
AUTHOR="Xcyber360 Inc."
USE_JSON=false
DAEMONS="xcyber360-clusterd xcyber360-modulesd xcyber360-monitord xcyber360-logcollector xcyber360-remoted xcyber360-syscheckd xcyber360-analysisd xcyber360-maild xcyber360-execd xcyber360-db xcyber360-authd xcyber360-agentlessd xcyber360-integratord xcyber360-dbd xcyber360-csyslogd xcyber360-apid xcyber360-comms-apid"
OP_DAEMONS="xcyber360-clusterd xcyber360-maild xcyber360-agentlessd xcyber360-integratord xcyber360-dbd xcyber360-csyslogd"
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting Xcyber360 $VERSION..."
    fi

    TEST=$(${DIR}/bin/xcyber360-logtest-legacy -t  2>&1 | grep "ERROR")
    if [ ! -z "$TEST" ]; then
        if [ $USE_JSON = true ]; then
        ...
        touch ${DIR}/var/run/xcyber360-analysisd.failed
        exit 1;
    fi
    ...
    for i in ${SDAEMONS}; do
        ## If xcyber360-maild is disabled, don't try to start it.
        if [ X"$i" = "Xxcyber360-maild" ]; then
             grep "<email_notification>no<" ${DIR}/etc/ossec.conf >/dev/null 2>&1
             if [ $? = 0 ]; then
                 continue
             fi
        fi
        ## If xcyber360-clusterd is disabled, don't try to start it.
        if [ X"$i" = "Xxcyber360-clusterd" ]; then
        ...
        ## If xcyber360-authd is disabled, don't try to start it.
        if [ X"$i" = "Xxcyber360-authd" ]; then
...
pstatus()
{
    ...
    if [ $? = 0 ]; then
        for pid in `cat ${DIR}/var/run/${pfile}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ]; then
                    echo "${pfile}: Process ${pid} not used by Xcyber360, removing..."
                fi
...
stop_service()
{
    ...
    if [ $USE_JSON = true ]; then
        ...
        echo "Xcyber360 $VERSION Stopped"
    fi
}
...
info()
{
    if [ "X${1}" = "X" ]; then
        if [ $USE_JSON = true ]; then
            echo -n '{"error":0,"data":['
            echo -n '{"XCYBER360_VERSION":"'${VERSION}'"},'
            echo -n '{"XCYBER360_REVISION":"'${REVISION}'"},'
            echo -n '{"XCYBER360_TYPE":"'${TYPE}'"}'
            echo -n ']}'
        else
            echo "XCYBER360_VERSION=\"${VERSION}\""
            echo "XCYBER360_REVISION=\"${REVISION}\""
            echo "XCYBER360_TYPE=\"${TYPE}\""
        fi
    else
        case "${1}" in
            -v) echo "${VERSION}" ;;
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/xcyber360-execd//')
    restart_service
    ;;
status)
    lock
