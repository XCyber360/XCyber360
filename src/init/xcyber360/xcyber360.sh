...
# Install functions for Xcyber360
# Xcyber360.com (https://github.com/xcyber360)
...
InstallSELinuxPolicyPackage(){
    ...
    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ -f selinux/xcyber360.pp ]; then
            if [ $(getenforce) != "Disabled" ]; then
                cp selinux/xcyber360.pp /tmp && semodule -i /tmp/xcyber360.pp
                rm -f /tmp/xcyber360.pp
                semodule -e xcyber360
            fi
        fi
    fi
}
...
Xcyber360Upgrade()
{
    ...
    if [ -f "$PREINSTALLEDDIR/var/db/global.db" ]; then
        ...
        if [ -f "$PREINSTALLEDDIR/queue/db/global.db" ]; then
            ...
            chown xcyber360:xcyber360 $PREINSTALLEDDIR/queue/db/global.db
    ...
    # Remove existing SQLite databases for Xcyber360 DB, only if upgrading from 3.2..3.6
    ...
    if [ -d $PREINSTALLEDDIR/logs/ossec ]; then
        if [ "$(ls -A $PREINSTALLEDDIR/logs/ossec)" ]; then
            mv -f $PREINSTALLEDDIR/logs/ossec/* $PREINSTALLEDDIR/logs/xcyber360
        fi
        rm -rf $PREINSTALLEDDIR/logs/ossec
    fi

    # Remove deprecated Xcyber360 tools
    ...
    if (grep "^ossec:" /etc/group > /dev/null 2>&1) || (dscl . -read /Groups/ossec > /dev/null 2>&1)  ; then
        if [ "X$1" = "Xserver" ]; then
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -print0 | xargs -0 chown root:xcyber360
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -print0 | xargs -0 chown xcyber360:xcyber360
        else
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -exec chown root:xcyber360 {} \;
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -exec chown xcyber360:xcyber360 {} \;
        fi
    fi
