...
# Init functions for Xcyber360
# Copyright (C) 2015, Xcyber360 Inc.
# Author: Daniel B. Cid <daniel.cid@gmail.com>

UN=${NUNAME};
service="xcyber360-server";
...
runInit()
{
    ...
    if hash ps 2>&1 > /dev/null && hash grep 2>&1 > /dev/null && [ -n "$(ps -e | egrep ^\ *1\ .*systemd$)" ]; then
        ...
        if [ "${DIST_NAME}" = "rhel" -a "${DIST_VER}" -ge "7" ] || [ "${DIST_NAME}" = "centos" -a "${DIST_VER}" -ge "7" ]; then
            SERVICE_UNIT_PATH=/usr/lib/systemd/system/xcyber360-server.service
            rm -f /etc/systemd/system/xcyber360-server.service
        else
            SERVICE_UNIT_PATH=/etc/systemd/system/xcyber360-server.service
        fi
        GenerateService xcyber360-server.service > ${SERVICE_UNIT_PATH}
        chown root:xcyber360 ${SERVICE_UNIT_PATH}
    ...
    if [ -r "/etc/redhat-release" ]; then
        if [ -d /etc/rc.d/init.d ]; then
            ...
            echo " - ${modifiedinit}"
            GenerateService xcyber360-server-rh.init > /etc/rc.d/init.d/${service}
            chmod 755 /etc/rc.d/init.d/${service}
            chown root:xcyber360 /etc/rc.d/init.d/${service}
    ...
    if [ "X${NUNAME}" = "XLinux" ]; then
        if [ -e "/etc/rc.d/rc.local" ]; then
            ...
            grep xcyber360-control /etc/rc.d/rc.local > /dev/null 2>&1
            if [ $? != 0 ]; then
                echo "echo \"${starting}\"" >> /etc/rc.d/rc.local
                echo "${INSTALLDIR}/bin/xcyber360-control start" >> /etc/rc.d/rc.local
            fi
            return 0;
        elif [ -d "/etc/rc.d/init.d" ]; then
            echo " - ${systemis} Linux (SysV)."
            echo " - ${modifiedinit}"
            GenerateService xcyber360-server.init > /etc/rc.d/init.d/${service}
            chmod 755 /etc/rc.d/init.d/${service}
            chown root:xcyber360 /etc/rc.d/init.d/${service}
            return 0;
        # Taken from Stephen Bunn ossec howto.
            ...
            echo " - ${modifiedinit}"
            GenerateService xcyber360-server-debian.init > /etc/init.d/${service}
            chmod +x /etc/init.d/${service}
            chmod go-w /etc/init.d/${service}
            chown root:xcyber360 /etc/init.d/${service}
