...
if [[ "$OS" == "Darwin" ]]; then
    if [ "${XCYBER360_HOME}" != "/Library/Ossec" ]; then
    ...
    if [ "${XCYBER360_HOME}" != "/var/ossec" ]; then
...
if [[ "$OS" == "Darwin" ]]; then
    installer -pkg ./var/upgrade/xcyber360-agent* -target / >> ./logs/upgrade.log 2>&1
elif [[ "$OS" == "Linux" ]]; then
    if find ./var/upgrade/ -mindepth 1 -maxdepth 1 -type f -name "*.rpm" | read; then
        if command -v rpm >/dev/null 2>&1; then
            rpm -Uvh ./var/upgrade/xcyber360-agent* >> ./logs/upgrade.log 2>&1
        else
        ...
        if command -v dpkg >/dev/null 2>&1; then
            dpkg -i ./var/upgrade/xcyber360-agent* >> ./logs/upgrade.log 2>&1
        else
        ...
        if command -v apk >/dev/null 2>&1; then
            apk add --allow-untrusted --force ./var/upgrade/xcyber360-agent* >> ./logs/upgrade.log 2>&1
        else
...
echo "$(date +"%Y/%m/%d %H:%M:%S") - Checking for Xcyber360 Agent control script." >> ./logs/upgrade.log

if [ -f "./bin/xcyber360-control" ]; then
    echo "$(date +"%Y/%m/%d %H:%M:%S") - Checking if Xcyber360 Agent is running." >> ./logs/upgrade.log
    if ./bin/xcyber360-control status >/dev/null 2>&1; then
        echo "$(date +"%Y/%m/%d %H:%M:%S") - Xcyber360 Agent is already running." >> ./logs/upgrade.log
    else
        echo "$(date +"%Y/%m/%d %H:%M:%S") - Starting Xcyber360 Agent." >> ./logs/upgrade.log
        ./bin/xcyber360-control start >> ./logs/upgrade.log 2>&1
    fi
elif [ -f "./bin/ossec-control" ]; then
    echo "$(date +"%Y/%m/%d %H:%M:%S") - Upgrade failed: xcyber360-control not found. Attempting to restart using ossec-control." >> ./logs/upgrade.log
    ./bin/ossec-control restart >> ./logs/upgrade.log 2>&1
    echo -ne "2" > ./var/upgrade/upgrade_result
    rm -f $LOCK
    exit 1
else
    echo "$(date +"%Y/%m/%d %H:%M:%S") - Upgrade failed: Neither xcyber360-control nor ossec-control were found." >> ./logs/upgrade.log
    echo -ne "2" > ./var/upgrade/upgrade_result
    rm -f $LOCK
    exit 1
fi
...
while [ "$status" != "connected" -a $COUNTER -gt 0 ]; do
    . ./var/run/xcyber360-agentd.state >> ./logs/upgrade.log 2>&1
