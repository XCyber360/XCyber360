...
cp -rf /tmp_volume/config/* /var/ossec/ && chown -R xcyber360:xcyber360 /var/ossec/api
...
sed -i "s:validate_responses=False:validate_responses=True:g" /var/ossec/api/core/scripts/xcyber360_apid.py
...
chown root:xcyber360 /var/ossec/etc/client.keys
chown -R xcyber360:xcyber360 /var/ossec/queue/db
chown -R xcyber360:xcyber360 /var/ossec/etc/shared
chmod --reference=/var/ossec/etc/shared/default /var/ossec/etc/shared/group*
cd /var/ossec/etc/shared && find -name merged.mg -exec chown xcyber360:xcyber360 {} \; && cd /
chown root:xcyber360 /var/ossec/etc/shared/ar.conf
...
if [ "$3" == "master" ]; then
  ...
  # Wait until Xcyber360 API is ready
  elapsed_time=0
