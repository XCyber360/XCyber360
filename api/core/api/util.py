...
def to_relative_path(full_path: str) -> str:
    """Return a relative path from Xcyber360 base directory.
    ...
    -------
    str
        Relative path from Xcyber360 base directory.
    """
    return os.path.relpath(full_path, common.XCYBER360_SHARE)
...
def _create_problem(exc: Exception, code: int = None):
    ...
    ----------
    exc : Exception
        If `exc` is an instance of `Xcyber360Exception` it will be casted into a ProblemException,
...
def only_master_endpoint(func):
    ...
    async def wrapper(*args, **kwargs):
        if not running_in_master_node():
            raise_if_exc(exception.Xcyber360ResourceNotFound(902))
        else:
