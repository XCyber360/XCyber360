...
def build_and_up(interval: int = 10, build: bool = True):
    ...
    if build:
        ...
        response = requests.get(f"https://github.com/xcyber360/xcyber360/tarball/{current_branch}")
        ...
        while retries < max_retries:
            if build:
                build_process = subprocess.Popen(["docker", "compose",
                    "build", "--build-arg", f"XCYBER360_BRANCH={current_branch}",
                    "--build-arg", "--no-cache"],
...
def check_health(node_type: str = 'manager', agents: list = None):
    """Check the Xcyber360 nodes health.
    ...
    if node_type == 'manager':
        for node in env_cluster_nodes:
            health = subprocess.check_output(
                f"docker inspect env-xcyber360-{node}-1 -f '{{{{json .State.Health.Status}}}}'",
                shell=True)
        ...
        for agent in agents:
            health = subprocess.check_output(
                f"docker inspect env-xcyber360-agent{agent}-1 -f '{{{{json .State.Health.Status}}}}'",
                shell=True)
            if not health.startswith(b'"healthy"'):
                return False
...
def save_logs(test_name: str):
    """Save API, cluster and Xcyber360 logs from every cluster node and Xcyber360 logs from every agent if tests fail.
    Save haproxy-lb log.
        ...
        for log in logs:
            try:
                subprocess.check_output(
                    f"docker cp env-xcyber360-{node}-1:{os.path.join(logs_path, log)} "
    ...
    for agent in agent_names:
        try:
            subprocess.check_output(
                f"docker cp env-xcyber360-{agent}-1:{os.path.join(logs_path, 'ossec.log')} "
...
def api_test(request: _pytest.fixtures.SubRequest):
    ...
    while retries < max_retries:
        ...
        try:
            error_message = subprocess.check_output(["docker", "exec", "-t", "env-xcyber360-master-1", "sh", "-c",
