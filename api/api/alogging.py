...
class Xcyber360JsonFormatter(jsonlogger.JsonFormatter):
    """
...
def set_logging(log_filepath, logging_config: RotatedLoggingConfig, foreground_mode: bool = False) -> dict:
    ...
    This function creates a logging configuration dictionary, configure the xcyber360-api logger
    ...
    log_config_dict = {
        ...
        "formatters": {
            ...
            "json" : {
                '()': 'api.alogging.Xcyber360JsonFormatter',
                'style': '%',
                'datefmt': "%Y/%m/%d %H:%M:%S"
            }
        },
        "filters": {
            'plain-filter': {'()': 'xcyber360.core.wlogging.CustomFilter',
                             'log_type': 'log' },
            'json-filter': {'()': 'xcyber360.core.wlogging.CustomFilter',
                             'log_type': 'json' }
        },
        "handlers": {
            "default": {
        ...
        "loggers": {
            "xcyber360-api": {"handlers": hdls, "level": log_level, "propagate": False},
            "start-stop-api": {"handlers": hdls, "level": INFO, "propagate": False},
            "xcyber360-comms-api": {"handlers": hdls, "level": log_level, "propagate": False}
        }
    }
    ...
    for handler, d in handlers.items():
        if d and 'filename' in d:
            if logging_config.max_size.enabled:
                ...
                d.update({
                    'class':'xcyber360.core.wlogging.SizeBasedFileRotatingHandler',
                    'maxBytes': max_size,
                    'backupCount': 1
                })
            else:
                d.update({
                    'class': 'xcyber360.core.wlogging.TimeBasedFileRotatingHandler',
                    'when': 'midnight'
                })
