...
with patch('xcyber360.common.xcyber360_uid'):
    with patch('xcyber360.common.xcyber360_gid'):
        ...
        from xcyber360.core.exception import Xcyber360Exception, Xcyber360PermissionError
        from xcyber360.core.results import AffectedItemsXcyber360Result
        from xcyber360.rbac import preprocessor
...
def mock_request():
    ...
    with TestContext(operation=operation):
        with patch('api.controllers.security_controller.request', MagicMock) as m_req:
            ...
            m_req.context = {
                'token_info': {'sub': 'xcyber360', 'run_as': 'manager', 'rbac_policies': {}}
            }
            yield m_req
...
async def test_login_user_ko(mock_token, mock_exc, mock_dapi, mock_remove, mock_dfunc, mock_bool):
    """Verify 'login_user' endpoint is handling Xcyber360Exception as expected."""
    mock_token.side_effect = Xcyber360Exception(999)
...
async def test_run_as_login_ko(mock_token, mock_exc, mock_dapi, mock_remove, mock_dfunc,
    ...
    """Verify 'run_as_login' endpoint is handling Xcyber360Exception as expected."""
    mock_token.side_effect = Xcyber360Exception(999)
...
async def test_get_user_me_policies(mock_request):
    ...
    with patch('api.controllers.security_controller.Xcyber360Result', return_value='mock_wr_result') as mock_wr:
...
@patch('api.controllers.security_controller.DistributedAPI.__init__', return_value=None)
@patch('api.controllers.security_controller.raise_if_exc', return_value=CustomAffectedItems())
@patch('api.controllers.security_controller.type', return_value=AffectedItemsXcyber360Result)
...
async def test_revoke_all_tokens_ko(mock_type, mock_len, mock_exc, mock_dapi, mock_remove, mock_dfunc,
                                    mock_request):
    """Verify 'revoke_all_tokens' endpoint is handling Xcyber360PermissionError as expected."""
    with patch('api.controllers.security_controller.get_system_nodes', return_value=AsyncMock()) as mock_snodes:
        ...
        mock_exc.assert_has_calls([call(mock_dfunc.return_value),
                                   call(Xcyber360PermissionError(4000, mock_exc.return_value.message))])
        assert mock_exc.call_count == 2
        mock_remove.assert_called_once_with({})
        assert isinstance(result, ConnexionResponse)
