...
from xcyber360 import security, __version__
...
from xcyber360.core.security import revoke_tokens
...
@deprecate_endpoint(link=f'https://documentation.xcyber360.com/{__version__}/user-manual/api/reference.html#'
...
async def deprecated_login_user(user: str, raw: bool = False) -> ConnexionResponse:
    ...
    return ConnexionResponse(body=token,
           ...
           ConnexionResponse(body=dumps(Xcyber360Result({'data': TokenResponseModel(token=token)})),
...
async def get_user_me_policies(pretty: bool = False, wait_for_complete: bool = False) -> ConnexionResponse:
    ...
    """
    data = Xcyber360Result({'data': request.context['token_info']['rbac_policies'],
...
async def revoke_all_tokens(pretty: bool = False) -> ConnexionResponse:
    ...
    data = raise_if_exc(await dapi.distribute_function())
    if type(data) == AffectedItemsXcyber360Result and len(data.affected_items) == 0:
        raise_if_exc(Xcyber360PermissionError(4000, data.message))
