...
_xcyber360_key = re.compile(r'[a-zA-Z0-9]+$')
_xcyber360_version = re.compile(r'^(?:xcyber360 |)v?\d+\.\d+\.\d+$', re.IGNORECASE)
...
XCYBER360_COMPONENT_CONFIGURATION_MAPPING = MappingProxyType(
    {
...
def is_safe_path(path: str, basedir: str = common.XCYBER360_ETC, relative: bool = True) -> bool:
    ...
    path : str
        Path to be checked.
    basedir : str
        Xcyber360 installation directory.
    relative : bool
...
def check_component_configuration_pair(component: str, configuration: str) -> Xcyber360Error:
    """

    Parameters
    ----------
    component : str
        Xcyber360 component name.
    configuration : str
    ...
    -------
    Xcyber360Error
        It can either return a `Xcyber360Error` or `None`, depending on the given component and configuration. The exception
        is returned and not raised because we use the object to create a problem on API level.
    """
    if configuration not in XCYBER360_COMPONENT_CONFIGURATION_MAPPING[component]:
        return Xcyber360Error(1128, extra_message=f"Valid configuration values for '{component}': "
                                              f"{XCYBER360_COMPONENT_CONFIGURATION_MAPPING[component]}")
...
@Draft4Validator.FORMAT_CHECKER.checks("xcyber360_key")
def format_xcyber360_key(value):
    return check_exp(value, _xcyber360_key)


@Draft4Validator.FORMAT_CHECKER.checks("xcyber360_version")
def format_xcyber360_version(value):
    return check_exp(value, _xcyber360_version)
