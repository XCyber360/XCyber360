...
from xcyber360.core.exception import Xcyber360Error, Xcyber360PermissionError, Xcyber360ResourceNotFound, \
...
@pytest.mark.parametrize('exception_type, code, extra_fields, returned_code, returned_exception', [
    ...
    (Xcyber360Error, 1000, ['remediation', 'code'], 400, ProblemException),
    (Xcyber360PermissionError, 4000, ['remediation', 'code'], 403, ProblemException),
    (Xcyber360ResourceNotFound, 1710, ['remediation', 'code'], 404, ProblemException),
    (Xcyber360InternalError, 1000, ['remediation', 'code'], 500, ProblemException)
])
...
@pytest.mark.parametrize('obj, code', [
    ((Xcyber360Error(6001), ['value0', 'value1']), 429),
    ((Xcyber360InternalError(1000), ['value0', 'value1']), None),
    ((Xcyber360PermissionError(4000), ['value0', 'value1']), None),
    ((Xcyber360ResourceNotFound(1710), ['value0', 'value1']), None)
])
...
@pytest.mark.parametrize('link', [
    '',
    'https://documentation.xcyber360.com/current/user-manual/api/reference.html'
])
@pytest.mark.asyncio
...
async def test_only_master_endpoint(mock_exc):
    ...
    with patch('api.util.running_in_master_node', return_value=False):
        await func_()
        mock_exc.assert_called_once_with(Xcyber360ResourceNotFound(902))
