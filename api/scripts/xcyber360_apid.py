...
from xcyber360.core.config.models.central_config import ManagementAPIConfig
...
API_MAIN_PROCESS = 'xcyber360-apid'
API_LOCAL_REQUEST_PROCESS = 'xcyber360-apid_exec'
API_AUTHENTICATION_PROCESS = 'xcyber360-apid_auth'
API_SECURITY_EVENTS_PROCESS = 'xcyber360-apid_events'
...
def configure_ssl(params: dict, config: APISSLConfig):
    ...
    try:
        ...
        # Check and assign ownership to xcyber360 user for server.key and server.crt files
        utils.assign_xcyber360_ownership(config.key)
        utils.assign_xcyber360_ownership(config.cert)
        ...
        if exc.errno == 22:
            ...
            msg = f'Xcyber360 API SSL ERROR. Please, ensure ' \
                    f'if path to certificates is correct in the configuration ' \
                    f'file {XCYBER360_SERVER_YML}'
            print(msg)
            logger.error(msg)
            raise exc from exc
...
def start(params: dict, config: ManagementAPIConfig):
    """Run the Xcyber360 API.

    If another Xcyber360 API is running, this function will fail because uvicorn server will
    ...
    app.add_middleware(Xcyber360AccessLoggerMiddleware, MiddlewarePosition.BEFORE_EXCEPTION)
...
def print_version():
    ...
    print('\n{} {} - {}\n\n{}'.format(__xcyber360_name__, __version__, __author__, __licence__))
...
def add_debug2_log_level_and_error():
    """Add a new debug level used by xcyber360 api and framework."""
...
if __name__ == '__main__':
    ...
    from xcyber360.core import common, pyDaemonModule, utils
    from xcyber360.rbac.orm import check_database_integrity
    ...
    from api.middlewares import (
        ...
        Xcyber360AccessLoggerMiddleware,
    ...
    try:
        ...
        print(f"Error when trying to start the Xcyber360 API. {api_log_error}")
        sys.exit(1)

    # set permission on log files
    for handler in uvicorn_params['log_config']['handlers'].values():
        if 'filename' in handler:
            utils.assign_xcyber360_ownership(handler['filename'])
            os.chmod(handler['filename'], 0o660)

    # Configure and create the xcyber360-api logger
    ...
    try:
        ...
        print(f"Error when trying to start the Xcyber360 API. {e}")
        sys.exit(1)
    except Exception as e:
        print(f'Internal error when trying to start the Xcyber360 API. {e}')
        sys.exit(1)
    finally:
