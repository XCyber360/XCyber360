...
from xcyber360.core.batcher.config import BatcherConfig
from xcyber360.core.batcher.mux_demux import MuxDemuxQueue, MuxDemuxManager
...
MAIN_PROCESS = 'xcyber360-comms-apid'
...
def configure_ssl(keyfile: str, certfile: str) -> None:
    ...
    try:
        ...
        raise Xcyber360CommsAPIError(2700, extra_message=str(exc))
    except IOError as exc:
        if exc.errno == 22:
            raise Xcyber360CommsAPIError(2701, extra_message=str(exc))
        elif exc.errno == 13:
            raise Xcyber360CommsAPIError(2702, extra_message=str(exc))
        else:
            raise Xcyber360CommsAPIError(2703, extra_message=str(exc))
...
def get_gunicorn_options(pid: int, foreground_mode: bool, log_config_dict: dict, config: CommsAPIConfig) -> dict:
    ...
    pidfile = common.XCYBER360_RUN / f'{MAIN_PROCESS}-{pid}.pid'
...
if __name__ == '__main__':
    ...
    logger = logging.getLogger('xcyber360-comms-api')
    ...
    except Xcyber360CommsAPIError as e:
        logger.error(f'Error when trying to start the Xcyber360 Communications API. {e}')
        exit(1)
    except Exception as e:
        logger.error(f'Internal error when trying to start the Xcyber360 Communications API. {e}')
        exit(1)
    finally:
