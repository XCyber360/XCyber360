...
@patch('xcyber360.core.indexer.Indexer._get_opensearch_client', new_callable=AsyncMock)
...
@patch('xcyber360.core.indexer.agent.AgentsIndex.update')
...
@pytest.mark.parametrize('exception,message', [
    (Xcyber360IndexerError(2200), 'Couldn\'t connect to the indexer: Error 2200 - Could not connect to the indexer'),
    (Xcyber360ResourceNotFound(1701), 'Agent does not exist'),
    (Xcyber360InternalError(6003), 'Couldn\'t get key pair: Error 6003 - Error trying to load the JWT secret'),
])
async def test_authentication_ko(exception, message):
    """Verify that the `authentication` handler catches exceptions successfully."""
    with patch('xcyber360.core.indexer.create_indexer', AsyncMock(side_effect=exception)):
