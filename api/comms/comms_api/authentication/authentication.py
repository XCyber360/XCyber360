...
JWT_AUDIENCE = 'Xcyber360 Communications API'
JWT_EXPIRATION = 900
...
class JWTBearer(HTTPBearer):
    ...
    async def __call__(self, request: Request) -> str:
        ...
        except Xcyber360CommsAPIError as exc:
...
def decode_token(token: str) -> dict:
    ...
    try:
        ...
        if (payload['exp'] - payload['iat']) != JWT_EXPIRATION:
            raise Xcyber360CommsAPIError(2706)

        current_timestamp = int(get_utc_now().timestamp())
        if payload['exp'] <= current_timestamp:
            raise Xcyber360CommsAPIError(2707)

        return payload
    except PyJWTError as exc:
        raise Xcyber360CommsAPIError(2706) from exc
