...
from xcyber360.core.indexer import Indexer
from xcyber360.core.indexer.bulk import Operation
from xcyber360.core.indexer.models.agent import Host, OS
from xcyber360.core.indexer.models.events import Agent, AgentMetadata, CommandResult, SCAEvent, TaskResult, StatefulEvent, \
    Header, Module, Result

INDEXER = Indexer(host='host', user='xcyber360', password='xcyber360')


@patch('xcyber360.core.engine.events.EventsModule.send', new_callable=AsyncMock)
...
async def test_send_stateless_events_ko():
    ...
    with pytest.raises(Xcyber360Error, match=r'2708'):
        await send_stateless_events(request=request)


@patch('xcyber360.core.indexer.create_indexer', return_value=AsyncMock())
...
async def test_parse_stateful_events_ko(headers, expected_code):
    ...
    with pytest.raises(Xcyber360Error, match=rf'{expected_code}'):
