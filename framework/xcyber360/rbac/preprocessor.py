...
from xcyber360.core.exception import Xcyber360Error, Xcyber360PermissionError
from xcyber360.core.results import Xcyber360Result
from xcyber360.rbac.auth_context import RBAChecker, get_policies_from_roles
from xcyber360.rbac.orm import AuthenticationManager
        ...
        for action in policy['actions']:
            ...
            for resource in policy['resources']:
                if not re.match(resource_regex, resource):
                    raise Xcyber360Error(4500)
                resource_type = PreProcessor.is_combination(resource)
                if len(resource_type[1]) > 2:
                    raise Xcyber360Error(4500, extra_remediation="The maximum length for permission combinations is two")
...
def get_permissions(user_id: int = None, auth_context: Union[dict, str] = None) -> Xcyber360Result:
