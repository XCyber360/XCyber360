...
from xcyber360.rbac.tests.utils import init_db, MockRolePolicy, MockedUserRole, MockRoleRules
from xcyber360.rbac.orm import XCYBER360_USER_ID, XCYBER360_WUI_USER_ID, MAX_ID_RESERVED, User, SecurityError, Roles, Rules, \
        ...
        with patch('sqlalchemy.create_engine', return_value=create_engine("sqlite://")):
            with patch('shutil.chown'), patch('os.chmod'):
                with patch('api.constants.SECURITY_PATH', new=test_data_path):
                    import xcyber360.rbac.orm as rbac
                    # Clear mappers
...
def test_database_init(db_setup):
    ...
    with db_setup.RolesManager() as rm:
        assert rm.get_role('xcyber360') != db_setup.SecurityError.ROLE_NOT_EXIST
...
def test_delete_all_expired_rules(db_setup):
    """Check that rules are correctly deleted"""
    with patch('xcyber360.rbac.orm.time', return_value=0):
        add_token(db_setup)
    with db_setup.TokenManager() as tm:
...
@patch("xcyber360.rbac.orm.create_engine")
@patch("xcyber360.rbac.orm.sessionmaker")
...
@patch("xcyber360.rbac.orm._Base.metadata.create_all")
...
def test_databasemanager_get_table(fresh_in_memory_db):
    ...
    with patch("xcyber360.rbac.orm._new_columns", new={"new_column"}):
...
@patch("xcyber360.rbac.orm.safe_move")
@patch("xcyber360.rbac.orm.os.remove")
@patch("xcyber360.rbac.orm.chown")
@patch("xcyber360.rbac.orm.os.chmod")
def test_check_database_integrity(chmod_mock, chown_mock, remove_mock, safe_move_mock, fresh_in_memory_db):
    ...
    db_mock = MagicMock()
    with patch("xcyber360.rbac.orm.db_manager", new=db_mock):
        with patch("xcyber360.rbac.orm.os.path.exists", return_value=True):
            with patch("xcyber360.rbac.orm.CURRENT_ORM_VERSION", new=99999):
                ...
                db_mock.assert_has_calls([
                    ...
                    call.migrate_data(source=fresh_in_memory_db.DB_FILE, target=fresh_in_memory_db.DB_FILE_TMP,
                                      from_id=fresh_in_memory_db.XCYBER360_USER_ID,
                                      to_id=fresh_in_memory_db.XCYBER360_WUI_USER_ID),
                    call.migrate_data(source=fresh_in_memory_db.DB_FILE, target=fresh_in_memory_db.DB_FILE_TMP,
        ...
        safe_move_mock.assert_called_once_with(fresh_in_memory_db.DB_FILE_TMP, fresh_in_memory_db.DB_FILE,
                                               ownership=(fresh_in_memory_db.xcyber360_uid(),
                                                          fresh_in_memory_db.xcyber360_gid()),
                                               permissions=0o640)
...
@patch("xcyber360.rbac.orm.DatabaseManager.close_sessions")
@patch("xcyber360.rbac.orm.os.remove")
def test_check_database_integrity_exceptions(remove_mock, close_sessions_mock, exception, fresh_in_memory_db):
    ...
    with patch("xcyber360.rbac.orm.os.path.exists", side_effect=mocked_exists) as mock_exists:
        with patch("xcyber360.rbac.orm.DatabaseManager.connect", side_effect=exception) as db_manager_mock:
...
@pytest.mark.parametrize('from_id, to_id, users', [
    (XCYBER360_USER_ID, XCYBER360_WUI_USER_ID, [
        User('xcyber360', 'test', user_id=XCYBER360_USER_ID),
        User('xcyber360-wui', 'test2', user_id=XCYBER360_WUI_USER_ID)
    ]),
...
def test_migrate_data(db_setup, from_id, to_id, users):
    ...
    if to_id == XCYBER360_WUI_USER_ID:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=users):
            with patch("xcyber360.rbac.orm.AuthenticationManager.update_user") as mock_update_user:
        ...
        empty_list = []
        user_exists = False
        with patch("xcyber360.rbac.orm.AuthenticationManager.add_user",
                   side_effect=[not user_exists, user_exists, not user_exists]) as mock_add_user:
            with patch("xcyber360.rbac.orm.db_manager.get_data",
...
@pytest.mark.parametrize('user_data',
                         [
                             {'user': User('xcyber360', 'test', user_id=XCYBER360_USER_ID), 'status': True,
                              'expected_update': True},
                             {'user': User('xcyber360', 'test', user_id=XCYBER360_WUI_USER_ID), 'status': True,
                              'expected_update': True},
                             {'user': User('xcyber360', 'test', user_id=110), 'status': True,
                              'expected_update': False},
                             {'user': User('xcyber360', 'test', user_id=101), 'status': False,
                              'expected_update': False},
                         ]
                         )
def test_authentication_manager_migrate_data(db_setup, user_data):
    ...
    with db_setup.AuthenticationManager() as auth_manager:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=[user]):
            with patch("xcyber360.rbac.orm.AuthenticationManager.update_user") as mock_update_user:
                with patch("xcyber360.rbac.orm.AuthenticationManager.add_user",
...
@pytest.mark.parametrize('role_data',
                         [
                             {'user': Roles(XCYBER360_USER_ID, 'xcyber360'), 'status': True},
                             {'user': Roles(XCYBER360_WUI_USER_ID, 'xcyber360'), 'status': True},
                             {'user': Roles(110, 'xcyber360'), 'status': True},
                             {'user': Roles(101, 'xcyber360'), 'status': SecurityError.ALREADY_EXIST},
                         ]
                         )
def test_roles_manager_migrate_data(db_setup, role_data):
    ...
    with db_setup.RolesManager() as role_manager:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=[role]):
            with patch("xcyber360.rbac.orm.RolesManager.add_role",
...
def test_roles_manager_migrate_data_with_reserved_ids(db_setup):
    ...
    role = Roles(XCYBER360_USER_ID, 'xcyber360')

    with db_setup.RolesManager() as role_manager:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=[role]):
            with patch("xcyber360.rbac.orm.RolesManager.add_role") as mock_add_role:
                role_manager.migrate_data(db_setup.db_manager, source=db_setup.DB_FILE, target=db_setup.DB_FILE,
                                          from_id=XCYBER360_USER_ID, to_id=XCYBER360_WUI_USER_ID)
...
def test_rules_manager_migrate_data(db_setup, rules_data):
    ...
    with db_setup.RulesManager() as rule_manager:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=[rule]):
            with patch('xcyber360.rbac.orm.db_manager.sessions', autospec=True) as mock_sessions:
                with patch("xcyber360.rbac.orm.RulesManager.add_rule", side_effect=[status, status]) as mock_add_rule:
                    with patch("xcyber360.rbac.orm.db_manager.get_table") as mock_get_table:
                        with patch("xcyber360.rbac.orm.RolesRulesManager.add_rule_to_role") as mock_add_rule_to_role:
...
def test_rules_manager_migrate_data_with_reserved_ids(db_setup):
    ...
    with db_setup.RulesManager() as rule_manager:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=[rule]):
            with patch("xcyber360.rbac.orm.RulesManager.add_rule") as mock_add_rule:
                rule_manager.migrate_data(db_setup.db_manager, source=db_setup.DB_FILE, target=db_setup.DB_FILE,
                                          from_id=XCYBER360_USER_ID, to_id=XCYBER360_WUI_USER_ID)
...
@pytest.mark.parametrize('policy_data',
                         [
                             {'policy': Policies('xcyber360', '{}', policy_id=XCYBER360_USER_ID), 'status': True},
                             {'policy': Policies('xcyber360', '{}', policy_id=XCYBER360_USER_ID),
...
def test_policies_manager_migrate_data(db_setup, policy_data):
    ...
    with db_setup.PoliciesManager() as policy_manager:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=[policy]):
            with patch('xcyber360.rbac.orm.db_manager.sessions', autospec=True) as mock_sessions:
                with patch("xcyber360.rbac.orm.db_manager.get_table") as mock_get_table:
                    mock_query = mock_sessions.return_value.query.return_value
                    mock_filter_by = mock_query.filter_by.return_value
                    mock_first = mock_filter_by.first.return_value
                    mock_first.id = 'id'
                    with patch("xcyber360.rbac.orm.PoliciesManager.add_policy",
...
def test_policies_manager_migrate_data_with_reserved_ids(db_setup):
    ...
    policy = Policies('xcyber360', 'some_policy')

    with db_setup.PoliciesManager() as policy_manager:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=[policy]):
            with patch("xcyber360.rbac.orm.PoliciesManager.add_policy") as mock_add_policy:
...
def test_user_roles_manager_migrate_data(db_setup, user_role_data):
    ...
    with db_setup.UserRolesManager() as user_roles_manager:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=[user_role]):
            with patch("xcyber360.rbac.orm.UserRolesManager.add_role_to_user") as mock_add_role_to_user:
                with patch('xcyber360.rbac.orm.AuthenticationManager.get_user', autospec=True,
                           return_value={'id': None}) as mock_get_user:
                    with patch('xcyber360.rbac.orm.RolesManager.get_role', autospec=True,
...
def test_roles_policies_manager_migrate_data(db_setup, role_policies_data):
    ...
    with db_setup.RolesPoliciesManager() as auth_manager:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=[role_policies]):
            with patch("xcyber360.rbac.orm.RolesPoliciesManager.add_policy_to_role") as mock_add_policy_to_role:
                with patch('xcyber360.rbac.orm.PoliciesManager.get_policy', autospec=True,
...
def test_roles_rules_manager_migrate_data(db_setup, role_rules_data):
    ...
    with db_setup.RolesRulesManager() as roles_rules_manager:
        with patch("xcyber360.rbac.orm.db_manager.get_data", return_value=[role_rules]):
            with patch("xcyber360.rbac.orm.RolesRulesManager.add_rule_to_role") as mock_add_rule_to_role:
                with patch('xcyber360.rbac.orm.RulesManager.get_rule_by_name', autospec=True,
