...
from xcyber360.core.common import xcyber360_uid, xcyber360_gid, DEFAULT_RBAC_RESOURCES, XCYBER360_LIB
from xcyber360.core.utils import get_utc_now, safe_move
from xcyber360.rbac.utils import clear_cache
...
class AuthenticationManager(RBACManager):
    ...
    def migrate_data(self, manager, source: str, target: str, from_id: Optional[int] = None,
        ...
        for user in old_users:
            if user.id in (XCYBER360_USER_ID, XCYBER360_WUI_USER_ID):
...
def check_if_reserved_id(from_id: Optional[str], to_id: Optional[str]):
    ...
    bool
        Condition result
    """
    return from_id == XCYBER360_USER_ID and to_id == XCYBER360_WUI_USER_ID
...
def check_database_integrity():
    ...
    def _set_permissions_and_ownership(database: str):
        """Set Xcyber360 ownership and permissions.
        ...
        """
        chown(database, xcyber360_uid(), xcyber360_gid())
    ...
    try:
        ...
        if os.path.exists(DB_FILE):
            ...
            if current_version < expected_version:
                ...
                db_manager.migrate_data(source=DB_FILE, target=DB_FILE_TMP, from_id=XCYBER360_USER_ID,
                                        to_id=XCYBER360_WUI_USER_ID)
                ...
                safe_move(DB_FILE_TMP, DB_FILE,
                          ownership=(xcyber360_uid(), xcyber360_gid()),
                          permissions=0o640)
        ...
        logger.error("Error retrieving the current Xcyber360 RBAC database version. Aborting database integrity check")
        db_manager.close_sessions()
        raise e
