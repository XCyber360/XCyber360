...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        ...
        from xcyber360.ciscat import get_ciscat_results
...
@patch('xcyber360.ciscat.get_agents_info', return_value=['001'])
...
def test_get_ciscat_results_select_ko(agents_info_mock, socket_mock, exists_mock):
    ...
    with patch('xcyber360.core.utils.Xcyber360DBConnection') as mock_wdb:
        mock_wdb.return_value = InitWDBSocketMock(sql_schema_file=db_file)
        with pytest.raises(Xcyber360Error, match=r'\b1724\b'):
