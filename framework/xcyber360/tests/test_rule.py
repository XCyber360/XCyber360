...
from xcyber360.core.common import USER_RULES_PATH
import pytest
...
def mock_xcyber360_paths():
    with patch('xcyber360.core.common.RULES_PATH', new=os.path.join(parent_directory, core_data_path)):
        with patch('xcyber360.core.common.USER_RULES_PATH', new=os.path.join(parent_directory, tests_data_path)):
            with patch('xcyber360.core.common.XCYBER360_PATH', new=parent_directory):
                with patch('xcyber360.rule.to_relative_path', side_effect=lambda x: os.path.relpath(x, parent_directory)):
                    yield
...
@patch("xcyber360.rule.configuration.get_ossec_conf", return_value=rule_ossec_conf)
def test_get_rules_files_status_include(mock_ossec, status, func):
    ...
    if status == 'random':
        ...
        with pytest.raises(Xcyber360Error, match='.* 1202 .*'):
            func(status=status)
    else:
...
@patch('xcyber360.core.configuration.get_ossec_conf', return_value=rule_ossec_conf)
...
@patch('xcyber360.core.configuration.get_ossec_conf', return_value=other_rule_ossec_conf)
...
def test_failed_get_rules():
    ...
    with pytest.raises(Xcyber360Error, match=".* 1203 .*"):
...
def test_get_rule_file_path(filename, relative_dirname, result, mock_xcyber360_paths):
    """Test get_rule_file_path function."""
    with patch('xcyber360.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        res = rule.get_rule_file_path(filename=filename, 
                                            relative_dirname=relative_dirname)
        assert res == os.path.join(xcyber360.core.common.XCYBER360_PATH, result) if result else not res
...
def test_get_rule_file(filename, raw, relative_dirname, contains):
    ...
    with patch('xcyber360.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        if raw:
            ...
            assert isinstance(d_files, AffectedItemsXcyber360Result)
...
@patch('xcyber360.rule.delete_rule_file')
@patch('xcyber360.rule.full_copy')
@patch('xcyber360.rule.upload_file')
@patch('xcyber360.rule.remove')
@patch('xcyber360.rule.safe_move')
@patch('xcyber360.rule.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_xml, mock_full_copy,
    ...
    with patch('xcyber360.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        with patch('xcyber360.rule.validate_upload_delete_dir', return_value=ret_validation):
            with patch('xcyber360.rule.exists', return_value=overwrite):
                ...
                if overwrite:
                    full_path = os.path.join(xcyber360.common.XCYBER360_PATH, rule_path)
...
@patch('xcyber360.rule.delete_rule_file', side_effect=Xcyber360Error(1019))
@patch('xcyber360.rule.upload_file')
@patch('xcyber360.rule.safe_move')
...
def test_delete_rule_file(file, relative_dirname):
    ...
    with patch('xcyber360.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        with patch('xcyber360.rule.exists', return_value=True):
            with patch('xcyber360.rule.remove'):
...
def test_delete_rule_file_ko():
    ...
    with patch('xcyber360.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        with patch('xcyber360.rule.remove', side_effect=IOError()):
