...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        with patch('xcyber360.core.utils.load_xcyber360_xml'):
            ...
            from xcyber360.manager import *
            from xcyber360.core.manager import LoggingFormat
            from xcyber360.core.tests.test_manager import get_logs
...
def mock_xcyber360_path():
    with patch('xcyber360.core.common.XCYBER360_PATH', new=test_data_path):
        yield
...
manager_status = {'xcyber360-agentlessd': 'running', 'xcyber360-analysisd': 'running', 'xcyber360-authd': 'running',
 'xcyber360-csyslogd': 'running', 'xcyber360-dbd': 'running', 'xcyber360-monitord': 'running',
 'xcyber360-execd': 'running', 'xcyber360-integratord': 'running', 'xcyber360-logcollector': 'running',
 'xcyber360-maild': 'running', 'xcyber360-remoted': 'running', 'xcyber360-reportd': 'running',
 'xcyber360-syscheckd': 'running', 'xcyber360-clusterd': 'running', 'xcyber360-modulesd': 'running',
 'xcyber360-db': 'running', 'xcyber360-apid': 'running', 'xcyber360-comms-apid': 'running'}


@patch('xcyber360.core.manager.status', return_value=manager_status)
def test_get_status(mock_status):
    """Tests get_status() function works"""
    result = get_status()

    # Assert there are no errors and type returned
    assert isinstance(result, AffectedItemsXcyber360Result), 'No expected result type'
...
@pytest.mark.parametrize('tag, level, total_items, sort_by, sort_ascending', [
    ...
    ('xcyber360-modulesd:database', None, 2, None, None),
    ('xcyber360-modulesd:syscollector', None, 2, None, None),
    ('xcyber360-modulesd:syscollector', None, 2, None, None),
    ('xcyber360-modulesd:aws-s3', None, 5, None, None),
    ('xcyber360-execd', None, 1, None, None),
    ('xcyber360-csyslogd', None, 2, None, None),
...
@patch("xcyber360.core.manager.get_xcyber360_active_logging_format", return_value=LoggingFormat.plain)
@patch("xcyber360.core.manager.exists", return_value=True)
def test_ossec_log(mock_exists, mock_active_logging_format, tag, level, total_items, sort_by, sort_ascending):
    ...
    tag : str
        Filters by log category (i.e. xcyber360-remoted).
    total_items : int
    ...
    """
    with patch('xcyber360.core.manager.tail') as tail_patch:
        ...
        if tag is not None and level != 'xcyber360-modulesd:syscollector':
...
def test_ossec_log_summary(mock_exists, mock_active_logging_format):
    ...
    expected_result = {
        'xcyber360-csyslogd': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'xcyber360-execd': {'all': 1, 'info': 0, 'error': 1, 'critical': 0, 'warning': 0, 'debug': 0},
        'xcyber360-modulesd:aws-s3': {'all': 5, 'info': 2, 'error': 1, 'critical': 0, 'warning': 2, 'debug': 0},
        'xcyber360-modulesd:database': {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 2},
        'xcyber360-modulesd:syscollector': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'xcyber360-rootcheck': {'all': 1, 'info': 1, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0}
    }

    logs = get_logs().splitlines()
    with patch('xcyber360.core.manager.tail', return_value=logs):
...
@patch('socket.socket')
@patch('xcyber360.core.cluster.utils.fcntl')
@patch('xcyber360.core.cluster.utils.open')
...
def test_restart_ko_socket(mock_exists, mock_fcntl, mock_open):
    ...
    with pytest.raises(Xcyber360InternalError, match='.* 1901 .*'):
        restart()

    # Socket error
    with patch("os.path.exists", return_value=True):
        with patch('socket.socket', side_effect=socket.error):
            with pytest.raises(Xcyber360InternalError, match='.* 1902 .*'):
                restart()

        with patch('socket.socket.connect'):
            with patch('socket.socket.send', side_effect=socket.error):
                with pytest.raises(Xcyber360InternalError, match='.* 1014 .*'):
                    restart()


@pytest.mark.parametrize('error_flag, error_msg', [
    (0, ""),
    (1, "2019/02/27 11:30:07 xcyber360-clusterd: ERROR: [Cluster] [Main] Error 3004 - Error in cluster configuration: "
        "Unspecified key"),
    (1, "2019/02/27 11:30:24 xcyber360-authd: ERROR: (1230): Invalid element in the configuration: "
        "'use_source_i'.\n2019/02/27 11:30:24 xcyber360-authd: ERROR: (1202): Configuration error at "
...
def test_validation(mock_exists, error_flag, error_msg):
    ...
    """
    with patch('xcyber360.core.manager.Xcyber360Socket') as sock:
        # Mock sock response
...
@pytest.mark.parametrize('exception', [
    Xcyber360InternalError(1013),
    Xcyber360Error(1013)
])
@patch('xcyber360.manager.validate_ossec_conf')
def test_validation_ko(mock_validate, exception):
    mock_validate.side_effect = exception

    if isinstance(exception, Xcyber360InternalError):
        with pytest.raises(Xcyber360InternalError, match='.* 1013 .*'):
            validation()
    else:
        result = validation()
...
@patch('xcyber360.core.configuration.get_active_configuration')
...
@patch('builtins.open')
@patch('xcyber360.manager.get_ossec_conf', return_value={})
...
@patch('xcyber360.manager.validate_ossec_conf', return_value={'status': 'OK'})
@patch('xcyber360.manager.write_ossec_conf')
@patch('xcyber360.manager.validate_xcyber360_xml')
@patch('xcyber360.manager.full_copy')
@patch('xcyber360.manager.exists', return_value=True)
@patch('xcyber360.manager.remove')
@patch('xcyber360.manager.safe_move')
