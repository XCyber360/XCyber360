...
import pytest
from xcyber360.core.common import USER_DECODERS_PATH
...
with patch('xcyber360.core.common.getgrnam'):
    with patch('xcyber360.core.common.getpwnam'):
        ...
        from xcyber360.core.exception import Xcyber360InternalError, Xcyber360Error
        from xcyber360.core.results import AffectedItemsXcyber360Result
        from xcyber360 import decoder
...
def mock_xcyber360_paths():
    """Mock xcyber360 paths."""
    with patch('xcyber360.core.common.XCYBER360_PATH', new=test_data_path):
        with patch('xcyber360.core.configuration.get_ossec_conf', return_value=decoder_ossec_conf):
            with patch('xcyber360.core.common.DECODERS_PATH',
                       new=os.path.join(test_data_path, "tests", "data", "decoders")):
                with patch('xcyber360.core.common.USER_DECODERS_PATH',
                           new=os.path.join(test_data_path, "tests", "data", "etc", "decoders")):
                    with patch('xcyber360.decoder.to_relative_path',
                               side_effect=lambda x: os.path.relpath(x, xcyber360.core.common.XCYBER360_PATH)):
                        yield
...
@pytest.mark.parametrize('names, status, filename, relative_dirname, parents, expected_names, expected_total_failed', [
    (None, None, None, None, False, {'agent-buffer', 'json', 'agent-upgrade', 'xcyber360', 'agent-restart'}, 0),
    (['agent-buffer'], None, None, None, False, {'agent-buffer'}, 0),
    (['agent-buffer', 'non_existing'], None, None, None, False, {'agent-buffer'}, 1),
    (None, 'enabled', None, None, False, {'agent-buffer', 'agent-upgrade', 'xcyber360', 'agent-restart'}, 0),
    (None, 'disabled', None, None, False, {'json'}, 0),
    (['agent-upgrade', 'non_existing', 'json'], 'enabled', None, None, False, {'agent-upgrade'}, 1),
    (None, None, 'test1_decoders.xml', None, False, {'agent-buffer', 'agent-upgrade', 'xcyber360', 'agent-restart'}, 0),
    (None, None, 'test2_decoders.xml', 'tests/data/decoders', False, {'json'}, 0),
    (None, 'all', None, 'tests/data/decoders', True, {'xcyber360', 'json'}, 0),
...
@pytest.mark.parametrize('conf, exception', [
    ...
    ({'ruleset': None}, Xcyber360InternalError(1500))
])
def test_get_decoders_files(conf, exception):
    """Test get_decoders_files function"""
    with patch('xcyber360.core.configuration.get_ossec_conf', return_value=conf):
        try:
            # UUT call
...
def test_get_decoder_file_path(filename, relative_dirname, result, mock_xcyber360_paths):
    """Test get_decoder_file_path function."""
    res = decoder.get_decoder_file_path(filename=filename, 
                                         relative_dirname=relative_dirname)
    assert res == os.path.join(xcyber360.core.common.XCYBER360_PATH, result) if result else not res and isinstance(res, str)
...
def test_get_decoder_file(filename, raw, relative_dirname, contains, mock_xcyber360_paths):
    ...
    if raw:
        ...
        # Assert the result is an AffectedItemsXcyber360Result
        assert isinstance(result, AffectedItemsXcyber360Result)
...
@patch('xcyber360.decoder.delete_decoder_file')
@patch('xcyber360.decoder.full_copy')
@patch('xcyber360.decoder.validate_xcyber360_xml')
@patch('xcyber360.decoder.upload_file')
@patch('xcyber360.decoder.remove')
@patch('xcyber360.decoder.safe_move')
@patch('xcyber360.decoder.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_upload_file,
                     mock_xml, mock_full_copy, mock_delete, mock_xcyber360_paths,
    ...
    with patch('xcyber360.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('xcyber360.decoder.exists', return_value=overwrite):
            with patch('xcyber360.decoder.to_relative_path',
                    side_effect=lambda x: os.path.relpath(x, xcyber360.core.common.XCYBER360_PATH)):
            ...
            if overwrite:
                full_path = os.path.join(xcyber360.common.XCYBER360_PATH, decoder_path)
...
@patch('xcyber360.decoder.delete_decoder_file', side_effect=Xcyber360Error(1019))
@patch('xcyber360.decoder.upload_file')
@patch('xcyber360.decoder.safe_move')
...
def test_upload_file_ko(*_):
    ...
    with patch('xcyber360.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('xcyber360.decoder.exists'):
    ...
    # clean backup files
    search_pattern = os.path.join(xcyber360.core.common.XCYBER360_PATH, "**", "*.backup")
...
def test_delete_decoder_file(filename, relative_dirname):
    ...
    with patch('xcyber360.decoder.exists', return_value=True):
        # Assert returned type is AffectedItemsXcyber360Result when everything is correct
        with patch('xcyber360.decoder.remove'):
...
def test_delete_decoder_file_ko():
    ...
    with patch('xcyber360.decoder.exists', return_value=True):
        with patch('xcyber360.decoder.remove', side_effect=IOError()):
