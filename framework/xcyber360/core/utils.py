...
import xcyber360.core.results as results
...
def assign_xcyber360_ownership(filepath: str):
    ...
    if not os.path.isfile(filepath):
        f = open(filepath, "w")
        f.close()
    if os.stat(filepath).st_gid != common.xcyber360_gid() or \
        os.stat(filepath).st_uid != common.xcyber360_uid():
        os.chown(filepath, common.xcyber360_uid(), common.xcyber360_gid())
...
def clean_pid_files(daemon: str) -> None:
    ...
    for pid_file in os.listdir(common.XCYBER360_RUN):
        if match := re.match(regex, pid_file):
            try:
                ...
                if daemon.replace('-', '_') in command:
                    ...
                    print(f"{daemon}: Process {pid} does not belong to {daemon}, removing from {common.XCYBER360_RUN}...")

            except (OSError, psutil.NoSuchProcess):
                print(f'{daemon}: Non existent process {pid}, removing from {common.XCYBER360_RUN}...')
            finally:
                os.remove(path.join(common.XCYBER360_RUN, pid_file))
...
def process_array(array: list, search_text: str = None, complementary_search: bool = False,
    ...
    """Process a Xcyber360 framework data array.
...
def cut_array(array: list, offset: int = 0, limit: int = common.DATABASE_LIMIT) -> list:
    ...
    if limit is not None:
        if limit > common.MAXIMUM_DATABASE_LIMIT:
            raise Xcyber360Error(1405, extra_message=str(limit))
        elif limit == 0:
            raise Xcyber360Error(1406)
    ...
    if offset < 0:
        raise Xcyber360Error(1400)
    elif limit < 1:
        raise Xcyber360Error(1401)
    else:
...
def sort_array(array: list, sort_by: list = None, sort_ascending: bool = True,
    ...
    def check_sort_fields(allowed_sort_fields, sort_by):
        ...
        if not sort_by.issubset(allowed_sort_fields):
            ...
            raise Xcyber360Error(1403, extra_remediation='Allowed sort fields: {0}. '
    ...
    if sort_by:  # array should be a dictionary or a Class
        ...
        if type(array) is set or (type(array[0]) is not dict and 'class \'xcyber360' not in str(type(array[0]))):
...
def select_array(array: list, select: list = None, required_fields: set = None,
    ...
    if allowed_select_fields and not select_no_nested.issubset(allowed_select_fields):
        raise Xcyber360Error(1724, "{}".format(', '.join(select_no_nested)))
    ...
    for item in array:
        ...
        if array and not allowed_select_fields and not selected_fields:
            raise Xcyber360Error(1724, "{}".format(', '.join(select)))
...
def delete_xcyber360_file(full_path: str) -> bool:
    """Delete a Xcyber360 file.
    ...
    if not full_path.startswith(common.XCYBER360_PATH) or '..' in full_path:
        raise Xcyber360Error(1907)
...
def _get_hashing_algorithm(hash_algorithm):
    ...
    if hash_algorithm not in algorithm_list:
        raise Xcyber360Error(1723, "Available algorithms are {0}.".format(', '.join(algorithm_list)))
...
def check_xcyber360_limits_unchanged(new_conf, original_conf):
    """Check if Xcyber360 limits remain unchanged.
    ...
    for disabled_limit in [conf for conf, allowed in limits_configuration.items() if not allowed['allow']]:
        ...
        if len(new_limits) != len(original_limits) or any(x != y for x, y in zip(new_limits, original_limits)):
            raise Xcyber360Error(1127, extra_message=f"global > limits > {disabled_limit}")
...
def check_indexer(new_conf: str, original_conf: str):
    ...
    if not upload_configuration['indexer']['allow']:
        ...
        if len(new_indexer) != len(original_indexer) or any(x != y for x, y in zip(new_indexer, original_indexer)):
            raise Xcyber360Error(1127, extra_message='indexer')
    ...
    if not blocked_configurations['public_key']['allow']:
        ...
        for api_key in api_keys:
            ...
            try:
                ...
                raise Xcyber360Error(1131, extra_message=f'{extra_msg}')
            if response_minimum_quota == minimum_quota:
                raise Xcyber360Error(1130, extra_message='integrations > virustotal')


def validate_xcyber360_configuration(data: str):
    """Check that the Xcyber360 configuration provided is valid.
...
def load_xcyber360_xml(xml_path, data=None):
    if not data:
        with open(xml_path) as f:
            try:
                data = f.read()
            except Exception as e:
                raise Xcyber360Error(1113, extra_message=str(e))
...
def load_xcyber360_yaml(filepath: str, data: str = None) -> dict:
    """Load Xcyber360 YAML configuration files.
    ...
    validate_xcyber360_configuration(data)

    try:
        parsed_data = yaml.safe_load(data)
    except yaml.YAMLError as e:
        raise Xcyber360Error(1132, extra_message=str(e))
...
def get_group_file_path(group_id: str) -> str:
    ...
    """
    return path.join(common.XCYBER360_SHARED, group_id+GROUP_FILE_EXT)


class Xcyber360Version:

    def __init__(self, version):

        pattern = r"(?:Xcyber360 )?v?(\d+)\.(\d+)\.(\d+)\-?(alpha|beta|rc)?(\d*)"
...
def get_timeframe_in_seconds(timeframe: str) -> int:
    ...
    if not timeframe.isdigit():
        if 'h' not in timeframe and 'd' not in timeframe and 'm' not in timeframe and 's' not in timeframe:
            raise Xcyber360Error(1411, timeframe)
        ...
        for or_clause in or_clauses:
            ...
            for and_clause in and_clauses:
                ...
                try:
                    ...
                    raise Xcyber360Error(1407, extra_message=f"Parameter 'q' is not valid: '{and_clause}'")
...
class Xcyber360DBBackend(AbstractDatabaseBackend):
    """
    This class describes a xcyber360 db backend that executes database queries.
    """
    ...
    def __init__(self, agent_id=None, query_format='agent', request_slice=500):
        if query_format == 'agent' and not path.exists(path.join(common.WDB_PATH, f"{agent_id}.db")):
            raise Xcyber360Error(2007, extra_message=f"There is no database for agent {agent_id}. "
    ...
    def connect_to_db(self):
        return Xcyber360DBConnection(request_slice=self.request_slice)
    ...
    def execute(self, query, request, count=False):
        """Execute SQL query through Xcyber360DB socket."""
        query = self._substitute_params(query, request)
        return self.conn.execute(query=self._render_query(query), count=count)


class Xcyber360DBQuery(object):
    """This class describes a database query for xcyber360."""
    ...
    def __init__(self, offset: int, limit: int, table: str, sort: dict, search: dict, select: list, query: str,
        ...
        """Xcyber360 DB Query constructor.
        ...
        # For example, the following regex -> (name!=xcyber360;id>5),group=webserver <- would return 3 different matches:
        #   (name != xcyber360 ;
        #    id   > 5      ),
        #    group=webserver
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        isinstance(self.backend, Xcyber360DBBackend) and self.backend.close_connection()
    ...
    def _add_limit_to_query(self):
        if self.limit:
            if self.limit > common.MAXIMUM_DATABASE_LIMIT:
                raise Xcyber360Error(1405, extra_message=str(self.limit))
        ...
        if self.sort:
            if self.sort['fields']:
                ...
                if not set(sort_fields).issubset(allowed_sort_fields):
                    raise Xcyber360Error(1403, "Allowed sort fields: {}. Fields: {}".format(
    ...
    def _parse_select_filter(self, select_fields):
        if select_fields:
            ...
            if not set_select_fields or not set_select_fields.issubset(set_fields_keys):
                raise Xcyber360Error(1724, "Allowed select fields: {0}. Fields {1}". \
    ...
    def _filter_date(self, date_filter, filter_db_name):
        ...
        if date_filter['value'].isdigit() or re.match(r'\d+[dhms]', date_filter['value']):
            ...
            raise Xcyber360Error(1412, date_filter['value'])
    ...
    def oversized_run(self) -> dict:
        ...
        final_ids = list()
        resources = list()
        if self.__class__.__name__ == 'Xcyber360DBQueryAgents':
            resource = 'id'
        elif self.__class__.__name__ == 'Xcyber360DBQueryGroups':
            resource = 'name'
        else:
            raise Xcyber360InternalError(1123)
...
class Xcyber360DBQueryDistinct(Xcyber360DBQuery):
    ...
    def _add_filters_to_query(self):
        Xcyber360DBQuery._add_filters_to_query(self)
        self.query += ' WHERE ' if not self.q and 'WHERE' not in self.query else ' AND '
        self.query += ' AND '.join(
            ["{0} IS NOT null AND {0} != ''".format(self.fields[field]) for field in self.select])
    ...
    def _add_select_to_query(self):
        if len(self.select) > 1:
            raise Xcyber360Error(1410)

        Xcyber360DBQuery._add_select_to_query(self)
...
def upload_file(content: str, file_path: str, check_xml_formula_values: bool = True):
    ...
    Raises
    ------
    Xcyber360InternalError(1005)
        Error reading file.
    Xcyber360InternalError(1016)
        Error moving file.
    Xcyber360Error(1006)
    ...
    try:
        ...
        raise Xcyber360InternalError(1005) from exc

    # Move temporary file to group folder
    try:
        new_conf_path = path.join(common.XCYBER360_PATH, file_path)
        safe_move(tmp_file_path, new_conf_path, ownership=(common.xcyber360_uid(), common.xcyber360_gid()), permissions=0o660)
    except PermissionError as exc:
        raise Xcyber360Error(1006) from exc
    except Error as exc:
        raise Xcyber360InternalError(1016) from exc

    return results.Xcyber360Result({'message': 'File was successfully updated'})
...
def to_relative_path(full_path: str, prefix: str = common.XCYBER360_PATH) -> str:
    """Return a relative path from the Xcyber360 base directory.
    ...
    full_path : str
        Absolute path.
    prefix : str, opt
        Prefix to strip from the absolute path. Default `common.XCYBER360_PATH`
