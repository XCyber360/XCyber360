...
class AsyncXcyber360DBConnection:
    """
    ...
    async def _send(self, msg, raw=False):
        """Format and send message to xcyber360-db socket without blocking event loop.

        Parameters
        ----------
        msg : str
            Message to be sent to xcyber360-db.
        raw : bool
            If `True`, the status message from xcyber360-db is included in the response.

        Returns
        -------
        str, list
            Result for the request sent to xcyber360-db.
        """
        try:
            ...
            try:
                ...
                raise Xcyber360InternalError(2010, extra_message=e)

            if raw:
                return data
            elif data[0] == "err":
                raise Xcyber360Error(2003, data[1])
            else:
                return json.loads(data[1], object_hook=Xcyber360DBConnection.json_decoder)
        except (FileNotFoundError, ConnectionError) as e:
            with contextlib.suppress(Exception):
                await self.open_connection()
            raise Xcyber360InternalError(2005, extra_message=e)
    ...
    async def run_wdb_command(self, command):
        ...
        ----------
        command : str
            Command to be executed inside xcyber360-db
        ...
        if len(result) > 1:
            if result[0] == 'err':
                raise Xcyber360InternalError(2007, extra_message=result[1])

        else:
            if result[0] != 'ok':
                raise Xcyber360InternalError(2007)

        return result


class Xcyber360DBConnection:
    """
    ...
    def __init__(self, request_slice=500):
        ...
        ----------
        request_slice : int
            Maximum number of items to request from xcyber360-db on the first call.
        """
        self.socket_path = common.WDB_SOCKET
        ...
        try:
            ...
            raise Xcyber360InternalError(2005, e)
    ...
    def __query_input_validation(self, query: str):
        ...
        for check, error_text in input_val_errors:
            if not check:
                raise Xcyber360Error(2004, error_text)
    ...
    def _send(self, msg: str, raw: bool = False) -> dict:
        ...
        Raises
        ------
        Xcyber360InternalError(2009)
            Pagination error. Response from xcyber360-db was over the maximum socket buffer size.
        Xcyber360Error(2003)
        ...
        if data_size >= MAX_SOCKET_BUFFER_SIZE:
            raise Xcyber360InternalError(2009)
        ...
        if data[0] == "err":
            ...
            return Xcyber360DBConnection.loads(data[1])
    ...
    def loads(string: str) -> dict:
        ...
        ----------
        string : str
            String response from `xcyber360-db`. It must be a dumped JSON.
        ...
        dict
            JSON object.
        """
        data = json.loads(string, object_hook=Xcyber360DBConnection.json_decoder)
        if '"(null)"' in string:
    ...
    def delete_agents_db(self, agents_id: List[str]) -> dict:
        """Delete agents db through xcyber360-db service.
        ...
        -------
        dict
            Dict received from xcyber360 db in the form: {"agents": {"ID": "MESSAGE"}}, where MESSAGE may be one of the
            following:
                - Ok
                - Invalid agent ID
                ...
                - DB not found
        """
        return self._send(f"xcyber360db remove {' '.join(agents_id)}")
    ...
    def send(self, query: str, raw: bool = True) -> Union[str, dict]:
        ...
        ----------
        query : str
            Query to be executed in xcyber360-db.
        raw : bool
    ...
    def execute(self, query, count=False, delete=False, update=False):
        ...
        def send_request_to_wdb(query_lower, step, off, response):
            try:
                ...
                response.extend(Xcyber360DBConnection.loads(request_response))
        ...
        if delete:
            ...
            if regex.match(query_lower) is None:
                raise Xcyber360Error(2004, "Delete query is wrong")
        ...
        if update:
            regex = re.compile(r"\w+ \d+? sql update ([\w\d,*_ ]+) set value = '([\w\d,*_ ]+)' where key (=|like)?"
                               r" '([a-z0-9,*_%\- ]+)'")
            if regex.match(query_lower) is None:
                raise Xcyber360Error(2004, "Update query is wrong")
            return self._send(query_lower)
        ...
        if not re.search(r'.?select count\([\w \*]+\)( as [^,]+)? from', query_without_where):
            ...
            try:
                ...
                raise Xcyber360Error(2006, str(e))
            except (Xcyber360Error, Xcyber360InternalError) as e:
                raise e
            except Exception as e:
                raise Xcyber360InternalError(2007, str(e))
