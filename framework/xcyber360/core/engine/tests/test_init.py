...
from xcyber360.core.engine import DEFAULT_RETRIES, DEFAULT_TIMEOUT, Engine, get_engine_client
...
async def test_get_engine_client_ko(socket_path: str, error_number: int):
    """Check that the `get_engine_client` returns a Xcyber360EngineError on an exception."""
    with pytest.raises(Xcyber360EngineError, match=f'.*{error_number}.*'):
        async with get_engine_client() as engine:
