...
class TimeBasedFileRotatingHandler(logging.handlers.TimedRotatingFileHandler):
    """
    Xcyber360 log rotation. It rotates the log at midnight and sets the appropriate permissions to the new log file.
    """
    ...
    def doRollover(self):
        ...
        # Save rotated file in /var/logs/xcyber360-server/api directory
...
class SizeBasedFileRotatingHandler(logging.handlers.RotatingFileHandler):
    """Xcyber360 log rotation. It rotates when the logging file size exceeds the maximum number of bytes configured."""
...
class Xcyber360Logger:
    """
    Define attributes of a Python Xcyber360 daemon's logger.
    """
    def __init__(self, foreground_mode: bool, log_path: str, debug_level: [int, str], logger_name: str = 'xcyber360',
        ...
        log_path : str
            Filepath of the file to send logs to. Relative to the Xcyber360 installation path.
        ...
        """
        self.log_path = common.XCYBER360_LOG / log_path
        self.logger = None
...
class CLIFilter(logging.Filter):
    """
    Define a custom filter to filter Xcyber360InternalErrors
    """

    messages_to_avoid = ['Xcyber360 Internal Error', 'Xcyber360InternalError']
