...
from xcyber360.core.exception import Xcyber360Exception, Xcyber360InternalError

SOCKET_COMMUNICATION_PROTOCOL_VERSION = 1


class Xcyber360Socket:
    MAX_SIZE = 65536
    ...
    def _connect(self):
        try:
            ...
            raise Xcyber360InternalError(1013, extra_message=os.path.basename(self.path))
        except ConnectionRefusedError:
            raise Xcyber360InternalError(1121, extra_message=f"Socket '{os.path.basename(self.path)}' cannot receive "
                                                         "connections")
        except Exception as e:
            raise Xcyber360Exception(1013, str(e))
    ...
    def send(self, msg_bytes, header_format="<I"):
        if not isinstance(msg_bytes, bytes):
            raise Xcyber360Exception(1105, "Type must be bytes")

        try:
            sent = self.s.send(pack(header_format, len(msg_bytes)) + msg_bytes)
            if sent == 0:
                raise Xcyber360Exception(1014, "Number of sent bytes is 0")
            return sent
        except Exception as e:
            raise Xcyber360Exception(1014, str(e))
...
class Xcyber360SocketJSON(Xcyber360Socket):
    MAX_SIZE = 65536

    def __init__(self, path):
        Xcyber360Socket.__init__(self, path)

    def send(self, msg, header_format="<I"):
        return Xcyber360Socket.send(self, msg_bytes=dumps(msg).encode(), header_format=header_format)

    def receive(self, header_format="<I", header_size=4, raw=False):
        response = loads(Xcyber360Socket.receive(self, header_format=header_format, header_size=header_size).decode())
        if not raw:
            if 'error' in response.keys():
                if response['error'] != 0:
                    raise Xcyber360Exception(response['error'], response['message'], cmd_error=True)
            return response['data']
        else:
            return response


class Xcyber360AsyncProtocol(asyncio.Protocol):
    """Xcyber360 implementation of asyncio.Protocol class."""
...
class Xcyber360AsyncSocket:
    ...
    async def connect(self, path_to_socket: str):
        ...
        try:
            ...
            raise Xcyber360Exception(1013, str(exc)) from exc
    ...
    async def send(self, msg_bytes: bytes, header_format: str = "<I"):
        ...
        Raises
        ------
        Xcyber360Exception(1014)

        """
        if not isinstance(msg_bytes, bytes):
            raise Xcyber360Exception(1014, "Type must be bytes")
        ...
        try:
            ...
            raise Xcyber360Exception(1014, "Socket connection was closed") from exc
    ...
    async def receive(self, header_format: str ="<I", header_size: int = 4) -> bytes:
        ...
        try:
            ...
            raise Xcyber360Exception(1014, str(exc)) from exc


class Xcyber360AsyncSocketJSON(Xcyber360AsyncSocket):
...
def create_xcyber360_socket_message(origin=None, command=None, parameters=None):
