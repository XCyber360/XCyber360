...
from xcyber360.core.configuration import get_active_configuration, get_cti_url
from xcyber360.core.utils import get_utc_now, get_utc_strptime, tail
from xcyber360.core.xcyber360_socket import Xcyber360Socket
...
XCYBER360_UID_KEY = 'xcyber360-uid'
XCYBER360_TAG_KEY = 'xcyber360-tag'
...
def get_ossec_log_fields(log: str, log_format: LoggingFormat = LoggingFormat.plain) -> Union[tuple, None]:
    ...
    if "rootcheck" in tag:  # Unify rootcheck category
        tag = "xcyber360-rootcheck"

    return get_utc_strptime(date, '%Y/%m/%d %H:%M:%S'), tag, level.lower(), description


def get_xcyber360_active_logging_format() -> LoggingFormat:
    """Obtain the Xcyber360 active logging format.

    Returns
    -------
    LoggingFormat
        Xcyber360 active log format. Can either be `plain` or `json`. If it has both types, `plain` will be returned.
    """
...
def get_ossec_logs(limit: int = 2000) -> list:
    ...
    log_format = get_xcyber360_active_logging_format()
    if log_format == LoggingFormat.plain and exists(common.XCYBER360_LOG):
        xcyber360_log_content = tail(common.XCYBER360_LOG, limit)
    elif log_format == LoggingFormat.json and exists(common.XCYBER360_LOG_JSON):
        xcyber360_log_content = tail(common.XCYBER360_LOG_JSON, limit)
    else:
        raise Xcyber360InternalError(1020)

    for line in xcyber360_log_content:
...
def validate_ossec_conf() -> str:
    ...
    # Message for checking Xcyber360 configuration
    wcom_msg = common.CHECK_CONFIG_COMMAND

    # Connect to wcom socket
    if exists(wcom_socket_path):
        try:
            wcom_socket = Xcyber360Socket(wcom_socket_path)
        except Xcyber360Exception as e:
            extra_msg = f'Socket: XCYBER360_PATH/queue/sockets/com. Error {e.message}'
            raise Xcyber360InternalError(1013, extra_message=extra_msg)
    else:
    ...
    try:
        ...
        raise Xcyber360InternalError(1904, extra_message=str(e))
...
def parse_execd_output(output: str) -> Dict:
    ...
    if error_flag != 0:
        ...
        raise Xcyber360Error(1908, extra_message=', '.join(errors))
    else:
        response = {'status': 'OK'}
...
def get_update_information_template(
        uuid: str,
        update_check: bool,
        current_version: str = f"v{xcyber360.__version__}",
    ...
    current_version : str, optional
        Indicates the current version of Xcyber360, by default xcyber360.__version__.
...
async def query_update_check_service(installation_uid: str) -> dict:
    ...
    installation_uid : str
        Xcyber360 UID to include in the query.
    ...
    update_information : dict
        Updates information.
    """
    current_version = f'v{xcyber360.__version__}'
    headers = {
        XCYBER360_UID_KEY: installation_uid,
        XCYBER360_TAG_KEY: current_version,
        USER_AGENT_KEY: f'Xcyber360 UpdateCheckService/{current_version}'
    }
