...
from xcyber360.core.cluster import client, cluster, common as c_common
from xcyber360.core.cluster.utils import log_subprocess_execution
from xcyber360.core.cluster.dapi import dapi
from xcyber360.core.utils import safe_move, get_utc_now
...
class ReceiveIntegrityTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.xcyber360_common.process_files_from_master
    ...
    def done_callback(self, future=None):
        ...
        """
        self.xcyber360_common.check_integrity_free = True
        super().done_callback(future)


class WorkerHandler(client.AbstractClient, c_common.Xcyber360Common):
    """
    ...
    def connection_result(self, future_result):
        ...
        if self.connected:
            ...
            worker_tmp_files = common.XCYBER360_QUEUE / self.name
    ...
    def get_manager(self):
        """Get the Worker object that created this WorkerHandler. Used in the class Xcyber360Common.
    ...
    def error_receiving_integrity(self, data: str) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : str
            Task ID and error formatted as Xcyber360JSONEncoder.
        ...
        while True:
            ...
            # If exception is raised during sync process, notify the master so it removes the file if received.
                ...
                if isinstance(e, exception.Xcyber360Exception):
                    exc = json.dumps(e, cls=c_common.Xcyber360JSONEncoder)
                else:
                    exc = json.dumps(exception.Xcyber360ClusterError(1000, extra_message=str(e)),
    ...
    async def sync_extra_valid(self, extra_valid: Dict):
        ...
        # If exception is raised during sync process, notify the master, so it removes the file if received.
            ...
            if isinstance(e, exception.Xcyber360Exception):
                ...
                exc = json.dumps(exception.Xcyber360ClusterError(1000, extra_message=str(e)), cls=c_common.Xcyber360JSONEncoder)
            with contextlib.suppress(Exception):
                await self.send_request(command=b'syn_i_w_m_r', data=f"None {exc}".encode())
    ...
    async def process_files_from_master(self, name: str, file_received: asyncio.Event):
        ...
        if isinstance(received_filename, Exception):
            exc_info = json.dumps(exception.Xcyber360ClusterError(
                1000, extra_message=str(self.sync_tasks[name].filename)), cls=c_common.Xcyber360JSONEncoder)
            with contextlib.suppress(Exception):
    ...
    def update_master_files_in_worker(ko_files: Dict, zip_path: str, server_config: ServerConfig):
        ...
        def overwrite_or_create_files(filename_: str, data_: Dict):
            ...
            """
            full_filename_path = common.XCYBER360_ETC / filename
            ...
            if data_['merged']:  # worker nodes can only receive agent-groups files
                ...
                # and then move each one to the destination directory (<xcyber360_path>/filename).
                # The TYPE string used in the 'unmerge_info' function is a placeholder. It corresponds to the
                # directory inside '{xcyber360_path}/queue/' path.
                for name, content, _ in cluster.unmerge_info('TYPE', zip_path, filename_):
                    full_unmerged_name = common.XCYBER360_ETC / name
                ...
                # Move the file from zipdir (directory containing unzipped files) to <xcyber360_path>/filename.
            ...
            # Remove local files marked as extra.
                ...
                for file_to_remove in files:
                    try:
                        ...
                        file_path = common.XCYBER360_ETC / file_to_remove
                        try:
                            os.remove(file_path)
                        except OSError as e:
        ...
        for directory in directories_to_check:
            try:
                full_path = common.XCYBER360_ETC / directory
