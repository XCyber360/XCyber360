...
import xcyber360.core.cluster.cluster
import xcyber360.core.cluster.utils
import xcyber360.core.manager
import xcyber360.core.results as wresults
from xcyber360 import agent
from xcyber360.cluster import get_node_wrapper, get_nodes_info
from xcyber360.core import common, exception
from xcyber360.core.cluster import local_client, common as c_common
from xcyber360.core.exception import Xcyber360Exception, Xcyber360ClusterError, Xcyber360Error
...
class DistributedAPI:
    ...
    def __init__(self, f: Callable, logger: logging.getLogger, f_kwargs: Dict = None, node: c_common.Handler = None,
        ...
        self.debug = debug
        self.node_info = xcyber360.core.cluster.cluster.get_node() if node is None else node.get_node()
        ...
        if not basic_services:
            self.basic_services = ('xcyber360-server',)
        else:
    ...
    def debug_log(self, message):
        ...
        message : str
            Full log message.
        """
        if self.logger.name == 'xcyber360-api':
            self.logger.debug2(message)
        else:
            self.logger.debug(message)

    async def distribute_function(self) -> [Dict, exception.Xcyber360Exception]:
        """
        Distribute an API call.

        Returns
        -------
        dict or Xcyber360Exception
            Dictionary with API response or Xcyber360Exception in case of error.
        """
        try:
            ...
            try:
                response = json.loads(response, object_hook=c_common.as_xcyber360_object) \
                    if isinstance(response, str) else response
            except json.decoder.JSONDecodeError:
                response = {'message': response}

            return response if isinstance(response, (wresults.AbstractXcyber360Result, exception.Xcyber360Exception)) \
                else wresults.Xcyber360Result(response)

        except json.decoder.JSONDecodeError:
            e = exception.Xcyber360InternalError(3036)
            e.dapi_errors = await self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}")
            return e
        except exception.Xcyber360InternalError as e:
            e.dapi_errors = await self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}", exc_info=not isinstance(e, exception.Xcyber360ClusterError))
            return e
        except exception.Xcyber360Error as e:
            e.dapi_errors = await self.get_error_info(e)
            return e
            ...
            return exception.Xcyber360InternalError(1000,
                                                dapi_errors=await self.get_error_info(e))

    def check_xcyber360_status(self):
        """
        There are some services that are required for xcyber360 to correctly process API requests. If any of those services
        is not running, the API must raise an exception indicating that:
            ...
            * There's an error in any of those services that must be addressed before using the API if any service is
              in failed status.
            * Xcyber360 must be started before using the API is the services are stopped.

        The basic service xcyber360 needs to be running is: xcyber360-clusterd.
        """
        if self.f == xcyber360.core.manager.status:
            return

        status = xcyber360.core.manager.status()
        ...
        if not_ready_daemons:
            ...
            raise exception.Xcyber360InternalError(1017, extra_message=extra_info)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise exception.Xcyber360InternalError(3021)
                except OperationalError as exc:
                    raise exception.Xcyber360InternalError(2008, extra_message=str(exc.orig))
                except process.BrokenProcessPool:
                    raise exception.Xcyber360InternalError(901)
            except json.decoder.JSONDecodeError:
                raise exception.Xcyber360InternalError(3036)
            except process.BrokenProcessPool:
                raise exception.Xcyber360InternalError(900)
            ...
            # Xcyber360ClusterError
            self.logger.error(f"{e.message}",
                              exc_info=e.code not in {3021, 3036, 1913, 1017} and not isinstance(e,
                                                                                           exception.Xcyber360ClusterError))
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.Xcyber360JSONEncoder)
        except (exception.Xcyber360Error, exception.Xcyber360ResourceNotFound) as e:
            e.dapi_errors = await self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.Xcyber360JSONEncoder)
            ...
            if self.debug:
                raise
            return json.dumps(exception.Xcyber360InternalError(1000,
                                                           dapi_errors=await self.get_error_info(e)),
                              cls=c_common.Xcyber360JSONEncoder)
    ...
    async def get_error_info(self, e) -> Dict:
        ...
        except exception.Xcyber360Exception as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.Xcyber360Exception) else exception.GENERIC_ERROR_MSG
        result = {node: {'error': error_message}
                  }

        # Give log path only in case of Xcyber360InternalError
        if isinstance(e, exception.Xcyber360InternalError):
            log_filename = None
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('XCYBER360_LOG', os.path.relpath(h.baseFilename, start=common.XCYBER360_LOG))
    ...
    async def send_tmp_file(self, node_name=None):
        ...
        res = json.loads(await client.send_file(os.path.join(common.XCYBER360_PATH,
                                                             self.f_kwargs['tmp_file']),
                                                node_name),
                         object_hook=c_common.as_xcyber360_object)
        os.remove(os.path.join(common.XCYBER360_PATH, self.f_kwargs['tmp_file']))
    ...
    async def execute_remote_request(self) -> Dict:
        ...
        node_response = await client.execute(command=b'dapi',
                                             data=json.dumps(self.to_dict(),
                                                             cls=c_common.Xcyber360JSONEncoder).encode())
        return json.loads(node_response,
                          object_hook=c_common.as_xcyber360_object)

    async def forward_request(self) -> [wresults.AbstractXcyber360Result, exception.Xcyber360Exception]:
        ...
        Returns
        -------
        wresults.AbstractXcyber360Result or exception.Xcyber360Exception
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractXcyber360Result, exception.Xcyber360Exception]:
            ...
            if node_name == self.node_info['node']:
                ...
                try:
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a Xcyber360Error exception
                if isinstance(result, Xcyber360ClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = Xcyber360Error.from_dict(dikt)

            return result if isinstance(result, (wresults.AbstractXcyber360Result, exception.Xcyber360Exception)) \
                else wresults.Xcyber360Result(result)
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsXcyber360Result()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractXcyber360Result):
        ...
        # It might be a Xcyber360Error after reducing
        if isinstance(response, wresults.AffectedItemsXcyber360Result):
...
class Xcyber360RequestQueue:
    """Represents a queue of Xcyber360 requests"""
...
class APIRequestQueue(Xcyber360RequestQueue):
    """
    ...
    def __init__(self, server):
        ...
        self.logger = logging.getLogger('xcyber360').getChild('dapi')
        self.logger.addFilter(xcyber360.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='D API'))
    ...
    async def run(self):
        while True:
            ...
            try:
                request = json.loads(request, object_hook=c_common.as_xcyber360_object)
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.Xcyber360JSONEncoder).encode())
            except Exception as e:
