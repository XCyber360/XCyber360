...
from xcyber360.core import common, exception
from xcyber360.core.cluster import cluster, utils as cluster_utils
...
class ReceiveFileTask:
    ...
    def __init__(self, xcyber360_common, logger, task_id: bytes = b''):
        """Class constructor.

        Parameters
        ----------
        xcyber360_common : Xcyber360Common object
            Instance of Xcyber360Common.
        ...
        """
        self.xcyber360_common = xcyber360_common
    ...
    def done_callback(self, future=None):
        ...
        """
        if self.task_id in self.xcyber360_common.sync_tasks:
            del self.xcyber360_common.sync_tasks[self.task_id]
...
class Handler(asyncio.Protocol):
    ...
    def __init__(self, server_config: ServerConfig, logger: logging.Logger = None, tag: str = "Handler"):
        ...
        self.logger = logging.getLogger('xcyber360') if not logger else logger
        # Logging tag.
        self.tag = tag
    ...
    def msg_build(self, command: bytes, counter: int, data: bytes) -> List[bytearray]:
        ...
        if cmd_len > self.cmd_len - len(InBuffer.divide_flag):
            raise exception.Xcyber360ClusterError(3024, extra_message=command)
    ...
    async def send_request(self, command: bytes, data: bytes) -> Union[exception.Xcyber360ClusterError, Any]:
        ...
        try:
            ...
            raise exception.Xcyber360ClusterError(3026)
        except Exception as e:
            raise exception.Xcyber360ClusterError(3018, extra_message=str(e))
        try:
    ...
    async def send_file(self, filename: str, task_id: bytes = None) -> int:
        ...
        if not os.path.exists(filename):
            raise exception.Xcyber360ClusterError(3034, extra_message=filename)

        sent_size = 0
        filename = Path(filename)
        relative_path = str(filename.relative_to(common.XCYBER360_RUN)).encode()
        try:
            # Tell to the destination node where (inside xcyber360_path) the file has to be written.
            await self.send_request(command=b'new_file', data=relative_path)
        except exception.Xcyber360ClusterError as e:
            if e.code != 3020:
                raise e
    ...
    async def send_string(self, my_str: bytes) -> bytes:
        ...
        try:
            ...
            with contextlib.suppress(exception.Xcyber360ClusterError):
    ...
    async def forward_dapi_response(self, data: bytes):
        ...
        try:
            ...
            if isinstance(e, exception.Xcyber360Exception):
                exc = json.dumps(e, cls=Xcyber360JSONEncoder)
            else:
                exc = json.dumps(exception.Xcyber360ClusterError(1000, extra_message=str(e)), cls=Xcyber360JSONEncoder)
    ...
    def dispatch(self, command: bytes, counter: int, payload: bytes) -> None:
        ...
        try:
            ...
            command, payload = b'err', json.dumps(e, cls=Xcyber360JSONEncoder).encode()
        except Exception as e:
            self.logger.error(f"Unhandled error processing request '{command}': {e}", exc_info=True)
            command, payload = b'err', json.dumps(exception.Xcyber360InternalError(1000, extra_message=str(e)),
                                                  cls=Xcyber360JSONEncoder).encode()
    ...
    def process_dapi_error(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if dapi_client.decode() in self.server.local_server.clients:
            ...
            raise exception.Xcyber360ClusterError(3032, extra_message=dapi_client.decode())
    ...
    def receive_file(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        bytes
            Response message.
        """
        self.in_file[data] = {'fd': open(common.XCYBER360_RUN / data.decode(), 'wb'), 'checksum': hashlib.sha256()}
    ...
    def cancel_task(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : bytes
            String containing task_id and Xcyber360JSONEncoded object with the exception details.
        ...
        error_json = json.loads(error_details, object_hook=as_xcyber360_object)
        if task_id != b'None':
    ...
    def process_error_from_peer(self, data: bytes) -> bytes:
        ...
        """
        try:
            exc = json.loads(data.decode(), object_hook=as_xcyber360_object)
        except json.JSONDecodeError as e:
            exc = exception.Xcyber360ClusterError(3000, extra_message=data.decode())
    ...
    async def wait_for_file(self, file, task_id):
        ...
        try:
            ...
            if isinstance(e, asyncio.TimeoutError):
                exc = exception.Xcyber360ClusterError(3039)
            else:
                exc = exception.Xcyber360ClusterError(3040, extra_message=str(e))
            # Notify the sending node to stop its task.
            with contextlib.suppress(Exception):
                await self.send_request(command=b"cancel_task",
                                        data=f"{task_id} {json.dumps(exc, cls=Xcyber360JSONEncoder)}".encode())
            raise exc
    ...
    async def send_orders(self, orders: bytes) -> tuple:
        ...
        transport = httpx.AsyncHTTPTransport(uds=common.XCYBER360_SOCKET / 'comms-api.sock')
        ...
        try:
            ...
            raise exception.Xcyber360Error(3041, extra_message=str(e))
...
class Xcyber360Common:
    """
    Task implementing common methods for both clients and servers that are Xcyber360 specific.
    """
    ...
    def end_receiving_file(self, task_and_file_names: str, logger_tag: str = '') -> Tuple[bytes, bytes]:
        ...
        if task_id not in self.sync_tasks:
            ...
            if os.path.exists(os.path.join(common.XCYBER360_RUN, filename)):
                try:
                    os.remove(os.path.join(common.XCYBER360_RUN, filename))
                except Exception as e:
                    self.get_logger(logger_tag).error(
                        f"Attempt to delete file {os.path.join(common.XCYBER360_RUN, filename)} failed: {e}")
            raise exception.Xcyber360ClusterError(3027, extra_message=task_id)

        # Set full path to file for task 'task_id' and notify it is ready to be read, so the lock is released.
        self.sync_tasks[task_id].filename = os.path.join(common.XCYBER360_RUN, filename)
    ...
    def error_receiving_file(self, task_id_and_error_details: str, logger_tag: str = '') -> Tuple[bytes, bytes]:
        ...
        task_id_and_error_details : str
             Xcyber360JSONEncoded object with the exception details.
        logger_tag : str
            Logger task to use. If empty, it will use main class logger.
        ...
        error_details_json = json.loads(error_details, object_hook=as_xcyber360_object)
    ...
    async def sync(self, files: Iterable, files_metadata: Dict, metadata_len: int, task_pool=None,
        ...
        try:
            ...
            await self.server.send_request(
                ...
                data=task_id + b' ' + os.path.relpath(compressed_data, common.XCYBER360_RUN).encode()
            )
        except Exception as e:
            self.logger.error(f"Error sending zip file: {e}")
            if isinstance(e, exception.Xcyber360Exception):
                exc = json.dumps(e, cls=Xcyber360JSONEncoder).encode()
            else:
                exc = json.dumps(exception.Xcyber360ClusterError(1000, extra_message=str(e)), cls=Xcyber360JSONEncoder).encode()
            with contextlib.suppress(Exception):
...
class Xcyber360JSONEncoder(json.JSONEncoder):
    """
    Define special JSON encoder for Xcyber360.
    """
    ...
    def default(self, obj):
        if callable(obj):
            ...
            if hasattr(obj, '__self__'):
                if isinstance(obj.__self__, Xcyber360):
                    attributes['__xcyber360__'] = obj.__self__.to_dict()
            attributes['__type__'] = type(obj).__name__
            return result
        elif isinstance(obj, exception.Xcyber360Exception):
            result = {'__xcyber360_exception__': {'__class__': obj.__class__.__name__,
                                              '__object__': obj.to_dict()}}
            return result
        elif isinstance(obj, wresults.AbstractXcyber360Result):
            result = {'__xcyber360_result__': {'__class__': obj.__class__.__name__,
                                           '__object__': obj.encode_json()}
                      }
            return result
        elif isinstance(obj, (datetime.datetime, datetime.date)):
            return {'__xcyber360_datetime__': obj.isoformat()}
...
def as_xcyber360_object(dct: Dict):
    try:
        if '__callable__' in dct:
            encoded_callable = dct['__callable__']
            funcname = encoded_callable['__name__']
            if '__xcyber360__' in encoded_callable:
                # Encoded Xcyber360 instance method.
                xcyber360 = Xcyber360()
                return getattr(xcyber360, funcname)
            else:
        ...
        elif '__xcyber360_exception__' in dct:
            xcyber360_exception = dct['__xcyber360_exception__']
            return getattr(exception, xcyber360_exception['__class__']).from_dict(xcyber360_exception['__object__'])
        elif '__xcyber360_result__' in dct:
            xcyber360_result = dct['__xcyber360_result__']
            return getattr(wresults, xcyber360_result['__class__']).decode_json(xcyber360_result['__object__'])
        elif '__xcyber360_datetime__' in dct:
            return datetime.datetime.fromisoformat(dct['__xcyber360_datetime__'])
        ...
        raise exception.Xcyber360InternalError(1000,
                                           extra_message=f"Xcyber360 object cannot be decoded from JSON {dct}",
                                           cmd_error=True)
