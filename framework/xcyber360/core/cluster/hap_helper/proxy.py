...
import httpx
from xcyber360.core.cluster.utils import ClusterFilter, context_tag
from xcyber360.core.exception import Xcyber360HAPHelperError
...
class ProxyAPI:
        ...
        try:
            async with httpx.AsyncClient(verify=self.haproxy_cert, cert=self.client_cert,
                ...
                if response.status_code == 401:
                    raise Xcyber360HAPHelperError(3046)
                elif response.status_code == 404:
                    raise Xcyber360HAPHelperError(3047)
        except httpx.ConnectError:
            raise Xcyber360HAPHelperError(
                3043, extra_message='Check connectivity and the configuration in the `ossec.conf`'
            )
        except httpx.RequestError as req_exc:
            raise Xcyber360HAPHelperError(3043, extra_message=str(req_exc))
    ...
    async def _make_hap_request(
        ...
        try:
            ...
            raise Xcyber360HAPHelperError(3044, extra_message=str(request_exc))
        ...
        if response.is_success:
            ...
            raise Xcyber360HAPHelperError(3045, extra_message=response.json()['message'])
...
class Proxy:
    def __init__(
        self,
        xcyber360_backend: str,
        proxy_api: ProxyAPI,
        tag: str,
        xcyber360_connection_port: int = 1514,
        resolver: str = None,
    ):
        self.tag = tag
        self.logger = self._get_logger(self.tag)
        self.xcyber360_backend = xcyber360_backend
        self.xcyber360_connection_port = xcyber360_connection_port
        self.api = proxy_api
    ...
    def _get_logger(tag: str) -> logging.Logger:
        ...
        """
        logger = logging.getLogger('xcyber360').getChild('Proxy')
    ...
    async def initialize(self):
        ...
        try:
            ...
            raise Xcyber360HAPHelperError(3048)
    ...
    async def get_current_backend_servers(self) -> dict:
        ...
        dict
            The backend servers.
        """
        api_response = await self.api.get_backend_servers(self.xcyber360_backend)
        self.logger.debug2('Obtained proxy servers')
        return {server['name']: server['address'] for server in api_response['data']}

    async def add_xcyber360_manager(self, manager_name: str, manager_address: str, resolver: Optional[str] = None) -> dict:
        """Add a new Xcyber360 manager to the Proxy.

        Parameters
        ----------
        manager_name : str
            Name of the Xcyber360 manager.
        manager_address : str
            IP or DNS for the Xcyber360 manager.
        resolver : Optional[str]
        ...
        api_response = await self.api.add_server_to_backend(
            backend=self.xcyber360_backend,
            server_name=manager_name,
            server_address=manager_address,
            port=self.xcyber360_connection_port,
            resolver=resolver,
        )
        self.logger.debug2(
            f"Added new server '{manager_name}' {manager_address}:{self.xcyber360_connection_port} to backend"
            f" '{self.xcyber360_backend}'"
        )
        return api_response

    async def remove_xcyber360_manager(self, manager_name: str) -> dict:
        """Delete the given Xcyber360 manager from the Proxy.
        ...
        """
        api_response = await self.api.remove_server_from_backend(backend=self.xcyber360_backend, server_name=manager_name)

        self.logger.debug2(f"Removed server {manager_name} from backend '{self.xcyber360_backend}'")
        return api_response
    ...
    async def restrain_server_new_connections(self, server_name: str) -> dict:
        ...
        api_response = await self.api.change_backend_server_state(
            backend_name=self.xcyber360_backend, server_name=server_name, state=ProxyServerState.DRAIN
        )
        self.logger.debug2(f"Changed Xcyber360 server '{server_name}' to {ProxyServerState.DRAIN.value.upper()} state")
        return api_response
    ...
    async def allow_server_new_connections(self, server_name: str) -> dict:
        ...
        """
        api_response = await self.api.change_backend_server_state(
            backend_name=self.xcyber360_backend, server_name=server_name, state=ProxyServerState.READY
        )
        self.logger.debug2(f"Changed Xcyber360 server '{server_name}' to {ProxyServerState.READY.value.upper()} state")
        return api_response

    async def get_xcyber360_server_stats(self, server_name: str) -> dict:
        ...
        """
        server_stats = (
            await self.api.get_backend_server_stats(backend_name=self.xcyber360_backend, server_name=server_name)
        )[0]['stats'][0]['stats']
    ...
    async def is_server_drain(self, server_name: str) -> bool:
        ...
        server_stats = await self.api.get_backend_server_runtime_settings(
            backend_name=self.xcyber360_backend, server_name=server_name
        )
        return server_stats['admin_state'] == ProxyServerState.DRAIN.value

    async def get_xcyber360_backend_stats(self, only_actives: bool = True) -> dict:
        """Get the statistics of the Xcyber360 backend.
        ...
        -------
        dict
            The statistics of the Xcyber360 backend.
        """
        backend_servers = [
            server['name'] for server in (await self.api.get_backend_servers(self.xcyber360_backend))['data']
        ]
        stats = {}

        for server_name in backend_servers:
            server_stats = await self.get_xcyber360_server_stats(server_name=server_name)
    ...
    async def get_xcyber360_backend_server_connections(self) -> dict:
        """Get the active connections of the Xcyber360 backend server.
        ...
        server_stats = await self.get_xcyber360_backend_stats()
