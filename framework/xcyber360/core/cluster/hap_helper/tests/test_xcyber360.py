...
import pytest
from xcyber360.agent import get_agents, reconnect_agents
from xcyber360.cluster import get_nodes_info
from xcyber360.core.results import AffectedItemsXcyber360Result

from framework.xcyber360.core.cluster.hap_helper.xcyber360 import Xcyber360Agent, Xcyber360DAPI


class TestXcyber360Agent:
    ...
    def test_can_reconnect(self, version: str, expected: bool):
        ...
        assert Xcyber360Agent.can_reconnect(version) == expected
    ...
    def test_get_agents_able_to_reconnect(self):
        ...
        assert Xcyber360Agent.get_agents_able_to_reconnect(agents_list=agents) == [2, 3]


@mock.patch('framework.xcyber360.core.cluster.hap_helper.xcyber360.DistributedAPI', autospec=True)
class TestXcyber360DAPI:
    xcyber360_dapi = Xcyber360DAPI(tag='test')

    @pytest.fixture
    def fixture_affected_items_result(self):
        return AffectedItemsXcyber360Result()
    ...
    @mock.patch('framework.xcyber360.core.cluster.hap_helper.xcyber360.get_system_nodes', return_value={})
    async def test_get_cluster_nodes(
        self,
        get_system_nodes_mock: mock.AsyncMock,
        dapi_mock: mock.MagicMock,
        fixture_affected_items_result: AffectedItemsXcyber360Result,
        nodes_data: list,
        excluded_nodes: list,
    ):
        """Check the correct output of `get_cluster_nodes` function."""

        self.xcyber360_dapi.excluded_nodes = excluded_nodes
        ...
        ret_val = await self.xcyber360_dapi.get_cluster_nodes()

        dapi_mock.assert_called_once_with(
            f=get_nodes_info,
            f_kwargs=None,
            logger=self.xcyber360_dapi.logger,
    ...
    async def test_reconnect_agents(
        ...
        ret_val = await self.xcyber360_dapi.reconnect_agents(agent_list=agent_list)
    ...
    async def test_get_agents_node_distribution(
        ...
        ret_val = await self.xcyber360_dapi.get_agents_node_distribution()
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'limit': self.xcyber360_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.xcyber360_dapi.logger,
            request_type='local_master',
        )
    ...
    async def test_get_agents_belonging_to_node(
        self, dapi_mock: mock.MagicMock, fixture_affected_items_result: AffectedItemsXcyber360Result, limit: Optional[int]
    ):
        ...
        ret_val = await self.xcyber360_dapi.get_agents_belonging_to_node(node_name=node_name, limit=limit)
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'limit': limit or self.xcyber360_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.xcyber360_dapi.logger,
            request_type='local_master',
        )
