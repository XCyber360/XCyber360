...
from xcyber360.core.cluster.hap_helper.proxy import Proxy, ProxyAPI, ProxyServerState
from xcyber360.core.cluster.hap_helper.xcyber360 import Xcyber360Agent, Xcyber360DAPI
from xcyber360.core.cluster.utils import (
    AGENT_CHUNK_SIZE,
...
from xcyber360.core.exception import Xcyber360Exception, Xcyber360HAPHelperError
...
class HAPHelper:
    """Helper to balance Xcyber360 agents through cluster calling HAProxy."""
    ...
    def __init__(
        ...
        self.proxy = proxy
        self.xcyber360_dapi = xcyber360_dapi
    ...
    def _get_logger(tag: str) -> logging.Logger:
        ...
        logger = logging.getLogger('xcyber360').getChild('HAPHelper')
    ...
    async def initialize_xcyber360_cluster_configuration(self):
        """Initialize main components of the Xcyber360 cluster."""
        if not await self.proxy.exists_backend(self.proxy.xcyber360_backend):
            self.logger.info(f"Could not find Xcyber360 backend '{self.proxy.xcyber360_backend}'")
            await self.proxy.add_new_backend(name=self.proxy.xcyber360_backend)
            self.logger.info('Added Xcyber360 backend')

        if not await self.proxy.exists_frontend(f'{self.proxy.xcyber360_backend}_front'):
            self.logger.info(f"Could not find Xcyber360 frontend '{self.proxy.xcyber360_backend}_front'")
            await self.proxy.add_new_frontend(
                name=f'{self.proxy.xcyber360_backend}_front',
                port=self.proxy.xcyber360_connection_port,
                backend=self.proxy.xcyber360_backend,
            )
            self.logger.info('Added Xcyber360 frontend')
    ...
    async def check_node_to_delete(self, node_name: str) -> bool:
        ...
        """
        node_stats = await self.proxy.get_xcyber360_server_stats(server_name=node_name)
    ...
    async def obtain_nodes_to_configure(
        self, xcyber360_cluster_nodes: dict, proxy_backend_servers: dict
    ) -> tuple[list, list]:
        """Returns the nodes able to add and delete.

        Parameters
        ----------
        xcyber360_cluster_nodes : dict
            Xcyber360 cluster nodes to check.
        ...
        for node_name, node_address in xcyber360_cluster_nodes.items():
            if node_name not in proxy_backend_servers:
                ...
                add_nodes.append(node_name)
        for node_name in proxy_backend_servers.keys() - xcyber360_cluster_nodes.keys():
            if node_name in self.xcyber360_dapi.excluded_nodes:
    ...
    async def update_agent_connections(self, agent_list: list[str]):
        ...
        for index in range(0, len(agent_list), self.agent_reconnection_chunk_size):
            await self.xcyber360_dapi.reconnect_agents(agent_list[index : index + self.agent_reconnection_chunk_size])
    ...
    async def force_agent_reconnection_to_server(self, chosen_server: str, agents_list: list[dict]):
        ...
        eligible_agents = Xcyber360Agent.get_agents_able_to_reconnect(agents_list=agents_list)
    ...
    async def migrate_old_connections(self, new_servers: list[str], deleted_servers: list[str]):
        ...
        """
        xcyber360_backend_stats = {}
        backend_stats_iteration = 1
        while any([server not in xcyber360_backend_stats for server in new_servers]):
            if backend_stats_iteration > self.UPDATED_BACKEND_STATUS_TIMEOUT:
                self.logger.error(f'Some of the new servers did not go UP: {set(new_servers) - xcyber360_backend_stats}')
                raise Xcyber360HAPHelperError(3041)

            self.logger.debug('Waiting for new servers to go UP')
            await asyncio.sleep(1)
            backend_stats_iteration += 1
            xcyber360_backend_stats = (await self.proxy.get_xcyber360_backend_stats()).keys()

        self.logger.debug('All new servers are UP')
        previous_agent_distribution = await self.xcyber360_dapi.get_agents_node_distribution()
        previous_connection_distribution = await self.proxy.get_xcyber360_backend_server_connections() | {
        ...
        for xcyber360_worker, agents in previous_agent_distribution.items():
            if xcyber360_worker in deleted_servers:
                agents_to_balance += [agent['id'] for agent in agents]
                continue
            try:
                agents_to_balance += [agent['id'] for agent in agents[: unbalanced_connections[xcyber360_worker]]]
                agents = agents[unbalanced_connections[xcyber360_worker] :]
            except KeyError:
                pass

            self.logger.info(f"Migrating {len(agents)} connections from server '{xcyber360_worker}'")
            await self.force_agent_reconnection_to_server(chosen_server=xcyber360_worker, agents_list=agents)

        if agents_to_balance:
            self.logger.info('Balancing exceeding connections after changes on the Xcyber360 backend')
    ...
    def check_for_balance(self, current_connections_distribution: dict) -> dict:
        """Checks if the Xcyber360 cluster is balanced.
    ...
    async def calculate_agents_to_balance(self, affected_servers: dict) -> dict:
        ...
        for server_name, n_agents in affected_servers.items():
            agent_candidates = await self.xcyber360_dapi.get_agents_belonging_to_node(node_name=server_name, limit=n_agents)
            eligible_agents = Xcyber360Agent.get_agents_able_to_reconnect(agents_list=agent_candidates)
    ...
    async def manage_xcyber360_cluster_nodes(self):
        """Main loop for check balance of Xcyber360 cluster."""

        while True:
            context_tag.set(self.tag)
            try:
                await self.backend_servers_state_healthcheck()
                current_xcyber360_cluster = await self.xcyber360_dapi.get_cluster_nodes()
                ...
                nodes_to_add, nodes_to_remove = await self.obtain_nodes_to_configure(
                    current_xcyber360_cluster, current_proxy_backend
                )
                if nodes_to_add or nodes_to_remove:
                    self.logger.info(
                        'Detected changes in Xcyber360 cluster nodes. Current cluster: ' f'{current_xcyber360_cluster}'
                    )
                    self.logger.info('Attempting to update proxy backend')

                    for node_to_remove in nodes_to_remove:
                        await self.proxy.remove_xcyber360_manager(manager_name=node_to_remove)

                    for node_to_add in nodes_to_add:
                        await self.proxy.add_xcyber360_manager(
                            manager_name=node_to_add,
                            manager_address=current_xcyber360_cluster[node_to_add],
                ...
                unbalanced_connections = self.check_for_balance(
                    current_connections_distribution=await self.proxy.get_xcyber360_backend_server_connections()
                )
                if not unbalanced_connections:
                    self.logger.debug(
                        'Current backend stats: ' f'{await self.proxy.get_xcyber360_backend_server_connections()}'
                    )
            ...
            except Xcyber360Exception as handled_exc:
    ...
    async def set_hard_stop_after(self, wait_connection_retry: bool = True, reconnect_agents: bool = True):
        ...
        agents_distribution = await self.xcyber360_dapi.get_agents_node_distribution()
        agents_id = [item['id'] for agents in agents_distribution.values() for item in agents]
        current_cluster = await self.xcyber360_dapi.get_cluster_nodes()
    ...
    async def start(cls):
        ...
        try:
            ...
            proxy = Proxy(
                xcyber360_backend=helper_config[HAPROXY_BACKEND],
                xcyber360_connection_port=connection_port,
                proxy_api=proxy_api,
                tag=tag,
            ...
            if await helper.proxy.check_multiple_frontends(
                port=connection_port, frontend_to_skip=f'{helper.proxy.xcyber360_backend}_front'
            ):
                logger.warning(
