...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        with patch('xcyber360.core.utils.load_xcyber360_xml'):
            ...
            from xcyber360.core.cluster import common as cluster_common, client, master
            from xcyber360.core.cluster.master import DEFAULT_DATE
            from xcyber360.core import common
            from xcyber360.core.cluster.dapi import dapi
            from xcyber360.core.common import DECIMALS_DATE_FORMAT
...
async def test_rit_init():
    ...
    with patch.object(master_common, 'integrity_check') as integrity_check_mock:
        receive_integrity_task = master.ReceiveIntegrityTask(xcyber360_common=master_common,
                                                            logger=logging.getLogger("xcyber360"))
        assert isinstance(receive_integrity_task.xcyber360_common, cluster_common.Xcyber360Common)
...
def test_rit_set_up_coro(create_task_mock):
    ...
    xcyber360_common_mock = Xcyber360CommonMock()
    receive_integrity_task = master.ReceiveIntegrityTask(xcyber360_common=xcyber360_common_mock,
                                                         logger=logging.getLogger("xcyber360"))
    assert receive_integrity_task.set_up_coro() == xcyber360_common_mock.integrity_check
    create_task_mock.assert_called_once()


@patch("asyncio.create_task")
@patch("xcyber360.core.cluster.common.ReceiveFileTask.done_callback")
def test_rit_done_callback(super_callback_mock, create_task_mock):
    ...
    assert xcyber360_common_mock.sync_integrity_free[0] is True


# Test ReceiveExtraValidTask class

@patch("asyncio.create_task")
@patch("xcyber360.core.cluster.master.ReceiveExtraValidTask.set_up_coro")
def test_revt_init(set_up_coro_mock, create_task_mock):
    """Test the correct initialization of the ReceiveExtraValidTask class."""

    receive_extra_valid_task = master.ReceiveExtraValidTask(xcyber360_common=cluster_common.Xcyber360Common(),
                                                            logger=logging.getLogger("xcyber360"))

    assert isinstance(receive_extra_valid_task.xcyber360_common, cluster_common.Xcyber360Common)
...
def test_revt_set_up_coro(create_task_mock):
    ...
    xcyber360_common_mock = Xcyber360CommonMock()
    receive_extra_valid_task = master.ReceiveExtraValidTask(xcyber360_common=xcyber360_common_mock,
                                                            logger=logging.getLogger("xcyber360"))
    assert receive_extra_valid_task.set_up_coro() == xcyber360_common_mock.sync_extra_valid
    create_task_mock.assert_called_once()
...
def test_revt_done_callback(set_up_coro_mock, super_callback_mock, create_task_mock):
    ...
    set_up_coro_mock.assert_called_once()
    assert xcyber360_common_mock.sync_integrity_free[0] is True
    assert xcyber360_common_mock.extra_valid_requested is False
...
def test_master_handler_init():
    ...
    with patch('xcyber360.core.cluster.master.context_tag', ContextVar('', default="")) as cv:
...
@patch.object(logging.getLogger("xcyber360"), "debug")
def test_master_handler_process_request(logger_mock):
    ...
    with patch("xcyber360.core.cluster.master.MasterHandler.get_permission", return_value=b"ok") as get_permission_mock:
    ...
    with patch("xcyber360.core.cluster.master.MasterHandler.setup_sync_integrity",
        ...
        setup_sync_integrity_mock.assert_has_calls(
            [call(b'syn_i_w_m', b"data"), call(b'syn_e_w_m', b"data")])

    # Test the syn_i_w_m_e|syn_e_w_m_e condition
    with patch("xcyber360.core.cluster.master.MasterHandler.end_receiving_integrity_checksums",
    ...
    with patch("xcyber360.core.cluster.master.MasterHandler.process_sync_error_from_worker",
    ...
    with patch("xcyber360.core.cluster.master.MasterHandler.process_dapi_res", return_value=b"ok") as process_dapi_res_mock:
    ...
    with patch("xcyber360.core.cluster.master.MasterHandler.get_nodes", return_value=(["cmd", "res"])) as get_nodes_mock:
    ...
    with patch("xcyber360.core.cluster.master.MasterHandler.get_health", return_value=(["cmd", "res"])) as get_health_mock:
        with patch("json.loads", return_value=b"ok") as json_loads_mock:
            with patch("json.dumps", return_value="ok") as json_dumps_mock:
    ...
    with patch("xcyber360.core.cluster.master.MasterHandler.distribute_orders", return_value=b'ok') as distribute_orders_mock:
        data = b'data'
    ...
    with patch("xcyber360.core.cluster.server.AbstractServerHandler.process_request",
    ...
    logger_mock.assert_has_calls([call("Command received: b'syn_i_w_m_p'"), call("Command received: b'syn_i_w_m'"),
                                  ...
                                  call("Command received: b'get_health'"), call("Command received: b'dist_orders'"),
                                  call("Command received: b'random'")])


@pytest.mark.asyncio
@patch("asyncio.wait_for")
@patch("xcyber360.core.cluster.master.uuid4", return_value=10101010)
async def test_master_handler_execute_ok(uuid4_mock, wait_for_mock):
    ...
    with patch("xcyber360.core.cluster.master.MasterHandler.send_request", return_value=b"result") as send_request_mock:
    ...
    with patch("xcyber360.core.cluster.master.MasterHandler.process_request",
...
async def test_master_handler_execute_ko(uuid4_mock):
    ...
    with pytest.raises(exception.Xcyber360ClusterError, match=r'.* 3022 .*'):
    ...
    with patch("xcyber360.core.cluster.master.MasterHandler.send_request", return_value=b"result"):
        with pytest.raises(exception.Xcyber360ClusterError, match=r".* 3021 .*"):
            await master_handler.execute(command=b'dapi', data=b"client request", wait_for_complete=False)

    # Test the third exception
    with patch("xcyber360.core.cluster.master.MasterHandler.process_request", return_value=[b"error", b""]):
        with pytest.raises(exception.Xcyber360ClusterError, match=r".* 3022 .*"):
            await master_handler.execute(command=b'random', data=b"client request", wait_for_complete=True)


@patch("xcyber360.core.cluster.common.SyncFiles", return_value="SyncFilesMock")
@patch("os.path.exists", return_value=False)
@patch("xcyber360.core.cluster.master.utils.mkdir_with_mode")
@patch("xcyber360.core.cluster.master.metadata.__version__", "version")
@patch("xcyber360.core.cluster.server.AbstractServerHandler.hello", return_value=(b"ok", "payload"))
def test_master_handler_hello_ok(super_hello_mock, mkdir_with_mode_mock, path_exists_mock, sync_files_mock):
    ...
    mkdir_with_mode_mock.assert_called_once_with(common.XCYBER360_RUN / "cluster" / "name")
    path_exists_mock.assert_called_once_with(common.XCYBER360_RUN / "cluster" / "name")
...
@patch("xcyber360.core.cluster.master.metadata.__version__", "random")
@patch("xcyber360.core.cluster.server.AbstractServerHandler.hello", return_value=(b"ok", "payload"))
def test_master_handler_hello_ko(super_hello_mock):
    ...
    with pytest.raises(exception.Xcyber360ClusterError, match=r".* 3031 .*"):
...
def test_master_handler_get_permission():
    ...
    class MockServer:
        ...
        def __init__(self):
            self.integrity_already_executed = ['not xcyber360']

    master_handler = get_master_handler()
    master_handler.server = MockServer()
    master_handler.name = 'xcyber360'
...
@patch("xcyber360.core.cluster.common.Xcyber360Common.setup_receive_file", return_value=b"ok")
...
@patch("xcyber360.core.cluster.common.Xcyber360Common.error_receiving_file", return_value=b"ok")
...
@patch("xcyber360.core.cluster.common.Xcyber360Common.end_receiving_file", return_value=b"ok")
...
@patch("shutil.rmtree")
@patch("asyncio.wait_for")
@patch("xcyber360.core.cluster.cluster.decompress_files", return_value=("files_metadata", "/decompressed/files/path"))
@patch('xcyber360.core.cluster.master.cluster.run_in_pool',
...
async def test_master_handler_sync_worker_files_ok(run_in_pool_mock, decompress_files_mock, wait_for_mock, rmtree_mock):
    ...
    await asyncio.gather(
        master_handler.sync_worker_files("task_id", event, logging.getLogger("xcyber360")),
        unlock_event(event))
...
@pytest.mark.asyncio
@patch("shutil.rmtree")
@patch('xcyber360.core.cluster.master.cluster.run_in_pool', side_effect=Exception)
@patch("xcyber360.core.cluster.cluster.decompress_files", return_value=("files_metadata", "/decompressed/files/path"))
@patch("xcyber360.core.cluster.master.MasterHandler.wait_for_file")
async def test_master_handler_sync_worker_files_ko(wait_for_mock, decompress_files_mock, run_in_pool_mock, rmtree_mock):
    ...
    with pytest.raises(Exception):
        await master_handler.sync_worker_files("task_id", asyncio.Event(), logging.getLogger("xcyber360"))

    # Test the second exception
    with pytest.raises(exception.Xcyber360ClusterError, match=r'.* 3038 .*'):
...
@pytest.mark.asyncio
@patch("xcyber360.core.cluster.master.MasterHandler.set_date_end_master")
@patch("xcyber360.core.cluster.master.MasterHandler.sync_worker_files")
async def test_master_handler_sync_extra_valid(sync_worker_files_mock, set_date_end_master_mock):
    ...
    master_handler = get_master_handler()
    master_handler.task_loggers["Integrity sync"] = logging.getLogger("xcyber360")
    await master_handler.sync_extra_valid("task_id", None)

    sync_worker_files_mock.assert_called_once_with("task_id", None, logging.getLogger("xcyber360"))
    set_date_end_master_mock.assert_called_once_with(logging.getLogger("xcyber360"))
...
@patch.object(logging.getLogger("xcyber360"), "info")
def test_set_date_end_master(info_mock):
    ...
    master_handler.set_date_end_master(logging.getLogger("xcyber360"))
...
@patch.object(logging.getLogger("xcyber360"), "info")
@patch.object(logging.getLogger("xcyber360"), "debug")
@patch("xcyber360.core.cluster.master.MasterHandler.integrity_sync")
@patch("xcyber360.core.cluster.master.MasterHandler.wait_for_file")
@patch("xcyber360.core.cluster.master.MasterHandler.send_request", return_value=b"ok")
@patch("xcyber360.core.cluster.cluster.decompress_files", return_value=("files_metadata", "/decompressed/files/path"))
async def test_master_handler_integrity_check(decompress_files_mock, send_request_mock, wait_for_file_mock,
    ...
    master_handler.server = ServerMock()
    master_handler.sync_tasks = {"task_id": TaskMock()}
    master_handler.task_loggers["Integrity check"] = logging.getLogger("xcyber360")

    with patch("xcyber360.core.cluster.cluster.compare_files", return_value=({"extra_valid": compare_result})
               ) as compare_mock:
...
@pytest.mark.asyncio
@patch("xcyber360.core.cluster.master.MasterHandler.wait_for_file", return_value=Exception())
...
@patch('xcyber360.core.cluster.master.utils.safe_move')
@patch("xcyber360.core.common.xcyber360_uid", return_value="xcyber360_uid")
@patch("xcyber360.core.common.xcyber360_gid", return_value="xcyber360_gid")
def test_master_handler_process_files_from_worker_ok(gid_mock, uid_mock, safe_move_mock, path_join_mock):
    ...
    worker_name = 'xcyber360'
    timeout = 0

    # Reset all the used mocks
    reset_mock(all_mocks)

    with patch("xcyber360.core.cluster.cluster.unmerge_info",
        ...
        with patch('os.path.isfile', return_value=True) as isfile_mock:
            with patch('os.stat', return_value=StatMock()) as os_stas_mock:
                ...
                path_join_mock.assert_has_calls([call(common.XCYBER360_PATH, 'data'),
                                                 call(common.XCYBER360_PATH, '/file/path'),
                                                 call(common.XCYBER360_PATH, 'queue', 'cluster', 'xcyber360', 'path')])
...
@patch.object(logging.getLogger("xcyber360"), "info")
@patch("xcyber360.core.cluster.master.server.AbstractServerHandler.connection_lost")
@patch("xcyber360.core.cluster.master.cluster.clean_up")
def test_master_handler_connection_lost(clean_up_mock, connection_lost_mock, logger_mock, worker_name):
    ...
    master_handler = get_master_handler()
    master_handler.logger = logging.getLogger("xcyber360")
...
@patch.object(logging.getLogger("xcyber360"), "warning")
@patch('asyncio.get_running_loop', return_value=loop)
@patch("xcyber360.core.cluster.master.ProcessPoolExecutor")
def test_master_init(pool_executor_mock, get_running_loop_mock, warning_mock):
    ...
    warning_mock.assert_has_calls([call("In order to take advantage of Xcyber360 4.3.0 cluster improvements, the directory "
                                   ...
                                   call('The Xcyber360 cluster will be run without the improvements added in Xcyber360 4.3.0 '
                                        'and higher versions.')])
...
@patch('asyncio.get_running_loop', return_value=loop)
@patch("xcyber360.core.cluster.master.metadata.__version__", "1.0.0")
...
async def test_master_file_status_update_ok(sleep_mock):
    ...
    with patch("xcyber360.core.cluster.master.cluster.run_in_pool", side_effect=run_in_pool) as run_in_pool_mock:
        with patch("xcyber360.core.cluster.master.Master.setup_task_logger",
...
@freeze_time("2021-11-02")
@patch('asyncio.sleep')
@patch('xcyber360.core.cluster.master.cluster.run_in_pool', return_value={})
...
@patch('asyncio.get_running_loop', return_value=loop)
@patch("xcyber360.core.agent.Agent.get_agents_overview", return_value={'totalItems': 5})
