...
from xcyber360.core.engine.base import APPLICATION_JSON
...
with patch('xcyber360.common.xcyber360_uid'):
    with patch('xcyber360.common.xcyber360_gid'):
        ...
        import xcyber360.core.cluster.common as cluster_common
        import xcyber360.core.results as wresults
...
xcyber360_common = cluster_common.Xcyber360Common()
...
@patch.object(logging.getLogger("xcyber360"), "error")
@patch("xcyber360.core.cluster.common.ReceiveFileTask.set_up_coro")
@patch("xcyber360.core.cluster.common.uuid4", return_value="e6c45993-b0ae-438f-8914-3b5175b2bbfd")
def test_rft_init(uuid_mock, setup_coro_mock, logger_mock, create_task_mock, event_mock):
    ...
    with patch.object(TaskMock, "add_done_callback") as done_callback_mock:
        file_task = cluster_common.ReceiveFileTask(xcyber360_common=cluster_common.Xcyber360Common(), logger=logger_mock,
                                                   task_id=b"010")
        assert isinstance(file_task.xcyber360_common, cluster_common.Xcyber360Common)
        ...
        string_task = cluster_common.ReceiveFileTask(xcyber360_common=cluster_common.Xcyber360Common(), logger=logger_mock,
                                                     task_id="")
...
@patch('logging.Logger')
@patch('xcyber360.core.cluster.common.Xcyber360Common')
def test_rft_set_up_coro(xcyber360_common_mock, logger_mock):
    """Test if the exception is being properly raised when an Exception takes place."""

    with pytest.raises(NotImplementedError):
        cluster_common.ReceiveFileTask(xcyber360_common_mock, logger_mock, b"task")
...
def test_rft_done_callback(set_up_coro_mock, create_task_mock, event_mock):
    ...
    class Xcyber360Common:

        def __init__(self):
            self.sync_tasks = {"010": b"123456789", "011": b"123456789"}

    create_task_mock.return_value = TaskMock()
    xcyber360_common_mock = Xcyber360Common()

    with patch.object(TaskMock, "add_done_callback"):
        with patch.object(logging.getLogger('xcyber360'), "error") as logger_mock:
            file_task = cluster_common.ReceiveFileTask(xcyber360_common=xcyber360_common_mock,
                                                       logger=logging.getLogger('xcyber360'), task_id=b"010")
            file_task.done_callback()
            assert file_task.xcyber360_common.sync_tasks == {"011": b"123456789"}
...
def test_handler_init():
    ...
    with patch('xcyber360.core.cluster.utils.context_tag', ContextVar('', default="")) as cv:
        ...
        assert handler.logger == logging.getLogger("xcyber360")
...
def test_handler_msg_build_ko():
    ...
    with pytest.raises(exception.Xcyber360ClusterError, match=r'.* 3024 .*'):
...
def test_handler_get_messages_ok():
    ...
    with patch('xcyber360.core.cluster.common.Handler.msg_parse', return_value=True) as handler_mock:
        # Test if
...
@pytest.mark.asyncio
@patch('xcyber360.core.cluster.common.Handler.push')
@patch('xcyber360.core.cluster.common.Handler.next_counter', return_value=30)
@patch('xcyber360.core.cluster.common.Handler.msg_build', return_value=["some", "messages"])
async def test_handler_send_request_ok(msg_build_mock, next_counter_mock, push_mock):
    ...
    with patch('xcyber360.core.cluster.common.Response.read', return_value="some value") as read_mock:
    ...
    with patch('xcyber360.core.cluster.common.Response.read', side_effect=delay):
        with pytest.raises(exception.Xcyber360ClusterError, match=r'\b3020\b'):
...
async def test_handler_send_request_ko():
    ...
    with patch('xcyber360.core.cluster.common.Handler.msg_build', side_effect=MemoryError):
        with pytest.raises(exception.Xcyber360ClusterError, match=r'.* 3026 .*'):
            await handler.send_request(b'some bytes', b'some data')

    with pytest.raises(exception.Xcyber360ClusterError, match=r'.* 3018 .*'):
        await handler.send_request(b'some bytes', b'some data')
...
@patch('xcyber360.core.cluster.common.Handler.send_request', return_value=b'some data')
async def test_handler_send_file_ok(send_request_mock, os_path_exists_mock):
    ...
    with patch('hashlib.sha256', return_value=MockHash()):
        assert (await handler.send_file(common.XCYBER360_RUN / 'some_file.txt', task_id=b'abcd') == 3)
        ...
        os_path_exists_mock.assert_called_once_with(common.XCYBER360_RUN / 'some_file.txt')
...
async def test_handler_send_file_ko():
    ...
    with pytest.raises(exception.Xcyber360ClusterError, match=r'.* 3034 .*'):
...
async def test_handler_send_string():
    ...
    with patch('xcyber360.core.cluster.common.Handler.send_request', return_value=b"some data"):
        assert (await handler.send_string(b"something") == b"some data")

    with patch('xcyber360.core.cluster.common.Handler.send_request', side_effect=exception.Xcyber360ClusterError(3020)):
        with patch.object(logging.getLogger("xcyber360"), "error") as logger_mock:
            assert exception.Xcyber360ClusterError(3020).message.encode() in await handler.send_string(b"something")
...
async def test_handler_forward_dapi_response_ok():
    ...
    with patch('xcyber360.core.cluster.common.Handler.get_manager', return_value=mock_manager):
...
async def test_handler_forward_dapi_response_ko():
    ...
    with patch('xcyber360.core.cluster.common.Handler.get_manager', return_value=mock_manager):
        ...
        with patch.object(mock_manager.local_server, "send_string",
                          side_effect=exception.Xcyber360Exception(1001)) as send_string_mock:
            with patch.object(logging.getLogger('xcyber360'), "error") as logger_mock:
                with patch('xcyber360.core.cluster.common.Handler.send_request', return_value="some value"):
                    with patch('json.dumps', return_value="some value"):
                        ...
                        logger_mock.assert_called_once_with(f"Error sending API response to local client: "
                                                            f"{exception.Xcyber360Exception(1001)}")
...
def test_handler_data_received_ok():
    ...
    # Test first if
    with patch('xcyber360.core.cluster.common.Handler.get_messages', return_value=[(b"bytes1", 123, b"bytes2", b"d")]):
        # Test try
    ...
    with patch('xcyber360.core.cluster.common.Handler.get_messages', return_value=[(b"bytes1", 123, b"bytes2", b"bytes3")]):
        ...
        with patch('xcyber360.core.cluster.common.Handler.dispatch') as dispatch_mock:
            handler.data_received(b"message")
            dispatch_mock.assert_called_once_with(b"bytes1", 123, b"bytes2")


@patch('xcyber360.core.cluster.common.Handler.msg_build', return_value=["msg"])
@patch('xcyber360.core.cluster.common.Handler.push')
@patch('xcyber360.core.cluster.common.Handler.process_request', return_value=(b"command", b"payload"))
def test_handler_dispatch(process_request_mock, push_mock, msg_build_mock):
    ...
    process_request_mock.side_effect = exception.Xcyber360Exception(1001)
    with patch.object(logging.getLogger('xcyber360'), "error") as logger_mock:
        handler.dispatch(None, 123, b"payload")
        logger_mock.assert_called_with(f"Internal error processing request '{None}': {exception.Xcyber360Exception(1001)}")
...
def test_handler_process_request():
    ...
    with patch('xcyber360.core.cluster.common.Handler.echo') as echo_mock:
        handler.process_request(b"echo", b"data")
        echo_mock.assert_called_once_with(b"data")
    with patch('xcyber360.core.cluster.common.Handler.receive_file') as receive_file_mock:
        handler.process_request(b"new_file", b"data")
        receive_file_mock.assert_called_once_with(b"data")
    with patch('xcyber360.core.cluster.common.Handler.receive_str') as receive_str_mock:
        handler.process_request(b"new_str", b"data")
        receive_str_mock.assert_called_once_with(b"data")
    with patch('xcyber360.core.cluster.common.Handler.update_file') as update_file_mock:
        handler.process_request(b"file_upd", b"data")
        update_file_mock.assert_called_once_with(b"data")
    with patch('xcyber360.core.cluster.common.Handler.str_upd') as str_upd_mock:
        handler.process_request(b"str_upd", b"data")
        str_upd_mock.assert_called_once_with(b"data")
    with patch('xcyber360.core.cluster.common.Handler.process_error_str') as process_error_str_mock:
        handler.process_request(b"err_str", b"data")
        process_error_str_mock.assert_called_once_with(b"data")
    with patch('xcyber360.core.cluster.common.Handler.end_file') as end_file_mock:
        handler.process_request(b"file_end", b"data")
        end_file_mock.assert_called_once_with(b"data")
    with patch('xcyber360.core.cluster.common.Handler.cancel_task') as cancel_task_mock:
        handler.process_request(b"cancel_task", b"data")
        cancel_task_mock.assert_called_once_with(b"data")
    with patch('xcyber360.core.cluster.common.Handler.process_dapi_error') as process_dapi_err_mock:
    ...
    with patch('xcyber360.core.cluster.common.Handler.process_unknown_cmd') as process_unknown_cmd_mock:
...
def test_handler_process_response():
    ...
    with patch('xcyber360.core.cluster.common.Handler.process_error_from_peer', return_value=b"payload"):
...
def test_handler_receive_file():
    ...
    with patch('xcyber360.core.cluster.common.open') as open_mock:
...
def test_handler_str_upd():
    ...
    with patch('xcyber360.core.cluster.common.InBuffer.receive_data'):
...
def test_handler_process_dapi_error_ko():
    ...
    handler = cluster_common.Handler(cluster_items)
    with pytest.raises(exception.Xcyber360ClusterError, match=r".* 3032 .*"):
        handler.server = ManagerMock()
...
def test_handler_process_error_from_peer():
    ...
    assert isinstance(handler.process_error_from_peer(b"data to decode"), exception.Xcyber360ClusterError)
...
def test_handler_setup_task_logger():
    ...
    with patch.object(logging.getLogger("xcyber360"), "getChild", return_value=TaskLoggerMock()) as get_child_mock:
...
@pytest.mark.asyncio
@patch('xcyber360.core.cluster.common.Handler.send_request')
async def test_handler_wait_for_file_ko(send_request_mock):
    """Check if expected exception is raised.
        Condition 1: when event.wait() exceeds the timeout, Xcyber360ClusterError 3039 is raised
        Condition 2: when any other exception occurs, Xcyber360ClusterError 3040  is raised"""
    ...
    with pytest.raises(exception.Xcyber360ClusterError, match=r'.* 3039 .*'):
    ...
    with pytest.raises(exception.Xcyber360ClusterError, match=r".* 3040 .*"):
...
@patch('xcyber360.core.cluster.common.httpx.AsyncClient.post')
@patch('xcyber360.core.cluster.common.httpx.AsyncHTTPTransport.aclose')
@patch('json.loads')
...
@patch('xcyber360.core.cluster.common.httpx.AsyncClient.post', side_effect=TimeoutException(''))
@patch('xcyber360.core.cluster.common.httpx.AsyncHTTPTransport.aclose')
@patch('json.loads')
async def test_handler_send_orders_ko(json_loads_mock, close_mock, post_mock):
    """Check if `send_orders` is called with expected parameters."""
    handler = cluster_common.Handler(cluster_items)
    with pytest.raises(exception.Xcyber360Error, match=r'.*3041.*'):
        await handler.send_orders(b'[{"source": "Users/Services", "user": "Management API"}]')


# Test 'Xcyber360Common' class methods

def test_xcyber360_common_init():
    """Test the '__init__' method correct functioning."""

    xcyber360_common_test = cluster_common.Xcyber360Common()
    assert xcyber360_common_test.sync_tasks == {}

def test_xcyber360_common_get_logger():
    """Check if a Logger object is properly returned."""

    with pytest.raises(NotImplementedError):
        xcyber360_common.get_logger()


def test_xcyber360_common_setup_receive_file():
    ...
    with patch('xcyber360.core.cluster.common.Xcyber360Common.get_logger'):
        first_output, second_output = xcyber360_common.setup_receive_file(mock_object)
        assert first_output == b'ok'
        assert isinstance(second_output, bytes)
        assert MyTaskMock().task_id in xcyber360_common.sync_tasks
        assert isinstance(xcyber360_common.sync_tasks[MyTaskMock().task_id], MyTaskMock)


@patch('xcyber360.core.cluster.common.Xcyber360Common')
@patch('logging.Logger')
def test_xcyber360_common_end_receiving_file_ok(logger_mock, xcyber360_common_mock):
    """Check if the full path to the received file is properly stored and availability is notified."""

    with patch('xcyber360.core.cluster.common.ReceiveFileTask.set_up_coro'):
        with patch('asyncio.create_task'):
            file_task = cluster_common.ReceiveFileTask(xcyber360_common_mock, logger_mock, b"task")

    xcyber360_common.sync_tasks = {'task_ID': file_task}
    assert xcyber360_common.end_receiving_file("task_ID filepath") == (b'ok', b'File correctly received')
    assert isinstance(xcyber360_common.sync_tasks["task_ID"], cluster_common.ReceiveFileTask)


@patch('os.remove')
@patch('os.path.exists', return_value=True)
def test_xcyber360_common_end_receiving_file_ko(path_exists_mock, os_remove_mock):
    """Test the 'end_receiving_file' correct functioning in a failure scenario."""

    with pytest.raises(exception.Xcyber360ClusterError, match=r'.* 3027 .*'):
        xcyber360_common.end_receiving_file("not_task_ID filepath")
...
@patch('json.loads')
def test_xcyber360_common_error_receiving_file_ok(json_loads_mock):
    """Check how error are handled by peer in the sent file process."""

    with patch('os.path.exists', return_value=True):
        with patch('os.remove'):
            # Test first condition and its nested condition
            assert xcyber360_common.error_receiving_file("task_ID error_details") == (b'ok', b'Error received')

    # Test second condition
    with patch('xcyber360.core.cluster.common.Xcyber360Common.get_logger'):
        assert xcyber360_common.error_receiving_file("not_task_ID error_details") == (b'ok', b'Error received')


def test_xcyber360_common_error_receiving_file_ko():
...
def test_xcyber360_common_get_node():
    """Check if it is possible to obtain basic information about the node."""

    class MockClass(cluster_common.Xcyber360Common, cluster_common.Handler, abc.ABC):
        def __init__(self):
            super().__init__()
    ...
    with patch('xcyber360.core.cluster.common.Handler.get_manager', return_value=mock_manager) as manager_mock:
        mock_class.get_node()
...
def test_xcyber360_json_encoder_default():
    """Test if a special JSON encoder is defined for Xcyber360."""

    xcyber360_encoder = cluster_common.Xcyber360JSONEncoder()

    # Test first condition
    assert isinstance(xcyber360_encoder.default(object), dict)
    assert xcyber360_encoder.default(object) == {'__callable__': {'__name__': 'object', '__module__': 'builtins',
                                                              '__qualname__': 'object', '__type__': 'type'}}

    class Xcyber360Mock(json.JSONEncoder):
        __self__ = Xcyber360()
        a = 0
        b = 0
    ...
    xcyber360_encoder.default(Xcyber360Mock)

    with patch('builtins.callable', return_value=False):
        # Test second condition
        assert isinstance(xcyber360_encoder.default(exception.Xcyber360Exception(3012)), dict)
        assert xcyber360_encoder.default(exception.Xcyber360Exception(3012)) == \
               {'__xcyber360_exception__': {'__class__': 'Xcyber360Exception',
                                        '__object__': {'type': 'about:blank', 'title': 'Xcyber360Exception', 'code': 3012,
                                                       'extra_message': None, 'extra_remediation': None,
                                                       'cmd_error': False, 'dapi_errors': {}}}}

        # Test third condition
        abstract_xcyber360_result = wresults.AffectedItemsXcyber360Result()

        assert isinstance(xcyber360_encoder.default(abstract_xcyber360_result), dict)
        assert xcyber360_encoder.default(abstract_xcyber360_result) == \
               {'__xcyber360_result__': {'__class__': 'AffectedItemsXcyber360Result',
        ...
        assert isinstance(xcyber360_encoder.default(date), dict)
        assert xcyber360_encoder.default(date) == {'__xcyber360_datetime__': '2021-10-15T00:00:00'}

        # Test fifth condition
        exc = ValueError('test')
        assert isinstance(xcyber360_encoder.default(exc), dict)
        assert xcyber360_encoder.default(exc) == {'__unhandled_exc__': {'__class__': 'ValueError',
                                                                    '__args__': ('test',)}}

        # Test simple return
        with pytest.raises(TypeError):
            xcyber360_encoder.default({"key": "value"})


def test_as_xcyber360_object_ok():
    """Test the different outputs taking into account the input values."""

    # Test the first condition and nested if
    assert cluster_common.as_xcyber360_object({"__callable__": {"__name__": "type", "__xcyber360__": "version"}}) == "server"

    # Test the first condition and nested else
    assert isinstance(
        cluster_common.as_xcyber360_object({"__callable__": {"__name__": "path", "__qualname__": "__loader__.value",
                                                         "__module__": "os"}}), str)

    assert cluster_common.as_xcyber360_object({"__callable__": {"__name__": "__name__", "__qualname__": "value",
                                                            "__module__": "itertools"}}) == "itertools"

    # Test the second condition
    assert isinstance(cluster_common.as_xcyber360_object(
        {"__xcyber360_exception__": {"__class__": "Xcyber360Exception", "__object__": {"code": 1001}}}),
        exception.Xcyber360Exception)

    # Test the third condition
    with patch('xcyber360.core.results.AbstractXcyber360Result.decode_json', return_value=wresults.AbstractXcyber360Result):
        assert isinstance(cluster_common.as_xcyber360_object(
            {"__xcyber360_result__": {"__class__": "AbstractXcyber360Result", "__object__": {"code": 1001}}}), abc.ABCMeta)

    # Test the fourth condition
    assert isinstance(cluster_common.as_xcyber360_object({"__xcyber360_datetime__": "2021-10-14"}), datetime)
    assert cluster_common.as_xcyber360_object({"__xcyber360_datetime__": "2021-10-14"}) == datetime(2021, 10, 14)

    # Test the fifth condition
    result = cluster_common.as_xcyber360_object({'__unhandled_exc__': {'__class__': 'ValueError',
                                                                   '__args__': ('test',)}})
    assert result == {"ValueError": ["test"]}

    result = cluster_common.as_xcyber360_object({'__unhandled_exc__': {'__class__': 'exit',
                                                                   '__args__': []}})
    assert result == {"exit": []}

    # No condition fulfilled
    assert isinstance(cluster_common.as_xcyber360_object({"__xcyber360_datetime_bad__": "2021-10-14"}), dict)
    assert cluster_common.as_xcyber360_object({"__xcyber360_datetime_bad__": "2021-10-14"}) == \
           {"__xcyber360_datetime_bad__": "2021-10-14"}


def test_as_xcyber360_object_ko():
    """Test if the exceptions are correctly raised."""

    with pytest.raises(exception.Xcyber360InternalError, match=r'.* 1000 .*'):
        cluster_common.as_xcyber360_object({"__callable__": {"__name__": "value", "__xcyber360__": "value"}})


def get_handler():
    """Return a Handler object. This is an auxiliary method."""
    return cluster_common.Handler(cluster_items=cluster_items, logger=logging.getLogger("xcyber360"))
...
def test_sync_task_init():
    ...
    sync_task = cluster_common.SyncTask(b"cmd", logging.getLogger("xcyber360"), get_handler())

    assert sync_task.cmd == b"cmd"
    assert sync_task.logger == logging.getLogger("xcyber360")
    assert isinstance(sync_task.server, cluster_common.Handler)


@pytest.mark.asyncio
@patch('xcyber360.core.cluster.common.Handler.send_request', side_effect=Exception())
async def test_sync_task_request_permission(send_request_mock):
    ...
    with patch.object(logging.getLogger("xcyber360"), "debug") as logger_mock:
        # Test second condition
...
@patch("xcyber360.core.cluster.cluster.compress_files", return_value=("files/path/", {}))
@patch("xcyber360.core.cluster.utils.log_subprocess_execution")
async def test_sync_files_sync_ok(log_subprocess_mock, compress_files_mock, unlink_mock, relpath_mock, json_dumps_mock):
    ...
    class WorkerMock:
        ...
        async def send_request(self, command, data):
            ...
            if command == b"cmd" and data == b"" and self.count == 1:
                raise exception.Xcyber360ClusterError(3020, extra_message=command.decode())
                ...
                raise exception.Xcyber360ClusterError(3016, extra_message=command.decode())
    ...
    sync_files = cluster_common.SyncFiles(b"cmd", logging.getLogger("xcyber360"), worker_mock)
    ...
    with patch.object(logging.getLogger("xcyber360"), "error") as logger_mock:
        ...
        json_dumps_mock.assert_called_once_with(
            exception.Xcyber360ClusterError(code=3020, extra_message="cmd"),
            cls=cluster_common.Xcyber360JSONEncoder)
    ...
    with patch.object(WorkerMock, "send_file", return_value=1000) as send_file_mock:
        ...
        with patch.object(logging.getLogger("xcyber360"), "debug") as logger_debug_mock:
            with patch.object(logging.getLogger("xcyber360"), "error") as logger_error_mock:
                await sync_files.sync(files_to_sync, files_metadata, 1, task_pool=None)
                ...
                relpath_mock.assert_called_once_with('files/path/', common.XCYBER360_RUN)
                ...
                logger_error_mock.assert_called_once_with(
                    f"Error sending zip file: {exception.Xcyber360Exception(3016, 'cmd_e')}")
...
@pytest.mark.asyncio
@patch('xcyber360.core.cluster.cluster.open')
@patch('xcyber360.core.cluster.cluster.mkdir_with_mode')
@patch('xcyber360.core.cluster.cluster.get_cluster_items')
@patch("xcyber360.core.cluster.common.Handler.send_request", side_effect=Exception())
async def test_sync_files_sync_ko(send_request_mock, get_cluster_items_mock, mkdir_with_mode_mock, open_mock):
    ...
    sync_files = cluster_common.SyncFiles(b"cmd", logging.getLogger("xcyber360"), handler)
    ...
    with patch('os.unlink', side_effect=FileNotFoundError):
        ...
        with patch("xcyber360.core.cluster.cluster.compress_files", return_value=(compressed_data, {})):
            send_request_mock.side_effect = None
...
def test_create_ssl_context(logger_mock, purpose, keyfile_password):
    ...
    ssl_mock = SSLMock()
    cafile = '/var/xcyber360_server/etc/sslmanager.ca'
    certfile = '/var/xcyber360_server/etc/sslmanager.cert'
    keyfile = '/var/xcyber360_server/etc/sslmanager.key'
