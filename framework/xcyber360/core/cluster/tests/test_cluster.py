...
with patch('xcyber360.common.xcyber360_uid'):
    with patch('xcyber360.common.xcyber360_gid'):
        ...
        import xcyber360.core.cluster.cluster as cluster
...
certificates_configuration = {
    'cafile': common.XCYBER360_ETC / 'rootCA.cert',
    'certfile': common.XCYBER360_ETC / 'test.cert',
    'keyfile': common.XCYBER360_ETC / 'test.key'
}


@patch('pathlib.Path.exists', return_value=True)
@patch.object(xcyber360.core.cluster.cluster.logger, "warning")
...
@pytest.mark.parametrize('read_config, message', [
    ...
    ({
        'cluster': {
            ...
            'port': 30000,
            'certfile': common.XCYBER360_ETC / 'test.cert',
            'keyfile': common.XCYBER360_ETC / 'test.cert'
        }
    }, "Paths to certificates and keys must be different."),
    ({
        'cluster': {
            'port': 30000,
            'cafile': common.XCYBER360_ETC / 'fail',
            'keyfile': common.XCYBER360_ETC / 'test.key'
        }
    }, 'does not exist.'),
    ({
        'cluster': {
            'port': 30000,
            'cafile': common.XCYBER360_ETC / '/test',
            'keyfile': common.XCYBER360_ETC / 'test.key'
        }
    }, f'is not inside {common.XCYBER360_ETC}.'),
    ({
        'cluster': {
            'port': 30000,
            'cafile': common.XCYBER360_ETC / '../test',
...
def test_check_cluster_config_ko(read_config, message):
    ...
    with patch('xcyber360.core.cluster.utils.get_ossec_conf', return_value=read_config) as m:
        with pytest.raises(Xcyber360Exception, match=rf'.* 3004 .* {message}'):
            configuration = xcyber360.core.cluster.utils.read_config()
...
def test_get_node():
    ...
    with patch('xcyber360.core.cluster.cluster.read_config', return_value=test_dict):
...
@patch('xcyber360.core.cluster.cluster.blake2b', return_value="hash")
@patch("xcyber360.core.cluster.cluster.path.join", return_value="/mock/foo/bar")
@patch('xcyber360.core.cluster.cluster.walk', return_value=[('foo/bar', (), ('spam', 'eggs', '.merged'))])
def test_walk_dir(walk_mock, path_join_mock, blake2b_mock, getmtime_mock):
    ...
    path_join_mock.assert_called_once_with(common.XCYBER360_ETC, '/foo/bar')
    ...
    walk_mock.assert_called_once_with(path_join_mock.return_value, topdown=True)
    path_join_mock.assert_has_calls([call(common.XCYBER360_ETC, 'foo/bar'),
...
@patch('xcyber360.core.cluster.cluster.walk', return_value=[('/foo/bar', (), ['spam'])])
...
def test_walk_dir_ko(mock_path_join, mock_walk):
    ...
    with patch('xcyber360.core.cluster.cluster.walk', side_effect=OSError):
        with pytest.raises(Xcyber360InternalError, match=r'.* 3015 .*'):
...
@patch('xcyber360.core.cluster.cluster.get_cluster_items', return_value={
    "files": {
        "etc/": {
            "permissions": 416,
            "source": "master",
...
def test_get_files_status(mock_get_cluster_items):
    ...
    with patch('xcyber360.core.cluster.cluster.walk_dir', return_value=(test_dict, {})):
    ...
    with patch('xcyber360.core.cluster.cluster.walk_dir', side_effect=Exception):
...
def test_get_ruleset_status(mock_get_cluster_items):
    ...
    with patch("xcyber360.core.cluster.cluster.walk_dir", return_value=(test_dict, {})) as walk_dir_mock:
    ...
    with patch("xcyber360.core.cluster.cluster.walk_dir", side_effect=Exception):
        with patch.object(xcyber360.core.cluster.cluster.logger, "warning") as logger_mock:
...
@patch('xcyber360.core.cluster.cluster.path.dirname', return_value='/some/path')
@patch('xcyber360.core.cluster.cluster.path.exists', return_value=False)
def test_compress_files_ok(mock_path_exists, mock_path_dirname, mock_mkdir_with_mode, mock_get_cluster_items,
    ...
    with patch('builtins.open', mock_open(read_data='test_content')) as open_mock:
        assert isinstance(cluster.compress_files('some_name', ['some/path', 'another/path'], {'ko_file': 'file'}),
                          tuple)
        assert open_mock.call_args_list == [call(ANY, 'ab'), call(os.path.join(common.XCYBER360_ETC, 'some/path'), 'rb'),
                                            call(os.path.join(common.XCYBER360_ETC, 'another/path'), 'rb')]
...
def test_compress_files_ko(mock_path_exists, mock_path_dirname, mock_mkdir_with_mode, mock_get_cluster_items):
    ...
    with patch('builtins.open', mock_open(read_data='test_content')):
        ...
        assert logs['warning']['some/path'] == [f'File too large to be synced: '
                                                f'{os.path.join(common.XCYBER360_ETC, "some/path")}']
        ...
        with patch('zlib.compress', side_effect=zlib.error):
            with pytest.raises(Xcyber360Error, match=r'.* 3001 .*'):
...
@pytest.mark.asyncio
@patch('xcyber360.core.cluster.cluster.decompress_files', return_value="OK")
...
@patch('xcyber360.core.cluster.cluster.remove')
@patch('xcyber360.core.cluster.cluster.mkdir_with_mode')
        ...
        with patch('builtins.open', mock_open(read_data=f'path{cluster.PATH_SEP}content'.encode())):
            with patch('os.path.exists', return_value=False):
                with patch('os.makedirs', side_effect=PermissionError) as mock_makedirs:
                    with patch('xcyber360.core.cluster.cluster.remove'):
...
def test_compare_files(mock_get_cluster_items):
    ...
    # First condition
    with patch('xcyber360.core.cluster.cluster.merge_info', return_values=[1, "random/path/"]):
...
@patch('xcyber360.core.cluster.cluster.get_cluster_items')
@patch.object(xcyber360.core.cluster.cluster.logger, "error")
...
def test_clean_up_ok():
    ...
    with patch('os.path.join', return_value="some/path/"):
        with patch.object(xcyber360.core.cluster.cluster.logger, "debug") as mock_logger:
            with patch('os.path.exists', return_value=False) as path_exists_mock:
                ...
                with patch('xcyber360.core.cluster.cluster.listdir',
...
def test_clean_up_ko():
    ...
    with patch('os.path.join') as path_join_mock:
        with patch.object(xcyber360.core.cluster.cluster.logger, "error") as mock_error_logger:
            with patch.object(xcyber360.core.cluster.cluster.logger, "debug") as mock_debug_logger:
...
@patch('xcyber360.core.cluster.cluster.listdir', return_value=['005', '006'])
@patch('xcyber360.core.cluster.cluster.stat')
def test_merge_info(stat_mock, listdir_mock):
    ...
    with patch('builtins.open', mock_open(read_data=agent_groups)) as open_mock:
        ...
        open_mock.assert_any_call(str(common.XCYBER360_RUN / 'cluster/worker1/testing-shared.merged'), 'wb')
        open_mock.assert_any_call(str(common.XCYBER360_RUN / 'cluster/testing/005'), 'rb')
        open_mock.assert_any_call(str(common.XCYBER360_RUN / 'cluster/testing/006'), 'rb')
...
def test_unmerge_info():
    ...
    with patch('builtins.open', mock_open(read_data=agent_info)):
        with patch('xcyber360.core.cluster.cluster.stat') as stat_mock:
            ...
            with patch.object(xcyber360.core.cluster.cluster.logger, "warning") as mock_logger:
...
async def test_run_in_pool(event_loop):
    ...
    with patch('xcyber360.core.cluster.cluster.wait_for', return_value="OK") as wait_for_mock:
...
def test_validate_haproxy_helper_config_ko():
    """Verify that validate_haproxy_helper_config raises Xcyber360Error when there is a validation error."""

    config = {cluster.AGENT_CHUNK_SIZE: 120, cluster.AGENT_RECONNECTION_TIME: 10}

    with patch.object(cluster, 'validate', side_effect=cluster.ValidationError(message='Error test', path=['test'])):
        with pytest.raises(cluster.Xcyber360Error, match='.* 3004 .*'):
            cluster.validate_haproxy_helper_config(config)
