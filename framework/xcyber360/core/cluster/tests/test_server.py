...
with patch('xcyber360.common.xcyber360_uid'):
    with patch('xcyber360.common.xcyber360_gid'):
        from xcyber360.core.cluster.server import *
        from xcyber360.core.cluster import common as c_common
        from xcyber360.core.exception import Xcyber360ClusterError, Xcyber360Error, Xcyber360ResourceNotFound
...
async def test_AbstractServerHandler_init(event_loop):
    ...
    with patch("xcyber360.core.cluster.server.context_tag", ContextVar("tag", default="")) as mock_contextvar:
...
@pytest.mark.asyncio
@patch("xcyber360.core.cluster.server.AbstractServerHandler.hello")
@patch("xcyber360.core.cluster.server.AbstractServerHandler.echo_master")
@patch("xcyber360.core.cluster.common.Handler.process_request")
async def test_AbstractServerHandler_process_request(mock_process_request, mock_echo_master, mock_hello, event_loop):
    ...
    abstract_server_handler.process_request(command=b"echo-c", data=b"xcyber360")
    mock_echo_master.assert_called_once_with(b"xcyber360")
...
async def test_AbstractServerHandler_echo_master(event_loop):
    ...
    assert abstract_server_handler.echo_master(data=b"xcyber360") == (b"ok-m ", b"xcyber360")
    abstract_server_handler.echo_master(data=b"xcyber360")
...
def test_AbstractServerHandler_hello(event_loop):
    ...
    with pytest.raises(Xcyber360ClusterError, match=".* 3029 .*"):
        abstract_server_handler.hello(b"elif_test")

    abstract_server_handler.server.clients["if_test"] = "testing"
    with pytest.raises(Xcyber360ClusterError, match=f".* 3028 .* if_test"):
        abstract_server_handler.hello(b"if_test")
    assert abstract_server_handler.name == ""


@pytest.mark.asyncio
@patch("xcyber360.core.cluster.common.Handler.process_response")
...
@pytest.mark.asyncio
@patch("asyncio.Queue")
@patch("xcyber360.core.cluster.server.functools")
...
@patch('xcyber360.core.cluster.server.AbstractServer.check_clients_keepalive')
@patch('xcyber360.core.cluster.server.AbstractServerHandler')
...
@patch("xcyber360.core.cluster.server.uuid4", return_value="abc123")
@patch("asyncio.get_running_loop", new=Mock())
...
@patch("xcyber360.core.cluster.server.utils.process_array")
@patch("asyncio.get_running_loop", new=Mock())
def test_AbstractServer_get_connected_nodes(mock_process_array):
    ...
    with patch.object(abstract_server, "to_dict", return_value=basic_dict):
        abstract_server.get_connected_nodes(search={"value": "xcyber360", "negation": True},
                                            sort={"fields": ["nothing"], "order": "desc"}, limit=501, offset=1)
        mock_process_array.assert_called_once_with([basic_dict["info"]], search_text="xcyber360", complementary_search=True,
...
def test_AbstractServer_get_connected_nodes_ko(mock_process_array):
    ...
    with patch.object(abstract_server, "to_dict", return_value=basic_dict):
        ...
        with pytest.raises(Xcyber360Error, match=".* 1724 .* Not a valid select field: no"):
            abstract_server.get_connected_nodes(select={"no": "exists"})

        with pytest.raises(Xcyber360Error, match=".* 1728 .*"):
            abstract_server.get_connected_nodes(filter_type="None")

        abstract_server.configuration = {"node_name": "master"}
        with pytest.raises(Xcyber360ResourceNotFound, match=".* 1730 .*"):
...
async def test_AbstractServer_check_clients_keepalive(sleep_mock):
    ...
    with patch.object(logger, "debug") as mock_debug:
        with patch.object(logger, "error") as mock_error:
            with patch("xcyber360.core.cluster.server.AbstractServer.setup_task_logger",
                       return_value=logger):
                abstract_server = AbstractServer(performance_test=1, concurrency_test=2, configuration={"test3": 3},
                                                 cluster_items={"test4": 4}, logger=logger)
...
@patch("asyncio.sleep", side_effect=IndexError)
@patch("asyncio.get_running_loop", new=Mock())
@patch('xcyber360.core.cluster.server.perf_counter', return_value=0)
...
async def test_AbstractServer_start_ko(keepalive_mock, set_event_loop_policy_mock, eventlooppolicy_mock,
    ...
    with patch("asyncio.get_running_loop", return_value=LoopMock()), \
        ...
        with pytest.raises(exception.Xcyber360ClusterError, match=r'.* 3007 .*'):
            abstract_server = AbstractServer(
                performance_test=1,
                concurrency_test=2,
                configuration={
