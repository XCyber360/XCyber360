...
with patch('xcyber360.common.xcyber360_uid'):
    with patch('xcyber360.common.xcyber360_gid'):
        ...
        import xcyber360.core.cluster.client as client
...
def test_acm_init():
    ...
    assert abstract_client_manager.logger == logging.getLogger("xcyber360")
...
@patch('xcyber360.core.cluster.client.AbstractClientManager.add_tasks', return_value=[])
async def test_acm_start(add_tasks_mock, starmap_mock, asyncio_sleep_mock):
    ...
    with patch.object(LoopMock, "create_connection",
        ...
        with patch.object(logging.getLogger("xcyber360"), "info") as logger_info_mock, \
            patch.object(logging.getLogger("xcyber360"), "error") as logger_error_mock, \
...
def test_ac_connection_result():
    ...
    with patch.object(logging.getLogger('xcyber360'), "error") as logger_mock:
        ...
        with patch.object(abstract_client.transport, "close") as close_mock:
            ...
            future.set_result([Xcyber360ClusterError(3020)])
            abstract_client.connection_result(future)
            logger_mock.assert_called_once_with(f"Could not connect to master: {str(Xcyber360ClusterError(3020))}.")
            ...
            future = asyncio.Future()
            future.set_exception(Xcyber360ClusterError(3020))
            abstract_client.connection_result(future)
            logger_mock.assert_called_once_with(f"Could not connect to master: {str(Xcyber360ClusterError(3020))}.")
            close_mock.assert_called_once()

    # Check second condition
    with patch.object(logging.getLogger('xcyber360'), "info") as logger_mock:
        abstract_client.transport = CloseMock()
        future = asyncio.Future()
...
@patch('xcyber360.core.cluster.client.AbstractClient._cancel_all_tasks')
def test_ac_connection_lost(cancel_tasks_mock):
    ...
    with patch.object(logging.getLogger('xcyber360'), "error") as logger_mock:
        abstract_client.connection_lost(exc=Xcyber360Exception(1001))
        logger_mock.assert_called_once_with(f"Connection closed due to an unhandled error: {Xcyber360Exception(1001)}\n",
                                            exc_info=False)
...
def test_ac_process_response():
    ...
    # Test the second condition
    with patch('xcyber360.core.cluster.common.Handler.process_response', return_value=b'ok') as pr_mock:
...
def test_ac_process_request():
    ...
    # Test the fist condition
    with patch('xcyber360.core.cluster.client.AbstractClient.echo_client', return_value=b'ok') as echo_mock:
        assert (abstract_client.process_request(command=b"echo-m", data=b"data") == b'ok')
        echo_mock.assert_called_once_with(b'data')

    # Test the second condition
    with patch('xcyber360.core.cluster.common.Handler.process_request', return_value=b'ok') as pr_mock:
...
@patch('xcyber360.core.cluster.client.AbstractClient.send_request', return_value=b"ok")
async def test_ac_client_echo_ok(send_request_mock, done_mock, asyncio_sleep_mock):
    ...
    # Test try
    with patch.object(logging.getLogger("xcyber360"), "info") as logger_mock:
        with patch('xcyber360.core.cluster.common.Handler.setup_task_logger',
                   return_value=logging.getLogger("xcyber360")) as setup_logger_mock:
            try:
...
@patch.object(FutureMock, "done", return_value=False)
@patch('xcyber360.core.cluster.client.perf_counter', return_value=0)
@patch('xcyber360.core.cluster.client.AbstractClient.send_request', return_value="ok")
...
@freeze_time("2022-01-01")
@patch('xcyber360.core.cluster.client.perf_counter', return_value=0)
@patch('xcyber360.core.cluster.client.AbstractClient.send_file', return_value="ok")
...
@freeze_time("2022-01-01")
@patch('xcyber360.core.cluster.client.perf_counter', return_value=0)
@patch('xcyber360.core.cluster.client.AbstractClient.send_string', return_value="ok")
