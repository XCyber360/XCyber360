...
with patch('xcyber360.common.xcyber360_uid'):
    with patch('xcyber360.common.xcyber360_gid'):
        from xcyber360.core.cluster.local_client import *
        from xcyber360.core.cluster.common import InBuffer
...
@patch('xcyber360.core.cluster.utils.read_config')
@patch("xcyber360.core.cluster.client.asyncio.get_running_loop")
...
async def test_localclient_start(read_config_mock, get_cluster_items_mock):
    ...
    with patch("uvloop.Loop.create_unix_connection", side_effect=create_unix_connection) as mock_create_unix_connection:
        ...
        with patch("xcyber360.core.cluster.client.asyncio.get_running_loop", return_value=mocked_loop):
            lc = LocalClient()
            await lc.start()
            assert mock_create_unix_connection.call_count == 1
            assert mock_create_unix_connection.call_args[1]["path"] == common.XCYBER360_SOCKET / common.LOCAL_SERVER_SOCKET
...
async def test_wait_for_response(read_config_mock, get_cluster_items_mock):
    ...
    lc.protocol.send_request.side_effect = [b"None", exception.Xcyber360ClusterError(3018)]

    with patch("asyncio.Event.wait", side_effect=asyncio.TimeoutError):
        with pytest.raises(Xcyber360InternalError, match=rf".* 3020 .*"):
...
async def test_localclient_execute(read_config_mock, get_cluster_items_mock):
    ...
    with patch("xcyber360.core.cluster.local_client.LocalClient.start"):
        with patch("xcyber360.core.cluster.local_client.LocalClient.send_api_request", return_value="Test"):
...
async def test_localclient_send_file(read_config_mock, get_cluster_items_mock):
    ...
    with patch("xcyber360.core.cluster.local_client.LocalClient.start"):
        with patch("xcyber360.core.cluster.local_client.LocalClient.send_api_request", return_value=b"xcyber360/test python"):
            lc = LocalClient()
            assert await lc.send_file(path="xcyber360/test", node_name="python") == b"xcyber360/test python"
