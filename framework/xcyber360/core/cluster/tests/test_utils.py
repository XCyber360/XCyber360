        ...
        with patch('xcyber360.core.common.xcyber360_uid'):
            with patch('xcyber360.core.common.xcyber360_gid'):
                with patch('xcyber360.core.utils.load_xcyber360_xml'):
                    ...
                    from xcyber360.core.cluster import utils
                    from xcyber360.core.exception import (
                        Xcyber360Error,
                        Xcyber360Exception,
                        Xcyber360InternalError,
                        Xcyber360PermissionError,
                        Xcyber360ResourceNotFound,
...
default_cluster_config = {
    ...
    'nodes': ['127.0.0.1'],
    'hidden': 'no',
    'cafile': common.XCYBER360_ETC / 'server.ca',
    'certfile': common.XCYBER360_ETC / 'server.crt',
    'keyfile': common.XCYBER360_ETC / 'server.key',
    'keyfile_password': '',
}
...
def test_read_cluster_config():
    ...
    with patch('xcyber360.core.cluster.utils.get_ossec_conf', side_effect=Xcyber360Error(1001)):
        with pytest.raises(Xcyber360Error, match='.* 3006 .*'):
            utils.read_cluster_config()

    with patch('xcyber360.core.configuration.load_xcyber360_xml', return_value=SystemExit):
        with pytest.raises(SystemExit) as pytest_wrapped_e:
            utils.read_cluster_config(from_import=True)
        assert pytest_wrapped_e.type == SystemExit
        assert pytest_wrapped_e.value.code == 0

    with patch('xcyber360.core.cluster.utils.get_ossec_conf', side_effect=KeyError(1)):
        with pytest.raises(Xcyber360Error, match='.* 3006 .*'):
            utils.read_cluster_config()

    with patch('xcyber360.core.cluster.utils.get_ossec_conf', return_value={'cluster': default_cluster_config}):
        ...
        with pytest.raises(Xcyber360Error, match='.* 3004 .*'):
            utils.read_cluster_config()
...
def test_parse_haproxy_helper_config_ko(config: dict, exception_type: Xcyber360Exception, expected_error_code: str):
...
def test_get_manager_status():
    ...
    with patch('xcyber360.core.cluster.utils.glob', return_value=['ossec-0.pid']):
        with patch('re.match', return_value='None'):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'

        # with patch('xcyber360.core.cluster.utils.join', return_value='failed') as join_mock:
        with patch('xcyber360.core.cluster.utils.os.path.exists', side_effect=exist_mock):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'
...
def test_get_manager_status_ko(mock_stat, exc):
    ...
    with pytest.raises(Xcyber360InternalError, match='.* 1913 .*'):
...
def test_get_cluster_status():
    ...
    Xcyber360InternalError is raised."""
    status = utils.get_cluster_status()
    assert {'running': 'no'} == status

    with patch('xcyber360.core.cluster.utils.get_manager_status', side_effect=Xcyber360InternalError(1913)):
        status = utils.get_cluster_status()
        assert {'running': 'no'} == status
...
def test_manager_restart():
    ...
    with patch('xcyber360.core.cluster.utils.open', side_effect=None):
            ...
            with patch('os.path.exists', return_value=True):
                ...
                with patch('socket.socket.connect', side_effect=None):
                    ...
                    with patch('socket.socket.send', side_effect=None):
                        ...
                        assert Xcyber360Result({'message': 'Restart request sent'}) == status
...
def test_get_cluster_items():
    ...
    with patch('builtins.open', side_effect=FileNotFoundError):
        with pytest.raises(Xcyber360Exception, match='.* 3005 .*'):
    ...
    with patch('builtins.open'):
        with patch('xcyber360.core.cluster.utils.json.loads', return_value=cluster_json):
...
def test_ClusterLogger():
    """Verify that ClusterLogger defines the logger used by xcyber360-clusterd."""
...
@patch('xcyber360.core.cluster.utils.pyDaemonModule.create_pid')
def test_process_spawn_sleep(pyDaemon_create_pid_mock, get_pid_mock):
    ...
    pyDaemon_create_pid_mock.assert_called_once_with(f'xcyber360-server_child_{child}', get_pid_mock.return_value)


@pytest.mark.asyncio
@patch('concurrent.futures.ThreadPoolExecutor')
@patch('xcyber360.core.cluster.dapi.dapi.DistributedAPI')
...
@patch('xcyber360.core.cluster.utils.read_cluster_config')
