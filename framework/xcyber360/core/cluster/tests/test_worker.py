...
import xcyber360.core.exception as exception
...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        with patch('xcyber360.core.utils.load_xcyber360_xml'):
            ...
            from xcyber360.core.cluster import client, worker, common as cluster_common
            from xcyber360.core import common as core_common
...
configuration = {'node_name': 'master', 'nodes': ['master'], 'port': 1111, "name": "xcyber360", "node_type": "master"}
...
async def test_rit_set_up_coro(event_loop):
    ...
    with patch('xcyber360.core.cluster.worker.WorkerHandler.process_files_from_master',
               return_value='') as process_files_mock:
        receive_task = worker.ReceiveIntegrityTask(xcyber360_common=get_worker_handler(event_loop), logger=None)
        receive_task.xcyber360_common = cluster_common.Xcyber360Common()
...
async def test_rit_done_callback(event_loop):
    ...
    with patch('xcyber360.core.cluster.worker.ReceiveIntegrityTask.set_up_coro', side_effect=return_coro):
        receive_task = worker.ReceiveIntegrityTask(xcyber360_common=get_worker_handler(event_loop), logger=None)
        receive_task.xcyber360_common = cluster_common.Xcyber360Common()
        ...
        assert receive_task.xcyber360_common.check_integrity_free is True
...
@patch("xcyber360.core.utils.mkdir_with_mode")
@patch("xcyber360.core.cluster.worker.client.AbstractClient.connection_result")
async def test_worker_handler_connection_result(connection_result_mock, mkdir_with_mode_mock, exists_mock,
    ...
    exists_mock.assert_called_once_with(core_common.XCYBER360_QUEUE / "Testing")
    mkdir_with_mode_mock.assert_called_once_with(core_common.XCYBER360_QUEUE / "Testing")
...
async def test_worker_handler_process_request_ok(logger_mock, event_loop):
    ...
    worker_handler = get_worker_handler(event_loop)
    worker_handler.logger = logging.getLogger("xcyber360")
    ...
    with patch("xcyber360.core.cluster.worker.WorkerHandler.sync_integrity_ok_from_master",
    ...
    with patch("xcyber360.core.cluster.worker.WorkerHandler.setup_receive_files_from_master",
    ...
    with patch("xcyber360.core.cluster.worker.WorkerHandler.end_receiving_integrity",
    ...
    with patch("xcyber360.core.cluster.worker.WorkerHandler.error_receiving_integrity",
    ...
    with patch("xcyber360.core.cluster.worker.WorkerHandler.forward_dapi_response",
    ...
    with patch("xcyber360.core.cluster.common.Handler.send_orders", side_effect=b"ok") as send_orders_mock:
    ...
    with patch("xcyber360.core.cluster.worker.client.AbstractClient.process_request",
...
@patch("xcyber360.core.cluster.worker.client.AbstractClient.connection_lost")
@patch("xcyber360.core.cluster.worker.cluster.clean_up")
...
async def test_worker_handler_process_request_ko(logger_mock, event_loop):
    ...
    class ClientsMock:
        ...
        def send_request(self, command, error_msg):
            raise exception.Xcyber360ClusterError(1001)
...
@patch.object(logging.getLogger("xcyber360.Integrity check"), "info")
@patch("xcyber360.core.cluster.common.Xcyber360Common.setup_receive_file", return_value="OK")
...
@pytest.mark.asyncio
@patch("xcyber360.core.cluster.common.Xcyber360Common.end_receiving_file", return_value=(b"OK", b"OK"))
...
@pytest.mark.asyncio
@patch("xcyber360.core.cluster.common.Xcyber360Common.error_receiving_file", return_value=(b"error", b"error"))
...
@freeze_time('1970-01-01')
@pytest.mark.asyncio
@patch.object(xcyber360.core.cluster.worker.json, "dumps", return_value="")
@patch.object(logging.getLogger("xcyber360.Integrity check"), "info")
@patch.object(logging.getLogger("xcyber360.Integrity check"), "error")
@patch("xcyber360.core.cluster.common.SyncFiles.sync", return_value=True)
@patch("xcyber360.core.cluster.cluster.get_files_status", return_value={})
@patch("xcyber360.core.cluster.worker.client.common.Handler.send_request")
@patch("xcyber360.core.cluster.common.SyncTask.request_permission", return_value=True)
async def test_worker_handler_sync_integrity(request_permission_mock,
    ...
    # Test the try
    with (patch('xcyber360.core.cluster.worker.cluster.run_in_pool', side_effect=cluster_run_in_pool_mock) as
          run_in_pool_mock):
        try:
        ...
        run_in_pool_mock.side_effect = exception.Xcyber360Exception(1001)
        try:
            await asyncio.wait_for(worker_handler.sync_integrity(), 0.2)
        except asyncio.exceptions.TimeoutError:
            pass

        error_mock.assert_called_with(f"Error synchronizing integrity: {exception.Xcyber360Exception(1001)}")
        json_dumps_mock.assert_called_with(exception.Xcyber360Exception(1001), cls=cluster_common.Xcyber360JSONEncoder)
        ...
        json_dumps_mock.assert_called_with(exception.Xcyber360ClusterError(code=1000, extra_message=str(Exception())),
...
@pytest.mark.asyncio
@freeze_time('1970-01-01')
@patch.object(logging.getLogger("xcyber360.Integrity sync"), "debug")
@patch("xcyber360.core.cluster.common.SyncFiles.sync", return_value=True)
@patch('xcyber360.core.cluster.worker.perf_counter', return_value=0)
@patch("xcyber360.core.cluster.cluster.merge_info", return_value=("n_files", "merged_file"))
async def test_worker_handler_sync_extra_valid(merge_info_mock, perf_counter_mock, sync_mock, logger_debug_mock,
    ...
    worker_handler = get_worker_handler(event_loop)
    worker_handler.server = ManagerMock()
    with patch.object(logging.getLogger("xcyber360.Integrity sync"), "info") as logger_info_mock:
    ...
    with patch("xcyber360.core.cluster.worker.WorkerHandler.send_request") as send_request_mock:
        with patch.object(logging.getLogger("xcyber360.Integrity sync"), "error") as logger_error_mock:
            merge_info_mock.side_effect = exception.Xcyber360Exception(1001)
            cls = cluster_common.Xcyber360JSONEncoder
            await worker_handler.sync_extra_valid(extra_valid)
            ...
            logger_error_mock.assert_called_once_with(
                f"Error synchronizing extra valid files: {exception.Xcyber360Exception(1001)}")
            send_request_mock.assert_called_once_with(command=b'syn_i_w_m_r',
                                                      data=b'None ' + json.dumps(exception.Xcyber360Exception(1001),
                                                                                 cls=cls).encode())
...
@patch("xcyber360.core.cluster.cluster.decompress_files")
@patch.object(logging.getLogger("xcyber360.Integrity sync"), "info")
@patch.object(logging.getLogger("xcyber360.Integrity sync"), "debug")
@patch("xcyber360.core.cluster.worker.client.common.Handler.send_request")
@patch("xcyber360.core.cluster.worker.WorkerHandler.update_master_files_in_worker")
async def test_worker_handler_process_files_from_master_ok(update_files_mock, send_request_mock, logger_debug_mock,
    ...
    with patch('xcyber360.core.cluster.cluster.run_in_pool', side_effect=cluster_run_in_pool_mock) as run_in_pool_mock:
        ...
        # Test first except
        worker_handler.sync_tasks["task_id"] = TaskMock()
        decompress_files_mock.side_effect = exception.Xcyber360Exception(1001)
        event = asyncio.Event()
        await asyncio.gather(worker_handler.process_files_from_master(name="task_id", file_received=event),
                             unlock_event(event))
        ...
        json_dumps_mock.assert_called_once_with(exception.Xcyber360Exception(1001), cls=cluster_common.Xcyber360JSONEncoder)
        ...
        logger_debug_mock.assert_not_called()
        logger_info_mock.assert_called_once_with("Starting.")
        json_dumps_mock.assert_called_once_with(exception.Xcyber360ClusterError(code=1000, extra_message=str(Exception())),
                                                cls=cluster_common.Xcyber360JSONEncoder)
        rmtree_mock.assert_not_called()
...
@patch("builtins.open")
@patch("os.path.exists", return_value=False)
@patch("xcyber360.core.cluster.worker.safe_move")
@patch("xcyber360.core.cluster.worker.utils.mkdir_with_mode")
...
async def test_worker_handler_update_master_files_in_worker_ok(xcyber360_gid_mock, xcyber360_uid_mock, path_join_mock,
    ...
    all_mocks = [xcyber360_gid_mock, xcyber360_uid_mock, path_join_mock, mkdir_with_mode_mock, safe_move_mock, open_mock,
                 path_exists_mock]
    ...
    with patch("xcyber360.core.cluster.cluster.unmerge_info", return_value=[("name", "content", "_")]):
        with patch("os.remove") as os_remove_mock:
            ...
            path_join_mock.assert_has_calls([call(core_common.XCYBER360_PATH, 'filename1'),
                                             call(core_common.XCYBER360_PATH, 'name'),
                                             call(core_common.XCYBER360_PATH, 'filename1'),
                                             call('/zip/path', 'filename1'),
                                             call(core_common.XCYBER360_PATH, 'filename3')])
            xcyber360_uid_mock.assert_called_with()
            xcyber360_gid_mock.assert_called_with()
    ...
    path_join_mock.assert_has_calls([call(core_common.XCYBER360_PATH, "filename1"),
                                     call(core_common.XCYBER360_PATH, "filename2"),
                                     call(core_common.XCYBER360_PATH, "filename3")])
    xcyber360_uid_mock.assert_not_called()
    xcyber360_gid_mock.assert_not_called()
    ...
    with patch("os.listdir", return_value="dir_files") as listdir_mock:
        with patch("shutil.rmtree") as rmtree_mock:
            ...
            path_join_mock.assert_has_calls([call(core_common.XCYBER360_PATH, "filename3"),
                                             call(core_common.XCYBER360_PATH, "")])
            xcyber360_uid_mock.assert_not_called()
            xcyber360_gid_mock.assert_not_called()
            mkdir_with_mode_mock.assert_not_called()
            safe_move_mock.assert_not_called()
...
@pytest.mark.asyncio
@patch("xcyber360.core.cluster.worker.metadata.__version__", "1.0.0")
@patch("xcyber360.core.cluster.worker.dapi.APIRequestQueue", return_value="APIRequestQueue object")
...
@pytest.mark.asyncio
@patch("xcyber360.core.cluster.client.AbstractClientManager.add_tasks", return_value=["task"])
@patch("xcyber360.core.cluster.worker.dapi.APIRequestQueue", return_value="APIRequestQueue object")
