...
from xcyber360.core.utils import blake2b, get_date_from_timestamp, get_utc_now, mkdir_with_mode, to_relative_path
...
def validate_file_path(config: dict, key: str):
    """Validate a file path is within XCYBER360_ETC and that the file exists.
    ...
    if not filepath.is_relative_to(common.XCYBER360_ETC):
        raise Xcyber360Error(3004, f'The {key} path ({filepath}) is not inside {common.XCYBER360_ETC}.')

    if '..' in str(filepath):
        raise Xcyber360Error(3004, f'The {key} path ({filepath}) contains "..".')

    if not filepath.exists():
        raise Xcyber360Error(3004, f'The {key} "{filepath}" does not exist.')
...
def check_cluster_config(config):
    ...
    if config['node_type'] != 'master' and config['node_type'] != 'worker':
        raise Xcyber360Error(3004, f'Invalid node type {config["node_type"]}. Correct values are master and worker')

    if not isinstance(config['port'], int):
        raise Xcyber360Error(3004, "Port has to be an integer.")

    if not MIN_PORT < config['port'] < MAX_PORT:
        raise Xcyber360Error(3004, f"Port must be higher than {MIN_PORT} and lower than {MAX_PORT}.")

    cert_keys = ['cafile', 'certfile', 'keyfile']
    if len(cert_keys) > len(set(config[key] for key in cert_keys)):
        raise Xcyber360Error(3004, 'Paths to certificates and keys must be different.')
...
def walk_dir(dirname, recursive, files, excluded_files, excluded_extensions, get_cluster_item_key, previous_status=None,
    ...
    full_dirname = path.join(common.XCYBER360_ETC, dirname)
    ...
    try:
                ...
                for file_ in files_:
                    ...
                    try:
                        ...
                        if files == ['all'] or file_ in files:
                            relative_file_path = path.join(path.relpath(root_, common.XCYBER360_ETC), file_)
        ...
        raise Xcyber360InternalError(3015, e)
...
def compress_files(name, list_path, cluster_control_json=None, max_zip_size=None):
    ...
    zip_file_path = path.join(common.XCYBER360_QUEUE, name, f'{name}-{get_utc_now().timestamp()}-{uuid4().hex}.zip')
    ...
    with open(zip_file_path, 'ab') as wf:
        for file in list_path:
            ...
            try:
                with open(path.join(common.XCYBER360_ETC, file), 'rb') as rf:
                    new_file = rf.read()
                    if len(new_file) > max_zip_size:
                        result_logs['warning'][file].append(f'File too large to be synced: '
                                                            f'{path.join(common.XCYBER360_ETC, file)}')
                        update_cluster_control(file, cluster_control_json)
                        continue
                ...
                raise Xcyber360Error(3001, str(e))
            except Exception as e:
                result_logs['debug'][file].append("Exception raised: " + str(Xcyber360Exception(3001, str(e))))
...
def clean_up(node_name=""):
    ...
    try:
        rm_path = path.join(common.XCYBER360_QUEUE, node_name)
...
def merge_info(merge_type, node_name, files=None, file_type=""):
    ...
    ----------
    merge_type : str
        Directory inside {xcyber360_path}/PATH where the files to merge can be found.
    node_name : str
    ...
    """
    merge_path = path.join(common.XCYBER360_QUEUE, merge_type)
    ...
    with open(path.join(common.XCYBER360_RUN, output_file), 'wb') as o_f:
...
def unmerge_info(merge_type, path_file, filename):
    ...
    ----------
    merge_type : str
        Name of the destination directory inside queue. I.e: {xcyber360_path}/PATH/{merge_type}/<unmerge_files>.
    path_file : str
