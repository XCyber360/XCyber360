...
from xcyber360.core.cluster import client, cluster, server
from xcyber360.core.cluster import common as c_common
from xcyber360.core.cluster.dapi import dapi
from xcyber360.core.cluster.utils import context_tag
...
class LocalServer(server.AbstractServer):
    ...
    async def start(self):
        ...
        socket_path = common.XCYBER360_SOCKET / common.LOCAL_SERVER_SOCKET
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            ...
            if node_name in self.server.node.clients:
                ...
                raise Xcyber360ClusterError(3022)
        else:
            return super().process_request(command, data)
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            if self.server.node.client is None:
                raise Xcyber360ClusterError(3023)
            asyncio.create_task(self.log_exceptions(
