...
from xcyber360.core import cluster as metadata, common, exception, utils
from xcyber360.core.agent import Agent
from xcyber360.core.cluster import server, cluster, common as c_common
from xcyber360.core.cluster.dapi import dapi
from xcyber360.core.cluster.utils import context_tag, log_subprocess_execution
...
class ReceiveIntegrityTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.xcyber360_common.integrity_check
    ...
    def done_callback(self, future=None):
        ...
        # if not self.xcyber360_common.extra_valid_requested:
        #     self.xcyber360_common.sync_integrity_free[0] = True
...
class ReceiveExtraValidTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.xcyber360_common.sync_extra_valid
    ...
    def done_callback(self, future=None):
        ...
        """
        super().done_callback(future)
        self.xcyber360_common.extra_valid_requested = False
        self.xcyber360_common.sync_integrity_free[0] = True


class MasterHandler(server.AbstractServerHandler, c_common.Xcyber360Common):
    """
    ...
    async def execute(self, command: bytes, data: bytes, wait_for_complete: bool = False) -> Dict:
        ...
        if command == b'dapi_fwd':
            ...
            if client in self.server.clients:
                ...
                raise exception.Xcyber360ClusterError(3022, extra_message=client)
        ...
        if command == b'dapi' or command == b'dapi_fwd':
            try:
                ...
                raise exception.Xcyber360ClusterError(3021)
        # Otherwise, immediately return the result obtained before.
        else:
            status, request_result = result
            if status != b'ok':
                raise exception.Xcyber360ClusterError(3022, extra_message=request_result.decode())
    ...
    def hello(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : bytes
            Node name, node type and xcyber360 version all separated by spaces.
    ...
    def get_manager(self) -> server.AbstractServer:
        """Get the Master object that created this MasterHandler. Used in the class Xcyber360Common.
    ...
    def process_dapi_res(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if req_id in self.server.pending_api_requests:
            ...
            raise exception.Xcyber360ClusterError(3032, extra_message=req_id)
    ...
    async def sync_worker_files(self, task_id: str, received_file: asyncio.Event, logger):
        ...
        try:
            ...
            raise exception.Xcyber360ClusterError(3038, extra_message=str(e))
        finally:
            shutil.rmtree(decompressed_files_path)
        ...
        try:
            with utils.Timeout(timeout):
                for file_path, data in files_metadata.items():
                    full_path = common.XCYBER360_RUN / file_path
                    ...
                    if data['merged']:
                        for unmerged_file_path, file_data, file_time in cluster.unmerge_info(
                            ...
                            try:
                                # Destination path.
                                full_unmerged_name = common.XCYBER360_QUEUE / unmerged_file_path
                                # Path where to create the file before moving it to the destination path.
                                tmp_unmerged_path = common.XCYBER360_QUEUE / worker_name / os.path.basename(
                                    unmerged_file_path
                                )
                                ...
                                utils.safe_move(tmp_unmerged_path, full_unmerged_name,
                                                ownership=(common.xcyber360_uid(), common.xcyber360_gid()),
                    ...
                    # If the file is not 'merged' type, move it directly to the destination path.
                        ...
                        try:
                            ...
                            utils.safe_move(zip_path, full_path, ownership=(common.xcyber360_uid(), common.xcyber360_gid()),
                                            permissions=file_config.permissions)
                        except TimeoutError as e:
                            raise e
                        except Exception as e:
