...
from xcyber360.core import common, utils
from xcyber360.core import xcyber360_socket
...
def totals_(date: datetime.datetime = utils.get_utc_now()) -> list:
    ...
    try:
        ...
        raise Xcyber360Error(1308, extra_message=stat_filename)
        ...
        if len(data) == 4:
            ...
            if len(data) != 5:
                if len(data) in (0, 1):
                    ...
                    raise Xcyber360InternalError(1309)
...
async def get_daemons_stats_socket(socket: str,
    ...
    """Send message to Xcyber360 socket to get statistical information.
    ...
    """
    # Create message
    full_message = xcyber360_socket.create_xcyber360_socket_message(
    ...
    # Connect to socket
    try:
        s = xcyber360_socket.Xcyber360AsyncSocketJSON()
        await s.connect(socket)
    except Exception as exc:
        raise Xcyber360InternalError(1121, extra_message=socket) from exc
...
def get_daemons_stats_(filename: str) -> list:
    ...
    try:
        ...
        try:
            ...
            raise Xcyber360InternalError(1104, extra_message=str(e))
    except IOError:
        raise Xcyber360Error(1308, extra_message=filename)
...
def send_command_to_socket(dest_socket: str, command: str) -> dict:
    ...
    except Xcyber360Exception:
        # Error connecting to socket
        raise Xcyber360InternalError(1121)
...
def get_daemons_stats_from_socket(agent_id: str, daemon: str) -> dict:
    ...
    if socket_response.get('error', 0) != 0:
        ...
        raise Xcyber360Error(1117, extra_message=rec_msg)
