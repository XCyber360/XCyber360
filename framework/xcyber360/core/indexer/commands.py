...
from xcyber360.core.indexer.models.commands import Action, Command, Result, Source, Status, Target, TargetType, \
...
class CommandsManager(BaseIndex):
    ...
    async def create(self, command: Command) -> CreateCommandResponse:
        ...
        try:
            ...
            raise Xcyber360Error(1761, extra_message=str(e))
    ...
    async def update(self, items: List[Union[Command, Result]]) -> None:
        ...
        Raises
        ------
        Xcyber360ResourceNotFound(2202)
        ...
        for item in response[IndexerKey.ITEMS]:
            if item[IndexerKey.UPDATE][STATUS_KEY] == 404:
                raise Xcyber360ResourceNotFound(2202)
...
def create_restart_command(agent_id: str) -> Command:
    ...
    # To be defined in https://github.com/xcyber360/xcyber360-agent/issues/54.
    return Command(
