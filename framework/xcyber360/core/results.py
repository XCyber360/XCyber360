...
import xcyber360.core.exception as wexception
from xcyber360.core import utils
from xcyber360.core.common import DATABASE_LIMIT

current_module = sys.modules[__name__]


class AbstractXcyber360Result(collections.abc.MutableMapping):
    """
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        Raises
        ------
        wexception.Xcyber360InternalError(1000)
            If dct is of a wrong type.
        """
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractXcyber360Result):
            self.dikt = dct.dikt
        else:
            raise wexception.Xcyber360InternalError(1000, extra_message=f"dct param must be a dict or "
                                                                    f"an AbstractXcyber360Result subclass, "
                                                                    f"not a {type(dct)}")
    ...
    def __or__(self, other):
        """ | operator used to merge two AbstractXcyber360Result objects. When merged with a Xcyber360Exception, the result is
        always a Xcyber360Exception

        Parameters
        ----------
        other : AbstractXcyber360Result or wexception.Xcyber360Exception

        Raises
        ------
        Xcyber360InternalError(1000)
            Xcyber360 internal error when two object could not be merged.

        Returns
        -------
        AbstractXcyber360Result or wexception.Xcyber360Exception
            Resultant object.
        """
        if isinstance(other, wexception.Xcyber360Exception):
            return other
        elif not isinstance(other, (dict, AbstractXcyber360Result)):
            raise wexception.Xcyber360InternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractXcyber360Result subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in Xcyber360Result subclasses.

        Parameters
        ----------
        limit : int
            Default: the value specified in xcyber360.core.common.DATABASE_LIMIT
        offset : int
            Default: 0.

        Returns
        -------
        AbstractXcyber360Result
            Filtered AbstractXcyber360Result
        """
        result = deepcopy(self)
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in Xcyber360Result subclasses.
        ...
        -------
        AbstractXcyber360Result
            Sorted AbstractXcyber360Result
        """
...
class Xcyber360Result(AbstractXcyber360Result):
    """
...
class AffectedItemsXcyber360Result(AbstractXcyber360Result):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.Xcyber360Exception = None):
        ...
        error : wexception.Xcyber360Exception
    ...
    def add_failed_items_from(self, other):
        ...
        Parameters
        ----------
        other : AffectedItemsXcyber360Result
        ...
        wexception.Xcyber360InternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsXcyber360Result):
            raise wexception.Xcyber360InternalError(1000,
    ...
    def __or__(self, other):
        """Merge a AffectedItemsXcyber360Result with self.

        Parameters
        ----------
        other : AffectedItemsXcyber360Result
            AffectedItemsXcyber360Result object to merge.
        ...
        if isinstance(other, wexception.Xcyber360Error):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.Xcyber360Exception):
            return result
        elif not isinstance(other, AffectedItemsXcyber360Result):
            raise wexception.Xcyber360InternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsXcyber360Result as a dict.
    ...
    def decode_json(cls, obj: dict):
        ...
        -------
        AbstractXcyber360Result
            Instance of AbstractXcyber360Result.
        """
        result = cls()
    ...
    def render(self) -> dict:
        """Render AffectedItemsXcyber360Result object.

        Returns
        -------
        dict
            Return AffectedItemsXcyber360Result as a dictionary with fields data, message and error.
        """
