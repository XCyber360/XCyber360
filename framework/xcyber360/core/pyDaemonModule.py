...
def create_pid(name: str, pid: int):
    ...
    Raises
    ------
    Xcyber360InternalError(3002)
        Error creating pidfile.
    """
    filename = common.XCYBER360_RUN / f'{name}-{pid}.pid'
    ...
    with open(filename, 'a') as fp:
        try:
            ...
            raise Xcyber360InternalError(3002, str(e))
...
def delete_child_pids(name: str, ppid: int, logger: logging.Logger):
    ...
    logger : logging.Logger
        Logger object.
    """
    filenames = [i for i in common.XCYBER360_RUN.glob(f'{name}*.pid')]
...
def get_xcyber360_server_pid(server_daemon_name: str, pids_path: str = common.XCYBER360_RUN) -> int:
    """Get the PID of the running xcyber360 server process contained in the given path.
    ...
    pids_path : str, optional
        Path to search the PID, by default common.XCYBER360_RUN

    Returns
    -------
    int
        The PID of the xcyber360 server.
...
def get_running_processes(pids_path: str = common.XCYBER360_RUN) -> list:
    """Get the running processes based on the PID files contained in the given path.

    Parameters
    ----------
    pids_path : str, optional
        Path to search for the PIDs, by default `common.XCYBER360_RUN`.
    ...
    """
    pids_path = Path(pids_path)
    running_processes = [i.stem for i in pids_path.glob('xcyber360-*-*') if '_' not in i.stem]
