...
from xcyber360.core.common import MAX_SOCKET_BUFFER_SIZE
from xcyber360.core.wdb import AsyncXcyber360DBConnection, Xcyber360DBConnection
...
def test_async_init():
    """Verify that AsyncXcyber360DBConnection attributes are correct."""
    async_wdb = AsyncXcyber360DBConnection('test')
...
async def test_async_open_connection(open_unix_connection_mock):
    ...
    async_wdb = AsyncXcyber360DBConnection(loop='test_loop')
...
def test_async_close():
    ...
    async_wdb = AsyncXcyber360DBConnection()
...
async def test_async_send_ko():
    ...
    with pytest.raises(exception.Xcyber360InternalError, match=".* 2005 .*"):
    ...
    with pytest.raises(exception.Xcyber360InternalError, match=r'\b2010\b'):
        await async_wdb._send('test')

    # Xcyber360-db error response.
    encoded_response = 'err Error message'.encode(encoding='utf-8')
    async_wdb._reader.readexactly.side_effect = [struct.pack('<I', len(encoded_response)), encoded_response]
    with pytest.raises(exception.Xcyber360Error, match=r'\b2003\b'):
        await async_wdb._send('test')


@pytest.mark.asyncio
async def test_run_wdb_command():
    """Test `Xcyber360DBConnection.run_wdb_command` method."""
    send_result = ('status', '["data"]')
    command = "any wdb command"

    wdb_con = AsyncXcyber360DBConnection()
    with patch('xcyber360.core.wdb.AsyncXcyber360DBConnection._send', return_value=send_result) as wdb_send_mock:
...
async def test_run_wdb_command_ko(wdb_response):
    """Test `Xcyber360DBConnection.run_wdb_command` method expected exceptions."""
    with patch('xcyber360.core.wdb.AsyncXcyber360DBConnection._send', return_value=wdb_response):
        wdb_con = AsyncXcyber360DBConnection()
        with pytest.raises(exception.Xcyber360InternalError, match=".* 2007 .*") as expected_exc:
...
def test_failed_connection():
    ...
    with patch('xcyber360.core.common.WDB_SOCKET', '/this/path/doesnt/exist'):
        with pytest.raises(exception.Xcyber360Exception, match=".* 2005 .*"):
            Xcyber360DBConnection()
...
def test_wrong_character_encodings_wdb(send_mock, connect_mock):
    """
    Tests receiving a text with a bad character encoding from xcyber360 db
    """
...
def test_failed_send_private(send_mock, connect_mock):
    ...
    with patch('socket.socket.recv', side_effect=recv_mock):
        mywdb = Xcyber360DBConnection()
        with pytest.raises(exception.Xcyber360Exception, match=".* 2003 .*"):
            mywdb._send('test_msg')

    with patch('socket.socket.recv', return_value=b'a' * (MAX_SOCKET_BUFFER_SIZE + 1)):
        mywdb = Xcyber360DBConnection()
        with pytest.raises(exception.Xcyber360Exception, match=".* 2009 .*"):
            mywdb._send('test_msg')
...
def test_query_input_validation_private(send_mock, connect_mock, error_query):
    mywdb = Xcyber360DBConnection()
    with pytest.raises(exception.Xcyber360Exception, match=".* 2004 .*"):
...
@patch("socket.socket.connect")
@patch("socket.socket.send")
@patch("xcyber360.core.wdb.Xcyber360DBConnection._send")
def test_execute(send_mock, socket_send_mock, connect_mock):
    ...
    with patch("xcyber360.core.wdb.Xcyber360DBConnection._send", new=send_mock):
...
def test_execute_pagination(socket_send_mock, connect_mock):
    ...
    # Test pagination
    with patch("xcyber360.core.wdb.Xcyber360DBConnection._send",
               side_effect=[[{'total': 5}], exception.Xcyber360InternalError(2009), ['ok', '{"total": 5}'],
                            ['ok', '{"total": 5}']]):
        mywdb.execute("agent 000 sql select test from test offset 1 limit 500")

    # Test pagination error
    with patch("xcyber360.core.wdb.Xcyber360DBConnection._send",
               side_effect=[[{'total': 5}], exception.Xcyber360InternalError(2009)]):
        with pytest.raises(exception.Xcyber360InternalError, match=".* 2009 .*"):
            mywdb.execute("agent 000 sql select test from test offset 1 limit 1")
...
def test_failed_execute(send_mock, connect_mock, error_query, error_type, expected_exception, delete, update):
    ...
    if not error_type:
        ...
        with patch("xcyber360.core.wdb.Xcyber360DBConnection._send", return_value=[{'total': 5}]):
            with patch("xcyber360.core.wdb.min", side_effect=error_type):
                with pytest.raises(exception.Xcyber360Exception, match=f'.* {expected_exception} .*'):
                    mywdb.execute(error_query, delete=delete, update=update)
...
def test_Xcyber360DBConnection_loads(string):
    """Test that the `loads` method from the class `Xcyber360DBConnection` cleans empty objects from the result."""
    result = Xcyber360DBConnection.loads(string)
    assert len(result) == 1
