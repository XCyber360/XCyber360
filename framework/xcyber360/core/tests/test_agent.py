...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        from xcyber360.core.agent import *
        from xcyber360.core.exception import Xcyber360Exception, Xcyber360IndexerError
        from api.util import remove_nones_to_dict
        from xcyber360.core.common import reset_context_cache
        from xcyber360.core.indexer.agent import Agent as IndexerAgent
        from xcyber360.core.utils import get_group_file_path
...
def get_Xcyber360DBQuery_params(wdb_class):
    """Get default parameters for the specified Xcyber360DBQuery class.

    Parameters
    ----------
    wdb_class : str
        Suffix of the Xcyber360DBQuery class. Example: 'Agents' to get default parameters from `Xcyber360DBQueryAgents` class.
    ...
    """
    with patch('xcyber360.core.agent.Xcyber360DBQuery.__init__') as wdbquery_mock:
        getattr(sys.modules[__name__], f'Xcyber360DBQuery{wdb_class}')()
        return wdbquery_mock.call_args.kwargs


# list with Xcyber360 packages availables with their hash
...
@patch("xcyber360.core.agent.Xcyber360DBBackend")
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Xcyber360DBQueryAgents__init__(socket_mock, send_mock, backend_mock, value):
    """Tests if method __init__ of Xcyber360DBQueryAgents works properly.
    ...
    if value:
        Xcyber360DBQueryAgents()
        backend_mock.assert_called_once()
    else:
        with pytest.raises(Xcyber360Exception, match=".* 2005 .*"):
            Xcyber360DBQueryAgents()


@patch('socket.socket.connect')
def test_Xcyber360DBQueryAgents_filter_date(mock_socket_conn):
    """Tests _filter_date of Xcyber360DBQueryAgents returns expected query"""
    query_agent = Xcyber360DBQueryAgents()
...
@patch('socket.socket.connect')
def test_Xcyber360DBQueryAgents_sort_query(mock_socket_conn, field, expected_query):
    """Tests _sort_query of Xcyber360DBQueryAgents returns expected result
    ...
    """
    query_agent = Xcyber360DBQueryAgents(sort={'order': 'asc'})
...
@patch('socket.socket.connect')
def test_Xcyber360DBQueryAgents_add_search_to_query(mock_socket_conn):
    """Tests _add_search_to_query of Xcyber360DBQueryAgents returns expected query"""
    query_agent = Xcyber360DBQueryAgents(search={'value': 'test', 'negation': True})
...
@patch('socket.socket.connect')
def test_Xcyber360DBQueryAgents_format_data_into_dictionary(mock_socket_conn, data):
    """Tests _format_data_into_dictionary of Xcyber360DBQueryAgents returns expected data"""
    query_agent = Xcyber360DBQueryAgents(offset=0, limit=1, sort=None,
...
@patch('socket.socket.connect')
def test_Xcyber360DBQueryAgents_parse_legacy_filters(mock_socket_conn):
    """Tests _parse_legacy_filters of Xcyber360DBQueryAgents returns expected query"""
    query_agent = Xcyber360DBQueryAgents(filters={'older_than': 'test'})
...
@patch('socket.socket.connect')
def test_Xcyber360DBQueryAgents_process_filter(mock_socket_conn, field_name, field_filter, q_filter):
    """Tests _process_filter of Xcyber360DBQueryAgents returns expected query
...
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Xcyber360DBQueryGroup__init__(socket_mock, send_mock, backend_mock, value):
    """Test if method __init__ of Xcyber360DBQueryGroup works properly.
...
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Xcyber360DBQueryGroup_filters(socket_mock, send_mock, filters):
    """Test if parameter filters of Xcyber360DBQueryGroup works properly.
        ...
        """
    query_group = Xcyber360DBQueryGroup(filters=filters)
...
@patch('socket.socket.connect')
def test_Xcyber360DBQueryGroupByAgents__init__(mock_socket_conn):
    """Tests if method __init__ of Xcyber360DBQueryGroupByAgents works properly."""
    query_group = Xcyber360DBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
...
@patch('socket.socket.connect')
def test_Xcyber360DBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    """Tests if method _format_data_into_dictionary of Xcyber360DBQueryGroupByAgents works properly."""
    query_group = Xcyber360DBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
                                            search={'value': 'test', 'negation': True},
                                            select={'os.name'}, query=None, count=5, get_data=None)

    query_group.filter_fields = {'fields': set(query_group.filter_fields)}
    query_group._data = [{'count': 1, 'name': 'xcyber360-master'},
                         {'count': 1, 'name': 'xcyber360-agent1'}]
...
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Xcyber360DBQueryGroupByAgents(mock_socket_conn, send_mock, filter_fields, expected_response):
    """Tests if Xcyber360DBQueryGroupByAgents works properly."""
    query_group = Xcyber360DBQueryGroupByAgents(filter_fields=filter_fields, offset=0, limit=None, sort=None,
...
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Xcyber360DBQueryGroup__add_sort_to_query(mock_socket_conn, send_mock):
    """Tests if _add_sort_to_query method of Xcyber360DBQueryGroup works properly"""
    query_group = Xcyber360DBQueryGroup()
...
@patch('xcyber360.core.agent.Agent._add')
...
def test_agent_load_info_from_db_ko(socket_mock, send_mock):
    ...
    with pytest.raises(Xcyber360ResourceNotFound, match='.* 1701 .*'):
...
@patch('xcyber360.core.agent.Xcyber360Queue.send_msg_to_agent')
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect(socket_mock, send_mock, mock_send_msg):
    ...
    agent.reconnect(Xcyber360Queue(common.AR_SOCKET))

    # Assert send_msg method is called with correct params
    mock_send_msg.assert_called_with(Xcyber360Queue.HC_FORCE_RECONNECT, agent_id)


@patch('xcyber360.core.agent.Xcyber360Queue')
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_ko(socket_mock, send_mock, mock_queue):
    ...
    with pytest.raises(Xcyber360Error, match='.* 1707 .*'):
        agent = Agent('003')
        agent.reconnect(mock_queue)


@patch('xcyber360.core.agent.Agent._remove_authd', return_value='Agent was successfully deleted')
def test_agent_remove(mock_remove_authd):
    """Tests if method remove() works as expected."""

    with patch('xcyber360.core.agent.get_manager_status', return_value={'xcyber360-authd': 'running'}):
        agent = Agent('001')
        result = agent.remove()
...
def test_agent_remove_ko(mock_remove_authd):
    ...
    with pytest.raises(Xcyber360Error, match='.* 1726 .*'):
        agent = Agent('001')
        agent.remove()


@patch('xcyber360.core.agent.Xcyber360SocketJSON')
def test_agent_remove_authd(mock_xcyber360_socket):
    """Tests if method remove_authd() works as expected"""
    agent = Agent('001')
    agent._remove_authd(purge=True)
    mock_xcyber360_socket.return_value.send.assert_called_once_with(
        {"function": "remove", "arguments": {"id": str(1).zfill(3), "purge": True}})
    mock_xcyber360_socket.return_value.receive.assert_called_once()
    mock_xcyber360_socket.return_value.close.assert_called_once()
...
@patch('xcyber360.core.agent.Agent._add_authd')
...
@patch('xcyber360.core.agent.get_manager_status', return_value={'xcyber360-authd': 'stopped'})
def test_agent_add_ko(mock_maganer_status):
    """Test if _add() method raises expected exception."""
    agent = Agent('001')

    with pytest.raises(Xcyber360Error, match='.* 1706 .*'):
...
@patch('xcyber360.core.agent.Xcyber360SocketJSON')
def test_agent_add_authd(mock_xcyber360_socket, name, ip, id, key, force):
    ...
    mock_xcyber360_socket.return_value.send.assert_called_once_with(socket_msg)


@pytest.mark.parametrize("mocked_exception, expected_exception", [
    (None, None),
    (Xcyber360Error(9008, cmd_error=True), ".* 1705 .*"),
    (Xcyber360Error(9007, cmd_error=True), ".* 1706 .*"),
    (Xcyber360Error(9012, cmd_error=True), ".* 1708 .*"),
    (Xcyber360Error(9000, cmd_error=True), ".* None")
])
@patch('xcyber360.core.agent.Xcyber360SocketJSON')
def test_agent_add_authd_ko(mock_xcyber360_socket, mocked_exception, expected_exception):
    """Tests if method _add_authd() raises expected exception"""
    agent = Agent('001')

    if not mocked_exception:
        with pytest.raises(Xcyber360Error, match=".* 1709 .*"):
            agent._add_authd('test_add', '192.168.0.1', '2', 'adsiojew')
    else:
        mock_xcyber360_socket.return_value.receive.side_effect = mocked_exception
        with pytest.raises(Xcyber360Error, match=expected_exception):
...
@pytest.mark.asyncio
@patch('xcyber360.core.agent.remove')
@patch('xcyber360.core.agent.path.exists', return_value=True)
@patch('xcyber360.core.common.XCYBER360_SHARED', new=os.path.join(test_data_path, 'etc', 'shared'))
@patch('xcyber360.core.indexer.Indexer._get_opensearch_client')
@patch('xcyber360.core.indexer.Indexer.connect')
@patch('xcyber360.core.indexer.Indexer.close')
@patch('xcyber360.core.indexer.agent.AgentsIndex.delete_group')
...
def test_agent_group_exists_ko():
    ...
    with pytest.raises(Xcyber360Error, match='.* 1722 .*'):
...
@patch('xcyber360.core.indexer.Indexer.connect')
@patch('xcyber360.core.indexer.Indexer.close')
@patch('xcyber360.core.indexer.agent.AgentsIndex.remove_agents_from_group')
@patch('xcyber360.core.indexer.agent.AgentsIndex.add_agents_to_group')
async def test_agent_set_agent_group_relationship(add_agents_to_group_mock, remove_agents_from_group_mock,
    ...
    with patch('xcyber360.core.indexer.agent.AgentsIndex.get', return_value=IndexerAgent(groups=','.join(groups))):
...
@pytest.mark.asyncio
@patch('xcyber360.core.indexer.Indexer._get_opensearch_client', side_effect=Xcyber360IndexerError(2200))
async def test_agent_set_agent_group_relationship_ko(get_client_mock):
    ...
    with pytest.raises(Xcyber360InternalError, match='.* 2200 .*'):
...
@patch('xcyber360.core.agent.Agent.set_agent_group_relationship')
@patch('xcyber360.core.agent.Agent.group_exists', return_value=True)
@patch('xcyber360.core.indexer.Indexer._get_opensearch_client')
@patch('xcyber360.core.indexer.Indexer.connect')
@patch('xcyber360.core.indexer.Indexer.close')
@patch('xcyber360.core.indexer.agent.AgentsIndex.get')
async def test_agent_unset_single_group_agent(get_agent_mock, close_mock, connect_mock, get_os_client_mock,
    ...
    """
    with patch('xcyber360.core.agent.Agent.get_agent_groups', return_value=previous_groups):
...
async def test_agent_unset_single_group_agent_ko(create_indexer_mock):
    ...
    with patch('xcyber360.core.agent.Agent.group_exists', return_value=False):
    ...
    with patch('xcyber360.core.agent.Agent.get_agent_groups', return_value=['new_group', 'new_group2']):
        # Group_id is not within group_list
        with pytest.raises(Xcyber360Error, match='.* 1734 .*'):
            await Agent.unset_single_group_agent('002', 'test_group', force=True)

    # Group ID is 'default' and it is the last only one remaining
    with patch('xcyber360.core.agent.Agent.get_agent_groups', return_value=['default']):
        # Agent file does not exists
        with pytest.raises(Xcyber360Error, match='.* 1745 .*'):
...
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config(socket_mock, send_mock, mock_xcyber360_socket):
    """Test getconfig method returns expected message."""
    agent = Agent('001')
    mock_xcyber360_socket.return_value.receive.return_value = b'ok {"test": "conf"}'
    result = agent.get_config('com', 'active-response', 'Xcyber360 v4.0.0')
...
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config_ko(socket_mock, send_mock, mock_xcyber360_socket):
    """Test getconfig method raises expected exceptions."""
    # Invalid component
    agent = Agent('003')
    with pytest.raises(Xcyber360Error, match=".* 1101 .*"):
        agent.get_config('invalid_component', 'active-response', 'Xcyber360 v4.0.0')

    # Component or config is none
    agent = Agent('003')
    with pytest.raises(Xcyber360Error, match=".* 1307 .*"):
        agent.get_config('com', None, 'Xcyber360 v4.0.0')
        agent.get_config(None, 'active-response', 'Xcyber360 v4.0.0')

    # Agent Xcyber360 version is lower than ACTIVE_CONFIG_VERSION
    agent = Agent('002')
    with pytest.raises(Xcyber360InternalError, match=".* 1735 .*"):
        agent.get_config('com', 'active-response', 'Xcyber360 v3.6.0')


@patch('xcyber360.core.xcyber360_socket.Xcyber360Socket')
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats(socket_mock, send_mock, mock_xcyber360_socket):
    """Test get_stats method returns expected message."""
    agent = Agent('001')
    mock_xcyber360_socket.return_value.receive.return_value = b'{"error":0, "data":{"global":{}, "interval":{}}}'
...
@patch('xcyber360.core.wdb.Xcyber360DBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats_ko(socket_mock, send_mock, mock_xcyber360_socket):
    """Test get_stats method raises expected exception when the agent's version is lower than required."""
    agent = Agent('002')
    with pytest.raises(Xcyber360InternalError, match=r'\b1735\b'):
        agent.get_stats('logcollector')


@pytest.mark.parametrize('agents_list, versions_list', [
    (['001', '002', '003', '004'],
     [{'version': ver} for ver in ['Xcyber360 v4.2.0', 'Xcyber360 v4.0.0', 'Xcyber360 v4.2.1', 'Xcyber360 v3.13.2']])
])
@patch('xcyber360.core.agent.Xcyber360Queue.send_msg_to_agent')
@patch('xcyber360.core.agent.Xcyber360Queue.__init__', return_value=None)
def test_send_restart_command(wq_mock, wq_send_msg, agents_list, versions_list):
    ...
    """
    with patch('xcyber360.core.agent.Agent.get_basic_information', side_effect=versions_list):
        for agent_id, agent_version in zip(agents_list, versions_list):
            wq = Xcyber360Queue(common.AR_SOCKET)
            send_restart_command(agent_id, agent_version['version'], wq)
            expected_msg = Xcyber360Queue.RESTART_AGENTS_JSON if Xcyber360Version(
                agent_version['version']) >= Xcyber360Version(common.AR_LEGACY_VERSION) else Xcyber360Queue.RESTART_AGENTS
...
def test_get_groups():
    ...
    with patch('xcyber360.core.common.XCYBER360_SHARED', new=shared):
        try:
            os.makedirs(shared)
...
@patch('xcyber360.core.agent.core_upgrade_agents')
