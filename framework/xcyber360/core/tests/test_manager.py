...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        with patch('xcyber360.core.utils.load_xcyber360_xml'):
            from xcyber360.core.manager import *
...
@patch('xcyber360.core.cluster.utils.glob')
...
def test_get_ossec_log_fields():
    ...
    assert result[1] == 'xcyber360-rootcheck'
...
def test_get_ossec_logs(log_format):
    ...
    with patch("xcyber360.core.manager.get_xcyber360_active_logging_format", return_value=log_format):
        with pytest.raises(Xcyber360InternalError, match=".*1020.*"):
            get_ossec_logs()

        with patch('xcyber360.core.manager.exists', return_value=True):
...
def test_get_logs_summary(mock_exists, mock_active_logging_format):
    ...
    with patch('xcyber360.core.manager.tail', return_value=logs):
        ...
        assert result['xcyber360-modulesd:database'] == {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0,
                                                     'debug': 2}


@patch('xcyber360.core.manager.exists', return_value=True)
@patch('xcyber360.core.manager.Xcyber360Socket')
def test_validate_ossec_conf(mock_xcyber360socket, mock_exists):
    with patch('socket.socket') as sock:
        # Mock sock response
        json_response = json.dumps({'error': 0, 'message': ""}).encode()
        mock_xcyber360socket.return_value.receive.return_value = json_response
        result = validate_ossec_conf()

        assert result == {'status': 'OK'}
        mock_exists.assert_called_with(os.path.join(common.XCYBER360_PATH, 'queue', 'sockets', 'com'))
...
def test_validation_ko(mock_exists):
    ...
    with patch('socket.socket.bind'):
        ...
        with patch("xcyber360.core.manager.exists", return_value=False):
            with pytest.raises(Xcyber360InternalError, match='.* 1901 .*'):
                validate_ossec_conf()

        with patch('socket.socket.connect'):
            # Socket send raise socket.error
            with patch('xcyber360.core.manager.Xcyber360Socket.send', side_effect=socket.error):
                with pytest.raises(Xcyber360InternalError, match='.* 1014 .*'):
                    validate_ossec_conf()

            with patch('socket.socket.send'):
                # Socket recv raise socket.error
                with patch('xcyber360.core.manager.Xcyber360Socket.receive', side_effect=socket.timeout):
                    with pytest.raises(Xcyber360InternalError, match='.* 1014 .*'):
                        validate_ossec_conf()

                # _parse_execd_output raise KeyError
                with patch('xcyber360.core.manager.Xcyber360Socket'):
                    with patch('xcyber360.core.manager.parse_execd_output', side_effect=KeyError):
                        with pytest.raises(Xcyber360InternalError, match='.* 1904 .*'):
                            validate_ossec_conf()
...
def test_parse_execd_output(error_flag, error_msg):
    ...
    if not error_flag:
        ...
        with pytest.raises(Xcyber360Exception, match=f'.* 1908 .*'):
            parse_execd_output(json_response)


@patch('xcyber360.core.manager.configuration.api_conf', new={'experimental_features': True})
...
def test_get_update_information_template(last_check_date, update_check, installation_uid):
    ...
    assert template['current_version'] == f"v{xcyber360.__version__}"
...
async def test_query_update_check_service_request(
    ...
    version = '4.8.0'
    with patch('framework.xcyber360.core.manager.xcyber360.__version__', version):
        await query_update_check_service(installation_uid)
        ...
        client_session_get_mock.assert_called_with(
            RELEASE_UPDATES_URL,
            headers={
                XCYBER360_UID_KEY: installation_uid,
                XCYBER360_TAG_KEY: f'v{version}',
                USER_AGENT_KEY: f'Xcyber360 UpdateCheckService/v{version}'
            },
