...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        ...
        from xcyber360.core.cdb_list import check_path, get_list_from_file, iterate_lists, \
...
RELATIVE_PATH = os.path.join("framework", "xcyber360", "core", "tests", "data", "test_cdb_list")
PATH_FILE = os.path.join(RELATIVE_PATH, "test_lists")

CONTENT_FILE = {'test-xcyber360-w': 'write',
                'test-xcyber360-r': 'read',
                'test-xcyber360-a': 'attribute',
                'test-xcyber360-x': 'execute',
                'test-xcyber360-c': 'command',
                'test-key': 'value:1',
                'test-key:1': 'value',
                'test-key:2': 'value:2',
                'test-key::::::3': 'value3',
...
def test_split_key_value_with_quotes(line, expected_key, expected_value):
    ...
    if expected_key and expected_value:
        ...
        with pytest.raises(Xcyber360Error) as e:
...
def test_get_list_from_file(raw):
    ...
    """
    full_path = os.path.join(common.XCYBER360_PATH, PATH_FILE)
    if raw:
...
@pytest.mark.parametrize("error_to_raise, xcyber360_error_code", [
...
def test_get_list_from_file_with_errors(error_to_raise, xcyber360_error_code):
    """Test `get_list_from_file` core functionality when using invalid files or paths as parameter.

    `get_list_from_file` must raise the proper Xcyber360Error when facing certain scenarios like a Permission Denied error
    when opening a file.
    ...
    xcyber360_error_code : int
        Error code of the `Xcyber360Error` that must be raised by `get_list_from_file` when the specified `OSError` occurrs.
    """
    with patch("builtins.open", mock_open()) as mock:
        ...
        try:
            ...
            assert e.code == xcyber360_error_code
...
def test_validate_cdb_list():
    ...
    with open(os.path.join(common.XCYBER360_PATH, PATH_FILE)) as f:
        xcyber360_cdb_list = f.read()

    try:
        validate_cdb_list(xcyber360_cdb_list)
    except Xcyber360Error:
...
def test_validate_cdb_list_ko():
    ...
    with pytest.raises(Xcyber360Error, match=r'\b1112\b'):
        validate_cdb_list('')

    # Raise exeption when using invalid CDB list
    with pytest.raises(Xcyber360Error, match=r'\b1800\b'):
        validate_cdb_list("test:key:testvalue\n")


@patch('xcyber360.core.cdb_list.chmod')
...
def test_create_list_file(mock_delete, mock_chmod):
    ...
    with open(os.path.join(common.XCYBER360_PATH, PATH_FILE)) as f:
        with patch('xcyber360.core.cdb_list.common.XCYBER360_PATH', new='/var/ossec'):
            with patch('builtins.open') as mock_open:
                xcyber360_cdb_list = f.read()
                result = create_list_file('/test/path', xcyber360_cdb_list, permissions=0o660)
                assert mock_open.return_value.__enter__().write.call_count == len(xcyber360_cdb_list.split('\n'))-1
...
def test_create_list_file_ko():
    ...
    with pytest.raises(Xcyber360Error, match=r'\b1806\b'):
        create_list_file(full_path='/test/path', content=' ')


@patch('xcyber360.core.cdb_list.remove')
@patch('xcyber360.core.cdb_list.delete_xcyber360_file')
def test_delete_list(mock_delete_xcyber360_file, mock_remove):
    """Check that delete_list function uses expected params."""
    path = 'etc/list/test_list'
    delete_list(path)
    mock_delete_xcyber360_file.assert_called_once_with(os.path.join(common.XCYBER360_PATH, path))
    mock_remove.assert_called_once_with(os.path.join(common.XCYBER360_PATH, path + '.cdb'))
