...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        ...
        from xcyber360.core import rule
...
@pytest.mark.parametrize('status, expected_result', [
    ...
    (None, 'all'),
    ('unexistent', Xcyber360Error(1202))
])
...
@pytest.mark.parametrize('rule_file, rule_path, rule_status, exception', [
    ...
    ('noexists.xml', 'tests/data/rules', 'enabled', Xcyber360Error(1201))
])
@patch("xcyber360.core.common.XCYBER360_PATH", new=parent_directory)
@patch("xcyber360.core.common.RULES_PATH", new=data_path)
...
@patch("xcyber360.core.rule.load_xcyber360_xml", side_effect=OSError(13, 'Error', 'Permissions'))
def test_load_rules_from_file_permissions(mock_load):
    """Test set_groups rule core function."""
    with pytest.raises(Xcyber360Error, match='.* 1207 .*'):
        rule.load_rules_from_file('nopermissions.xml', 'tests/data/rules', 'disabled')


@patch("xcyber360.core.rule.load_xcyber360_xml", side_effect=OSError(8, 'Error', 'Unknown'))
