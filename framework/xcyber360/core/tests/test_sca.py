...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        from xcyber360.core import sca as core_sca
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.__init__', return_value=None)
@patch('xcyber360.core.utils.Xcyber360DBQuery.__init__')
def test_Xcyber360DBQuerySCA__init__(mock_wdbq, mock_backend, mock_get_basic_info, distinct, agent_id, offset, limit, sort,
                                 search, query, count, get_data, select):
    """Test if method __init__ of Xcyber360DBQuerySCA works properly.
    ...
    """
    wdbq_sca = core_sca.Xcyber360DBQuerySCA(agent_id=agent_id, offset=offset, limit=limit, sort=sort, search=search,
    ...
    mock_wdbq.assert_called_once_with(ANY, offset=offset, limit=limit, table='sca_policy', sort=sort, search=search,
                                      select=select, fields=core_sca.Xcyber360DBQuerySCA.DB_FIELDS,
...
def test_Xcyber360DBQuerySCA__format_data_into_dictionary(agent_id, offset, limit, sort, search, select, query, count,
                                                      get_data):
    """Check if Xcyber360DBQuerySCA's method _format_data_into_dictionary works properly.
...
@patch('xcyber360.core.sca.Xcyber360DBQuerySCA.__init__')
def test_Xcyber360DBQuerySCACheck__init__(mock_wdbqsca, select, sca_checks_test_list, expected_default_query):
    """Test if method __init__ of Xcyber360DBQuerySCACheck works properly.
    ...
    """
    core_sca.Xcyber360DBQuerySCACheck(agent_id='000', select=select, sort={'fields': ['title'], 'order': 'asc'},
...
def test_Xcyber360DBQuerySCACheck_parse_select_filter(mock_exists, mock_get_basic_info, mock_backend):
    """Test if method _parse_select_filter of Xcyber360DBQuerySCACheck works properly."""
    wdbq_sca_check = core_sca.Xcyber360DBQuerySCACheck(agent_id='000', sort={'value': 'test'},
...
@patch('xcyber360.core.sca.Xcyber360DBQuerySCA.__init__')
def test_Xcyber360DBQuerySCACheckIDs__init__(mock_wdbqsca, query):
    """Test if method __init__ of Xcyber360DBQuerySCACheckIDs works properly.

    Parameters
    ----------
    query : str
        Query used to initialize the Xcyber360DBQuerySCACheckIDs object.
    """
    ...
    core_sca.Xcyber360DBQuerySCACheckIDs(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
                                     search={'value': 'test'},
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.__init__', return_value=None)
@patch('xcyber360.core.agent.Agent.get_basic_information')
def test_Xcyber360DBQuerySCACheckIDs_protected_pass_filter(mock_get_basic_info, mock_backend, field, value, expected):
    """Test Xcyber360DBQuerySCACheckIDs._pass_filter function."""
    query = core_sca.Xcyber360DBQuerySCACheckIDs(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
...
@patch('xcyber360.core.sca.Xcyber360DBQuerySCA.__init__')
def test_Xcyber360DBQuerySCACheckRelational__init__(mock_wdbqsca, select, table, sca_checks_test_list):
    """Test if method __init__ of Xcyber360DBQuerySCACheckRelational works properly.
    ...
    select : list or None
        Fields to select.
    table : str
        Table used to initialize the Xcyber360DBQuerySCACheckRelational object.
    sca_checks_test_list : list
        List of SCA checks IDs.
    """
    query_sca_check_relational = core_sca.Xcyber360DBQuerySCACheckRelational(agent_id='000', table=table,
...
@patch('xcyber360.core.sca.Xcyber360DBQuerySCA.__enter__')
...
def test_Xcyber360DBQueryDistinctSCACheck__init__(mock_exit, mock_wdbqsca, mock_enter, select):
    """Test if method __init__ of Xcyber360DBQueryDistinctSCACheck works properly."""
    ...
    core_sca.Xcyber360DBQueryDistinctSCACheck(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
    ...
    mock_wdbqsca.assert_has_calls(
        [call(agent_id='000', offset=0, limit=None, sort={'fields': ['title'], 'order': 'asc'},
              ...
              default_query=core_sca.Xcyber360DBQueryDistinctSCACheck.INNER_QUERY_PATTERN, search={'value': 'test'}),
