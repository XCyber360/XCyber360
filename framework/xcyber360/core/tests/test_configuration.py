...
from xcyber360.core.common import REMOTED_SOCKET
...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        ...
        from xcyber360.core import configuration
...
def test_get_ossec_conf():
    with patch('xcyber360.core.configuration.load_xcyber360_xml', return_value=Exception):
...
def test_get_group_conf():
    with pytest.raises(Xcyber360Error, match=".* 1710 .*"):
        configuration.get_group_conf(group_id='noexists')

    with patch('xcyber360.core.common.XCYBER360_SHARED', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with patch('xcyber360.core.configuration.load_xcyber360_yaml', side_effect=Xcyber360Error(1101)):
            with pytest.raises(Xcyber360Error, match=".* 1101 .*"):
...
@patch('xcyber360.core.configuration.common.xcyber360_gid')
@patch('xcyber360.core.configuration.common.xcyber360_uid')
@patch('builtins.open')
def test_update_group_configuration(mock_open, mock_xcyber360_uid, mock_xcyber360_gid):
    with pytest.raises(Xcyber360Error, match=".* 1710 .*"):
        configuration.update_group_configuration('noexists', 'noexists')

    with patch('xcyber360.core.common.XCYBER360_SHARED', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with patch('xcyber360.core.configuration.open', return_value=Exception):
            with pytest.raises(Xcyber360Error, match=".* 1006 .*"):
                configuration.update_group_configuration('default', '')

    with patch('xcyber360.core.common.XCYBER360_SHARED', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with patch('xcyber360.core.configuration.open'):
...
@patch('xcyber360.core.configuration.common.xcyber360_uid')
@patch('builtins.open')
def test_update_group_file(mock_open, mock_xcyber360_uid, mock_xcyber360_gid):
    with pytest.raises(Xcyber360Error, match=".* 1710 .*"):
    ...
    with patch('xcyber360.core.common.XCYBER360_SHARED', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with pytest.raises(Xcyber360Error, match=".* 1112 .*"):
...
@pytest.mark.parametrize("agent_id, component, socket, socket_dir, rec_msg", [
    ...
    (None, 'xcyber360-db', 'wdb', 'db', {"error": 0, "data": {"enabled": "yes"}}),
...
@patch('xcyber360.core.xcyber360_socket.create_xcyber360_socket_message')
@patch('os.path.exists')
@patch('xcyber360.core.common.XCYBER360_PATH', new='/var/ossec')
@pytest.mark.xfail(reason="This module it is deprecated.", run=False)
def test_get_active_configuration(mock_exists, mock_create_xcyber360_socket_message, agent_id, component, socket,
                                  socket_dir, rec_msg):
    """This test checks the proper working of get_active_configuration function."""
    sockets_json_protocol = {'remote', 'analysis', 'wdb'}
    config = MagicMock()

    socket_class = "Xcyber360Socket" if socket not in sockets_json_protocol or agent_id else "Xcyber360SocketJSON"
    with patch(f'xcyber360.core.xcyber360_socket.{socket_class}.close') as mock_close:
        with patch(f'xcyber360.core.xcyber360_socket.{socket_class}.send') as mock_send:
            with patch(f'xcyber360.core.xcyber360_socket.{socket_class}.__init__', return_value=None) as mock__init__:
                with patch(f'xcyber360.core.xcyber360_socket.{socket_class}.receive',
                           return_value=rec_msg.encode() if socket_class == "Xcyber360Socket" else rec_msg) as mock_receive:
                    ...
                    if socket_class == "Xcyber360Socket":
                        mock_send.assert_called_with(f"getconfig {config}".encode() if not agent_id else \
                                                         f"{agent_id} {component} getconfig {config}".encode())
                    else:  # socket_class == "Xcyber360SocketJSON"
                        mock_create_xcyber360_socket_message.assert_called_with(origin={'module': ANY},
                                                                            command="getconfig",
                                                                            parameters={'section': config})
                        mock_send.assert_called_with(mock_create_xcyber360_socket_message.return_value)
...
@pytest.mark.parametrize('agent_id, component, config, socket_exist, socket_class, expected_error, expected_id', [
    ...
    (None, 'test_component', None, ANY, 'Xcyber360Socket', Xcyber360Error, 1307),  # No configuration
    (None, None, 'test_config', ANY, 'Xcyber360Socket', Xcyber360Error, 1307),  # No component
    (None, 'test_component', 'test_config', ANY, 'Xcyber360Socket', Xcyber360Error, 1101),  # Component not in components
    ('001', 'syscheck', 'syscheck', ANY, 'Xcyber360Socket', Xcyber360Error, 1116),  # Cannot send request
    ('001', 'syscheck', 'syscheck', ANY, 'Xcyber360Socket', Xcyber360Error, 1117),  # No such file or directory

    # Checks for manager - Simple messages
    (None, 'syscheck', 'syscheck', False, 'Xcyber360Socket', Xcyber360Error, 1121),  # Socket does not exist
    (None, 'syscheck', 'syscheck', True, 'Xcyber360Socket', Xcyber360InternalError, 1121),  # Error connecting with socket
    (None, 'syscheck', 'syscheck', True, 'Xcyber360Socket', Xcyber360InternalError, 1118),  # Data could not be received

    # Checks for manager - JSON messages
    (None, 'request', 'global', False, 'Xcyber360SocketJSON', Xcyber360Error, 1121),  # Socket does not exist
    (None, 'request', 'global', True, 'Xcyber360SocketJSON', Xcyber360InternalError, 1121),  # Error connecting with socket
    (None, 'request', 'global', True, 'Xcyber360SocketJSON', Xcyber360InternalError, 1118),  # Data could not be received

    # Checks for 001
    ('001', 'syscheck', 'syscheck', ANY, 'Xcyber360Socket', Xcyber360InternalError, 1121),  # Error connecting with socket
    ('001', 'syscheck', 'syscheck', ANY, 'Xcyber360Socket', Xcyber360InternalError, 1118)  # Data could not be received
...
def test_get_active_configuration_ko(mock_exists, agent_id, component, config, socket_exist, socket_class,
    ...
    with patch(f'xcyber360.core.xcyber360_socket.{socket_class}.__init__',
               return_value=MagicMock() if expected_id == 1121 and socket_exist else None):
        with patch(f'xcyber360.core.xcyber360_socket.{socket_class}.send'):
            with patch(f'xcyber360.core.xcyber360_socket.{socket_class}.receive',
                ...
                with patch(f'xcyber360.core.xcyber360_socket.{socket_class}.close'):
...
@patch('xcyber360.core.configuration.get_ossec_conf')
...
def test_update_check_is_enabled_exceptions(error_id, value):
    ...
    with patch('xcyber360.core.configuration.get_ossec_conf', side_effect=Xcyber360Error(error_id), return_value=value):
        if value is not None:
            assert configuration.update_check_is_enabled() == value
        else:
            with pytest.raises(Xcyber360Error, match=f'.* {error_id} .*'):
