...
from xcyber360.core.common import OSSEC_TMP_PATH
...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        from xcyber360 import Xcyber360Exception
        from xcyber360.core.agent import Xcyber360DBQueryAgents
        from xcyber360.core import utils, exception
        from xcyber360.core.common import XCYBER360_PATH, AGENT_NAME_LEN_LIMIT
...
def test_assign_xcyber360_ownership(mock_gid, mock_uid, mock_chown):
    """Test assign_xcyber360_ownership function."""
    with TemporaryDirectory() as tmp_dirname:
        tmp_file = NamedTemporaryFile(dir=tmp_dirname, delete=False)
        filename = os.path.join(tmp_dirname, tmp_file.name)
        utils.assign_xcyber360_ownership(filename)
...
@patch('xcyber360.core.common.xcyber360_uid')
@patch('xcyber360.core.common.xcyber360_gid')
def test_assign_xcyber360_ownership_write_file(mock_gid, mock_uid, mock_chown):
    """Test assign_xcyber360_ownership function with a non-regular file."""
    with TemporaryDirectory() as tmp_dirname:
        tmp_file = NamedTemporaryFile(dir=tmp_dirname, delete=False)
        filename = os.path.join(tmp_dirname, tmp_file.name)
...
@patch('xcyber360.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
...
def test_cut_array_ko(limit, offset, expected_exception):
    ...
    """
    with pytest.raises(exception.Xcyber360Exception, match=f'.* {expected_exception} .*'):
...
@patch('xcyber360.core.utils.len', return_value=1)
@patch('xcyber360.core.utils.cut_array')
@patch('xcyber360.core.utils.select_array', return_value=ANY)
@patch('xcyber360.core.utils.filter_array_by_query', return_value=ANY)
@patch('xcyber360.core.utils.search_array', return_value=ANY)
@patch('xcyber360.core.utils.sort_array', return_value=ANY)
...
@patch('xcyber360.core.utils.chmod')
...
@patch('xcyber360.core.utils.chown')
...
@patch('xcyber360.core.utils.common.XCYBER360_PATH', new='/test/path')
@patch('xcyber360.core.utils.path.exists', return_value=True)
@patch('xcyber360.core.utils.remove')
def test_delete_xcyber360_file(mock_remove, mock_exists):
    """Check delete_file calls functions with expected params"""
    assert utils.delete_xcyber360_file('/test/path/etc/file')
    mock_remove.assert_called_once_with('/test/path/etc/file')


@patch('xcyber360.core.utils.common.XCYBER360_PATH', new='/test/path')
def test_delete_xcyber360_file_ko():
    """Check delete_file calls functions with expected params"""
    with pytest.raises(utils.Xcyber360Error, match=r'\b1907\b'):
        utils.delete_xcyber360_file('/test/different_path/etc/file')

    with pytest.raises(utils.Xcyber360Error, match=r'\b1907\b'):
        utils.delete_xcyber360_file('/test/path/file/../../home')

    with patch('xcyber360.core.utils.path.exists', return_value=False):
        with pytest.raises(utils.Xcyber360Error, match=r'\b1906\b'):
            utils.delete_xcyber360_file('/test/path/etc/file')

    with patch('xcyber360.core.utils.path.exists', return_value=True):
        with pytest.raises(utils.Xcyber360Error, match=r'\b1907\b'):
            utils.delete_xcyber360_file('/test/path/etc/file')
...
@patch('xcyber360.core.utils.chown')
@patch('xcyber360.core.utils.chmod')
@patch('xcyber360.core.utils.utime')
...
def test_safe_move_exception(mock_utime, mock_chmod, mock_chown):
    ...
    with TemporaryDirectory() as tmpdirname:
        tmp_file = NamedTemporaryFile(dir=tmpdirname, delete=False)
        target_file = os.path.join(tmpdirname, 'target')
        with patch('xcyber360.core.utils.rename', side_effect=OSError(1)):
...
@patch('xcyber360.core.utils.chmod')
@patch('xcyber360.core.utils.mkdir')
@patch('xcyber360.core.utils.curdir', new='var')
def test_mkdir_with_mode(mock_mkdir, mock_chmod, dir_name, path_exists):
    """Test mkdir_with_mode function."""
    with patch('xcyber360.core.utils.path.exists', return_value=path_exists):
...
@patch('xcyber360.core.utils.mkdir', side_effect=OSError)
def test_mkdir_with_mode_ko(mock_mkdir, dir_name, exists):
    """Test mkdir_with_mode function errors work."""
    with patch('xcyber360.core.utils.path.exists', return_value=exists):
        with pytest.raises(OSError):
            utils.mkdir_with_mode(dir_name)


@patch('xcyber360.core.utils.open')
@patch('xcyber360.core.utils.iter', return_value=['1', '2'])
def test_md5(mock_iter, mock_open):
    """Test md5 function."""
    with patch('xcyber360.core.utils.hashlib.md5') as md:
...
def test_blake2b(mock_iter, mock_open):
    """Test md5 function."""
    with patch('xcyber360.core.utils.hashlib.blake2b') as blake2b_mock:
...
def test_protected_get_hashing_algorithm_ko():
    ...
    with pytest.raises(exception.Xcyber360Exception, match=".* 1723 .*"):
...
def test_get_hash(mock_open):
    ...
    with patch('xcyber360.core.utils.iter', return_value=['1', '2']):
        with patch('xcyber360.core.utils.hashlib.new') as md:
            md.return_value.update.side_effect = None
    ...
    with patch('xcyber360.core.utils.iter', return_value=[]):
...
def test_basic_load_xcyber360_xml():
    """Test basic load_xcyber360_xml functionality."""
    with patch('xcyber360.core.utils.open') as f:
        f.return_value.__enter__.return_value = StringIO(test_xml)
        result = utils.load_xcyber360_xml('test_file')

        assert isinstance(result, Element)


def test_load_xcyber360_xml():
    """Test load_xcyber360_xml function."""
    ...
    for rule_file in os.listdir(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data/test_load_xcyber360_xml')):
        path = os.path.join(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data/test_load_xcyber360_xml'),
                            rule_file)
        original = parse(path).getroot()
        result = utils.load_xcyber360_xml(path)

        assert elements_equal(original, result.find('dummy_tag'))


@pytest.mark.parametrize('expected_exception', [1113])
def test_load_xcyber360_xml_ko(expected_exception):
    """Test `load_xcyber360_xml` fails gracefully when reading an invalid utf-8 character sequence."""
    file_path = os.path.join(test_data_path, 'test_load_xcyber360_xml_ko', 'invalid_utf8.xml')
    with pytest.raises(Xcyber360Exception, match=f'.* {expected_exception} .*'):
        utils.load_xcyber360_xml(file_path)


def test_basic_load_xcyber360_yaml():
    """Test basic `load_xcyber360_yaml` functionality."""
    with patch('xcyber360.core.utils.open') as f:
        f.return_value.__enter__.return_value = StringIO(test_yaml)
        result = utils.load_xcyber360_yaml('test_file')

        assert isinstance(result, dict)


def test_load_xcyber360_yaml_read_ko():
    """Test `load_xcyber360_yaml` fails gracefully when reading invalid files."""
    file_path = os.path.join(test_data_path, 'test_load_xcyber360_yaml_ko', 'invalid_utf8.yaml')
    with pytest.raises(Xcyber360Exception, match=f'.*{1006}.*'):
        utils.load_xcyber360_yaml(file_path)


@patch('xcyber360.core.utils.yaml.safe_load', side_effect=yaml.YAMLError)
def test_load_xcyber360_yaml_data_ko(safe_load_mock):
    """Test `load_xcyber360_yaml` fails gracefully when parsing invalid data."""
    with pytest.raises(Xcyber360Exception, match=f'.*{1132}.*'):
        utils.load_xcyber360_yaml('', data='1')


@patch('xcyber360.core.common.XCYBER360_SHARED', new='/test')
...
@pytest.mark.parametrize('version1, version2', [
    ('Xcyber360 v3.5.0', 'Xcyber360 v3.5.2'),
    ('Xcyber360 v3.6.1', 'Xcyber360 v3.6.3'),
    ('Xcyber360 v3.7.2', 'Xcyber360 v3.8.0'),
    ('Xcyber360 v3.8.0', 'Xcyber360 v3.8.1'),
    ('Xcyber360 v3.9.0', 'Xcyber360 v3.9.2'),
    ('Xcyber360 v3.9.10', 'Xcyber360 v3.9.14'),
    ('Xcyber360 v3.10.1', 'Xcyber360 v3.10.10'),
    ('Xcyber360 v4.10.10', 'Xcyber360 v4.11.0'),
    ('Xcyber360 v5.1.15', 'Xcyber360 v5.2.0'),
    ('v3.6.0', 'v3.6.1'),
    ('v3.9.1', 'v3.9.2'),
    ('v4.0.0', 'v4.0.1'),
    ('3.6.0', '3.6.1'),
...
def test_version_ok(version1, version2):
    """Test Xcyber360Version class."""
    current_version = utils.Xcyber360Version(version1)
    new_version = utils.Xcyber360Version(version2)
...
@pytest.mark.parametrize('version1, version2', [
    ('v3.6.0', 'v.3.6.1'),
    ('Xcyber360 v4', 'Xcyber360 v5'),
    ('Xcyber360 v3.9', 'Xcyber360 v3.10'),
    ('ABC v3.10.1', 'ABC v3.10.12'),
    ('Xcyber360v3.9.0', 'Xcyber360v3.9.2'),
    ('3.9', '3.10'),
    ('3.9.0', '3.10'),
    ('3.10', '4.2'),
    ('3', '3.9.1')
...
def test_version_ko(version1, version2):
    """Test Xcyber360Version class."""
    try:
        utils.Xcyber360Version(version1)
        utils.Xcyber360Version(version2)
    except ValueError:
        return


@pytest.mark.parametrize('version1, version2', [
    ('Xcyber360 v3.10.10', 'Xcyber360 v3.10.10'),
    ('Xcyber360 v5.1.15', 'Xcyber360 v5.1.15'),
    ('v3.6.0', 'v3.6.0'),
    ('v3.9.2', 'v3.9.2')
])
...
def test_Xcyber360Version_to_array():
    """Test Xcyber360Version.to_array function."""
    version = utils.Xcyber360Version('Xcyber360 v3.10.0-alpha4')

    assert isinstance(version.to_array(), list)


def test_Xcyber360Version__str__():
    """Test Xcyber360Version.__str__ function."""
    version = utils.Xcyber360Version('Xcyber360 v3.10.0-alpha4')
...
@pytest.mark.parametrize('version1, version2', [
    ('Xcyber360 v3.5.2', 'Xcyber360 v4.0.0'),
    ('Xcyber360 v3.10.0-alpha', 'Xcyber360 v3.10.0'),
    ('Xcyber360 v3.10.0-alpha4', 'Xcyber360 v3.10.0-beta4'),
    ('Xcyber360 v3.10.0-alpha3', 'Xcyber360 v3.10.0-alpha4'),
])
def test_Xcyber360Version__ge__(version1, version2):
    """Test Xcyber360Version.__ge__ function."""
    current_version = utils.Xcyber360Version(version1)
    new_version = utils.Xcyber360Version(version2)
...
def test_failed_test_get_timeframe_in_seconds():
    ...
    with pytest.raises(exception.Xcyber360Exception, match=".* 1411 .*"):
...
@patch('xcyber360.core.utils.path.exists', return_value=True)
@patch('xcyber360.core.utils.glob.glob', return_value=True)
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('xcyber360.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_Xcyber360DBQuery_protected_clean_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                             query_filter, expected_query_filter, expected_wef):
    """Test Xcyber360DBQuery._clean_filter function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=500, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.Xcyber360DBBackend(agent_id=1),
...
@patch('socket.socket.connect')
@patch('xcyber360.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_Xcyber360DBQuery_protected_add_limit_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                                   limit, error, expected_exception):
    """Test Xcyber360DBQuery._add_limit_to_query function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=limit, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.Xcyber360DBBackend(agent_id=1),
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_sort_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Tests Xcyber360DBQuery._sort_query function works"""

    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.Xcyber360DBBackend(agent_id=1), count=5,
                               get_data=None)
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_add_sort_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                                  sort, expected_exception):
    """Test Xcyber360DBQuery._add_sort_to_query function."""
    fields = {'1': 'one', '2': 'two', '3': 'three', '4': 'four'}
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=sort,
                               search=None, select=None, filters=None,
                               fields=fields,
                               default_sort_field=None, query=None,
                               backend=utils.Xcyber360DBBackend(agent_id=1),
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_add_search_to_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test Xcyber360DBQuery._add_search_to_query function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('xcyber360.core.utils.glob.glob', return_value=True)
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_parse_select_filter(mock_socket_conn, mock_glob, mock_conn_db, mock_exists,
                                                    selector_fields, error, expected_exception):
    """Test Xcyber360DBQuery._parse_select_filter function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('xcyber360.core.utils.Xcyber360DBQuery._parse_select_filter')
def test_Xcyber360DBQuery_protected_add_select_to_query(mock_parse, mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test Xcyber360DBQuery._add_select_to_query function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
@patch('xcyber360.core.utils.path.exists', return_value=True)
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
def test_Xcyber360DBQuery_protected_parse_query_regex(mock_backend_connect, mock_exists, q, expected_query_filters):
    """Test Xcyber360DBQuery._parse_query function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_parse_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_parse_legacy_filters(mock_socket_conn, mock_conn_db, mock_glob,
                                                     mock_exists, filter_):
    """Test Xcyber360DBQuery._parse_legacy_filters function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=filter_,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.Xcyber360DBBackend(agent_id=0),
                               count=5, get_data=None)
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('xcyber360.core.utils.Xcyber360DBQuery._parse_legacy_filters')
@patch('xcyber360.core.utils.Xcyber360DBQuery._parse_query')
def test_Xcyber360DBQuery_parse_filters(mock_query, mock_filter, mock_socket_conn, mock_conn_db, mock_glob,
                                    mock_exists, filter, q):
    """Test Xcyber360DBQuery._parse_filters function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search={"negation": True, "value": "1"}, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=q,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('xcyber360.core.utils.Xcyber360DBQuery._filter_status')
@patch('xcyber360.core.utils.Xcyber360DBQuery._filter_date')
def test_Xcyber360DBQuery_protected_process_filter(mock_date, mock_status, mock_socket_conn, mock_conn_db,
                                               mock_glob, mock_exists, field_name, field_filter, q_filter):
    """Tests Xcyber360DBQuery._process_filter."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04', 'status': 'active'},
                               default_sort_field=None, query=None,
                               backend=utils.Xcyber360DBBackend(agent_id=0), count=5,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('xcyber360.core.utils.Xcyber360DBQuery._process_filter')
def test_Xcyber360DBQuery_protected_add_filters_to_query(mock_process, mock_socket_conn,
                                                     mock_conn_db, mock_glob, mock_exists):
    """Test Xcyber360DBQuery._add_filters_to_query function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=['os.name'],
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query='os.name=ubuntu',
                               backend=utils.Xcyber360DBBackend(agent_id=0), distinct=True,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('xcyber360.core.utils.path.exists', return_value=True)
def test_Xcyber360DBQuery_protected_add_filters_to_query_final_query(mock_conn_db, mock_file_exists,
                                                                 filters, expected_query):
    """Test Xcyber360DBQuery._add_filters_to_query final query."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None, search=None, select=None,
                               fields={'id': 'id', 'status': 'status', 'group': 'group'},
                               default_sort_field=None, query='', backend=utils.Xcyber360DBBackend(agent_id=0),
                               count=5, get_data=None)
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_get_total_items(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test Xcyber360DBQuery._get_total_items function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_get_total_items_mitre(mock_socket_conn, mock_conn_db, mock_glob,
                                                      mock_exists):
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.Xcyber360DBBackend(agent_id=0, query_format='mitre'), count=5,
                               get_data=None, date_fields=['date1', 'date2'])
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_substitute_params(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.Xcyber360DBQuery._get_total_items function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.Xcyber360DBQuery._format_data_into_dictionary."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_filter_status(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.Xcyber360DBQuery._filter_status function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_filter_date(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            date_filter, filter_db_name, time, error):
    """Test utils.Xcyber360DBQuery._filter_date function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
    ...
    with patch('xcyber360.core.utils.get_timeframe_in_seconds', return_value=time):
        if error:
            with pytest.raises(exception.Xcyber360Exception, match=".* 1412 .*"):
...
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_general_run(mock_socket_conn, execute_value, expected_result):
    """Test utils.Xcyber360DBQuery.general_run function."""
    with patch('xcyber360.core.utils.Xcyber360DBBackend.execute', return_value=execute_value):
        query = Xcyber360DBQueryAgents(offset=0, limit=None, sort=None, search=None, select={'id'},
...
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_oversized_run(mock_socket_conn, execute_value, rbac_ids, negate,
                                    final_rbac_ids, expected_result):
    """Test utils.Xcyber360DBQuery.oversized_run function."""
    with patch('xcyber360.core.utils.Xcyber360DBBackend.execute', side_effect=[execute_value, final_rbac_ids]):
        query = Xcyber360DBQueryAgents(offset=0, limit=None, sort=None, search=None, select={'id'},
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('xcyber360.core.utils.Xcyber360DBQuery._default_query')
def test_Xcyber360DBQuery_reset(mock_query, mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.Xcyber360DBQuery.reset function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'os.name'},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_default_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.Xcyber360DBQuery._default_query function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test utils.Xcyber360DBQuery._default_count_query function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQuery_protected_pass_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            value):
    """Test utils.Xcyber360DBQuery._pass_filter function."""
    query = utils.Xcyber360DBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQueryDistinct_protected_default_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                      mock_exists):
    """Test utils.Xcyber360DBQueryDistinct._default_query function."""
    query = utils.Xcyber360DBQueryDistinct(offset=0, limit=1, sort=None, search=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQueryDistinct_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.Xcyber360DBQueryDistinct._default_count_query function."""
    query = utils.Xcyber360DBQueryDistinct(offset=0, limit=1, sort=None, search=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('xcyber360.core.utils.Xcyber360DBQuery._add_filters_to_query')
def test_Xcyber360DBQueryDistinct_protected_add_filters_to_query(mock_add, mock_socket_conn, mock_conn_db,
                                                             mock_glob, mock_exists):
    """Test utils.Xcyber360DBQueryDistinct._add_filters_to_query function."""
    query = utils.Xcyber360DBQueryDistinct(offset=0, limit=1, sort=None, search=None,
                                       query=None, select={'name'},
                                       fields={'name': '`group`'}, count=True,
                                       get_data=True, default_sort_field='`group`',
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('xcyber360.core.utils.Xcyber360DBQuery._add_select_to_query')
def test_Xcyber360DBQueryDistinct_protected_add_select_to_query(mock_add, mock_socket_conn, mock_conn_db,
                                                            mock_glob, mock_exists, select):
    """Test utils.Xcyber360DBQueryDistinct._add_select_to_query function."""
    query = utils.Xcyber360DBQueryDistinct(offset=0, limit=1, sort=None, search=None,
    ...
    if len(select) > 1:
        with pytest.raises(exception.Xcyber360Exception, match=".* 1410 .*"):
            query._add_select_to_query()
    else:
        query._add_select_to_query()
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQueryDistinct_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db,
                                                                    mock_glob, mock_exists):
    """Test utils.Xcyber360DBQueryDistinct._format_data_into_dictionary function."""
    query = utils.Xcyber360DBQueryDistinct(offset=0, limit=1, sort=None, search=None,
                                       query=None, select={'fields': ['name']},
                                       fields={'name': '`group`'}, count=True,
                                       get_data=True,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_Xcyber360DBQueryGroupBy__init__(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Tests utils.Xcyber360DBQueryGroupBy.__init__ function works"""
    utils.Xcyber360DBQueryGroupBy(filter_fields=None, offset=0, limit=1, table='agent',
                              ...
                              backend=utils.Xcyber360DBBackend(agent_id=0), get_data=None,
...
@patch('xcyber360.core.utils.Xcyber360DBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('xcyber360.core.utils.Xcyber360DBQuery._get_total_items')
def test_Xcyber360DBQueryGroupBy_protected_get_total_items(mock_total, mock_socket_conn, mock_conn_db,
                                                       mock_glob, mock_exists):
    """Test utils.Xcyber360DBQueryGroupBy._get_total_items function."""
    query = utils.Xcyber360DBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
...
@patch('xcyber360.core.utils.Xcyber360DBQuery._add_select_to_query')
@patch('xcyber360.core.utils.Xcyber360DBQuery._parse_select_filter')
def test_Xcyber360DBQueryGroupBy_protected_add_select_to_query(mock_parse, mock_add, mock_socket_conn,
                                                           mock_conn_db, mock_glob, mock_exists):
    """Test utils.Xcyber360DBQueryGroupBy._add_select_to_query function."""
    query = utils.Xcyber360DBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
                                      limit=1, table='agent', sort=None, search=None,
                                      select={'name'}, filters=None,
                                      fields={'name': '`group`'}, query=None,
...
def test_filter_array_by_query(q, return_length):
    ...
    if return_length == -1:
        with pytest.raises(exception.Xcyber360Error, match='.* 1407 .*'):
...
def test_select_array(select, required_fields, expected_result):
    ...
    except utils.Xcyber360Error as e:
        assert e.code == 1724
...
@patch('xcyber360.core.utils.check_xcyber360_limits_unchanged')
@patch('xcyber360.core.utils.check_remote_commands')
@patch('xcyber360.core.utils.check_agents_allow_higher_versions')
@patch('xcyber360.core.utils.check_virustotal_integration')
@patch('xcyber360.core.utils.check_indexer')
@patch('xcyber360.core.common.XCYBER360_ETC', new=test_files_path)
def test_validate_xcyber360_xml(mock_check_indexer, mock_virus_total_integration,
                            mock_agents_versions, mock_remote_commands, mock_unchanged_limits):
    """Test validate_xcyber360_xml method works and methods inside are called with expected parameters"""
    ...
    with patch('builtins.open', m):
        utils.validate_xcyber360_xml(xml_file)
    ...
    with patch('builtins.open', m):
        utils.validate_xcyber360_xml(xml_file, config_file=True)
...
def test_validate_xcyber360_xml_ko(effect, expected_exception):
    """Tests validate_xcyber360_xml function works when open function raises an exception.
    Parameters
    ----------
    effect : Exception
    ...
    with patch('xcyber360.core.utils.load_xcyber360_xml', side_effect=effect):
        with pytest.raises(Xcyber360Exception, match=f'.* {expected_exception} .*'):
            utils.validate_xcyber360_xml(input_file)


@patch('xcyber360.core.utils.full_copy')
...
@patch('xcyber360.core.utils.full_copy', side_effect=IOError)
def test_delete_file_with_backup_ko(mock_copyfile):
    """Test delete_file_with_backup function exceptions."""
    with pytest.raises(utils.Xcyber360Error, match='.* 1019 .*'):
...
def test_to_relative_path():
    ...
    path = 'etc/ossec.conf'
    assert utils.to_relative_path(os.path.join(XCYBER360_PATH, path)) == path

    assert utils.to_relative_path(path, prefix='etc') == os.path.basename(path)


@patch('xcyber360.core.utils.common.RULES_PATH', new=test_files_path)
@patch('xcyber360.core.utils.common.USER_RULES_PATH', new=test_files_path)
...
@patch('xcyber360.core.utils.common.DECODERS_PATH', new=test_files_path)
@patch('xcyber360.core.utils.common.USER_DECODERS_PATH', new=test_files_path)
...
@patch('xcyber360.core.utils.common.LISTS_PATH', new=test_files_path)
@patch('xcyber360.core.utils.common.USER_LISTS_PATH', new=test_files_path)
...
@patch('xcyber360.core.utils.copy2', new=copyfile)
...
def test_check_xcyber360_limits_unchanged(new_conf, unchanged_limits_conf, original_conf, limits_conf):
    ...
    with patch('xcyber360.core.utils.configuration.api_conf', new=api_conf):
        if limits_conf['eps']['allow'] or unchanged_limits_conf:
            utils.check_xcyber360_limits_unchanged(new_conf, original_conf)
        else:
            with pytest.raises(exception.Xcyber360Error, match=".* 1127 .*"):
                utils.check_xcyber360_limits_unchanged(new_conf, original_conf)
...
def test_agents_allow_higher_versions(new_conf, agents_conf):
    ...
    with patch('xcyber360.core.utils.configuration.api_conf', new=api_conf):
        if agents_conf['allow_higher_versions']['allow'] or new_conf.find('no') != -1:
            ...
            with pytest.raises(exception.Xcyber360Error, match=".* 1129 .*"):
                utils.check_agents_allow_higher_versions(new_conf)
...
@patch('xcyber360.core.utils.tempfile.mkstemp',
...
def test_upload_file(mock_uid, mock_gid,
    ...
    ----------
    mock_uid: Mock
        mock of the xcyber360.core.common.xcyber360_uid function.
    mock_gid: Mock
        mock of the xcyber360.core.common.xcyber360_gid function.
    mock_chmod: Mock
        mock of the xcyber360.core.utils.chmod function.
    mock_mks
        Mock of the xcyber360.core.utils.tempfile.mkstemp function.
    chk_xml: bool
    ...
    with patch('xcyber360.core.utils.open', mko):
        with patch('xcyber360.core.utils.safe_move') as mock_safe_move:
            result = utils.upload_file(content, file_path=filename,
                                        check_xml_formula_values=chk_xml)
            assert isinstance(result, Xcyber360Result)
            handle.write.assert_called_once_with(content)
            tmp_path = os.path.join(OSSEC_TMP_PATH, 'file.tmp')
            file_path = os.path.join(XCYBER360_PATH, filename)
...
@pytest.mark.parametrize(
    'sm_side_effect, w_side_effect, upload_error', [
        ...
        (None, True, (utils.Xcyber360InternalError, 1005)),
        # safe_move raises an Error()
        (Error(), False, (utils.Xcyber360InternalError, 1016)),
        # safe_move raises a PermissionError()
        (PermissionError(), False, (utils.Xcyber360Error, 1006))]
)
@patch('xcyber360.core.utils.tempfile.mkstemp',
        return_value=('handle', os.path.join(OSSEC_TMP_PATH, 'file.tmp')))
@patch('xcyber360.core.utils.chmod')
...
def test_upload_file_ko(mock_uid, mock_gid, mock_chmod, mock_mks,
    ...
    with patch('xcyber360.core.utils.open', mko):
        with patch('xcyber360.core.utils.safe_move', side_effect=sm_side_effect) as mock_safe_move:
...
def test_check_virustotal_integration(integrations_conf, new_conf):
    ...
    with patch('xcyber360.core.utils.get') as mock_requests_get:
        if not integrations_conf['virustotal']['public_key']['allow']:
            with pytest.raises(exception.Xcyber360Error, match=".* 1130 .*"):
                mock_response = Mock()
            ...
            with pytest.raises(exception.Xcyber360Error, match=".* 1131 .*"):
