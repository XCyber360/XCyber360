...
with patch('xcyber360.core.common.xcyber360_uid'):
    with patch('xcyber360.core.common.xcyber360_gid'):
        from xcyber360.core import wlogging
...
def test_sizebasedfilerotatinghandler_compute_log_directory(mock_mkdir):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("xcyber360.core.wlogging.os.path.exists", new=_mock_exists):
...
@patch('xcyber360.core.wlogging.SizeBasedFileRotatingHandler')
@patch('xcyber360.core.wlogging.TimeBasedFileRotatingHandler')
def test_xcyber360_logger_setup_logger(mock_time_handler, mock_size_handler, mock_add_handler, mock_add_level_name,
                                   max_size):
    """Test if method setup_logger of Xcyber360Logger setups the logger attribute properly.
    ...
    # To bypass the checking of the existence of a valid Xcyber360 install
    # Check time handler
    with patch('os.path.join', return_value=tmp_dir.name):
        w_logger = wlogging.Xcyber360Logger(foreground_mode=True, log_path=tmp_dir.name,
...
@patch.object(wlogging.Xcyber360Logger, 'log_path', create=True, new_callable=PropertyMock(Path('test')))
@patch.object(wlogging.Xcyber360Logger, 'tag', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Xcyber360Logger, 'logger', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Xcyber360Logger, 'foreground_mode', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Xcyber360Logger, 'debug_level', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Xcyber360Logger, 'logger_name', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Xcyber360Logger, 'custom_formatter', create=True, new_callable=PropertyMock)
@patch.object(wlogging.Xcyber360Logger, 'max_size', create=True, new_callable=PropertyMock)
@patch('logging.Formatter')
def test_xcyber360_logger__init__(mock_lformatter, mock_max_size, mock_formatter, mock_logger_name, mock_debug_level,
                              mock_foreground_mode, mock_logger, mock_tag, mock_log_path):
    """Test if Xcyber360Logger __init__ method initialize all attributes properly."""
    # To bypass the checking of the existence of a valid Xcyber360 install
    with patch('os.path.join'):
        wlogging.Xcyber360Logger(foreground_mode=mock_foreground_mode, log_path=mock_log_path, tag=mock_tag,
...
@patch('xcyber360.core.wlogging.SizeBasedFileRotatingHandler')
@patch('xcyber360.core.wlogging.TimeBasedFileRotatingHandler')
def test_xcyber360_logger_getattr(mock_time_handler, mock_size_handler, attribute, expected_exception, expected_value):
    """Test if Xcyber360Logger __getattr__ method works properly."""
    tmp_dir = tempfile.TemporaryDirectory()
    # To bypass the checking of the existence of a valid Xcyber360 install
    with patch('os.path.join'):
        w_logger = wlogging.Xcyber360Logger(foreground_mode=True, log_path=tmp_dir.name, tag='%(test)s %(test)s: %(test)s',
...
@pytest.mark.parametrize('value, expected', [
    ('Example log', True),
    ('Xcyber360 Internal Error', False),
    ('Xcyber360InternalError', False),
    ('Xcyber360Error', True),
    ('InternalError', True)
])
