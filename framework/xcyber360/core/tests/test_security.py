...
from xcyber360.tests.test_security import db_setup  # noqa
...
def test_revoke_tokens(db_setup):
    """Checks that the return value of revoke_tokens is a Xcyber360Result.
    ...
    db_setup: callable
        This function creates the rbac.db file.
    """
    with patch('xcyber360.core.security.generate_keypair', side_effect=None):
        security, Xcyber360Result, _ = db_setup
        mock_current_user = ContextVar('current_user', default='xcyber360')
        with patch("xcyber360.sca.common.current_user", new=mock_current_user):
...
def test_invalid_roles_tokens(db_setup, role_list, expected_roles):
    ...
    expected_roles : set
        Expected roles.
    """
    with patch('xcyber360.core.security.TokenManager.add_user_roles_rules') as TM_mock:
...
@patch('xcyber360.core.security.TokenManager.add_user_roles_rules')
...
@patch("xcyber360.core.security.revoke_tokens")
@patch("xcyber360.core.security.check_database_integrity")
@patch("xcyber360.core.security.os.remove")
