...
from xcyber360.core import common, configuration, stats
from xcyber360.core.InputValidator import InputValidator
from xcyber360.core.cluster.utils import get_manager_status
from xcyber360.core.common import AGENT_COMPONENT_STATS_REQUIRED_VERSION, DATE_FORMAT
from xcyber360.core.exception import Xcyber360Exception, Xcyber360Error, Xcyber360InternalError, Xcyber360ResourceNotFound
...
from xcyber360.core.utils import Xcyber360Version, plain_dict_to_nested_dict, get_fields_to_nest, Xcyber360DBQuery, \
    Xcyber360DBQueryDistinct, Xcyber360DBQueryGroupBy, Xcyber360DBBackend, get_utc_now, get_utc_strptime, \
    get_date_from_timestamp, get_group_file_path, GROUP_FILE_EXT
from xcyber360.core.xcyber360_queue import Xcyber360Queue
from xcyber360.core.xcyber360_socket import Xcyber360Socket, Xcyber360SocketJSON, create_xcyber360_socket_message
...
class Xcyber360DBQueryAgents(Xcyber360DBQuery):
    """Class used to query Xcyber360 agents."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        if filters is None:
            filters = {}
        unify_xcyber360_version_format(filters)
        if min_select_fields is None:
            min_select_fields = {'id'}
        backend = Xcyber360DBBackend(query_format='global')
        Xcyber360DBQuery.__init__(self, offset=offset, limit=limit, table='agent', sort=sort, search=search, select=select,
    ...
    def _filter_date(self, date_filter: dict, filter_db_name: str):
        """Add date filter to the Xcyber360 query."""
        Xcyber360DBQuery._filter_date(self, date_filter, filter_db_name)
        self.query += ' AND id != 0'
    ...
    def _sort_query(self, field: str) -> str:
        ...
        return Xcyber360DBQuery._sort_query(self, field)

    def _add_search_to_query(self):
        """Add search to the Xcyber360 query with the specific implications.

        Since IDs are stored in the database as integers, id searches must be turned into integers to work as expected.
        """
        if self.search:
            del self.fields['id']
            Xcyber360DBQuery._add_search_to_query(self)
            self.fields['id'] = 'id'
    ...
    def _process_filter(self, field_name: str, field_filter: str, q_filter: dict):
        ...
        if field_name == 'group' and q_filter['value'] is not None:
            ...
            if q_filter['operator'] == '=':
                ...
                raise Xcyber360Error(1409, f"Valid operators for 'group' field: {', '.join(valid_group_operators)}. "
                                       f"Used operator: {q_filter['operator']}")
        else:
            Xcyber360DBQuery._process_filter(self, field_name, field_filter, q_filter)


class Xcyber360DBQueryGroup(Xcyber360DBQuery):
    """Class used to query Xcyber360 groups."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        backend = Xcyber360DBBackend(query_format='global')
        Xcyber360DBQuery.__init__(self, offset=offset, limit=limit, table='`group`', sort=sort, search=search,
                              select=select,
...
class Xcyber360DBQueryDistinctAgents(Xcyber360DBQueryDistinct, Xcyber360DBQueryAgents):
    pass


class Xcyber360DBQueryGroupByAgents(Xcyber360DBQueryGroupBy, Xcyber360DBQueryAgents):
    ...
    def __init__(self, filter_fields: dict, *args: dict, **kwargs: dict):
        ...
        filter_fields : dict
            Fields to filter by.
        """
        Xcyber360DBQueryAgents.__init__(self, *args, **kwargs)
        Xcyber360DBQueryGroupBy.__init__(self, *args, table=self.table, fields=self.fields, filter_fields=filter_fields,
    ...
    def _format_data_into_dictionary(self) -> str:
        ...
        return Xcyber360DBQuery._format_data_into_dictionary(self)


class Agent:
    """Xcyber360 Agent object."""
    ...
    def load_info_from_db(self, select: list = None):
        ...
        Raises
        ------
        Xcyber360ResourceNotFound(1701)
            Agent does not exist.
        """
        with Xcyber360DBQueryAgents(offset=0, limit=None, sort=None, search=None, select=select,
    ...
    def reconnect(self, wq: Xcyber360Queue) -> str:
        """Force reconnect to the manager.

        Parameters
        ----------
        wq : Xcyber360Queue
            Xcyber360Queue used for the active response message.
        ...
        -------
        str
            Message generated by Xcyber360.
        """
        ...
        # Send force reconnect message to the Xcyber360Queue
        ret_msg = wq.send_msg_to_agent(Xcyber360Queue.HC_FORCE_RECONNECT, self.id)
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        str
            Message generated by Xcyber360.
        """
        # Check that xcyber360-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except Xcyber360InternalError as e:
            # xcyber360-authd is not running due to a problem with /proc availability
            raise Xcyber360Error(1726, extra_message=str(e))

        if manager_status.get('xcyber360-authd') != 'running':
            # xcyber360-authd is not running
            raise Xcyber360Error(1726)
        ...
        try:
            ...
            raise Xcyber360InternalError(1757, extra_message=str(e))
    ...
    def _remove_authd(self, purge: bool = False) -> dict:
        ...
        authd_socket = Xcyber360SocketJSON(common.AUTHD_SOCKET)
    ...
    def _add(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        ...
        if ip != 'any':
            if ip.find('/') > 0:
                try:
                    ...
                    raise Xcyber360Error(1706, extra_message=ip)
            else:
                try:
        ...
        try:
            ...
            raise Xcyber360InternalError(1725, extra_message=str(e))

    def _add_authd(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to Xcyber360 using authd.
        2 uses:
            - name and ip [force]: Add an agent like manage_agents (generate id and key).
            - name, ip, id, key [force]: Insert an agent with an existing id and key.
        ...
        try:
            ...
            if e.code == 9008:
                raise Xcyber360Error(1705, extra_message=name)
            elif e.code == 9007:
                raise Xcyber360Error(1706, extra_message=ip)
            elif e.code == 9012:
                raise Xcyber360Error(1708, extra_message=id)
            raise e
    ...
    async def delete_single_group(group_name: str) -> dict:
        ...
        if path.exists(group_path):
            try:
                ...
                raise Xcyber360Error(1006, extra_message=str(e))
    ...
    def get_agent_os_name(self) -> str:
        ...
        query = Xcyber360DBQueryAgents(select=['os.name'], filters={'id': [self.id]})
    ...
    def get_agents_overview(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: str = None,
        ...
        db_query = Xcyber360DBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
    ...
    async def unset_single_group_agent(agent_id: str, group_id: str, force: bool = False) -> str:
        ...
        Raises
        ------
        Xcyber360ResourceNotFound(1710)
        ...
        if not force:
            ...
            if not Agent.group_exists(group_id):
                raise Xcyber360ResourceNotFound(1710)
        ...
        if group_id not in group_list:
            raise Xcyber360Error(1734)
        elif len(group_list) == 1:
            if group_id == 'default':
                raise Xcyber360Error(1745)
    ...
    def get_config(self, component: str = '', config: str = '', agent_version: str = '') -> dict:
        ...
        agent_version : str
            Agent version to compare with the required version. The format is vX.Y.Z or Xcyber360 vX.Y.Z.
        ...
        """
        if Xcyber360Version(agent_version) < Xcyber360Version(common.ACTIVE_CONFIG_VERSION):
            raise Xcyber360InternalError(1735, extra_message=f"Minimum required version is {common.ACTIVE_CONFIG_VERSION}")
    ...
    def get_stats(self, component: str) -> dict:
        ...
        Raises
        ------
        Xcyber360InternalError(1015)
            Agent version is null.
        Xcyber360InternalError(1735)
        ...
        if self.version is None:
            raise Xcyber360InternalError(1015)
        agent_version = Xcyber360Version(self.version.split(" ")[1])
        required_version = Xcyber360Version(AGENT_COMPONENT_STATS_REQUIRED_VERSION.get(component))
        if agent_version < required_version:
            raise Xcyber360InternalError(1735, extra_message="Minimum required version is " + str(required_version))

        return stats.get_daemons_stats_from_socket(self.id, component)


def unify_xcyber360_upgrade_version_format(upgrade_version: str) -> str:
...
def unify_xcyber360_version_format(filters: dict):
    """Verify and format the specified xcyber360 version into the 'xcyber360 vX.Y.Z' standard.
    ...
    if wv is not None:
        if re.match(r'^v?\d+\.\d+\.\d+$', wv, re.IGNORECASE):
            filters['version'] = f"xcyber360 {'v' if 'v' not in wv else ''}{wv}"
        elif re.match(r'^xcyber360 \d+\.\d+\.\d+$', wv, re.IGNORECASE):
...
def send_restart_command(agent_id: str = '', agent_version: str = '', wq: Xcyber360Queue = None) -> str:
    ...
    wq : Xcyber360Queue
        Xcyber360Queue used for the active response messages.
    ...
    str
        Message generated by Xcyber360.
    """
    # If the Xcyber360 agent version is newer or equal to the AR legacy version,
    # the message sent will have JSON format
    if Xcyber360Version(agent_version) >= Xcyber360Version(common.AR_LEGACY_VERSION):
        ret_msg = wq.send_msg_to_agent(Xcyber360Queue.RESTART_AGENTS_JSON, agent_id)
    else:
        ret_msg = wq.send_msg_to_agent(Xcyber360Queue.RESTART_AGENTS, agent_id)
...
def get_groups() -> set:
    ...
    """
    groups = set()
    for group_file in listdir(common.XCYBER360_SHARED):
...
def core_upgrade_agents(agents_chunk: list, command: str = 'upgrade_result', wpk_repo: str = None, version: str = None,
    ...
    """
    msg = create_xcyber360_socket_message(origin={'module': 'api'},
                                      command=command,
                                      parameters={
                                          'agents': agents_chunk,
                                          'version': unify_xcyber360_upgrade_version_format(version),
                                          'force_upgrade': force,
                                          'use_http': use_http,
    ...
    s = Xcyber360Socket(common.UPGRADE_SOCKET)
