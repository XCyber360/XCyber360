...
from xcyber360.core import common, xcyber360_socket
from xcyber360.core.exception import Xcyber360Error, Xcyber360InternalError, Xcyber360ResourceNotFound
from xcyber360.core.InputValidator import InputValidator
from xcyber360.core.utils import load_xcyber360_yaml, validate_xcyber360_configuration, get_group_file_path
import xcyber360.core.config.client
...
DEFAULT_CTI_URL = 'https://cti.xcyber360.com'
...
def _replace_custom_values(opt_value: Union[list, dict, str]) -> Union[list, dict, str]:
    """Replace custom values introduced by 'load_xcyber360_xml' with their real values.
...
def get_group_conf(group_id: str = None, raw: bool = False) -> Union[dict, str]:
    ...
    if not os_path.exists(filepath):
        raise Xcyber360ResourceNotFound(1710, group_id)
    ...
    if raw:
        try:
            ...
            raise Xcyber360Error(1006, str(e))

    # Parse YAML
    data = load_xcyber360_yaml(filepath)
...
def update_group_configuration(group_id: str, file_content: str) -> str:
    ...
    validate_xcyber360_configuration(file_content)
...
def get_active_configuration(component: str, configuration: str, agent_id: str = None) -> dict:
    ...
    Xcyber360InternalError(1121)
        If the socket cant be created.
    Xcyber360InternalError(1118)
    ...
    component_socket_mapping = {'agent': 'analysis', 'agentless': 'agentless', 'analysis': 'analysis', 'auth': 'auth',
                                ...
                                'request': 'remote', 'syscheck': 'syscheck', 'xcyber360-db': 'wdb', 'wmodules': 'wmodules'}
    component_socket_dir_mapping = {'agent': 'sockets', 'agentless': 'sockets', 'analysis': 'sockets',
                                    ...
                                    'request': 'sockets', 'syscheck': 'sockets', 'xcyber360-db': 'db',
                                    'wmodules': 'sockets'}

    if not component or not configuration:
        raise Xcyber360Error(1307)

    # Check if the component is correct
    components = component_socket_mapping.keys()
    if component not in components:
        raise Xcyber360Error(1101, f'Valid components: {", ".join(components)}')
    ...
    def get_active_configuration_manager():
        ...
        dest_socket = common.XCYBER360_QUEUE / component_socket_dir_mapping[component] / component_socket_mapping[component]

        # Verify component configuration
        if not os.path.exists(dest_socket):
            raise Xcyber360Error(1121,
        ...
        if component_socket_mapping[component] not in sockets_json_protocol:
            ...
            # Socket connection
            try:
                s = xcyber360_socket.Xcyber360Socket(dest_socket)
            except Xcyber360InternalError:
                raise
            except Exception as unhandled_exc:
                raise Xcyber360InternalError(1121, extra_message=str(unhandled_exc))
            ...
            try:
                ...
                raise Xcyber360InternalError(1118, extra_message="Data could not be received")
            finally:
                s.close()
        ...
        # JSON socket message
            ...
            msg = xcyber360_socket.create_xcyber360_socket_message(origin={'module': common.origin_module.get()},
                                                           command=GETCONFIG_COMMAND,
                                                           parameters={'section': configuration})

            # Socket connection
            try:
                s = xcyber360_socket.Xcyber360SocketJSON(dest_socket)
            except Xcyber360InternalError:
                raise
            except Exception as unhandled_exc:
                raise Xcyber360InternalError(1121, extra_message=str(unhandled_exc))
    ...
    if rec_error == 'ok' or rec_error == 0:
        ...
        if data.get('auth', {}).get('use_password') == 'yes':
            try:
                with open(os_path.join(common.XCYBER360_PATH, "etc", "authd.pass"), 'r') as f:
        ...
        raise Xcyber360Error(1117 if "No such file or directory" in rec_data or "Cannot send request" in rec_data else 1116,
...
def update_check_is_enabled() -> bool:
    ...
    """
    try:
        config_value = xcyber360.core.config.client.CentralizedConfig.get_server_config().cti.update_check
        return config_value
...
def get_cti_url() -> str:
    ...
    """
    try:
        return xcyber360.core.config.client.CentralizedConfig.get_server_config().cti.url
    except Xcyber360Error as e:
        if e.code != 1106:
            raise e
