...
from xcyber360.core.exception import Xcyber360Error, Xcyber360Exception, Xcyber360InternalError, Xcyber360ResourceNotFound
from xcyber360.core.indexer import get_indexer_client
from xcyber360.core.indexer.base import IndexerKey
from xcyber360.core.indexer.models.agent import Host as IndexerAgentHost
from xcyber360.core.indexer.models.commands import ResponseResult
from xcyber360.core.indexer.commands import create_restart_command, create_set_group_command, create_update_group_command
...
from xcyber360.core.utils import (
    full_copy,
    get_hash,
    process_array,
    get_group_file_path
...
# The rest of codes generated by the socket will be raised as Xcyber360 internal errors (500)
...
def build_agents_query(agent_list: list, filters: dict) -> dict:
    ...
    """
    # TODO: The query build should be improved in https://github.com/xcyber360/xcyber360/issues/25289
...
def get_distinct_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
                        search: dict = None, fields: list = None, q: str = None) -> AffectedItemsXcyber360Result:
    ...
    result = AffectedItemsXcyber360Result(all_msg='All selected agents information was returned',
    ...
    if agent_list:
        ...
        with Xcyber360DBQueryGroupByAgents(filter_fields=fields, select=fields, offset=offset, limit=limit, sort=sort,
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agents_summary_status(agent_list: list[str] = None) -> Xcyber360Result:
    ...
    if agent_list:
        ...
        with Xcyber360DBQueryAgents(limit=None, select=['status', 'group_config_status'],
    ...
    return Xcyber360Result({'data': {'connection': connection, 'configuration': sync_configuration}})


@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agents_summary_os(agent_list: list[str] = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Affected items.
    """
    result = AffectedItemsXcyber360Result(none_msg='Could not get the operative system of the agents',
                                      all_msg='Showing the operative system of all specified agents',
                                      some_msg='Could not get the operative system of some agents')
    if agent_list:
        rbac_filters = get_rbac_filters(system_resources=get_agents_info(), permitted_resources=agent_list)

        with Xcyber360DBQueryAgents(select=['os.platform'], default_sort_field='os_platform', min_select_fields=set(),
...
def reconnect_agents(agent_list: Union[list, str] = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Affected items.
    """
    result = AffectedItemsXcyber360Result(all_msg='Force reconnect command was sent to all agents',
...
async def restart_agents(agent_list: list) -> AffectedItemsXcyber360Result:
    ...
    async with get_indexer_client() as indexer_client:
        ...
        if len(agent_list) == 0:
            ...
            for not_found_id in set(agent_list) - set(available_agents):
                result.add_failed_item(not_found_id, error=Xcyber360ResourceNotFound(1701))
        ...
        for agent_id in agent_list:
            ...
            if response.result is ResponseResult.CREATED:
                ...
                result.add_failed_item(id_=agent_id, error=Xcyber360Error(1762, extra_message=response.result.value))
...
def restart_agents_by_node(agent_list: list = None) -> AffectedItemsXcyber360Result:
...
async def get_agents_in_group(group_list: list, offset: int = 0, limit: int = None, sort_by: list = None,
                     ...
                     q: str = None, select: str = None, distinct: bool = False) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Affected items.
    """
    result = AffectedItemsXcyber360Result(all_msg='All selected groups information was returned',
...
def get_agents_keys(agent_list: list = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Affected items.
    """
    result = AffectedItemsXcyber360Result(all_msg='Obtained keys for all selected agents',
...
async def delete_agents(agent_list: list, filters: Optional[dict] = None,) -> AffectedItemsXcyber360Result:
...
async def add_agent(
    ...
    if len(name) > common.AGENT_NAME_LEN_LIMIT:
        raise Xcyber360Error(1738)

    for group in groups:
        if not Agent.group_exists(group):
            raise Xcyber360Error(1710, extra_message=group)
    ...
    async with get_indexer_client() as indexer_client:
        ...
        if response.result is not ResponseResult.CREATED:
            raise Xcyber360Error(1762, extra_message=response.result.value)

    return Xcyber360Result({'data': new_agent})
...
async def get_agent_groups(group_list: list = None, offset: int = 0, limit: int = None, sort_by: list = None,
    ...
    if group_list:
        ...
        for invalid_group in set(group_list) - system_groups:
            ...
            result.add_failed_item(id_=invalid_group, error=Xcyber360ResourceNotFound(1710))
...
async def create_group(group_id: str) -> Xcyber360Result:
    """Creates a group.
    ...
    -------
    Xcyber360Result
        Xcyber360Result object with a operation message.
    """
    ...
    if group_id.lower() == "agent-template":
        raise Xcyber360Error(1713, extra_message=group_id)

    group_path = get_group_file_path(group_id)

    if group_id.lower() == "default" or path.exists(group_path):
        raise Xcyber360Error(1711, extra_message=group_id)
    ...
    try:
        ...
        chown(group_path, common.xcyber360_uid(), common.xcyber360_gid())
        chmod(group_path, 0o660)
        msg = f"Group '{group_id}' created."
    except Exception as e:
        raise Xcyber360InternalError(1005, extra_message=str(e))

    return Xcyber360Result({'message': msg})


@expose_resources(actions=["group:delete"], resources=["group:id:{group_list}"],
                  post_proc_kwargs={'exclude_codes': [1710, 1712]})
async def delete_groups(group_list: list = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Affected items.
    """
    result = AffectedItemsXcyber360Result(all_msg='All selected groups were deleted',
...
async def assign_agents_to_group(group_list: list = None, agent_list: list = None, replace: bool = False,
                           replace_list: list = None) -> AffectedItemsXcyber360Result:
    ...
    """
    group_id = group_list[0]
    result = AffectedItemsXcyber360Result(all_msg=f'All selected agents were assigned to {group_id}'
    ...
    async with get_indexer_client() as indexer_client:
        ...
        for i, agent in enumerate(available_agents):
            if group_id in agent.groups:
                result.add_failed_item(id_=agent.id, error=Xcyber360Error(1766))
...
async def remove_agent_from_groups(agent_list: list = None, group_list: list = None) -> AffectedItemsXcyber360Result:
    ...
    result = AffectedItemsXcyber360Result(all_msg='Specified agent was removed from returned groups',
...
async def remove_agents_from_group(agent_list: list = None, group_list: list = None) -> AffectedItemsXcyber360Result:
    ...
    """
    group_id = group_list[0]
    result = AffectedItemsXcyber360Result(all_msg=f'All selected agents were removed from group {group_id}',
...
def get_outdated_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
                        search: dict = None, select: str = None, q: str = None) -> AffectedItemsXcyber360Result:
    ...
    if agent_list:
        ...
        with Xcyber360DBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
...
def upgrade_agents(agent_list: list = None, wpk_repo: str = None, version: str = None, force: bool = False,
                   ...
                   filters: dict = None, q: str = None) -> AffectedItemsXcyber360Result:
    ...
    """
    result = AffectedItemsXcyber360Result(all_msg='All upgrade tasks were created',
    ...
    if agent_list:
        ...
        with Xcyber360DBQueryAgents(limit=None, select=["id", "status"], query=q, **rbac_filters) as db_query:
            data = db_query.run()
        ...
        [result.add_failed_item(id_=agent, error=Xcyber360Error(1707)) for agent in non_active_agents]
        ...
        [result.add_failed_item(id_=ag, error=Xcyber360Error(
            1731,
        ...
        for agent_result_chunk in tasks_results:
            for agent_result in agent_result_chunk['data']:
                ...
                # Upgrade error for specific agents
                    ...
                    error = Xcyber360Error(error_code, cmd_error=True, extra_message=agent_result['message'])
                ...
                # Upgrade error for all agents, bad request
                    ...
                    raise Xcyber360Error(error_code, cmd_error=True, extra_message=agent_result['message'])

                # Upgrade error for all agents, internal server error
                else:
                    raise Xcyber360InternalError(error_code, cmd_error=True, extra_message=agent_result['message'])
...
def get_upgrade_result(agent_list: list = None, filters: dict = None, q: str = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Upgrade results.
    """
    result = AffectedItemsXcyber360Result(all_msg='All upgrade tasks were returned',
        ...
        for task_result_chunk in task_results:
            for task_result in task_result_chunk['data']:
                ...
                # Upgrade error for specific agents (no task in DB)
                    ...
                    error = Xcyber360Error(error_code, cmd_error=True, extra_message=task_result['message'])
                    result.add_failed_item(id_=str(task_result['agent']).zfill(3), error=error)

                # Upgrade error for all agents, internal server error
                else:
                    raise Xcyber360InternalError(error_code, cmd_error=True, extra_message=task_result['message'])
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agent_config(agent_list: list = None, component: str = None, config: str = None) -> Xcyber360Result:
    ...
    if my_agent.status != "active":
        raise Xcyber360Error(1740)

    return Xcyber360Result(
        {'data': my_agent.get_config(component=component, config=config, agent_version=my_agent.version)})


@expose_resources(actions=["group:read"], resources=["group:id:{group_list}"], post_proc_func=None)
async def get_group_conf(group_list: list = None) -> Xcyber360Result:
...
async def update_group_file(group_list: list = None, file_data: str = None) -> Xcyber360Result:
    ...
    -------
    Xcyber360Result
        Xcyber360Result object with the confirmation message.
    """
    # We access unique group_id from list, this may change if and when we decide to add option to update files for
    # a list of groups
    group_id = group_list[0]

    return Xcyber360Result({'message': configuration.update_group_file(group_id, file_data)})


def get_full_overview() -> Xcyber360Result:
    """Get information about agents.

    Returns
    -------
    Xcyber360Result
        Xcyber360Result object with information about agents.
    """
    ...
    return Xcyber360Result({'data': result})
