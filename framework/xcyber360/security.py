...
from xcyber360.core.security import invalid_users_tokens, invalid_roles_tokens, invalid_run_as_tokens, revoke_tokens, \
...
from xcyber360.rbac.orm import AuthenticationManager, PoliciesManager, RolesManager, RolesPoliciesManager
from xcyber360.rbac.orm import SecurityError, MAX_ID_RESERVED
from xcyber360.rbac.orm import UserRolesManager, RolesRulesManager, RulesManager
...
def get_user_me(token: dict) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        User information.
    """
    result = AffectedItemsXcyber360Result(all_msg='Current user information was returned')
    affected_items = list()
...
async def get_users(user_ids: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
              ...
              q: str = None, distinct: bool = False) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        User information.
    """
    result = AffectedItemsXcyber360Result(none_msg='No user was returned',
    ...
    with AuthenticationManager() as auth:
        for user_id in user_ids:
            ...
            affected_items.append(user) if user else result.add_failed_item(id_=user_id, error=Xcyber360Error(5001))
...
async def edit_run_as(user_id: str = None, allow_run_as: bool = False) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Status message.
    """
    result = AffectedItemsXcyber360Result(none_msg=f"The parameter allow_run_as could not be "
    ...
    with AuthenticationManager() as auth:
        ...
        if query is False:
            result.add_failed_item(id_=user_id, error=Xcyber360Error(5001))
        elif query == SecurityError.INVALID:
            result.add_failed_item(id_=user_id, error=Xcyber360Error(5010))
        else:
...
async def create_user(username: str = None, password: str = None) -> AffectedItemsXcyber360Result:
    """Create a new user.
    ...
    if len(password) > 64 or len(password) < 8:
        raise Xcyber360Error(5009)
    elif not _user_password.match(password):
        raise Xcyber360Error(5007)

    result = AffectedItemsXcyber360Result(none_msg='User could not be created',
    ...
    with AuthenticationManager() as auth:
        if auth.add_user(username, password):
            ...
            if operation:
                ...
                result.add_failed_item(id_=username, error=Xcyber360Error(5000))
        else:
            result.add_failed_item(id_=username, error=Xcyber360Error(5000))
...
async def update_user(user_id: str = None, password: str = None, current_user: str = None) -> AffectedItemsXcyber360Result:
    ...
    if password:
        ...
        if int(user_id[0]) <= MAX_ID_RESERVED and current_user is not None:
            ...
            if current_user_id > MAX_ID_RESERVED:
                raise Xcyber360Error(5011)

    result = AffectedItemsXcyber360Result(all_msg='User was successfully updated',
                                      none_msg='User could not be updated')
    with AuthenticationManager() as auth:
        query = auth.update_user(int(user_id[0]), password)
        if query is False:
            result.add_failed_item(id_=int(user_id[0]), error=Xcyber360Error(5001))
        else:
...
async def remove_users(user_ids: list) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Status message.
    """
    result = AffectedItemsXcyber360Result(none_msg='No user was deleted',
    ...
    with AuthenticationManager() as auth:
        for user_id in user_ids:
            ...
            if not isinstance(current_user, bool) and user_id == int(current_user['id']) and user_id > MAX_ID_RESERVED:
                result.add_failed_item(id_=user_id, error=Xcyber360Error(5008))
                continue
            user = auth.get_user_id(user_id)
            ...
            if not query:
                ...
                result.add_failed_item(id_=user_id, error=Xcyber360Error(5004))
            elif query == SecurityError.RELATIONSHIP_ERROR:
                result.add_failed_item(id_=user_id, error=Xcyber360Error(4025))
            elif user:
                invalid_users_tokens(users=[user_id])
...
async def get_roles(role_ids: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    """
    affected_items = list()
    result = AffectedItemsXcyber360Result(none_msg='No role was returned',
    ...
    with RolesManager() as rm:
        for r_id in role_ids:
            ...
            if role != SecurityError.ROLE_NOT_EXIST:
                ...
                result.add_failed_item(id_=int(r_id), error=Xcyber360Error(4002))
...
async def remove_roles(role_ids: list) -> AffectedItemsXcyber360Result:
    ...
    """
    result = AffectedItemsXcyber360Result(none_msg='No role was deleted',
    ...
    with RolesManager() as rm:
        for r_id in role_ids:
            ...
            if role_delete == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(r_id), error=Xcyber360Error(4008))
            elif role_delete == SecurityError.RELATIONSHIP_ERROR:
                result.add_failed_item(id_=int(r_id), error=Xcyber360Error(4025))
...
async def add_role(name: str = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Result of the operation.
    """
    result = AffectedItemsXcyber360Result(none_msg='Role was not created',
                                      all_msg='Role was successfully created')
    with RolesManager() as rm:
        status = rm.add_role(name=name)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=name, error=Xcyber360Error(4005))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=name, error=Xcyber360Error(4003))
        else:
...
async def update_role(role_id: str = None, name: str = None) -> AffectedItemsXcyber360Result:
    ...
    if name is None:
        raise Xcyber360Error(4001)
    result = AffectedItemsXcyber360Result(none_msg='Role was not updated',
                                      all_msg='Role was successfully updated')
    with RolesManager() as rm:
        status = rm.update_role(role_id=role_id[0], name=name)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=int(role_id[0]), error=Xcyber360Error(4005))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=int(role_id[0]), error=Xcyber360Error(4003))
        elif status == SecurityError.ROLE_NOT_EXIST:
            result.add_failed_item(id_=int(role_id[0]), error=Xcyber360Error(4002))
        elif status == SecurityError.ADMIN_RESOURCES:
            result.add_failed_item(id_=int(role_id[0]), error=Xcyber360Error(4008))
        else:
...
async def get_policies(policy_ids: list, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    """
    result = AffectedItemsXcyber360Result(none_msg='No policy was returned',
    ...
    with PoliciesManager() as pm:
        for p_id in policy_ids:
            ...
            if policy != SecurityError.POLICY_NOT_EXIST:
                ...
                result.add_failed_item(id_=int(p_id), error=Xcyber360Error(4007))
...
async def remove_policies(policy_ids: list = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Result of operation.
    """
    result = AffectedItemsXcyber360Result(none_msg='No policy was deleted',
    ...
    with PoliciesManager() as pm:
        for p_id in policy_ids:
            ...
            if policy_delete == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(p_id), error=Xcyber360Error(4008))
            elif policy_delete == SecurityError.RELATIONSHIP_ERROR:
                result.add_failed_item(id_=int(p_id), error=Xcyber360Error(4025))
...
async def add_policy(name: str = None, policy: dict = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Result of the operation.
    """
    result = AffectedItemsXcyber360Result(none_msg='Policy was not created',
                                      all_msg='Policy was successfully created')
    sanitize_rbac_policy(policy)
    with PoliciesManager() as pm:
        status = pm.add_policy(name=name, policy=policy)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=name, error=Xcyber360Error(4009))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=name, error=Xcyber360Error(4006))
        else:
...
async def update_policy(policy_id: str = None, name: str = None, policy: dict = None) -> AffectedItemsXcyber360Result:
    ...
    if name is None and policy is None:
        raise Xcyber360Error(4001)
    result = AffectedItemsXcyber360Result(none_msg='Policy was not updated',
    ...
    with PoliciesManager() as pm:
        ...
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=int(policy_id[0]), error=Xcyber360Error(4013))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=int(policy_id[0]), error=Xcyber360Error(4006))
        elif status == SecurityError.POLICY_NOT_EXIST:
            result.add_failed_item(id_=int(policy_id[0]), error=Xcyber360Error(4007))
        elif status == SecurityError.ADMIN_RESOURCES:
            result.add_failed_item(id_=int(policy_id[0]), error=Xcyber360Error(4008))
        else:
...
async def get_rules(rule_ids: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    """
    affected_items = list()
    result = AffectedItemsXcyber360Result(none_msg='No security rule was returned',
    ...
    with RulesManager() as rum:
        for ru_id in rule_ids:
            ...
            if rule != SecurityError.RULE_NOT_EXIST:
                ...
                result.add_failed_item(id_=ru_id, error=Xcyber360Error(4022))
...
@expose_resources(actions=['security:create'], resources=['*:*:*'])
async def add_rule(name: str = None, rule: dict = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Result of the operation.
    """
    result = AffectedItemsXcyber360Result(none_msg='Security rule was not created',
...
async def remove_rules(rule_ids: list = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Result of the operation.
    """
    result = AffectedItemsXcyber360Result(none_msg='No security rule was deleted',
    ...
    with RulesManager() as rum:
        for r_id in rule_ids:
            ...
            if role_delete == SecurityError.ADMIN_RESOURCES:
                ...
                result.add_failed_item(id_=int(r_id), error=Xcyber360Error(4022))
            elif rule:
...
async def update_rule(rule_id: str = None, name: str = None, rule: dict = None) -> AffectedItemsXcyber360Result:
    ...
    if name is None and rule is None:
        raise Xcyber360Error(4001)
    result = AffectedItemsXcyber360Result(none_msg='Security rule was not updated',
                                      all_msg='Security rule was successfully updated')
    with RulesManager() as rum:
        status = rum.update_rule(rule_id=rule_id[0], name=name, rule=rule)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=int(rule_id[0]), error=Xcyber360Error(4005))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=int(rule_id[0]), error=Xcyber360Error(4003))
        elif status == SecurityError.RULE_NOT_EXIST:
            result.add_failed_item(id_=int(rule_id[0]), error=Xcyber360Error(4022))
        elif status == SecurityError.ADMIN_RESOURCES:
            result.add_failed_item(id_=int(rule_id[0]), error=Xcyber360Error(4008))
        else:
...
def get_username(user_id: str) -> AffectedItemsXcyber360Result:
...
async def set_user_role(user_id: list, role_ids: list, position: int = None) -> AffectedItemsXcyber360Result:
    ...
    if position is not None and position < 0:
        raise Xcyber360Error(4018)

    username = get_username(user_id=user_id)
    result = AffectedItemsXcyber360Result(none_msg=f'No link was created to user {username}',
    ...
    with UserRolesManager() as urm:
        for role_id in role_ids:
            ...
            if user_role == SecurityError.ALREADY_EXIST:
                result.add_failed_item(id_=int(role_id), error=Xcyber360Error(4017))
            elif user_role == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id), error=Xcyber360Error(4002))
            elif user_role == SecurityError.USER_NOT_EXIST:
                result.add_failed_item(id_=int(user_id[0]), error=Xcyber360Error(5001))
                break
            elif user_role == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(user_id[0]), error=Xcyber360Error(4008))
            else:
                success = True
                result.total_affected_items += 1
...
async def remove_user_role(user_id: str, role_ids: list) -> AffectedItemsXcyber360Result:
    ...
    Raises
    ------
    Xcyber360ResourceNotFound(5001)
    ...
    username = get_username(user_id=user_id)
    if username == 'unknown':
        raise Xcyber360ResourceNotFound(5001)

    result = AffectedItemsXcyber360Result(none_msg=f'No role was unlinked from user {username}',
    ...
    with UserRolesManager() as urm:
        for role_id in role_ids:
            ...
            if user_role == SecurityError.INVALID:
                result.add_failed_item(id_=int(role_id), error=Xcyber360Error(4016))
            elif user_role == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id), error=Xcyber360Error(4002))
            elif user_role == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(user_id[0]), error=Xcyber360Error(4008))
...
async def set_role_rule(role_id: str, rule_ids: list, run_as: bool = False) -> AffectedItemsXcyber360Result:
    ...
    result = AffectedItemsXcyber360Result(none_msg=f'No link was created to role {role_id[0]}',
    ...
    with RolesRulesManager() as rrm:
        for rule_id in rule_ids:
            ...
            if role_rule == SecurityError.ALREADY_EXIST:
                result.add_failed_item(id_=int(rule_id), error=Xcyber360Error(4023))
            elif role_rule == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id[0]), error=Xcyber360Error(4002))
            elif role_rule == SecurityError.RULE_NOT_EXIST:
                result.add_failed_item(id_=int(rule_id), error=Xcyber360Error(4022))
...
async def remove_role_rule(role_id: str, rule_ids: list) -> AffectedItemsXcyber360Result:
    ...
    Raises
    ------
    Xcyber360ResourceNotFound(4002)
    ...
    if not role:
        raise Xcyber360ResourceNotFound(4002)

    result = AffectedItemsXcyber360Result(none_msg=f'No security rule was unlinked from role {role_id[0]}',
    ...
    with RolesRulesManager() as rrm:
        for rule_id in rule_ids:
            ...
            if role_rule == SecurityError.INVALID:
                result.add_failed_item(id_=rule_id, error=Xcyber360Error(4024))
            elif role_rule == SecurityError.RULE_NOT_EXIST:
                result.add_failed_item(id_=rule_id, error=Xcyber360Error(4022))
            elif role_rule == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(role_id[0]), error=Xcyber360Error(4008))
            else:
                success = True
...
async def set_role_policy(role_id: str, policy_ids: list, position: int = None) -> AffectedItemsXcyber360Result:
    ...
    with RolesPoliciesManager() as rpm:
        for policy_id in policy_ids:
            ...
            if role_policy == SecurityError.ALREADY_EXIST:
                result.add_failed_item(id_=policy_id, error=Xcyber360Error(4011))
            elif role_policy == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id[0]), error=Xcyber360Error(4002))
            elif role_policy == SecurityError.POLICY_NOT_EXIST:
                result.add_failed_item(id_=policy_id, error=Xcyber360Error(4007))
...
async def remove_role_policy(role_id: str, policy_ids: list) -> AffectedItemsXcyber360Result:
    ...
    result = AffectedItemsXcyber360Result(none_msg=f'No policy was unlinked from role {role_id[0]}',
    ...
    with RolesPoliciesManager() as rpm:
        for policy_id in policy_ids:
            ...
            if role_policy == SecurityError.INVALID:
                result.add_failed_item(id_=policy_id, error=Xcyber360Error(4010))
            elif role_policy == SecurityError.POLICY_NOT_EXIST:
                result.add_failed_item(id_=policy_id, error=Xcyber360Error(4007))
            elif role_policy == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(role_id[0]), error=Xcyber360Error(4008))
...
def revoke_current_user_tokens() -> Xcyber360Result:
    ...
    return Xcyber360Result({'message': f'User {common.current_user.get()} was successfully logged out'})
...
async def wrapper_revoke_tokens() -> Xcyber360Result:
    """Revoke all tokens.
    ...
    return Xcyber360Result({'message': 'Tokens were successfully revoked'})
...
@lru_cache(maxsize=None)
def get_rbac_resources(resource: str = None) -> Xcyber360Result:
    ...
    """
    if not resource:
        return Xcyber360Result({'data': load_spec()['x-rbac-catalog']['resources']})
    else:
        if resource not in load_spec()['x-rbac-catalog']['resources'].keys():
            raise Xcyber360Error(4019)
        return Xcyber360Result({'data': {resource: load_spec()['x-rbac-catalog']['resources'][resource]}})


@lru_cache(maxsize=None)
def get_rbac_actions(endpoint: str = None) -> Xcyber360Result:
    ...
    if endpoint and endpoint not in endpoints_list:
        raise Xcyber360Error(4020, extra_remediation=endpoints_list)
    info_data = load_spec()
    data = dict()
    for path, path_info in info_data['paths'].items():
    ...
    return Xcyber360Result({'data': data})


@expose_resources(actions=['security:read_config'], resources=['*:*:*'])
async def get_security_config() -> Xcyber360Result:
    ...
    """
    return Xcyber360Result({'data': get_security_conf()})
