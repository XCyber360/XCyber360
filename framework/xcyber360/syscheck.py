...
from xcyber360.core.exception import Xcyber360InternalError, Xcyber360Error, Xcyber360ResourceNotFound
from xcyber360.core.results import AffectedItemsXcyber360Result
from xcyber360.core.syscheck import Xcyber360DBQuerySyscheck, syscheck_delete_agent
...
def run(agent_list: Union[str, None] = None) -> AffectedItemsXcyber360Result:
    ...
    """
    result = AffectedItemsXcyber360Result(all_msg='Syscheck scan was restarted on returned agents',
    ...
    with Xcyber360DBQueryAgents(limit=None, select=["id", "status"], query='status!=active', **rbac_filters) as db_query:
...
def clear(agent_list: list = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Confirmation/Error message.
    """
    result = AffectedItemsXcyber360Result(all_msg='Syscheck database was cleared on returned agents',
    ...
    list(map(lambda ag: result.add_failed_item(id_=ag, error=Xcyber360ResourceNotFound(1701)), not_found_agents))

    wdb_conn = None
    rbac_filters = get_rbac_filters(system_resources=system_agents, permitted_resources=agent_list)
    db_query = Xcyber360DBQueryAgents(select=["id", "version"], **rbac_filters)
    data = db_query.run()
    ...
    for item in data['items']:
        ...
        if agent_version is not None:
            if Xcyber360Version(agent_version) < Xcyber360Version('v3.12.0'):
                try:
                    if wdb_conn is None:
                ...
                result.add_failed_item(id_=agent_id,
                                       error=Xcyber360Error(1760, extra_message="Agent version should be < v3.12.0."))
        else:
            result.add_failed_item(id_=agent_id, error=Xcyber360Error(1015))
...
def last_scan(agent_list: list) -> AffectedItemsXcyber360Result:
    ...
    result = AffectedItemsXcyber360Result(all_msg='Last syscheck scan of the agent was returned',
    ...
    with Xcyber360DBQuerySyscheck(agent_id=agent_list[0], query='module=fim', offset=0, sort=None,
...
def files(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
          ...
          summary: bool = False, distinct: bool = False) -> AffectedItemsXcyber360Result:
    ...
    result = AffectedItemsXcyber360Result(all_msg='FIM findings of the agent were returned',
    ...
    with Xcyber360DBQuerySyscheck(agent_id=agent_list[0], offset=offset, limit=limit, sort=sort, search=search,
