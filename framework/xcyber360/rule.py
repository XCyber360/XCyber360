...
from xcyber360.core.rule import (check_status, load_rules_from_file, REQUIRED_FIELDS,
...
def get_rules(rule_ids: list = None, status: str = None, group: str = None, pci_dss: str = None, gpg13: str = None,
    ...
    """
    result = AffectedItemsXcyber360Result(none_msg='No rule was returned',
    ...
    for rule_id in no_existent_ids:
        result.add_failed_item(id_=rule_id, error=Xcyber360Error(1208))
...
def get_rules_files(status: str = None, relative_dirname: str = None, filename: list = None, offset: int = 0,
    ...
    ------
    Xcyber360Error(1200)
        Error reading rules from `XCYBER360_HOME/etc/ossec.conf`.
...
def get_groups(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None, sort_ascending: bool = True,
               search_text: str = None, complementary_search: bool = False,
               search_in_fields: list = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Dictionary: {'items': array of items, 'totalItems': Number of items (without applying the limit)}
    """
    result = AffectedItemsXcyber360Result(none_msg='No groups in rules were returned',
...
def get_requirement(requirement: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    if requirement not in RULE_REQUIREMENTS:
        result.add_failed_item(id_=requirement,
                               error=Xcyber360Error(1205, extra_message=requirement,
...
def get_rule_file_path(filename: str = None, relative_dirname: str = None) -> str:
    ...
    if len(rules) == 0:
        ...
        return join(common.XCYBER360_PATH, rule['relative_dirname'], filename)
    else:
        return normpath(join(common.XCYBER360_PATH, rules[0]['relative_dirname'], filename))


def get_rule_file(filename: str = None, raw: bool = False,
                  relative_dirname: str = None) -> Union[str, AffectedItemsXcyber360Result]:
    ...
    Returns
    -------
    str or AffectedItemsXcyber360Result
        Content of the file. AffectedItemsXcyber360Result format if `raw=False`.
    """
    result = AffectedItemsXcyber360Result(none_msg='No rule was returned',
    ...
    if not full_path:
        result.add_failed_item(id_=filename,
                               error=Xcyber360Error(1415, extra_message=f"{filename}"))
        return result
    ...
    try:
        ...
        result.add_failed_item(id_=filename,
                               error=Xcyber360Error(1413, extra_message=f"{filename}: {str(exc)}"))
    except OSError:
        result.add_failed_item(id_=filename,
                               error=Xcyber360Error(1414, extra_message=f"{filename}"))
...
def upload_rule_file(filename: str, content: str, relative_dirname: str = None, 
                     overwrite: bool = False) -> AffectedItemsXcyber360Result:
...
def delete_rule_file(filename: Union[str, list], relative_dirname: str = None) -> AffectedItemsXcyber360Result:
    """Delete a rule file.
