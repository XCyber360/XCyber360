...
from xcyber360.core.sca import (
    Xcyber360DBQueryDistinctSCACheck, Xcyber360DBQuerySCA, Xcyber360DBQuerySCACheck, Xcyber360DBQuerySCACheckIDs,
    Xcyber360DBQuerySCACheckRelational, SCA_CHECK_COMPLIANCE_DB_FIELDS, SCA_CHECK_RULES_DB_FIELDS, SCA_CHECK_DB_FIELDS)
...
def get_sca_list(agent_list: list = None, q: str = "", offset: int = 0, limit: int = common.DATABASE_LIMIT,
                 ...
                 filters: dict = None) -> AffectedItemsXcyber360Result:
    ...
    AffectedItemsXcyber360Result
        Affected items.
    """
    result = AffectedItemsXcyber360Result(all_msg='All selected SCA information was returned',
    ...
    if len(agent_list) != 0:
        if agent_list[0] in get_agents_info():
            ...
            with Xcyber360DBQuerySCA(agent_id=agent_list[0], offset=offset, limit=limit, sort=sort, search=search,
            ...
            result.add_failed_item(id_=agent_list[0], error=Xcyber360ResourceNotFound(1701))
...
def get_sca_checks(policy_id: str = None, agent_list: list = None, q: str = "", offset: int = 0,
                   ...
                   filters: dict = None, distinct: bool = False) -> AffectedItemsXcyber360Result:
    ...
    Raises
    ------
    Xcyber360InternalError(2007)
        If there was an error retrieving data from Xcyber360 DB.
    ...
    AffectedItemsXcyber360Result
        Affected items.
    """
    result = AffectedItemsXcyber360Result(all_msg='All selected SCA/policy information was returned',
    ...
    if len(agent_list) != 0:
        if agent_list[0] in get_agents_info():
            ...
            if not distinct:
                ...
                with Xcyber360DBQuerySCACheckIDs(agent_id=agent_list[0], offset=offset, limit=limit, filters=filters,
                ...
                if id_check_list:
                    ...
                    if not select or select_compliance:
                        with Xcyber360DBQuerySCACheckRelational(
            ...
            # Workaround for distinct=True
                ...
                with Xcyber360DBQueryDistinctSCACheck(agent_id=agent_list[0], offset=offset, limit=limit, filters=filters,
