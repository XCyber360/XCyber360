...
from xcyber360.core.decoder import load_decoders_from_file, check_status, REQUIRED_FIELDS, SORT_FIELDS, DECODER_FIELDS
...
def get_decoders(names: list = None, status: str = None, filename: list = None, relative_dirname: str = None,
    ...
    AffectedItemsXcyber360Result
        Affected items.
    """
    result = AffectedItemsXcyber360Result(none_msg='No decoder was returned',
    ...
    for decoder_name in no_existent_files:
        result.add_failed_item(id_=decoder_name, error=Xcyber360Error(1504))
...
def get_decoder_file_path(filename: str,
    ...
    if len(decoders) == 0:
        ...
        return join(common.XCYBER360_PATH, decoder['relative_dirname'], filename)
    else:
        return normpath(join(common.XCYBER360_PATH, decoders[0]['relative_dirname'], filename))
...
def get_decoder_file(filename: str, raw: bool = False,
    ...
    if not full_path:
        result.add_failed_item(id_=filename,
                               error=Xcyber360Error(1503, extra_message=f"{filename}"))
        return result
    ...
    try:
        ...
        result.add_failed_item(id_=filename,
                               error=Xcyber360Error(1501, extra_message=f"{filename}: {str(exc)}"))
    except OSError:
        result.add_failed_item(id_=filename,
                               error=Xcyber360Error(1502, extra_message=f"{filename}"))
...
def delete_decoder_file(filename: Union[str, list], relative_dirname: str = None) -> AffectedItemsXcyber360Result:
