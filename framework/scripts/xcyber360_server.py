...
from xcyber360.core.common import BIN_ROOT, XCYBER360_SHARE, XCYBER360_LOG
from xcyber360.core.config.client import CentralizedConfig
from xcyber360.core.utils import clean_pid_files
from xcyber360.core.wlogging import Xcyber360Logger
from xcyber360.core.config.models.server import ServerConfig

BIN_PATH = '/bin'
SERVER_DAEMON_NAME = 'xcyber360-server'
COMMS_API_SCRIPT_PATH = XCYBER360_SHARE / 'apis' / 'scripts' / 'xcyber360_comms_apid.py'
COMMS_API_DAEMON_NAME = 'xcyber360-comms-apid'
EMBEDDED_PYTHON_PATH = XCYBER360_SHARE / 'framework' / 'python' / 'bin' / 'python3'
ENGINE_BINARY_PATH = BIN_ROOT / 'xcyber360-engine'
ENGINE_DAEMON_NAME = 'xcyber360-engined'
MANAGEMENT_API_SCRIPT_PATH = XCYBER360_SHARE / 'api' / 'scripts' / 'xcyber360_apid.py'
MANAGEMENT_API_DAEMON_NAME = 'xcyber360-apid'
CLUSTER_LOG = XCYBER360_LOG / 'cluster.log'
...
def set_logging(foreground_mode=False, debug_mode=0) -> Xcyber360Logger:
    """Set cluster logger.
...
def print_version():
    """Print Xcyber360 metadata."""
    from xcyber360.core.cluster import __author__, __licence__, __version__, __xcyber360_name__

    print(f'\n{__xcyber360_name__} {__version__} - {__author__}\n\n{__licence__}')
...
# Master main
#
async def master_main(args: argparse.Namespace, server_config: ServerConfig, logger: Xcyber360Logger):
    ...
    logger : Xcyber360Logger
        Cluster logger.
    """
    from xcyber360.core.cluster import local_server, master
...
# Worker main
#
async def worker_main(args: argparse.Namespace, server_config: ServerConfig, logger: Xcyber360Logger):
    ...
    from xcyber360.core.cluster import local_server, worker
    ...
    # Handle exception when the user running Xcyber360 cannot access /dev/shm
        ...
        main_logger.warning(
            "In order to take advantage of Xcyber360 4.3.0 cluster improvements, the directory '/dev/shm' must be "
            "accessible by the 'xcyber360' user. Check that this file has permissions to be accessed by all users. "
            'Changing the file permissions to 777 will solve this issue.'
        )
        main_logger.warning(
            'The Xcyber360 cluster will be run without the improvements added in Xcyber360 4.3.0 and higher versions.'
        )
        task_pool = None
...
def get_script_arguments() -> argparse.Namespace:
    ...
    start_parser = subparsers.add_parser('start', help='Start Xcyber360 server.')
    ####################################################################################################################
    ...
    stop_parser = subparsers.add_parser('stop', help='Stop Xcyber360 server.')
    stop_parser.set_defaults(func=stop, foreground=True)

    status_parser = subparsers.add_parser('status', help='Show the Xcyber360 server status.')
...
def main():
    """Main function of the xcyber360-clusterd script in charge of starting the cluster process."""
    import xcyber360.core.cluster.cluster
    from xcyber360.core.config.client import CentralizedConfig
    from xcyber360.core.authentication import generate_keypair, keypair_exists

    # Set correct permissions on cluster.log file
    if os.path.exists(CLUSTER_LOG):
        os.chown(CLUSTER_LOG, common.xcyber360_uid(), common.xcyber360_gid())
    ...
    xcyber360.core.cluster.cluster.clean_up()
    ...
    # Drop privileges to xcyber360
    if not args.root:
        os.setgid(common.xcyber360_gid())
        os.setuid(common.xcyber360_uid())
    ...
    try:
        ...
        main_logger.error("Directory '/tmp' needs read, write & execution " "permission for 'xcyber360' user")
    except Exception as e:
...
def stop():
    """Stop the Xcyber360 server running in background."""

    try:
        server_pid = pyDaemonModule.get_xcyber360_server_pid(SERVER_DAEMON_NAME)
    except StopIteration:
        main_logger.error('Xcyber360 server is not running.')
        sys.exit(1)
...
def status():
    """Show the status of the Xcyber360 server."""
