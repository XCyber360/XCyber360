...
# Set framework path
path.append(dirname(argv[0]) + '/../framework')  # It is necessary to import Xcyber360 package

# Import framework
try:
    import xcyber360.agent
    from xcyber360.agent import upgrade_agents, get_upgrade_result, get_agents
    from xcyber360.core import common
    from xcyber360.core.exception import Xcyber360Error
    from xcyber360.core.cluster import utils as cluster_utils
    from xcyber360.core.wlogging import CLIFilter
except Exception as e:
    print("Error importing 'Xcyber360' package.\n\n{0}\n".format(e))
    exit()
...
def get_script_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("-v", "--version", type=str, help="Version to upgrade. [Default: latest Xcyber360 version]")
...
def list_outdated():
    ...
    agents = xcyber360.agent.get_outdated_agents()
...
async def main():
    ...
    except Xcyber360Error as xcyber360_err:
        print(f"Error {xcyber360_err.code}: {xcyber360_err.message}")
        if args.debug:
            raise
