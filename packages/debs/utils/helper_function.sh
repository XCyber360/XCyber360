...
get_package_and_checksum(){
    xcyber360_version="$1"
    short_commit_hash="$2"
    base_name="xcyber360-server_${xcyber360_version}-${REVISION}"
    symbols_base_name="xcyber360-server-dbg_${xcyber360_version}-${REVISION}"
    ...
    if [[ "${checksum}" == "yes" ]]; then
        cd ${pkg_path} && sha512sum xcyber360-server_*deb > /var/local/xcyber360/${deb_file}.sha512
        sha512sum xcyber360-server-dbg_*deb > /var/local/xcyber360/${symbols_deb_file}.sha512
    fi
    ls -lh ${pkg_path}
    find ${pkg_path} -type f -name "xcyber360-server_*deb" -exec mv {} /var/local/xcyber360/${deb_file} \;
    find ${pkg_path} -type f -name "xcyber360-server-dbg_*deb" -exec mv {} /var/local/xcyber360/${symbols_deb_file} \;
}
