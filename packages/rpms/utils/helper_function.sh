...
setup_build(){
    ...
    cp ${specs_path}/xcyber360-server.spec ${rpm_build_dir}/SPECS/${package_name}.spec
...
build_package(){
    ...
    $rpmbuild --define "_sysconfdir /etc" --define "_topdir ${rpm_build_dir}" \
        ...
        --define "_version ${xcyber360_version}" --define "_hashcommit ${short_commit_hash}" \
...
get_package_and_checksum(){
    src="$3"
    RPM_NAME=$(ls -R ${rpm_build_dir}/RPMS | grep "xcyber360-server" | grep -v "debuginfo")
    SRC_NAME=$(ls -R ${rpm_build_dir}/SRPMS | grep "\.src\.rpm$")
    SYMBOLS_NAME=$(ls -R ${rpm_build_dir}/RPMS)
    SYMBOLS_NAME=$(echo "${SYMBOLS_NAME}" | grep "xcyber360-server-debuginfo" || true)

    echo "RPM_NAME: ${RPM_NAME}"
    echo "SYMBOLS_NAME:${SYMBOLS_NAME}"

    if [[ "${checksum}" == "yes" ]]; then
        cd "${rpm_build_dir}/RPMS" && sha512sum $RPM_NAME > /var/local/xcyber360/$RPM_NAME.sha512
        if [[ "${src}" == "yes" ]]; then
            cd "${rpm_build_dir}/SRPMS" && sha512sum $SRC_NAME > /var/local/xcyber360/$SRC_NAME.sha512
        fi

        if [ -n "${SYMBOLS_NAME}" ]; then
            sha512sum $SYMBOLS_NAME > /var/local/xcyber360/$SYMBOLS_NAME.sha512
        fi
    fi

    if [[ "${src}" == "yes" ]]; then
        mv ${rpm_build_dir}/SRPMS/$SRC_NAME /var/local/xcyber360
    else
        mv ${rpm_build_dir}/RPMS/$RPM_NAME /var/local/xcyber360
        if [ -n "${SYMBOLS_NAME}" ]; then
            mv ${rpm_build_dir}/RPMS/$SYMBOLS_NAME /var/local/xcyber360
        fi
    fi
}
