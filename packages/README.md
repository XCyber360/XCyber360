## Xcyber360 Package Builder Script

This script automates the process of building Xcyber360 packages (manager or agent) for various architectures within a Docker container.
...
| *--sources    | Path containing local Xcyber360 source code (optional)       | script path            |
...
./xcyber360_package_builder.sh -t manager -a amd64 -s /tmp --system rpm

2. Build a debug agent package for i386 architecture with checksum generation:
./xcyber360_package_builder.sh -t agent -a i386 -s /tmp -d -c --system rpm

3. Build a legacy RPM package for CentOS 5 (agent):
./xcyber360_package_builder.sh -t agent -l -s /tmp --system rpm

4. Build a package using local Xcyber360 source code:
./xcyber360_package_builder.sh -t manager -a amd64 --sources /path/to/xcyber360/source --system rpm
...
```bash
curl -L -X POST -H "Accept: application/vnd.github+json" -H "Authorization: Bearer $GH_WORKFLOW_TOKEN" -H "X-GitHub-Api-Version: 2022-11-28" --data-binary "@$(pwd)/xcyber360-agent-test-amd64-rpm.json" "https://api.github.com/repos/xcyber360/xcyber360/actions/workflows/packages-upload-agent-images-amd.yml/dispatches"
```

Where the JSON looks like this:

```json
# cat xcyber360-agent-test-amd64-rpm.json
{
    "ref":"4.9.0",
    "inputs":
        {
...
```json
curl -L -X POST -H "Accept: application/vnd.github+json" -H "Authorization: Bearer $GH_WORKFLOW_TOKEN" -H "X-GitHub-Api-Version: 2022-11-28" --data-binary "@$(pwd)/xcyber360-agent-test-amd64-rpm.json" "https://api.github.com/repos/xcyber360/xcyber360/actions/workflows/packages-build-linux-agent-amd.yml/dispatches"
```
