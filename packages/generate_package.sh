...
XCYBER360_PATH="$(cd $CURRENT_PATH/..; pwd -P)"
ARCHITECTURE="amd64"
SYSTEM="deb"
...
clean() {
    ...
    # Clean the files
    find "${DOCKERFILE_PATH}" \( -name '*.sh' -o -name '*.tar.gz' -o -name 'xcyber360-*' \) ! -name 'docker_builder.sh' -exec rm -rf {} +
...
build_pkg() {
    ...
    docker run -t --rm -v ${OUTDIR}:/var/local/xcyber360:Z \
        -e SYSTEM="$SYSTEM" \
        -e ARCHITECTURE_TARGET="${ARCHITECTURE}" \
        -e INSTALLATION_PATH="${INSTALLATION_PATH}" \
        -e IS_STAGE="${IS_STAGE}" \
        -e XCYBER360_BRANCH="${BRANCH}" \
        ${CUSTOM_CODE_VOL} \
...
help() {
    ...
    echo "    -p, --path <path>          [Optional] Installation path for the package. By default: /var/xcyber360-server."
    ...
    echo "    --sources <path>           [Optional] Absolute path containing xcyber360 source code. This option will use local source code instead of downloading it from GitHub. By default use the script path."
...
main() {
    ...
    do
        ...
        "--sources")
            if [ -n "$2" ]; then
               CUSTOM_CODE_VOL="-v $2:/xcyber360-local-src:Z"
               shift 2
            else
                help 1
            fi
    ...
    if [ -z "${CUSTOM_CODE_VOL}" ]; then
        CUSTOM_CODE_VOL="-v $XCYBER360_PATH:/xcyber360-local-src:Z"
    fi
