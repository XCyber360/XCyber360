...
XCYBER360_CONFIG_MOUNT=/xcyber360-config-mount
...
function runOpensearchDashboards {
    ...
    /usr/share/xcyber360-dashboard/bin/opensearch-dashboards -c $OPENSEARCH_DASHBOARDS_HOME/config/opensearch_dashboards.yml \
        --cpu.cgroup.path.override=/ \
        --cpuacct.cgroup.path.override=/
}

mount_files() {
  if [ -e $XCYBER360_CONFIG_MOUNT/* ]
  then
    print "Identified Xcyber360 cdashboard onfiguration files to mount..."
    exec_cmd_stdout "cp --verbose -r $XCYBER360_CONFIG_MOUNT/* $INSTALL_DIR"
  else
    print "No Xcyber360 dashboard configuration files to mount..."
  fi
}
...
# Create and configure Xcyber360 dashboard keystore
...
##############################################################################
# Start Xcyber360 dashboard
##############################################################################

/xcyber360_app_config.sh $XCYBER360_UI_REVISION
