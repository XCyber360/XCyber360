...
export NAME=xcyber360-indexer
export TARGET_DIR=${CURDIR}/debian/${NAME}

# Package build options
export USER=${NAME}
export GROUP=${NAME}
export VERSION=${XCYBER360_VERSION}-${XCYBER360_TAG_REVISION}
...
export INDEXER_FILE=xcyber360-indexer-base.tar.xz
export BASE_FILE=xcyber360-indexer-base-${VERSION}-linux-x64.tar.xz
export REPO_DIR=/unattended_installer

## Variables
CERT_TOOL=xcyber360-certs-tool.sh
PASSWORD_TOOL=xcyber360-passwords-tool.sh
PACKAGES_URL=https://packages.xcyber360.com/5.0/
PACKAGES_DEV_URL=https://packages-dev.xcyber360.com/5.0/
...
# Copy Xcyber360's config files for the security plugin
...
# Copy Xcyber360 indexer's certificates
cp -pr /xcyber360-certificates/demo.indexer.pem ${TARGET_DIR}${CONFIG_DIR}/certs/indexer.pem
cp -pr /xcyber360-certificates/demo.indexer-key.pem ${TARGET_DIR}${CONFIG_DIR}/certs/indexer-key.pem
cp -pr /xcyber360-certificates/root-ca.key ${TARGET_DIR}${CONFIG_DIR}/certs/root-ca.key
cp -pr /xcyber360-certificates/root-ca.pem ${TARGET_DIR}${CONFIG_DIR}/certs/root-ca.pem
cp -pr /xcyber360-certificates/admin.pem ${TARGET_DIR}${CONFIG_DIR}/certs/admin.pem
cp -pr /xcyber360-certificates/admin-key.pem ${TARGET_DIR}${CONFIG_DIR}/certs/admin-key.pem

# Delete xms and xmx parameters in jvm.options
sed '/-Xms/d' -i /etc/xcyber360-indexer/jvm.options
sed '/-Xmx/d' -i /etc/xcyber360-indexer/jvm.options
sed -i 's/-Djava.security.policy=file:\/\/\/etc\/xcyber360-indexer\/opensearch-performance-analyzer\/opensearch_security.policy/-Djava.security.policy=file:\/\/\/usr\/share\/xcyber360-indexer\/opensearch-performance-analyzer\/opensearch_security.policy/g' /etc/xcyber360-indexer/jvm.options
