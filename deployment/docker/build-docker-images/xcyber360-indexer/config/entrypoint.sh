...
export USER=xcyber360-indexer
export INSTALLATION_DIR=/usr/share/xcyber360-indexer
...
# is idempotent.
source /usr/share/xcyber360-indexer/bin/opensearch-env-from-file
...
if [[ -f bin/opensearch-users ]]; then
  ...
  if [[ -n "$INDEXER_PASSWORD" ]]; then
    [[ -f /usr/share/xcyber360-indexer/opensearch.keystore ]] || (run_as_other_user_if_needed opensearch-keystore create)
...
if [[ "$(id -u)" == "0" ]]; then
  ...
  if [[ -n "$TAKE_FILE_OWNERSHIP" ]]; then
    chown -R 1000:0 /usr/share/xcyber360-indexer/{data,logs}
  fi
fi


run_as_other_user_if_needed /usr/share/xcyber360-indexer/bin/opensearch <<<"$KEYSTORE_PASSWORD"