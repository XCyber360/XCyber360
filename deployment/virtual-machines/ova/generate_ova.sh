...
# Program to build the Xcyber360 Virtual Machine
...
help () {
    echo -e ""
    echo -e "NAME"
    echo -e "$(basename "$0") - Build Xcyber360 OVA."
    echo -e ""
    echo -e "SYNOPSIS"
    ...
    echo -e "                By default the same branch name as xcyber360-virtual-machines will be used."
    echo -e ""
    echo -e "        -r,  --repository"
...
clean() {
    ...
    cd "${scriptpath}"
    vagrant destroy -f
    OVA_VMDK="xcyber360-${OVA_VERSION}-disk001.vmdk"
    rm -f "${OVA_VM}" "${OVF_VM}" "${OVA_VMDK}" "${OVA_FIXED}" "${INSTALLER}"
    rm -rf "${XCYBER360_INSTALLATION_ASSISTANT}"
...
build_ova() {
    ...
    OVA_VM="xcyber360-${OVA_VERSION}.ova"
    OVF_VM="xcyber360-${OVA_VERSION}.ovf"
    OVA_FIXED="xcyber360-${OVA_VERSION}-fixed.ova"
    ...
    # Get machine name
    VM_EXPORT=$(vboxmanage list vms | grep -i vm_xcyber360 | cut -d "\"" -f2)

    # Create OVA with machine
    vboxmanage export "${VM_EXPORT}" -o "${OVA_VM}" \
    --vsys 0 \
    --product "Xcyber360 v${OVA_VERSION} OVA" \
    --producturl "https://packages.xcyber360.com/vm/xcyber360-${OVA_VERSION}.ova" \
    --vendor "Xcyber360, inc <info@xcyber360.com>" --vendorurl "https://xcyber360.com" \
    --version "$OVA_VERSION" --description "Xcyber360 enhances security visibility in your infrastructure by monitoring endpoints at the operating system and application levels. Its capabilities include log analysis, file integrity monitoring, intrusion detection, and compliance monitoring." \
    || clean 1
...
main() {
    ...
    INSTALLER="xcyber360-install.sh"
    XCYBER360_INSTALLATION_ASSISTANT="xcyber360-installation-assistant"
    XCYBER360_INSTALLATION_ASSISTANT_URL="https://github.com/xcyber360/${XCYBER360_INSTALLATION_ASSISTANT}.git"
    BUILDER_ARGS="-i"

    echo "Building Xcyber360 OVA version ${OVA_VERSION}"
    if git ls-remote ${REMOTE_TYPE} ${XCYBER360_INSTALLATION_ASSISTANT_URL} ${INSTALLATION_ASSISTANT_BRANCH} | grep -q "${INSTALLATION_ASSISTANT_BRANCH}"; then
        echo "Cloning Xcyber360 installation assistant repository"
        git clone ${XCYBER360_INSTALLATION_ASSISTANT_URL} -b ${INSTALLATION_ASSISTANT_BRANCH} >> /dev/null 2>&1
        echo "Using ${INSTALLATION_ASSISTANT_BRANCH} branch of ${XCYBER360_INSTALLATION_ASSISTANT} repository"
        cd ${XCYBER360_INSTALLATION_ASSISTANT}
        WIA_VERSION=$(cat VERSION)
        if [ "${OVA_VERSION}" != "${WIA_VERSION}" ]; then
            echo "Xcyber360 installation assistant version ${WIA_VERSION} does not match with OVA version ${OVA_VERSION}"
            clean 1
        fi
        echo "Building Xcyber360 installation assistant from ${INSTALLATION_ASSISTANT_BRANCH} branch"
    else
        echo "Branch ${INSTALLATION_ASSISTANT_BRANCH} not found in xcyber360-installation-assistant repository"
        clean 1
    fi
    ...
    # Standarize OVA
    bash setOVADefault.sh "${scriptpath}" "${OUTPUT_DIR}/${OVA_VM}" "${OUTPUT_DIR}/${OVA_VM}" "${scriptpath}/xcyber360_ovf_template" "${OVA_VERSION}"  || clean 1
