...
from pubsub.subscriber import Xcyber360GCloudSubscriber
...
def get_wodle_config(credentials_file: str = "credentials.json", project: str = "test_project",
    ...
    """Return a dict containing every parameter supported by Xcyber360GCloudSubscriber. Used to simulate different
...
def test_Xcyber360GCloudSubscriber__init__(mock_client):
    """Test if an instance of Xcyber360GCloudSubscriber is created properly."""
    pubsub = Xcyber360GCloudSubscriber(**get_wodle_config())
...
def test_Xcyber360GCloudSubscriber__init__ko(credentials_file, errcode):
    """Test that the appropriate exceptions are raised when the Xcyber360GCloudSubscriber constructor is called with
    invalid parameters.
    """
    with pytest.raises(GCloudError) as e:
        Xcyber360GCloudSubscriber(**get_wodle_config(credentials_file=os.path.join(data_path, credentials_file)))
    assert e.value.errcode == errcode


@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Xcyber360GCloudSubscriber_get_subscriber_client(mock_credentials):
    """Test get_subscriber_client attempts to create a client object using the provided credentials file."""
    Xcyber360GCloudSubscriber.get_subscriber_client("credentials.json")
    mock_credentials.assert_called_with("credentials.json")


@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Xcyber360GCloudSubscriber_get_subscription_path(mock_credentials):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Xcyber360GCloudSubscriber_check_permissions(mock_credentials):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Xcyber360GCloudSubscriber_check_permissions_ko(mock_credentials, errcode, msg):
    """Test check_permissions raises the expected GCloudError when the subscriber does not have the required
    permissions."""
    pubsub = Xcyber360GCloudSubscriber(**get_wodle_config(credentials_file="credentials"))
    if errcode != 1206:
...
@patch('pubsub.subscriber.Xcyber360GCloudSubscriber.send_msg')
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Xcyber360GCloudSubscriber_pull_request(mock_credentials, mock_send_msg, num_messages):
...
@patch('pubsub.subscriber.Xcyber360GCloudSubscriber.send_msg')
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Xcyber360GCloudSubscriber_pull_request_ko(mock_credentials, mock_send_msg):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_Xcyber360GCloudSubscriber_process_messages(mock_credentials):
