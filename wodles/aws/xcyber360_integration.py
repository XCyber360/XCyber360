...
class Xcyber360Integration:
    ...
    def __init__(self, profile, iam_role_arn, service_name=None, region=None,
        ...
        self.xcyber360_path = utils.find_xcyber360_path()
        self.xcyber360_version = utils.get_xcyber360_version()
        self.xcyber360_queue = path.join(self.xcyber360_path, "queue", "sockets", "queue")
        self.xcyber360_wodle = path.join(self.xcyber360_path, "wodles", "aws")
    ...
    def default_config(profile: str) -> dict:
        ...
        This method is called when Xcyber360 Integration is instantiated and sets a default config using .aws/config file
    ...
    def send_msg(self, msg, dump_json=True):
        """
        Sends an AWS event to the Xcyber360 Queue
        ...
        try:
            ...
            s.connect(self.xcyber360_queue)
            encoded_msg = f"{constants.XCYBER360_AWS_MESSAGE_HEADER}{json_msg if dump_json else msg}".encode()
            ...
            if e.errno == 111:
                aws_tools.error("Xcyber360 must be running.")
                sys.exit(11)
            elif e.errno == 90:
                aws_tools.error("Message too long to send to Xcyber360.  Skipping message...")
                aws_tools.debug('+++ ERROR: Message longer than buffer socket for Xcyber360. Consider increasing rmem_max. '
                                'Skipping message...', 1)
            else:
                aws_tools.error("Error sending message to xcyber360: {}".format(e))
                sys.exit(13)
        except Exception as e:
            aws_tools.error("Error sending message to xcyber360: {}".format(e))
            sys.exit(13)
...
class Xcyber360AWSDatabase(Xcyber360Integration):
    """
    ...
    def __init__(self, profile, iam_role_arn, db_name,
        ...
        Xcyber360Integration.__init__(self, service_name=service_name,
                                  profile=profile,
        ...
        self.db_path = "{0}/{1}.db".format(self.xcyber360_wodle, db_name)
        ...
        try:
            if self.db_cursor.execute(self.sql_find_table, {'name': 'metadata'}).fetchone():
                ...
                try:
                    ...
                    if metadata_version != self.xcyber360_version:
                        self.db_cursor.execute(self.sql_update_version_metadata, {'xcyber360_version': self.xcyber360_version})
                ...
                try:
                    ...
                    self.db_cursor.execute(self.sql_insert_version_metadata, {'xcyber360_version': self.xcyber360_version})
