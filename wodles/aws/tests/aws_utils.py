...
def get_xcyber360_integration_parameters(service_name: str = test_constants.TEST_SERVICE_NAME,
    ...
    """Return a dict containing every parameter supported by Xcyber360Integration. Used to simulate different ossec.conf
    configurations.
...
def get_xcyber360_aws_database_parameters(service_name: str = test_constants.TEST_SERVICE_NAME,
                                      profile: str = test_constants.TEST_AWS_PROFILE,
                                      db_name: str = test_constants.TEST_DATABASE,
                                      iam_role_arn: str = None, region: str = None, discard_field: str = None,
                                      discard_regex: str = None, sts_endpoint: str = None, service_endpoint: str = None,
                                      iam_role_duration: str = None, external_id: str = None):
    """Return a dict containing every parameter supported by Xcyber360AWSDatabase.
...
def get_mocked_xcyber360_integration(**kwargs):
    with patch('xcyber360_integration.Xcyber360Integration.get_client'), \
        ...
        return xcyber360_integration.Xcyber360Integration(**get_xcyber360_integration_parameters(**kwargs))


def get_mocked_xcyber360_aws_database(**kwargs):
    with patch('xcyber360_integration.Xcyber360AWSDatabase.check_metadata_version'), \
            patch('xcyber360_integration.Xcyber360Integration.get_client'), \
            patch('xcyber360_integration.sqlite3.connect'), \
            patch('xcyber360_integration.utils.find_xcyber360_path', return_value=test_constants.TEST_XCYBER360_PATH), \
            patch('xcyber360_integration.utils.get_xcyber360_version', return_value=test_constants.TEST_HARDCODED_XCYBER360_VERSION):
        return xcyber360_integration.Xcyber360AWSDatabase(**get_xcyber360_aws_database_parameters(**kwargs))
