(function () {
  ...
  const cliName = 'Xcyber360 alerts generator';
  const cliDescription = 'Generates sample data of Xcyber360 alerts';
  ...
  const defaultConfiguration = {
    ...
    alertGenerationParams: {
      manager: {
        name: 'xcyber360-manager',
      },
      cluster: {
        name: 'xcyber360-cluster',
        node: 'xcyber360-manager',
      },
    },
  };
...
- Generate alerts for all the modules in OpenSearch/Elasticsearch Bulk API format to a xcyber360-alerts index and save to a output.ndjson file
node ${cliFilePath} --all-modules --format bulk-api --index xcyber360-alerts > output.ndjson
