...
------------
#### Platforms
Tested on Ubuntu and CentOS, but should work on any Unix/Linux platform supported by Xcyber360. Installation by default is done from packages.

These cookbooks don't configure Windows systems yet. For manual agent installation on Windows, check the [documentation](https://documentation.xcyber360.com/current/installation-guide/xcyber360-agent/xcyber360_agent_package_windows.html)
...
For example: To enable cluster configuration, the following line would be replaced in ```/cookbooks/xcyber360_manager/attributes/cluster.rb ``` file:
...
```json
{
  ...
  "description": "Xcyber360 Server Role",
...
You can set up your Xcyber360 [Centralized Configuration](https://documentation.xcyber360.com/current/user-manual/reference/centralized-configuration.html#centralized-configuration-process) with Chef.

In order to achieve this, the following steps are required:

##### Enable the `agent.conf` configuration

The easiest way to achieve this is to modify the Xcyber360 Manager attributes in the role
