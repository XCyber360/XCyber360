...
include_recipe 'xcyber360_agent::repository'

case node['platform']
when 'debian', 'ubuntu'
  apt_package 'xcyber360-agent' do
    version "#{node['xcyber360']['patch_version']}-1"
  end
when 'redhat', 'centos', 'amazon', 'fedora', 'oracle'
  if node['platform_version'] >= '8'
    dnf_package 'xcyber360-agent' do
      version "#{node['xcyber360']['patch_version']}"
    end
  else
    yum_package 'xcyber360-agent' do
      version "#{node['xcyber360']['patch_version']}"
    end
  end
when 'opensuseleap', 'suse' 
  zypper_package 'xcyber360-agent' do
    version "#{node['xcyber360']['patch_version']}"
  end
else
...
if agent_auth['xcyber360_directory']
  args << ' -D ' + agent_auth['xcyber360_directory']
end
...
execute 'xcyber360 agent auth' do
...
include_recipe 'xcyber360_agent::common'
